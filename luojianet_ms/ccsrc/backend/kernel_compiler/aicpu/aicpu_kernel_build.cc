...
namespace kernel {
using FNodeAttrHandle = std::function<void(const std::shared_ptr<AnfNode> &anf_node, luojianet_ms::NodeDef *proto)>;
...
void ParseAttrValue(const std::string &type, const std::string &attr_name, const luojianet_ms::ValuePtr &value,
                    ::google::protobuf::Map<::std::string, ::luojianet_ms::AttrValue> *node_attr) {
  ...
  if (type == "int") {
    ...
    luojianet_ms::AttrValue input_shape_attr;
    luojianet_ms::AttrValue_ArrayValue *input_shape_attr_list = input_shape_attr.mutable_array();
...
void SetNodeAttr(const std::shared_ptr<AnfNode> &anf_node, luojianet_ms::NodeDef *proto) {
  MS_EXCEPTION_IF_NULL(anf_node);
  ...
  auto op_info_ptr = luojianet_ms::kernel::OpLib::FindOp(op_name, OpImplyType::kAICPU);
  MS_EXCEPTION_IF_NULL(op_info_ptr);
  auto attrs_ptr = op_info_ptr->attrs_ptr();
  auto primitive = AnfAlgo::GetCNodePrimitive(anf_node);
  MS_EXCEPTION_IF_NULL(primitive);
  ::google::protobuf::Map<::std::string, ::luojianet_ms::AttrValue> *node_attr = proto->mutable_attrs();
...
void SetNodeInputs(const std::shared_ptr<AnfNode> &anf_node, luojianet_ms::NodeDef *proto) {
  MS_EXCEPTION_IF_NULL(proto);
  MS_EXCEPTION_IF_NULL(anf_node);
  size_t input_num = AnfAlgo::GetInputTensorNum(anf_node);
  if (input_num == 0) {
  ...
  for (size_t input_index = 0; input_index < input_num; input_index++) {
    ::luojianet_ms::Tensor *node_inputs = proto->add_inputs();
    ...
    luojianet_ms::TensorShape *tensorShape = node_inputs->mutable_tensor_shape();
    MS_EXCEPTION_IF_NULL(tensorShape);
    for (auto item : input_shape) {
      luojianet_ms::TensorShape_Dim *dim = tensorShape->add_dim();
...
void SetNodeOutputs(const std::shared_ptr<AnfNode> &anf_node, luojianet_ms::NodeDef *proto) {
  MS_EXCEPTION_IF_NULL(proto);
  MS_EXCEPTION_IF_NULL(anf_node);
  size_t output_num = AnfAlgo::GetOutputTensorNum(anf_node);
  if (output_num == 1 && HasAbstractMonad(anf_node)) {
  ...
  for (size_t output_index = 0; output_index < output_num; output_index++) {
    ::luojianet_ms::Tensor *node_outputs = proto->add_outputs();
    MS_EXCEPTION_IF_NULL(node_outputs);
    std::vector<size_t> output_shape = AnfAlgo::GetOutputDeviceShape(anf_node, output_index);
    luojianet_ms::TensorShape *tensorShape = node_outputs->mutable_tensor_shape();
    MS_EXCEPTION_IF_NULL(tensorShape);
...
void SetNodedefProto(const std::shared_ptr<AnfNode> &anf_node, luojianet_ms::NodeDef *proto) {
  MS_EXCEPTION_IF_NULL(anf_node);
  MS_EXCEPTION_IF_NULL(proto);
  std::string op_name = AnfAlgo::GetCNodeName(anf_node);
  if (op_name == kInitDataSetQueue) {
