...
void MatMulAvx512Fp32(const float *a, const float *b, float *c, const float *bias, const int act_type, const int depth,
  ...
  GemmAvx512Kernel kernel[C4NUM][C21NUM];
  int max_shape[C4NUM] = {C20NUM, C12NUM, C8NUM, C6NUM};
  for (int i = 0; i < C4NUM; ++i) {
    for (int j = 1; j < C21NUM; ++j) {
      if (j <= max_shape[i]) {
  ...
  kernel[C3NUM][C2NUM] = nnacl_gemm_avx512_2x64_kernel_nhwc_fp32;
  kernel[C3NUM][C3NUM] = nnacl_gemm_avx512_3x64_kernel_nhwc_fp32;
  kernel[C3NUM][C4NUM] = nnacl_gemm_avx512_4x64_kernel_nhwc_fp32;
  kernel[C3NUM][C5NUM] = nnacl_gemm_avx512_5x64_kernel_nhwc_fp32;
  kernel[C3NUM][C6NUM] = nnacl_gemm_avx512_6x64_kernel_nhwc_fp32;
  int inc_flag;
