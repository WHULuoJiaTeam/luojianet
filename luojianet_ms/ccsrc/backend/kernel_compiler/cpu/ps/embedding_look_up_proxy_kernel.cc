...
void EmbeddingLookUpProxyKernel::InitKernel(const CNodePtr &kernel_node) {
  ...
  if (luojianet_ms::ps::PSContext::instance()->is_worker()) {
    luojianet_ms::ps::Worker::GetInstance().AddEmbeddingTable(key_, input_shape[axis]);
    luojianet_ms::ps::ParamInitInfoMessage info;
    if (!luojianet_ms::ps::Worker::GetInstance().InitPSEmbeddingTable(key_, input_shape, indices_shape, output_shape,
                                                                   info)) {
...
bool EmbeddingLookUpProxyKernel::Launch(const std::vector<kernel::AddressPtr> &inputs,
  ...
  if (!luojianet_ms::ps::Worker::GetInstance().DoPSEmbeddingLookup(key_, lookup_ids, &lookup_result,
                                                                luojianet_ms::ps::kEmbeddingLookupCmd)) {
