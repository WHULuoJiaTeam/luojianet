...
#ifndef LUOJIANET_MS_CCSRC_BACKEND_KERNEL_COMPILER_PS_PUSH_KERNEL_H_
#define LUOJIANET_MS_CCSRC_BACKEND_KERNEL_COMPILER_PS_PUSH_KERNEL_H_
...
class PushKernel : public CPUKernel {
 public:
  ...
  bool Launch(const std::vector<AddressPtr> &inputs, const std::vector<AddressPtr> &,
    ...
    luojianet_ms::ps::Worker::GetInstance().Push(keys, addrs, sizes);
  ...
  void Init(const CNodePtr &kernel_node) {
    ...
    for (size_t i = 0; i < optim_input_shapes.size(); i++) {
      ...
      luojianet_ms::ps::Worker::GetInstance().SetOptimInputShapes(key_, shape);
...
#endif  // LUOJIANET_MS_CCSRC_BACKEND_KERNEL_COMPILER_PS_PUSH_KERNEL_H_
