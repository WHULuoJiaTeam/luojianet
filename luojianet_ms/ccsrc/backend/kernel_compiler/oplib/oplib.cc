...
bool OpLib::RegOpFromLocalInfo() {
  ...
  MS_LOG(INFO) << "Start";
  has_load = true;
  std::string dir = common::GetEnv("LUOJIANET_MS_OP_INFO_PATH");
  if (dir.empty()) {
    MS_LOG(INFO) << "LuoJiaNet op info path does not been set. use op info from python pass.";
    return true;
  }
  char real_path[PATH_MAX] = {0};
  if (dir.size() >= PATH_MAX) {
    MS_LOG(ERROR) << "Invalid environment variable 'LUOJIANET_MS_OP_INFO_PATH', the path length should be smaller than "
  ...
  if (realpath(common::SafeCStr(dir), real_path) == nullptr) {
    MS_LOG(ERROR) << "Invalid environment variable 'LUOJIANET_MS_OP_INFO_PATH', the path is: " << dir
  ...
  if (strlen(real_path) >= PATH_MAX) {
    MS_LOG(ERROR) << "Invalid environment variable 'LUOJIANET_MS_OP_INFO_PATH', the absolute path length should be smaller"
...
bool OpLib::DecodeOpInfo(const nlohmann::json &obj, const luojianet_ms::kernel::OpImplyType imply_type,
