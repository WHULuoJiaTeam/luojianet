...
bool TbeAdapter::IsSpecialFusionComputeNode(const std::vector<luojianet_ms::AnfNodePtr> &compute_nodes) {
...
bool TbeAdapter::GetSpecInputLayers(const std::string &op_name, const std::vector<luojianet_ms::AnfNodePtr> &reorder_layer,
...
bool TbeAdapter::GetSpecDataInput(const FusionScopeInfo &fusion_scope_info,
  ...
  for (const auto &compute_node : compute_nodes) {
    ...
    std::vector<luojianet_ms::AnfNodePtr> layer = {};
    std::vector<luojianet_ms::AnfNodePtr> reorder_layer = {};
