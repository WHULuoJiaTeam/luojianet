...
static luojianet_ms::HashSet<std::string> commutative_ops{"Add", "Mul"};
...
using ParaMap = luojianet_ms::HashMap<char, inner::NodePtr>;
using ConstMap = luojianet_ms::HashMap<std::string, inner::NodePtr>;
...
class PatternTree {
 ...
 protected:
  ...
  virtual luojianet_ms::HashMap<PatternNodePtr, inner::DAttrs> SetAttributes(const inner::NodePtr &) {
    auto right_pattern = std::make_shared<PatternNodePtrList>();
    DfsTraverse(right_pattern, rhs_root_);
    luojianet_ms::HashMap<PatternNodePtr, inner::DAttrs> attrs_map;
...
inner::NodePtr PatternTree::AlterGraph(const std::shared_ptr<ParaMap> &para_to_ref,
  ...
  luojianet_ms::HashMap<PatternNodePtr, inner::NodePtr> pattern_to_ref;
  for (auto &n : (*res)) {
...
class ExtraReduce1PatternTree : public PatternTree {
 ...
 protected:
  ...
  luojianet_ms::HashMap<PatternNodePtr, inner::DAttrs> SetAttributes(const inner::NodePtr &origin_root) override {
    ...
    if (keep_dims) {
      ...
      luojianet_ms::HashMap<int64_t, int64_t> mp;
      int64_t shift = 0;
...
bool OutsideRely(const inner::NodePtrList &nodes, const inner::NodePtr &root) {
  luojianet_ms::HashSet<inner::Node *> nodes_can_simplify;
...
luojianet_ms::HashMap<std::string, std::vector<PatternTreePtr>> GetExpressions() {
  const auto &flags = GraphKernelFlags::GetInstance();
  luojianet_ms::HashMap<std::string, std::vector<PatternTreePtr>> expression_map;
  luojianet_ms::HashSet<std::string> enable_ids{flags.enable_simplify_exprs_only.begin(),
                                             flags.enable_simplify_exprs_only.end()};
  luojianet_ms::HashSet<std::string> disable_ids{flags.disable_simplify_exprs.begin(), flags.disable_simplify_exprs.end()};
