...
void KernelGraph::EnqueueActiveNodes(const AnfNodePtr &node, std::queue<AnfNodePtr> *visit_queue,
                                     luojianet_ms::HashSet<AnfNodePtr> *visited_nodes, bool comm_first) {
...
AnfNodePtr KernelGraph::TransValueNodeTuple(const AbstractBasePtr &abstract, const ValuePtr &value) {
  ...
  std::vector<AnfNodePtr> make_tuple_inputs = {
    luojianet_ms::NewValueNode(std::make_shared<Primitive>(prim::kPrimMakeTuple->name()))};
...
AnfNodePtr KernelGraph::CreatTupleGetItemNode(const AnfNodePtr &node, size_t output_idx) {
  auto idx = luojianet_ms::NewValueNode(SizeToLong(output_idx));
  MS_EXCEPTION_IF_NULL(idx);
  auto imm = std::make_shared<Int64Imm>(SizeToLong(output_idx));
  auto abstract_scalar = std::make_shared<abstract::AbstractScalar>(imm);
  idx->set_abstract(abstract_scalar);
  AnfNodePtr tuple_getitem = NewCNode({luojianet_ms::NewValueNode(prim::kPrimTupleGetItem), node, idx});
...
AnfNodePtr KernelGraph::TransCNodeTuple(const CNodePtr &node) {
  ...
  std::vector<AnfNodePtr> make_tuple_inputs_list = {luojianet_ms::NewValueNode(prim::kPrimMakeTuple)};
...
ValueNodePtr KernelGraph::GetValueNodeByTensor(const luojianet_ms::tensor::TensorPtr &tensor) {
