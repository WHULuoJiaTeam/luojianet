...
inline Status DLSoPath(std::string *so_path) {
  ...
  std::string libluojianet_so = dl_info.dli_fname;

  auto pos = libluojianet_so.find("libluojianet.so");
  if (pos == std::string::npos) {
    return Status(kMEFailed, "Could not find libluojianet.so, check path.");
  }

  std::string parent_dir = libluojianet_so.substr(0, pos) + "../";
