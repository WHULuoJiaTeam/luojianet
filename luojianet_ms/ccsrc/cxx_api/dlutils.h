 ...
 */
#ifndef LUOJIANET_MS_CCSRC_CXX_API_DLUTILS_H_
#define LUOJIANET_MS_CCSRC_CXX_API_DLUTILS_H_
...
inline Status DLSoPath(std::string *so_path) {
  ...
  std::string libluojianet_ms_so = dl_info.dli_fname;

  auto pos = libluojianet_ms_so.find("libluojianet_ms.so");
  if (pos == std::string::npos) {
    return Status(kMEFailed, "Could not find libluojianet_ms.so, check path.");
  }

  std::string parent_dir = libluojianet_ms_so.substr(0, pos) + "../";
...
#endif
#endif  // LUOJIANET_MS_CCSRC_CXX_API_DLUTILS_H_
