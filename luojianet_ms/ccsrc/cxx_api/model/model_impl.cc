...
Status ModelImpl::Preprocess(const std::vector<std::vector<MSTensor>> &inputs, std::vector<MSTensor> *outputs) {
  ...
  auto ExecuteRun =
    (void (*)(const std::vector<std::shared_ptr<dataset::Execute>> &, const std::vector<luojianet_ms::MSTensor> &,
              std::vector<luojianet_ms::MSTensor> *, Status *))(function);
  ...
  for (size_t i = 0; i < output_batched.size(); i++) {
    ...
    output_batched[i] = luojianet_ms::MSTensor("outputs", output_unbatch[0][i].DataType(), ori_shape, nullptr,
