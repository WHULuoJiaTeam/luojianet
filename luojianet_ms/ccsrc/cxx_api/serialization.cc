  ...
  if (model_type == kMindIR) {
    ...
    try {
      if (dec_key.len > dec_key.max_key_len) {
        ...
        auto plain_data = luojianet_ms::Decrypt(&plain_data_size, reinterpret_cast<const unsigned char *>(model_data),
  ...
  if (model_type == kMindIR) {
...
#if !defined(_WIN32) && !defined(_WIN64)
    // Config preprocessor, temporary way to let luojianet_ms.so depends on _c_dataengine
    ...
    if (!preprocessor.empty()) {
      ...
      auto ParseMindIRPreprocessFun =
        (void (*)(const std::vector<std::string> &, std::vector<std::shared_ptr<luojianet_ms::dataset::Execute>> *,
                  Status *))(function);
