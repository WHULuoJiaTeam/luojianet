...
void DumpAttrs(const luojianet_ms::HashMap<std::string, ValuePtr> &attrs, const std::shared_ptr<SubGraphIRInfo> &gsub,
...
void DumpDebugInfo(const CNodePtr &node, const std::shared_ptr<SubGraphIRInfo> &gsub,
  ...
  if (dump_location == kTopStack) {
    ...
    auto traces = luojianet_ms::trace::GetSourceLineList(node);
...
void SetDumpConfigByString(const std::string &str, DumpConfig *dump_config) {
  static luojianet_ms::HashMap<std::string, enum LocDumpMode> dump_level_map = {
