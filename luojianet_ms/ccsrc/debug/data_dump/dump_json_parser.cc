...
constexpr auto kLuoJiaNETDumpConfig = "LUOJIANET_MS_DUMP_CONFIG";
}  // namespace
...
bool DumpJsonParser::IsDumpEnabled() {
  ...
  auto config_path = common::GetEnv(kLuoJiaNETDumpConfig);
 ...
 * Description: Parse the configuration option in dump json file pointed by environment variable LUOJIANET_MS_DUMP_CONFIG.
 */
void DumpJsonParser::Parse() {
  ...
  auto dump_config_file = Common::GetConfigFile(kLuoJiaNETDumpConfig);
...
void DumpJsonParser::CopyHcclJsonToDir(uint32_t rank_id) {
  ...
  std::string config_path = common::GetEnv("LUOJIANET_MS_HCCL_CONFIG_PATH");
  if (config_path.empty()) {
 ...
 * Target device group: Ascend, GPU and CPU.
 * Runtime category: Old runtime, MindRT.
 * Description: Copy the luojianet_ms configuration file to the root directory of dump path. It provides the device and
