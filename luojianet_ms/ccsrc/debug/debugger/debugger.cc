...
using luojianet_ms::runtime::DeviceTensorStore;
  ...
  if (reply_metadata.status() == EventReply::OK) {
    if (version_check) {
      ...
      if (cmd != DebuggerCommand::kVersionMatchedCMD) {
        MS_LOG(ERROR) << "MindInsight version is too old, LuoJiaNET version is " << version_;
        Exit();
      } else {
        if (GetMiVersionMatched(reply_metadata)) {
          MS_LOG(INFO) << "LuoJiaNET version is " << version_ << " matches MindInsight version.";
          ret = true;
        } else {
          MS_LOG(ERROR) << "LuoJiaNET version " << version_ << ", did not match MindInsight version.";
          CommandLoop();
        }
      }
    } else {
