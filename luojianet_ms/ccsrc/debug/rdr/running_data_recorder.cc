...
bool RecordTaskDebugInfo(SubModuleId module, const std::string &name,
  ...
  bool ans = luojianet_ms::RecorderManager::Instance().RecordObject(std::move(task_debug_info_recorder));
  return ans;
}
#endif  // ENABLE_D
...
bool RecordAnfGraph(const SubModuleId module, const std::string &name, const FuncGraphPtr &graph,
  ...
  bool ans = luojianet_ms::RecorderManager::Instance().RecordObject(std::move(graph_recorder));
  return ans;
}
...
bool RecordGraphExecOrder(const SubModuleId module, const std::string &name,
  ...
  bool ans = luojianet_ms::RecorderManager::Instance().RecordObject(std::move(graph_exec_order_recorder));
  return ans;
}
...
bool RecordStreamExecOrder(const SubModuleId module, const std::string &name, const std::vector<CNodePtr> &exec_order) {
  ...
  bool ans = luojianet_ms::RecorderManager::Instance().RecordObject(std::move(stream_exec_order_recorder));
  return ans;
}
...
bool RecordMemAddressInfo(const SubModuleId module, const std::string &name) {
  ...
  bool ans = luojianet_ms::RecorderManager::Instance().RecordObject(std::move(mem_info_recorder));
  return ans;
}
...
bool UpdateMemAddress(const SubModuleId module, const std::string &name, const std::string &op_name,
  ...
  std::string submodule_name = std::string(GetSubModuleName(module));
  auto recorder = luojianet_ms::RecorderManager::Instance().GetRecorder(submodule_name, name);
  bool ans = false;
  if (recorder != nullptr) {
...
void TriggerAll() { luojianet_ms::RecorderManager::Instance().TriggerAll(); }

void Snapshot() { luojianet_ms::RecorderManager::Instance().Snapshot(); }

void ResetRecorder() { luojianet_ms::RecorderManager::Instance().ClearAll(); }
