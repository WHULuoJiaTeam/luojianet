...
#ifndef LUOJIANET_MS_CCSRC_DISTRIBUTED_COLLECTIVE_COLLECTIVE_MANAGER_H_
#define LUOJIANET_MS_CCSRC_DISTRIBUTED_COLLECTIVE_COLLECTIVE_MANAGER_H_
...
// LuoJiaNet uses OpenMPI on CPU, NCCL on GPU, HCCL on Ascend, to achieve distributed training.
// Besides, LuoJiaNet also has its own communication library which is implemented on the CPU side.
class CollectiveManager {
 public:
  ...
  // Initialize the collective communication for distributed training. The backend type is read from LuoJiaNet context.
  bool Initialize();
  ...
  // In some cases global rank id and rank size should be set by caller, e.g., when using LuoJiaNet communication
 ...
 private:
  ...
  // The device type read from LuoJiaNet context.
  ...
  // Host communication library refers to the communication libaray for CPU, e.g., OpenMPI and LuoJiaNet communication
  // framework.
...
#endif  // LUOJIANET_MS_CCSRC_DISTRIBUTED_COLLECTIVE_COLLECTIVE_MANAGER_H_
