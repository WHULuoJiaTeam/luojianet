...
bool CipherInit::Init(const CipherPublicPara &param, size_t time_out_mutex, size_t cipher_exchange_keys_cnt,
  ...
  if (param.encrypt_type == luojianet_ms::ps::kPWEncryptType) {
  ...
  if (param.encrypt_type == luojianet_ms::ps::kStablePWEncryptType) {
...
bool CipherInit::ReInitForScaling() {
  if (ps::PSContext::instance()->encrypt_type() == luojianet_ms::ps::kPWEncryptType) {
    cipher_meta_storage_.RegisterClass();
