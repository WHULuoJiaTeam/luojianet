...
bool DecodeExecutor::DeQuantSparseDiff(std::map<std::string, std::vector<float>> *weight_map,
  ...
  size_t param_num = 0;
  const auto &iter_to_model = luojianet_ms::fl::server::ModelStore::GetInstance().iteration_to_model();
  size_t latest_iter_num = iter_to_model.rbegin()->first;
  std::map<std::string, AddressPtr> feature_maps =
    luojianet_ms::fl::server::ModelStore::GetInstance().GetModelByIterNum(latest_iter_num);
