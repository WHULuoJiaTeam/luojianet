...
void ModelStore::Initialize(uint32_t rank_id, uint32_t max_count) {
  ...
  for (const auto &item : luojianet_ms::fl::compression::kCompressTypeMap) {
...
std::shared_ptr<MemoryRegister> ModelStore::AssignNewCompressModelMemory(
  ...
  std::map<std::string, luojianet_ms::fl::compression::CompressWeight> compressWeights;
  bool status = luojianet_ms::fl::compression::CompressExecutor::GetInstance().construct_compress_weight(
...
void ModelStore::SaveCheckpoint(size_t iteration, const std::map<std::string, AddressPtr> &model) {
  ...
  namespace python_adapter = luojianet_ms::python_adapter;
