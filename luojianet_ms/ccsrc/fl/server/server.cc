...
void Server::InitPkiCertificate() {
  if (ps::PSContext::instance()->pki_verify()) {
    ...
    bool ret = luojianet_ms::ps::server::CertVerify::initRootCertAndCRL(root_first_ca_path_, root_second_ca_path_,
...
void Server::InitCipher() {
  ...
  luojianet_ms::armour::CipherPublicPara param;
  param.g = cipher_g;
  param.t = cipher_t;
  ...
  if (prim == NULL) {
    ...
    ret = luojianet_ms::armour::GetPrime(prim);
  }
  if (ret == 0) {
