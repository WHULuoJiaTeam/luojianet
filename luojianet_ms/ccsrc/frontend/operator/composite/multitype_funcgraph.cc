...
void MultitypeFuncGraph::Register(const TypePtrList &types, specialize_fn s_fn) {
  MS_LOG(DEBUG) << "Register type (" << ::luojianet_ms::ToString(types) << ".";
  auto fn = fn_cache_.find(types);
  if (fn != fn_cache_.end()) {
    MS_LOG(EXCEPTION) << "Cannot register as (" << ::luojianet_ms::ToString(types) << ", already registered.";
  }
  fn_cache_[types] = s_fn;
}

void MultitypeFuncGraph::Register(const TypePtrList &types, const py::function &py_fn) {
  MS_LOG(DEBUG) << "Register type (" << ::luojianet_ms::ToString(types) << ", " << py::str(py_fn.cast<py::object>())
                << ").";
  auto fn = fn_cache_.find(types);
  if (fn != fn_cache_.end()) {
    MS_LOG(EXCEPTION) << "Cannot register as (" << ::luojianet_ms::ToString(types) << ", already registered.";
...
void MultitypeFuncGraph::PyRegister(const py::tuple &tuple, const py::function &py_fn) {
  ...
  for (size_t it = 0; it < tuple.size(); ++it) {
    ...
    if (py::isinstance<py::str>(type_in)) {
      ...
      MS_LOG(EXCEPTION) << "Register must be string or `luojianet_ms.dtype.Type`";
    }
