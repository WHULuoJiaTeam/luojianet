...
namespace ad {
luojianet_ms::HashMap<FuncGraphPtr, DFunctorPtr> DFunctor::func_graph_to_functor_;
luojianet_ms::HashMap<AnfNodePtr, AdjointPtr> DFunctor::anfnode_to_adjoin_definition_;
...
void DFunctor::BackPropagateSwitchLayer(const CNodePtr &cnode_morph, const CNodePtr &env) {
  ...
  luojianet_ms::HashMap<AnfNodePtr, FuncGraphPtr> node_to_fg;
...
void DFunctor::BackPropagate(const CNodePtr &cnode_morph, const CNodePtr &k_app, const AdjointPtr &node_adjoint) {
  ...
  if (HasSideEffectBackProp(cnode_morph)) {
    ...
    tape_->set_flag(luojianet_ms::kFuncGraphFlagReAutoMonad, true);
  } else {
...
FuncGraphPtr DFunctor::KUserDefined(const FuncGraphPtr &primal) {
  ...
  if (bprop != primal->transforms().end()) {
    ...
    bprop_graph->set_flag(luojianet_ms::kFuncGraphFlagBackPropEntry, true);
    bprop_graph->set_flag(luojianet_ms::kFuncGraphFlagReAutoMonad, true);
...
static luojianet_ms::HashMap<CNodePtr, std::vector<CNodePtr>> FindPrimalJPair(const FuncGraphManagerPtr &manager,
  ...
  luojianet_ms::HashMap<CNodePtr, std::vector<CNodePtr>> primal_user_to_j_users;
