...
#include "luojianet_ms/core/load_mindir/load_model.h"
...
constexpr char bprop_mindir_module[] = "luojianet_ms.ops.bprop_mindir";

#ifndef _WIN32
std::string GetBpropDir() {
  static std::string bprop_dir;
  if (bprop_dir.empty()) {
    py::module mod = py::module::import("luojianet_ms.ops._grad");
...
// Get the serializable bprop list from the module luojianet_ms.ops.bprop_mindir in python.
luojianet_ms::HashSet<std::string> GetSerializableBpropList() {
  luojianet_ms::HashSet<std::string> serializable_bprop_list;
...
bool IsSerializableBprop(const std::string &prim_name) {
  static luojianet_ms::HashSet<std::string> serializable_bprop_list = GetSerializableBpropList();
...
void GetFilesHash(const std::string &dir, luojianet_ms::HashMap<std::string, std::string> *bprop_hash_to_file) {
  if (dir.empty()) {
...
luojianet_ms::HashMap<std::string, std::string> GetAllBpropFileHash() {
  luojianet_ms::HashMap<std::string, std::string> bprop_hash_to_file;
...
AnfNodePtr GetPythonOps(const FuncGraphPtr &fg, const AnfNodePtr &origin_node, const PrimitivePtr &prim) {
  ...
  static luojianet_ms::HashMap<std::string, std::pair<std::string, std::string>> python_ops{
    {"S-Prim-zeros_like_leaf", {"zeros_like", ""}},
    {"S-Prim-getitem", {"getitem", "luojianet_ms.ops.composite.multitype_ops.getitem_impl"}}};
  ...
  if (serializable) {
    ...
    if (func_graph != nullptr) {
      ...
      if (GetPrimitiveFlag(prim, GRAPH_FLAG_SIDE_EFFECT_BACKPROP)) {
        func_graph->set_flag(luojianet_ms::kFuncGraphFlagReAutoMonad, true);
      }
      return func_graph;
    }
  }
...
FuncGraphPtr KPrim::GetFprop(const PrimitivePtr &prim) {
  static const std::string ad_module = "luojianet_ms.ops._grad.grad_implementations";
...
FuncGraphPtr KPrim::KPrimitive(const CNodePtr &cnode, const ValueNodePtr &value_node,
  ...
  luojianet_ms::HashMap<std::string, ValuePtr> primal_attrs;
...
AnfNodePtr KPrim::BuildOutput(const FuncGraphPtr &bprop_fg, const FuncGraphPtr &current_primal_fg) {
  ...
  std::string model_name("luojianet_ms.ops.composite.multitype_ops.add_impl");
...
void KPrim::CheckBprop(const FuncGraphPtr &bprop_fg, const string &prim_to_check) {
  ...
  // bprop_fg has been checked in caller
  auto check_bprop_class = prim::GetPythonOps("CheckBprop", "luojianet_ms.ops.operations.other_ops");
