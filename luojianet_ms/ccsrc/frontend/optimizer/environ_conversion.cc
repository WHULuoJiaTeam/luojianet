...
using SymbolicKeyConversionMap =
  luojianet_ms::HashMap<SymbolicKeyInstancePtr, int64_t, SymbolicKeyInstanceHash, SymbolicKeyInstanceEqual>;
...
AnfNodePtr GetTransformedKeyNode(const AnfNodePtr &old_key_node, SymbolicKeyConversionMap *para_symbol_map) {
  ...
  auto tensor_key = std::make_shared<luojianet_ms::tensor::Tensor>(transformed_key);
