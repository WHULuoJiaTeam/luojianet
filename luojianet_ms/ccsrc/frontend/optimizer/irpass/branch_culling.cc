...
AnfNodePtr GenerateSwitchNode(const FuncGraphPtr &graph, const AnfNodePtr &cond, const AnfNodePtr &data,
                              int64_t switch_idx) {
  auto switch_node = prim::GetPythonOps("geswitch", "luojianet_ms.ops.functional")->cast<PrimitivePtr>();
...
using NodeInputReplMap = luojianet_ms::HashMap<std::pair<AnfNodePtr, size_t>, AnfNodePtr, PairHasher>;
...
void RunSwitchNodeReplace(const FuncGraphManagerPtr &manager, std::vector<std::pair<CNodePtr, CNodePtr>> nodes_changed,
                          luojianet_ms::HashMap<AnfNodePtr, AnfNodePtr> repl_node, NodeInputReplMap repl_node_inputs,
...
FuncGraphPtr TransformGraphCondBranchNodes(
  ...
  luojianet_ms::HashMap<AnfNodePtr, AnfNodePtr> repl_node;
...
CNodePtr SquareOp(const FuncGraphPtr &graph, const AnfNodePtr &cond, int64_t switch_idx,
  ...
  auto PrimSquare = prim::GetPythonOps("square", "luojianet_ms.ops.functional")->cast<PrimitivePtr>();
...
CNodePtr MergeNode(const FuncGraphPtr &graph, const AnfNodePtr &cond, int64_t switch_idx,
  ...
  auto PrimMerge = prim::GetPythonOps("merge", "luojianet_ms.ops.functional")->cast<PrimitivePtr>();
...
void GenerateReplNodeForDependMakeTuple(
  ...
  const std::shared_ptr<luojianet_ms::HashMap<AnfNodePtr, AnfNodePtr>> &repl_node,
  const std::function<AnfNodePtr(FuncGraphPtr graph, AnfNodePtr cond, AnfNodePtr data)> &generate_func) {
...
FuncGraphPtr TransformGraphDependNode(
  ...
  ResetSharedOp();
  std::shared_ptr<luojianet_ms::HashMap<AnfNodePtr, AnfNodePtr>> repl_node =
    std::make_shared<luojianet_ms::HashMap<AnfNodePtr, AnfNodePtr>>();  // record the node to be replaced
  const AnfNodeSet &nodes = graph->nodes();
  for (auto &node : nodes) {
    MS_EXCEPTION_IF_NULL(node);
    if (!node->isa<CNode>()) {
