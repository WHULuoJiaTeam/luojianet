...
#ifndef LUOJIANET_MS_CCSRC_FRONTEND_OPTIMIZER_IRPASS_PARAMETER_ELIMINATE_H
#define LUOJIANET_MS_CCSRC_FRONTEND_OPTIMIZER_IRPASS_PARAMETER_ELIMINATE_H
#include <vector>
#include <utility>
#include <memory>
...
static inline std::pair<luojianet_ms::HashSet<size_t>, luojianet_ms::HashMap<size_t, size_t>> EraseUnusedParameters(
  ...
  const auto &parameters = fg->parameters();
  luojianet_ms::HashSet<size_t> unused_parameter_indexes;
  luojianet_ms::HashMap<size_t, size_t> only_return_parameter_indexes;
...
static inline void AdjustCallerArgs(const CNodePtr &caller,
                                    const luojianet_ms::HashSet<size_t> &unused_parameter_indexes) {
...
static inline void AdjustGetItemCall(const CNodePtr &caller,
                                     const luojianet_ms::HashMap<size_t, size_t> &only_return_parameter_indexes) {
  const FuncGraphManagerPtr &manager = caller->func_graph()->manager();
  MS_EXCEPTION_IF_NULL(manager);
...
#endif  // LUOJIANET_MS_CCSRC_FRONTEND_OPTIMIZER_IRPASS_PARAMETER_ELIMINATE_H
