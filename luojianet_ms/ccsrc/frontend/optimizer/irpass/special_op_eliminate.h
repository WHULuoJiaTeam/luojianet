...
#ifndef LUOJIANET_MS_CCSRC_FRONTEND_OPTIMIZER_IRPASS_SPECIAL_OP_ELIMINATE_H_
#define LUOJIANET_MS_CCSRC_FRONTEND_OPTIMIZER_IRPASS_SPECIAL_OP_ELIMINATE_H_
...
class ZeroLikeFillZero : public AnfVisitor {
 public:
  ZeroLikeFillZero()
      : PrimFill_(prim::GetPythonOps("fill", "luojianet_ms.ops.functional")->cast<PrimitivePtr>()),
        PrimShape_(prim::GetPythonOps("shape", "luojianet_ms.ops.functional")->cast<PrimitivePtr>()),
        PrimDType_(prim::GetPythonOps("dtype", "luojianet_ms.ops.functional")->cast<PrimitivePtr>()) {}
  ...
  AnfNodePtr operator()(const OptimizerPtr &, const AnfNodePtr &node) override {
    ...
    if (pattern.TryCapture(node) && IsVNode(x.GetNode(node)) &&
      ...
      if (reduce_op->ToString() == "sum") {
        ...
        // Multiply nodes
        auto mul_prim = prim::GetPythonOps("tensor_mul", "luojianet_ms.ops.functional");
...
#endif  // LUOJIANET_MS_CCSRC_FRONTEND_OPTIMIZER_IRPASS_SPECIAL_OP_ELIMINATE_H_
