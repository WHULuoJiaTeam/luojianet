...
luojianet_ms::HashSet<std::string> taylor_ops{prim::kPrimAdd->name(), prim::kPrimSub->name(), prim::kPrimRealDiv->name(),
                                           prim::kPrimMul->name(), prim::kPrimSin->name(), prim::kPrimCos->name(),
                                           prim::kPrimExp->name()};
// The ops below are excluded when considering taylor rules.
luojianet_ms::HashSet<std::string> taylor_exception_ops{prim::kPrimReturn->name(), prim::kPrimMakeTuple->name(),
                                                     prim::kPrimTupleGetItem->name(), prim::kPrimCast->name()};

// Cache list of primitive ops which have been replaced by taylor rule.
luojianet_ms::HashMap<PrimitivePtr, FuncGraphPtr> taylor_ops_cache_;
