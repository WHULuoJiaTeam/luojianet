...
luojianet_ms::HashSet<std::string> throughtout_op{prim::kPrimMakeTuple->name(), prim::kPrimMakeList->name(),
...
AnfNodePtr GetVmapRule(const PrimitivePtr &prim, const pipeline::ResourceBasePtr &resource, int axis_size) {
  ...
  if (!vmap_rule_fn || py::isinstance<py::none>(vmap_rule_fn)) {
    ...
    if (is_side_effect) {
      vmap_rule_fn = python_adapter::GetPyFn("luojianet_ms.ops._vmap", "vmap_monad_rule")(prim->name(), axis_size);
    } else {
      vmap_rule_node =
  ...
  if (vmap_rule_node == nullptr) {
    ...
    if (vmap_rule_flag) {
      vmap_rule_fg->set_flag(luojianet_ms::kFuncGraphFlagReAutoMonad, true);
    }
...
void ExpandVmapValueNode(const FuncGraphPtr &vmap_fg, const pipeline::ResourceBasePtr &resource,
                         luojianet_ms::HashSet<AnfNodePtr> *visited_node, int axis_size) {
  // Map ValueNode.
...
void ExpandVmapFreeVariable(const FuncGraphPtr &vmap_fg, const FuncGraphManagerPtr &manager,
                            const luojianet_ms::HashSet<AnfNodePtr> &visited_node) {
  // Map free variable.
...
AnfNodePtr ExpandVmap(const ValueNodePtr &vnode, const pipeline::ResourceBasePtr &resource, int axis_size) {
  ...
  if (IsValueNode<FuncGraph>(vnode)) {
    ...
    luojianet_ms::HashSet<AnfNodePtr> visited_node;
