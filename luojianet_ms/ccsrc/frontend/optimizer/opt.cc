...
void SubstitutionList::DisplayStatusOfSubstitution(const luojianet_ms::HashMap<std::string, std::vector<bool>> &status,
...
bool SubstitutionList::ApplySubstitutionsToIR(const OptimizerPtr &optimizer, const FuncGraphPtr &func_graph) const {
  ...
  size_t space = 0;
  luojianet_ms::HashMap<std::string, std::vector<bool>> status;
