...
bool CanNotRecomputed(const CNodePtr &node) {
  static luojianet_ms::HashSet<PrimitivePtr> not_recomputed_op_list{
...
void GetMaxSubGraph(const FuncGraphManagerPtr &mng, luojianet_ms::HashSet<CNodePtr> *recomputed_nodes, bool get_inputs,
                    bool get_outputs) {
  MS_EXCEPTION_IF_NULL(mng);
...
void GetOriginRecomputeAndTargetNodes(const FuncGraphManagerPtr &mng,
                                      const luojianet_ms::HashSet<CNodePtr> &max_recomputed_sub_graph,
                                      luojianet_ms::HashSet<CNodePtr> *recompute_nodes,
                                      luojianet_ms::HashSet<CNodePtr> *target_nodes) {
  MS_EXCEPTION_IF_NULL(mng);
...
std::vector<AnfNodePtr> GetFirstTargetInputs(const std::vector<CNodePtr> &origin_nodes_topological,
                                             const luojianet_ms::HashSet<CNodePtr> &recomputed_origin_nodes,
                                             const luojianet_ms::HashSet<CNodePtr> &target_nodes) {
...
bool HasGradInputs(const AnfNodePtr &node, luojianet_ms::HashMap<AnfNodePtr, bool> *has_grad_inputs_map) {
  MS_EXCEPTION_IF_NULL(node);
...
void SetRecomputedAttr(const FuncGraphPtr &graph, const std::vector<CNodePtr> &origin_nodes_topological) {
  ...
  MS_EXCEPTION_IF_NULL(mng);
  luojianet_ms::HashMap<AnfNodePtr, bool> has_grad_inputs_map;
  for (const auto &node : origin_nodes_topological) {
    MS_EXCEPTION_IF_NULL(node);
...
CNodePtr NewRecomputedNode(const FuncGraphPtr &graph, const CNodePtr &origin_node,
                           ...
                           const luojianet_ms::HashSet<CNodePtr> &recomputed_origin_nodes,
                           luojianet_ms::HashMap<CNodePtr, CNodePtr> *origin_to_recomputed_nodes) {
  MS_EXCEPTION_IF_NULL(graph);
...
void DuplicateRecomputedNodes(const FuncGraphPtr &graph, const luojianet_ms::HashSet<CNodePtr> &target_nodes,
                              const luojianet_ms::HashSet<CNodePtr> &origin_recomputed_nodes,
                              const std::vector<AnfNodePtr> &first_target_inputs,
                              luojianet_ms::HashMap<CNodePtr, CNodePtr> *origin_to_recomputed_nodes) {
  MS_EXCEPTION_IF_NULL(graph);
  auto mng = graph->manager();
...
void InsertRecomputedNodes(const FuncGraphPtr &graph) {
  ...
  luojianet_ms::HashSet<CNodePtr> visited_nodes;
  for (const auto &candidate_recomputed_node : candidate_recomputed_nodes) {
    ...
    luojianet_ms::HashSet<CNodePtr> max_recomputed_sub_graph = {candidate_recomputed_node};
    ...
    luojianet_ms::HashSet<CNodePtr> origin_recomputed_nodes;
    luojianet_ms::HashSet<CNodePtr> target_nodes;
