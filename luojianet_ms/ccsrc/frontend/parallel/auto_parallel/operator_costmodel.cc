...
// Return the per device computation cost in the forward phase. The cost is calculated according to the bytes
// this operator uses
double SoftmaxCost::GetBackwardComputationCost(const std::vector<luojianet_ms::parallel::TensorInfo> &,
                                               const std::vector<luojianet_ms::parallel::TensorInfo> &, int64_t) const {
  return 0.0;
}
...
// return the per device communication cost in the forward phase.
double TmpIdentityCost::GetForwardCommCost(const std::vector<luojianet_ms::parallel::TensorInfo> &,
                                           const std::vector<luojianet_ms::parallel::TensorInfo> &, int64_t) const {
...
// return the per device communication cost in the backward phase.
double TmpIdentityCost::GetBackwardCommCost(const std::vector<luojianet_ms::parallel::TensorInfo> &,
                                            const std::vector<luojianet_ms::parallel::TensorInfo> &, int64_t) const {
...
// Return the per device computation cost in the forward phase. The cost is calculated according to the bytes
// this operator uses
double TmpIdentityCost::GetForwardComputationCost(const std::vector<luojianet_ms::parallel::TensorInfo> &,
                                                  const std::vector<luojianet_ms::parallel::TensorInfo> &, int64_t) const {
  return 0.0;
}
...
// this operator uses
double TmpIdentityCost::GetBackwardComputationCost(const std::vector<luojianet_ms::parallel::TensorInfo> &,
                                                   const std::vector<luojianet_ms::parallel::TensorInfo> &,
                                                   int64_t) const {
  return 0.0;
}
...
double BatchParallelCost::GetForwardComputationCost(const std::vector<luojianet_ms::parallel::TensorInfo> &inputs,
                                                    const std::vector<luojianet_ms::parallel::TensorInfo> &,
                                                    int64_t) const {
  double cost = 0.0;
  for (size_t i = 0; i < inputs.size(); ++i) {
...
double BatchParallelCost::GetBackwardComputationCost(const std::vector<luojianet_ms::parallel::TensorInfo> &,
                                                     const std::vector<luojianet_ms::parallel::TensorInfo> &,
                                                     int64_t) const {
  return 0.0;
}
...
// Return the per device computation cost in the backward phase. The cost is calculated according to the bytes
// this operator uses
double PReLUCost::GetBackwardComputationCost(const std::vector<luojianet_ms::parallel::TensorInfo> &inputs,
                                             const std::vector<luojianet_ms::parallel::TensorInfo> &,
                                             int64_t stage_id) const {
  // In backward phase, the computation cost = (0 or 1) allreduce(slice(B))
  double result = 0.0;
...
// Return the per device computation cost in the backward phase. The cost is calculated according to the bytes
// this operator uses
double ReshapeCost::GetBackwardComputationCost(const std::vector<luojianet_ms::parallel::TensorInfo> &,
                                               const std::vector<luojianet_ms::parallel::TensorInfo> &, int64_t) const {
  return 0.0;
}
