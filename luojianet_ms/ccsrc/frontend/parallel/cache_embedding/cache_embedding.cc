...
namespace parallel {
using ParamMap = luojianet_ms::HashMap<ParameterPtr, ParameterPtr>;
using ParamSet = luojianet_ms::HashSet<ParameterPtr>;
using NodePairList = std::vector<std::pair<AnfNodePtr, AnfNodePtr>>;
using AnfMap = luojianet_ms::HashMap<AnfNodePtr, AnfNodePtr>;
using AnfSet = luojianet_ms::HashSet<AnfNodePtr>;
...
ParamMap AddCacheParameters(const FuncGraphPtr &graph, const ParamSet &parameter_cache_enable_set) {
  ...
  for (auto &param : parameter_cache_enable_set) {
    ...
    if (param_info && param_info->cache_enable()) {
      ...
      auto data_element_type = data_type->cast<luojianet_ms::TensorTypePtr>()->element();
...
AnfNodePtr CreateMapCacheIdx(const FuncGraphPtr &func_graph, const AnfNodePtr &indices,
  ...
  auto indices_element_type = indices_type->cast<luojianet_ms::TensorTypePtr>()->element();
...
AnfNodePtr CreateOutputNodeParam(const FuncGraphPtr &graph, const AnfNodePtr &ori_input, const std::string &name) {
  ...
  auto ori_input_element_type = ori_input_type->cast<luojianet_ms::TensorTypePtr>()->element();
