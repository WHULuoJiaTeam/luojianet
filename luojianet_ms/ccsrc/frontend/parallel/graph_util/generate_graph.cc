...
#include "luojianet_ms/ccsrc/pipeline/jit/parse/parse_base.h"
...
static luojianet_ms::HashMap<int64_t, AnfNodePtr> int_tensor_map = {};
AnfNodePtr CreateInt32Tensor(int64_t value) {
  auto it = int_tensor_map.find(value);
  if (it != int_tensor_map.end()) {
    return it->second;
  }
  luojianet_ms::tensor::TensorPtr tensor_ptr = std::make_shared<tensor::Tensor>(value, kInt32);
