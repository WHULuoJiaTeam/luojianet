...
size_t GetInputsTypeLen(const AnfNodePtr &input) {
  ...
  if (type->isa<luojianet_ms::TensorType>()) {
    auto input_element_type = type->cast<luojianet_ms::TensorTypePtr>()->element();
...
std::vector<TypePtr> ExtractOutputTypeByNode(const CNodePtr &node) {
  ...
  if (primary_output_type->isa<luojianet_ms::Tuple>()) {
    // in this case, the output is a tuple
    auto tuple_output_type = primary_output_type->cast<luojianet_ms::TuplePtr>();
    auto elements = tuple_output_type->elements();
    for (auto &ele : elements) {
      if (ele->isa<luojianet_ms::TensorType>()) {
        auto ele_element_type = ele->cast<luojianet_ms::TensorTypePtr>()->element();
    ...
    if (primary_output_type->isa<luojianet_ms::TensorType>()) {
      auto element_type = primary_output_type->cast<luojianet_ms::TensorTypePtr>()->element();
