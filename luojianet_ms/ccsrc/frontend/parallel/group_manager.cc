...
Status GroupManager::CreateGroup(const std::string &group_name, const std::vector<Device> &devices,
                                 luojianet_ms::parallel::Group *const group) {
...
Status GroupManager::DestroyGroup(luojianet_ms::parallel::Group *const group) {
...
Status GroupManager::FindGroup(const std::string &name, luojianet_ms::parallel::Group **group) {
  auto it = groups_.find(name);
  if (it == groups_.end()) {
    return Status::FAILED;
  }
