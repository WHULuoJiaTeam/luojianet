...
ForwardOp CreateReduceMeanForwardOp(const std::vector<Group> &forward_group, const TypePtr &dtype) {
  ...
  luojianet_ms::tensor::TensorPtr tensor_ptr = std::make_shared<luojianet_ms::tensor::Tensor>(divisor, dtype);
...
Status ReduceMeanInfo::InferForwardCommunication() {
  ...
  if (!forward_group.empty()) {
    if ((outputs_dtype_ == nullptr) || !outputs_dtype_->isa<luojianet_ms::TensorType>()) {
      MS_LOG(ERROR) << name_ << ": The dtype of output is not Array";
      return FAILED;
    }

    auto element_type = outputs_dtype_->cast<luojianet_ms::TensorTypePtr>()->element();
