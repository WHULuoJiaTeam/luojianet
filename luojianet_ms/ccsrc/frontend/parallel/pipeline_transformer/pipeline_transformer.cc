...
namespace parallel {
luojianet_ms::HashMap<AnfNodePtr, std::set<int64_t>> parameter_color_map;
// map<rank, tag>
luojianet_ms::HashMap<int64_t, int64_t> send_tag_map;
luojianet_ms::HashMap<int64_t, int64_t> recv_tag_map;
...
static std::pair<ValueListPtr, TypePtr> GetShapeType(const AnfNodePtr &node, const Shape &shape) {
  ...
  auto tensor_type = type->cast<luojianet_ms::TensorTypePtr>();
...
void PipelineTransformer::RedundancyNode(const AnfNodePtr &node,
                                         luojianet_ms::HashMap<CNodePtr, std::vector<AnfNodePtr>> *make_tuple_map) {
  auto node_users = manager_->node_users()[node];
