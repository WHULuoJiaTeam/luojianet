...
#include "luojianet_ms/core/utils/parallel_node_check.h"
...
bool StrategyFound(const luojianet_ms::HashMap<std::string, ValuePtr> &attrs) {
  auto iter = attrs.find(IN_STRATEGY);
  return !((iter == attrs.end()) || (iter->second->type_name() == NONE));
}

bool AttrFound(const luojianet_ms::HashMap<std::string, ValuePtr> &attrs, const std::string &target) {
...
void StepReplaceGraph(const ReplaceGraphPtr &replace_graph, const CNodePtr &node) {
  ...
  luojianet_ms::HashMap<AnfNodePtr, int> input_map = {};
...
bool InsertMirrorBeforeCast(const CNodePtr &node, size_t index) {
  ...
  if (!node_type->isa<luojianet_ms::TensorType>()) {
    MS_LOG(EXCEPTION) << "Unknown type.";
  }
  auto input_element_type = node_type->cast<luojianet_ms::TensorTypePtr>()->element();
