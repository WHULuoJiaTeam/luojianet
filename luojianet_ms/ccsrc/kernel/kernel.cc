...
bool KernelMod::InferShapeForDefiniteOutputNode(const CNodePtr &cnode) {
  ...
  auto cur_shape = dynamic_cast<luojianet_ms::abstract::Shape *>(cnode->Shape().get())->shape();
  ...
  luojianet_ms::abstract::BaseShapePtr shape = std::make_shared<luojianet_ms::abstract::Shape>(output_shape);
