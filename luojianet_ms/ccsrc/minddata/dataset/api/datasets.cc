...
std::vector<luojianet_ms::DataType> Dataset::GetOutputTypes() {
  ...
  if (consumer == nullptr) {
    ...
    return std::vector<luojianet_ms::DataType>();
  }
  ...
  std::vector<luojianet_ms::DataType> ret_types;
  std::transform(
    types.begin(), types.end(), std::back_inserter(ret_types),
    [](const DataType &d) -> luojianet_ms::DataType { return static_cast<luojianet_ms::DataType>(DETypeToMSType(d)); });
  return ret_types;
}
...
Status SchemaObj::add_column_char(const std::vector<char> &name, luojianet_ms::DataType de_type,
