...
Status Execute::operator()(const luojianet_ms::MSTensor &input, luojianet_ms::MSTensor *output) {
  ...
  if (device_type_ == MapTargetDevice::kCpu) {
    // Convert luojianet_ms::Tensor to dataset::Tensor
    ...
    *output = luojianet_ms::MSTensor(std::make_shared<DETensor>(de_tensor));
    ...
    std::shared_ptr<luojianet_ms::dataset::DeviceTensor> device_input;
    ...
    *output = luojianet_ms::MSTensor(std::make_shared<DETensor>(device_input, true));
#endif
  } else {
...
Status Execute::operator()(const std::vector<MSTensor> &input_tensor_list, std::vector<MSTensor> *output_tensor_list) {
  ...
  if (device_type_ == MapTargetDevice::kCpu) {  // Case CPU
    ...
    for (auto &tensor : de_tensor_list) {
      ...
      auto ms_tensor = luojianet_ms::MSTensor(std::make_shared<DETensor>(tensor));
    ...
    for (auto &input_tensor : input_tensor_list) {
      ...
      std::shared_ptr<luojianet_ms::dataset::Tensor> host_output;
      RETURN_IF_NOT_OK(device_resource_->Pop(device_input, &host_output));

      auto ms_tensor = luojianet_ms::MSTensor(std::make_shared<DETensor>(host_output));
      output_tensor_list->emplace_back(ms_tensor);
...
Status Execute::Run(const std::vector<std::shared_ptr<dataset::Execute>> &data_graph,
                    const std::vector<luojianet_ms::MSTensor> &inputs, std::vector<luojianet_ms::MSTensor> *outputs) {
...
// In the current stage, there is a cyclic dependency between libluojianet_ms.so and c_dataengine.so,
...
extern "C" {
// ExecuteRun_C has C-linkage specified, but returns user-defined type 'luojianet_ms::Status' which is incompatible with C
