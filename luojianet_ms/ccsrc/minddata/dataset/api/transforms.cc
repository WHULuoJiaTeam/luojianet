...
#include "luojianet_ms/ccsrc/minddata/dataset/core/type_id.h"
...
// Constructor to Mask
struct Mask::Data {
  explicit Data(RelationalOp op, const MSTensor &constant, luojianet_ms::DataType ms_type)
      : op_(op), constant_(constant), ms_type_(ms_type) {}
  RelationalOp op_;
  MSTensor constant_;
  luojianet_ms::DataType ms_type_;
};

Mask::Mask(RelationalOp op, const MSTensor &constant, luojianet_ms::DataType ms_type)
...
TypeCast::TypeCast(luojianet_ms::DataType data_type) : data_(std::make_shared<Data>()) {
