...
DETensor::DETensor(std::shared_ptr<dataset::Tensor> tensor_impl)
    : tensor_impl_(tensor_impl),
      ...
      type_(static_cast<luojianet_ms::DataType>(DETypeToMSType(tensor_impl_->type()))),
...
enum luojianet_ms::DataType DETensor::DataType() const {
#ifndef ENABLE_ANDROID
  if (is_device_) {
    EXCEPTION_IF_NULL(device_tensor_impl_);
    return static_cast<luojianet_ms::DataType>(DETypeToMSType(device_tensor_impl_->DeviceDataType()));
  }
#endif
  EXCEPTION_IF_NULL(tensor_impl_);
  return static_cast<luojianet_ms::DataType>(DETypeToMSType(tensor_impl_->type()));
}
...
std::shared_ptr<luojianet_ms::MSTensor::Impl> DETensor::Clone() const {
#ifndef ENABLE_ANDROID
  if (is_device_) {
    EXCEPTION_IF_NULL(device_tensor_impl_);
