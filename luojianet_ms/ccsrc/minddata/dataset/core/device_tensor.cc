...
Status DeviceTensor::DataPop_(std::shared_ptr<Tensor> *host_tensor) {
  ...
  luojianet_ms::dataset::dsize_t dvppDataSize = this->DeviceDataSize();
  ...
  RETURN_IF_NOT_OK(luojianet_ms::dataset::Tensor::CreateFromMemory(dvpp_shape, dvpp_data_type, ret_ptr, host_tensor));
