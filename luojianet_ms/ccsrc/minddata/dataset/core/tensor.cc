...
Status Tensor::GetSliceOption(const SliceOption &slice_option, const int32_t &slice_index,
  ...
  if (slice_option.indices_.empty()) {
    ...
    luojianet_ms::dataset::Slice slice_copy = slice_option.slice_;
...
Status Tensor::Slice(std::shared_ptr<Tensor> *out, const std::vector<SliceOption> slice_options_) {
  ...
  for (size_t k = 0; k < slice_options_.size(); k++) {
    ...
    if (slice_option.all_) {
      luojianet_ms::dataset::Slice slice = luojianet_ms::dataset::Slice(shape_[static_cast<dsize_t>(k)]);
  ...
  if (slice_options_.size() != Rank()) {
    for (dsize_t j = static_cast<dsize_t>(slice_options_.size()); j < Rank(); j++) {
      luojianet_ms::dataset::Slice slice = luojianet_ms::dataset::Slice(0, shape_[j]);
