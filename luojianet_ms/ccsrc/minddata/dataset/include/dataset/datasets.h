...
#ifndef LUOJIANET_MS_CCSRC_MINDDATA_DATASET_INCLUDE_DATASET_DATASETS_H_
#define LUOJIANET_MS_CCSRC_MINDDATA_DATASET_INCLUDE_DATASET_DATASETS_H_
...
class MS_API Dataset : public std::enable_shared_from_this<Dataset> {
 public:
  ...
  std::vector<luojianet_ms::DataType> GetOutputTypes();
...
class MS_API SchemaObj {
 public:
  ...
  /// \param[in] ms_type Data type of the column(luojianet_ms::DataType).
  /// \return Status code.
  Status add_column(const std::string &name, luojianet_ms::DataType ms_type) {
  ...
  /// \param[in] shape Shape of the column.
  /// \return Status code.
  Status add_column(const std::string &name, luojianet_ms::DataType ms_type, const std::vector<int32_t> &shape) {
 ...
 private:
  ...
  Status add_column_char(const std::vector<char> &name, luojianet_ms::DataType ms_type);

  Status add_column_char(const std::vector<char> &name, const std::vector<char> &ms_type);

  Status add_column_char(const std::vector<char> &name, luojianet_ms::DataType ms_type, const std::vector<int32_t> &shape);
...
///    Check API `luojianet_ms::dataset::Schema` and find more usage.
...
///      schema->add_column("image", luojianet_ms::DataType::kNumberTypeUInt8, {2});
///      schema->add_column("label", luojianet_ms::DataType::kNumberTypeUInt8, {1});
...
///      std::shared_ptr<SchemaObj> schema = Schema();
///      schema->add_column("column1", luojianet_ms::DataType::kNumberTypeUInt8, {2});
///      schema->add_column("column2", luojianet_ms::DataType::kNumberTypeUInt8, {1});
///      std::shared_ptr<Dataset> ds = RandomData(50, schema);
///
///      /* Create iterator to read dataset */
///      std::shared_ptr<Iterator> iter = ds->CreateIterator();
...
///    compiler option '_GLIBCXX_USE_CXX11_ABI' while char is free of this restriction.
///    Check API `luojianet_ms::dataset::CreateDatasetCache` and find more usage.
...
#endif  // LUOJIANET_MS_CCSRC_MINDDATA_DATASET_INCLUDE_DATASET_DATASETS_H_
