...
Status DvppDecodeJpegOp::Compute(const std::shared_ptr<DeviceTensor> &input, std::shared_ptr<DeviceTensor> *output) {
  ...
  try {
    ...
    RETURN_IF_NOT_OK(luojianet_ms::dataset::DeviceTensor::CreateEmpty(dvpp_shape, dvpp_data_type, output));
...
Status DvppDecodeJpegOp::Compute(const std::shared_ptr<Tensor> &input, std::shared_ptr<Tensor> *output) {
  ...
  try {
    ...
    const DataType dvpp_data_type(DataType::DE_UINT8);
    RETURN_IF_NOT_OK(luojianet_ms::dataset::Tensor::CreateFromMemory(dvpp_shape, dvpp_data_type, ret_ptr, output));
