 ...
 */
APP_ERROR MDAclProcess::H2D_Sink(const std::shared_ptr<luojianet_ms::dataset::Tensor> &input,
  ...
  const luojianet_ms::dataset::DataType dvpp_data_type(luojianet_ms::dataset::DataType::DE_UINT8);
  const luojianet_ms::dataset::TensorShape dvpp_shape({1, 1, 1});
  auto rc = luojianet_ms::dataset::DeviceTensor::CreateEmpty(dvpp_shape, dvpp_data_type, &device_input);
  if (rc.IsError()) {
...
APP_ERROR MDAclProcess::D2H_Pop(const std::shared_ptr<luojianet_ms::dataset::DeviceTensor> &device_output,
                                std::shared_ptr<luojianet_ms::dataset::Tensor> &output) {
  ...
  luojianet_ms::dataset::dsize_t dvppDataSize = device_output->DeviceDataSize();
  const luojianet_ms::dataset::TensorShape dvpp_shape({dvppDataSize, 1, 1});
  ...
  const luojianet_ms::dataset::DataType dvpp_data_type(luojianet_ms::dataset::DataType::DE_UINT8);
  auto rc = luojianet_ms::dataset::Tensor::CreateFromMemory(dvpp_shape, dvpp_data_type, ret_ptr, &output);
  if (rc.IsError()) {
    MS_LOG(ERROR) << "Failed to allocate memory, error msg is " << rc;
    return APP_ERR_ACL_BAD_ALLOC;
  }
