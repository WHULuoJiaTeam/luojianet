...
void *operator new(std::size_t s, luojianet_ms::Status *rc, std::shared_ptr<luojianet_ms::dataset::MemoryPool> b) {
  void *ptr = nullptr;
  *rc = b->Allocate(s, &ptr);
  return ptr;
}
...
void operator delete(void *p, std::shared_ptr<luojianet_ms::dataset::MemoryPool> b) {
