...
class AnalyzeFailExporter : public AnfExporter {
 ...
 protected:
  ...
  luojianet_ms::HashMap<FuncGraphPtr, TaggedNodeMap> CreateTaggedNodeMap(
...
luojianet_ms::HashMap<FuncGraphPtr, TaggedNodeMap> AnalyzeFailExporter::CreateTaggedNodeMap(
  const std::vector<abstract::AnfNodeConfigPtr> &node_config_stack) {
  luojianet_ms::HashSet<abstract::AnfNodeConfigPtr> forwarded_configs;  // Check if config. is forwarded.
  luojianet_ms::HashMap<FuncGraphPtr, TaggedNodeMap> tagged_func_graphs;
  size_t index = 0;
...
bool AnalyzeFailExporter::ExportFuncGraph(const std::string &filename, const TraceCNodeEvalStack &node_config_stack) {
  ...
  luojianet_ms::HashSet<FuncGraphPtr> printed_func_graphs;  // Check if func graph has been printed.
