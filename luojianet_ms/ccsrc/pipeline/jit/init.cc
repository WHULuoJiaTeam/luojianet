...
using RecoveryContext = luojianet_ms::distributed::recovery::RecoveryContext;
...
PYBIND11_MODULE(_c_expression, m) {
  ...
  (void)py::module::import("atexit").attr("register")(py::cpp_function{[&]() -> void {
    ...
    if (luojianet_ms::python_adapter::IsPythonEnv()) {
...
#endif
  (void)m.def("_ms_memory_recycle", &luojianet_ms::pipeline::MemoryRecycle, "Recycle memory used by luojianet_ms.");
}
