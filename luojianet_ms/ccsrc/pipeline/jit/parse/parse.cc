...
#include "luojianet_ms/core/ir/cell.h"
...
FunctionBlockPtr Parser::ParseStatement(const FunctionBlockPtr &block, const py::object &node) {
  ...
  if (stmt_method_map_.count(node_name)) {
    ...
    MS_LOG(EXCEPTION) << "Unsupported statement '" << node_name
                      << "'.\nMore details please refer to syntax support at https://www.luojianet_ms.cn";
  }
}
...
FunctionBlockPtr Parser::ParseForLoop(const FunctionBlockPtr &block, const py::object &node) {
  ...
  auto scalar_to_tensor = prim::GetPythonOps("ScalarToTensor", "luojianet_ms.ops.operations");
  ...
  auto prim_less = prim::GetPythonOps("Less", "luojianet_ms.ops.operations");
  ...
  // Create 'i = i + 1'
  auto prim_add = prim::GetPythonOps("Add", "luojianet_ms.ops.operations");
...
AnfNodePtr FindPhis(const luojianet_ms::HashMap<ParameterPtr, AnfNodePtr> &removable_phis, const AnfNodePtr &node) {
  MS_EXCEPTION_IF_NULL(node);
...
void Parser::RemoveUnnecessaryPhis() {
  ...
  luojianet_ms::HashMap<ParameterPtr, AnfNodePtr> removable_phis;
...
bool UpdateFuncGraphFlags(const py::object &obj, const FuncGraphPtr &func_graph) {
  ...
  if (!py::hasattr(obj, PYTHON_EXTERN_LUOJIANET_MS_FLAG)) {
    MS_LOG(DEBUG) << "No flags";
    return true;
  }
  py::dict flags = python_adapter::GetPyObjAttr(obj, PYTHON_EXTERN_LUOJIANET_MS_FLAG);
  for (auto &item : flags) {
