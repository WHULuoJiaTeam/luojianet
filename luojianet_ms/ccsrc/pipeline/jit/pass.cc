...
using luojianet_ms::validator::Validate;
namespace {
...
static luojianet_ms::HashMap<std::string, std::shared_ptr<Optimizer>> g_pass_opts = {};
    ...
    for (const auto &value_pair : value_nodes) {
      ...
      if (users.size() == 1) {
        ...
        if (IsPrimitiveCNode(cnode, prim::kPrimAllReduce) && cnode->inputs().size() > 1 &&
          ...
          auto fusion_id = attrs.find(luojianet_ms::parallel::FUSION);
