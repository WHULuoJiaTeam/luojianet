...
#ifdef ENABLE_D
#include "luojianet_ms/ccsrc/profiler/device/ascend/parallel_strategy_profiling.h"
#endif
#endif
...
using luojianet_ms::abstract::AbstractTensorPtr;
using luojianet_ms::abstract::AbstractTuple;
using luojianet_ms::abstract::AbstractTuplePtr;

#ifdef ENABLE_D
#ifndef ENABLE_SECURITY
using luojianet_ms::device::ascend::ProfilingManager;
#endif
using HcclCollectiveGroup = luojianet_ms::device::ascend::collective::HcclCollectiveGroup;
#endif
    ...
    if (tensor->data_type() == kNumberTypeBool) {
      MS_LOG(WARNING) << "The data types of Tensor:" << py::str(arg)
                      << " is bool, which may cause SelectKernelInfo failure for operator [AddN]. "
                         "For more details, please refer to the FAQ at https://www.luojianet_ms.cn.";
    }
  }
  ...
  for (size_t i = 0; i < args.size(); i++) {
    if (!CheckArgValid(args[i])) {
      MS_EXCEPTION(TypeError)
        ...
        << "For more details, please search 'outermost network' at https://www.luojianet_ms.cn.";
    }
  }
}
...
py::dict GraphExecutorPy::GetParallelGraphInfo(const std::string &phase) {
  ...
  return luojianet_ms::parallel::GetParallelCNodeInfoFromGraph(graph);
}
...
py::dict GraphExecutorPy::GetParameterLayout(const std::string &phase) {
  ...
  if (graph == nullptr) {
    ...
    return luojianet_ms::parallel::GetParameterLayoutFromResource(resource);
  }
  return luojianet_ms::parallel::GetParameterLayoutFromGraph(graph);
}
...
py::list GraphExecutorPy::GetParallelParameterNameList(const std::string &phase) {
  ...
  if (graph == nullptr) {
    auto resource = info_[phase]->resource;
    return luojianet_ms::parallel::GetParallelParameterNameListFromResource(resource);
  }
  return luojianet_ms::parallel::GetParallelParameterNameListFromGraph(graph);
}
...
py::dict GraphExecutorPy::GetAllreduceFusion(const std::string &phase) {
  ...
  return luojianet_ms::parallel::GetAllreduceFusion(graph);
}
...
#ifdef ENABLE_DUMP_IR
  (void)luojianet_ms::RDR::Snapshot();
#endif
  ...
  if (luojianet_ms::RecorderManager::Instance().RdrEnable()) {
    ...
    if (graph != nullptr) {
      ...
      if (graph_clone != nullptr) {
        ...
        (void)luojianet_ms::RDR::RecordAnfGraph(SUBMODULE_ID, filename, graph_clone, dump_params, ".ir");
      } else {
...
py::object GraphExecutorPy::Run(const py::tuple &args, const py::object &phase_obj) {
  // LuoJiaNET debugger notify main thread to exit after one step, and will not run next step
#ifdef ENABLE_DEBUGGER
  TerminateDebugger();
#endif
  std::size_t size = args.size();
...
bool InitExecDatasetVm(const std::string &queue_name, int64_t size, int64_t batch_size,
  ...
  luojianet_ms::python_adapter::set_python_env_flag(true);
  ...
  ConfigManager::GetInstance().ResetIterNum();
#ifdef ENABLE_DUMP_IR
  luojianet_ms::RDR::ResetRecorder();
#endif
...
void ResetOpId() { luojianet_ms::id_generator::reset_id(); }
...
py::bytes PyEncrypt(char *plain_data, size_t plain_len, char *key, size_t key_len, const std::string &enc_mode) {
  size_t encrypt_len;
  auto encrypt_data = luojianet_ms::Encrypt(&encrypt_len, reinterpret_cast<Byte *>(plain_data), plain_len,
...
py::bytes PyDecrypt(const std::string &encrypt_data_path, char *key, size_t key_len, const std::string &dec_mode) {
  size_t decrypt_len;
  auto decrypt_data =
    luojianet_ms::Decrypt(&decrypt_len, encrypt_data_path, reinterpret_cast<Byte *>(key), key_len, dec_mode);
...
bool PyIsCipherFile(const std::string &file_path) { return luojianet_ms::IsCipherFile(file_path); }
}  // namespace pipeline
}  // namespace luojianet_ms
