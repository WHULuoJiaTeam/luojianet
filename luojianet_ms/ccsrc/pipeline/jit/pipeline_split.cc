...
static CNodePtr CreateVirtualDataset(const FuncGraphPtr &func_graph) {
  luojianet_ms::parallel::OperatorAttrs attrs;
  ValuePtr pyop_instance = luojianet_ms::parallel::CreateOpInstance(attrs, luojianet_ms::parallel::VIRTUAL_DATA_SET,
                                                                 luojianet_ms::parallel::VIRTUAL_DATA_SET);
...
static std::set<FuncGraphPtr> FindForwardGraph(const FuncGraphPtr &root, const std::vector<AnfNodePtr> &all_nodes) {
  ...
  for (auto &node : all_nodes) {
    ...
    if (expect_prim->name() == luojianet_ms::parallel::J || expect_prim->name() == luojianet_ms::parallel::SHARD) {
    ...
    for (size_t index = 0; index < graph_inputs.size(); index++) {
      ...
      for (auto node_user : node_users) {
        ...
        for (size_t input_index = 1; input_index < cnode->inputs().size(); input_index++) {
          ...
          bool is_node_input_flag = !(IsValueNode<luojianet_ms::tensor::Tensor>(cnode->inputs()[input_index]) ||
...
std::vector<ValuePtr> GetStrategyElements(const CNodePtr &cnode, const std::vector<AnfNodePtr> &parameters,
  ...
  for (size_t i = 1; i < current_inputs.size(); ++i) {
    ...
    if (current_input->isa<ValueNode>()) {
      ...
      if (!current_value->isa<luojianet_ms::tensor::Tensor>()) {
        continue;
      }
    }
