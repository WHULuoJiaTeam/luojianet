...
// --------------------------------------------------------------------
using SccVector = luojianet_ms::HashSet<FuncGraphPtr>;
using SccPtr = std::shared_ptr<SccVector>;
using SccMap = luojianet_ms::HashMap<FuncGraphPtr, SccPtr>;
...
class SccFinder {
 ...
 private:
  ...
  luojianet_ms::HashMap<FuncGraphPtr, std::unique_ptr<State>> visited_;
...
class SideEffectFinder {
 ...
 private:
  ...
  luojianet_ms::HashMap<FuncGraphPtr, luojianet_ms::HashSet<CNodePtr>> graph_callers_;
  ...
  luojianet_ms::HashMap<CNodePtr, FuncGraphVector> switch_calls_;
...
bool AutoMonad(const FuncGraphPtr &func_graph) {
  ...
  for (auto &fg : fg_used_total) {
    auto top_flag = fg->has_flag(luojianet_ms::kFuncGraphFlagBackPropEntry);
    if (fg->stage() != -1) {
      top_flag = true;
    }
...
bool ReAutoMonad(const FuncGraphPtr &func_graph) {
  ...
  for (auto &fg : func_graph->func_graphs_used_total()) {
    if (fg->has_flag(luojianet_ms::kFuncGraphFlagReAutoMonad)) {
      auto_monaded_fg.push_back(fg);
      for (auto &used_fg : fg->func_graphs_used_total()) {
        used_fg->set_flag(luojianet_ms::kFuncGraphFlagReAutoMonad, true);
  ...
  if (need_auto_monad) {
    ...
    for (auto &fg : auto_monaded_fg) {
      fg->erase_flag(luojianet_ms::kFuncGraphFlagReAutoMonad);
    }
