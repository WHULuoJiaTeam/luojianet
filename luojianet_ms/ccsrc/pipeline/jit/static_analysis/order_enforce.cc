...
class OrderEnforcer {
 ...
 private:
  ...
  PrimitivePtr GetPrimitive(const AnfNodePtr &node) {
    ...
    auto do_sig = dyn_cast<luojianet_ms::prim::DoSignaturePrimitive>(prim);
    if (do_sig) {
  ...
  void HandleSideEffectNode(const CNodePtr &cnode, const CNodePtr &update_state) {
    ...
    for (size_t i = 1; i < inputs.size(); ++i) {
      ...
      for (auto &load : loads) {
        ...
        luojianet_ms::HashSet<AnfNodePtr> real_users;
  ...
  void AddInputEdges(const CNodePtr &update_state, const luojianet_ms::HashSet<AnfNodePtr> &load_users) {
  ...
  std::vector<AnfNodePtr> SortLoadUsers(const luojianet_ms::HashSet<AnfNodePtr> &load_users) {
  ...
  luojianet_ms::HashSet<AnfNodePtr> FindNodeUsers(const AnfNodePtr &node, const PredFunc &pred = nullptr) {
    auto &node_users = manager_->node_users();
    auto iter = node_users.find(node);
    if (iter == node_users.end()) {
      return {};
    }
    luojianet_ms::HashSet<AnfNodePtr> users;
  ...
  luojianet_ms::HashSet<AnfNodePtr> FindParallelNodeUsers(const AnfNodePtr &node) {
    auto &node_users = manager_->node_users();
    auto iter = node_users.find(node);
    if (iter == node_users.end()) {
      return {};
  ...
  luojianet_ms::HashSet<AnfNodePtr> FindLoadUsers(const AnfNodePtr &load_or_param) {
  ...
  luojianet_ms::HashSet<AnfNodePtr> FindLoadNodes(const AnfNodePtr &param) {
  ...
  luojianet_ms::HashMap<AnfNodePtr, size_t> topo_sort_map_;
  luojianet_ms::HashSet<AnfNodePtr> processed_nodes_;
};
}  // namespace
