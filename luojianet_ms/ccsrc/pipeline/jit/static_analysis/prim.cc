...
namespace abstract {
using luojianet_ms::parse::PyObjectWrapper;

luojianet_ms::HashSet<std::string> prims_to_skip_undetermined_infer{
...
luojianet_ms::HashMap<std::string, std::vector<int>> prims_transparent_pass_sequence{
...
AnfNodePtr MixedPrecisionCastHelper(const AnfNodePtr &source_node, const AbstractBasePtr &node_type,
  ...
  if (node_type->isa<AbstractTensor>()) {
    ...
    if (x->element()->BuildType()->isa<Float>()) {
      auto cast = prim::GetPythonOps("cast", "luojianet_ms.ops.functional");
...
bool CheckType(const TypePtr &expected_type, const TypePtr &x) {
  // As x and predicate both are luojianet_ms type statically, here we only to judge whether
...
FuncGraphPtr PyObjToGraph(const AnalysisEnginePtr &engine, const ValuePtr &method) {
  ...
  FuncGraphPtr func_graph = luojianet_ms::parse::ConvertToFuncGraph(obj->obj());
...
EvalResultPtr GetEvaluatedValueForBuiltinTypeAttrOrMethod(const AnalysisEnginePtr &engine, const ValuePtr &item_value,
  ...
  if (require.empty()) {
    ...
    if (require.empty()) {
      MS_LOG(EXCEPTION) << "LuoJiaNET not support to get attribute \'" << item_name << "\' of a type["
                        << data_type->ToString() << "]";
    }
  ...
  EvalResultPtr Run(AnalysisEnginePtr engine, const ConfigPtrList &args_conf_list,
    ...
    if (cnode->size() != (args_conf_list.size() + 1)) {
      MS_LOG(EXCEPTION) << "Out_conf node: " << cnode->DebugString()
                        << ", args_conf_list: " << luojianet_ms::ToString(args_conf_list);
    }
...
using PrimitiveToImplMap = luojianet_ms::HashMap<PrimitivePtr, PrimitiveImplInferValue, PrimitiveHasher, PrimitiveEqual>;
