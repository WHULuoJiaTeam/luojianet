...
const char kOpsFunctionModelName[] = "luojianet_ms.ops.functional";
...
std::string GetId(const py::handle &obj) {
  ...
  } else if (py::isinstance<luojianet_ms::Type>(obj)) {
    auto type_ptr = py::cast<luojianet_ms::TypePtr>(obj);
...
void GetDstType(const py::tuple &py_args,
                const luojianet_ms::HashMap<SignatureEnumDType, std::vector<size_t>> &type_indexes,
                luojianet_ms::HashMap<SignatureEnumDType, TypeId> *dst_type) {
...
void PynativeInfer(const PrimitivePyPtr &prim, OpExecInfo *const op_exec_info,
  ...
  MS_EXCEPTION_IF_NULL(prim);
  MS_LOG(DEBUG) << "Prim " << prim->name() << " input infer " << luojianet_ms::ToString(args_spec_list);
...
bool RunOpConvertConstInputToAttr(const py::object &input_object, size_t input_index, const PrimitivePtr &op_prim,
                                  const luojianet_ms::HashSet<size_t> &input_attrs, std::vector<size_t> *input_to_attr_idx,
...
void ConstructInputTensor(const OpExecInfoPtr &op_run_info, std::vector<int64_t> *tensors_mask,
  ...
  if (op_run_info->op_name == prim::kPrimCustom->name()) {
    ...
    luojianet_ms::HashSet<size_t> attr_indexes;
...
ValuePtr ShallowCopyValue(const OpExecInfoPtr &op_exec_info, const ValuePtr &value) {
  ...
  if (value->isa<luojianet_ms::tensor::Tensor>()) {
    auto tensor_value = value->cast<luojianet_ms::tensor::TensorPtr>();
    return std::make_shared<luojianet_ms::tensor::Tensor>(tensor_value->data_type(), new_shape->shape(),
...
void ForwardExecutor::GetOpOutputAbstract(const OpExecInfoPtr &op_exec_info,
  ...
  if (temp != prim_abs_list_.end()) {
    MS_LOG(DEBUG) << "Match prim input args " << op_name << luojianet_ms::ToString(args_spec_list);
...
void ForwardExecutor::DoSignatureCast(const PrimitivePyPtr &prim,
                                      const luojianet_ms::HashMap<SignatureEnumDType, TypeId> &dst_type,
...
AnfNodePtr GradExecutor::GetInput(const py::object &obj, bool op_mask) {
  ...
  if (curr_graph_info->node_map.find(obj_id) != curr_graph_info->node_map.end()) {
    ...
    SetNodeMapInGraphInfoMap(curr_g(), GetId(obj), cnode);
    node = cnode;
  } else {
...
void GradExecutor::SaveForwardTensorInfoInBpropGraph(const pipeline::ResourcePtr &resource) const {
  ...
  luojianet_ms::HashSet<std::string> forward_op_tensor_id;
...
void GradExecutor::InitResourceAndDfBuilder(const std::string &cell_id, const py::args &args) {
  if (cell_stack_.empty() || IsNestedGrad()) {
    if (cell_stack_.empty() && !grad_is_running_) {
      ...
      top_cell()->graph_info_map()[fg] = graph_info_cg;
  ...
  if (!top_cell()->is_init_kpynative()) {
    auto graph_info_cg = std::make_shared<GraphInfo>(cell_id);
    top_cell()->graph_info_map()[curr_g()] = graph_info_cg;
...
void GradExecutor::CheckNeedCompileGraph() {
  ...
  if (pre_all_op_info != new_all_op_info) {
    ...
    if (!cell_stack_.empty()) {
      pre_top_cell->graph_info_map()[pre_top_cell->df_builder()] =
        new_top_cell->graph_info_map()[new_top_cell->df_builder()];
    }
...
void GradExecutor::DoParameterReplace(const FuncGraphPtr &first_grad_fg, const py::tuple &forward_args,
  ...
  luojianet_ms::HashSet<std::string> params_weights_set;
  luojianet_ms::HashSet<std::string> params_inputs_set;
