...
#ifndef LUOJIANET_MS_CCSRC_PIPELINE_PYNATIVE_PYNATIVE_EXECUTE_H_
#define LUOJIANET_MS_CCSRC_PIPELINE_PYNATIVE_PYNATIVE_EXECUTE_H_
...
using OpInfoWithTensorId = luojianet_ms::HashMap<std::string, std::vector<std::string>>;
using TensorIdWithTensorObject = luojianet_ms::HashMap<std::string, std::vector<tensor::TensorPtr>>;
using OpInfoWithMsFuncForwardTensors = luojianet_ms::HashMap<std::string, std::vector<tensor::TensorPtr>>;
using CellIdWithBackwardHookOp = luojianet_ms::HashMap<std::string, std::vector<AnfNodePtr>>;
...
struct GraphInfo {
  ...
  luojianet_ms::HashMap<std::string, std::pair<AnfNodePtr, std::vector<int64_t>>> node_map;
  GraphInfo() = default;
...
class TopCellInfo {
 public:
  ...
  luojianet_ms::HashSet<std::string> &sub_cell_list() { return sub_cell_list_; }
 ...
 private:
  ...
  luojianet_ms::HashSet<std::string> sub_cell_list_;
  ...
  luojianet_ms::HashSet<std::string> sub_cell_hook_changed_;
...
class GradExecutor {
 ...
 private:
  ...
  luojianet_ms::HashMap<std::string, TopCellInfoPtr> already_run_top_cell_;
  // Use vector for keep order
...
class ForwardExecutor {
 public:
  ...
  luojianet_ms::HashMap<std::string, abstract::AbstractBasePtr> &node_abs_map() { return node_abs_map_; }
  void ClearRes();
 ...
 private:
  ...
  void DoSignatureCast(const PrimitivePyPtr &prim, const luojianet_ms::HashMap<SignatureEnumDType, TypeId> &dst_type,
 ...
 private:
  ...
  luojianet_ms::HashMap<std::string, abstract::AbstractBasePtr> node_abs_map_;
...
#endif  // LUOJIANET_MS_CCSRC_PIPELINE_PYNATIVE_PYNATIVE_EXECUTE_H_
