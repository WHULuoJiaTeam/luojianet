...
void DumpFusionScopeInfo(const kernel::FusionScopeInfo &info) {
  ...
  for (auto &node : info.compute_nodes) {
    MS_LOG(INFO) << "=== Compute: (" << node->DebugString() << ")-("
                 << luojianet_ms::kernel::GetFusionNameByType(AnfAlgo::GetFusionType(node)) << ")";
  }
...
void ReplaceInputNodeInOtherFusionScope(luojianet_ms::HashMap<int64_t, BufferFusionInfo_t> *buffer_fusion_infos,
...
void ReplaceOldNode(luojianet_ms::HashMap<int64_t, BufferFusionInfo_t> *buffer_fusion_infos, int64_t fusion_id,
...
void GetFusionScopeComputeNodeList(const session::KernelGraph *kernel_graph,
                                   luojianet_ms::HashMap<int64_t, BufferFusionInfo_t> *buffer_fusion_infos) {
  MS_EXCEPTION_IF_NULL(buffer_fusion_infos);
  MS_EXCEPTION_IF_NULL(kernel_graph);
...
void SetOutputUsedNumAttr(const session::KernelGraph &kernel_graph,
                          const luojianet_ms::HashMap<int64_t, BufferFusionInfo_t> &buffer_fusion_infos) {
...
bool CheckCircle(const session::KernelGraph &kernel_graph, const BufferFusionInfo_t &fusion_info) {
  ...
  luojianet_ms::HashSet<AnfNodePtr> visited_nodes;
...
bool UbPatternFusion::FuseBufferFusionPattern(session::KernelGraph *kernel_graph) const {
  ...
  std::vector<luojianet_ms::kernel::FusionScopeInfo> fusion_scope_infos;
  (void)std::transform(buffer_fusion_infos.begin(), buffer_fusion_infos.end(), std::back_inserter(fusion_scope_infos),
                       [](const auto &buffer_fusion_info) -> luojianet_ms::kernel::FusionScopeInfo {
                         return luojianet_ms::kernel::FusionScopeInfo(
...
bool UbPatternFusion::ReplaceFusionOp(luojianet_ms::HashMap<int64_t, BufferFusionInfo_t> *buffer_fusion_infos,
