...
bool SyncBatchNormFusion::CreateOutputsOfBNTrainingReduce(const FuncGraphPtr &graph, const CNodePtr &bn_cnode,
...
AnfNodePtr SyncBatchNormFusion::CreateOutputsOfBNTrainingUpdate(
  const FuncGraphPtr &graph, const CNodePtr &bn_cnode, const std::vector<AnfNodePtr> &bn_training_reduce_outputs,
  bool is_dynamic) const {
  MS_EXCEPTION_IF_NULL(graph);
  MS_EXCEPTION_IF_NULL(bn_cnode);
...
AnfNodePtr SyncBatchNormFusion::SyncBNFusionForGPU(const FuncGraphPtr &func_graph, const AnfNodePtr &node) const {
  MS_EXCEPTION_IF_NULL(func_graph);
  MS_EXCEPTION_IF_NULL(node);
  std::cout<<"==============SyncBNFusionForGPU=====================";
...
const BaseRef SyncBatchNormFusion::DefinePattern() const {
  VarPtr Xs = std::make_shared<SeqVar>();
  return VectorRef({prim::kPrimSyncBatchNorm, Xs});
}

const AnfNodePtr SyncBatchNormFusion::Process(const FuncGraphPtr &func_graph, const AnfNodePtr &node,
                                              const EquivPtr &) const {
  return SyncBNFusionForGPU(func_graph, node);
}
}  // namespace opt
}  // namespace luojianet_ms
