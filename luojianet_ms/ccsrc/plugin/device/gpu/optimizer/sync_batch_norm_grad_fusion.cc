...
namespace opt {
void SyncBatchNormGradFusion::CreateOutputsOfUpdateGrad(const FuncGraphPtr &graph, const CNodePtr &bn_grad_node,
...
void SyncBatchNormGradFusion::CreateOutputsOfReduceGrad(const FuncGraphPtr &graph, const CNodePtr &bn_grad_node,
...
CNodePtr SyncBatchNormGradFusion::SyncBNGradFusionForGPU(const FuncGraphPtr &func_graph, const CNodePtr &cnode) const {
...
const BaseRef SyncBatchNormGradFusion::DefinePattern() const {
  VarPtr Xs = std::make_shared<SeqVar>();
  return VectorRef({prim::kPrimSyncBatchNormGrad, Xs});
}

const AnfNodePtr SyncBatchNormGradFusion::Process(const FuncGraphPtr &func_graph, const AnfNodePtr &node,
                                                  const EquivPtr &) const {
  MS_EXCEPTION_IF_NULL(node);
  auto cnode = node->cast<CNodePtr>();
  return SyncBNGradFusionForGPU(func_graph, cnode);
}
}  // namespace opt
}  // namespace luojianet_ms
