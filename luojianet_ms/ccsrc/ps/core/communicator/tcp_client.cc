...
void TcpClient::Start() {
  ...
  MSLOG_IF(luojianet_ms::ERROR, ret == 1, NoExceptionType)
    << "Event base dispatch failed with no events pending or active!";
  MSLOG_IF(luojianet_ms::ERROR, ret == -1, NoExceptionType) << "Event base dispatch failed with error occurred!";
  MSLOG_IF(luojianet_ms::EXCEPTION, ret < -1, AbortedError) << "Event base dispatch with unexpected error code!";
}
...
void TcpClient::StartWithNoBlock() {
  ...
  MSLOG_IF(luojianet_ms::ERROR, ret == 1, NoExceptionType) << "Event base loop failed with no events pending or active!";
  MSLOG_IF(luojianet_ms::ERROR, ret == -1, NoExceptionType) << "Event base loop failed with error occurred!";
  MSLOG_IF(luojianet_ms::EXCEPTION, ret < -1, AbortedError) << "Event base loop with unexpected error code!";
}
