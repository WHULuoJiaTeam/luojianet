...
bool ParameterServer::Init(const FuncGraphPtr &func_graph) {
  pserver_num_ = std::strtol(luojianet_ms::common::GetEnv(kEnvPServerNum).c_str(), nullptr, kBase);
  worker_num_ = std::strtol(luojianet_ms::common::GetEnv(kEnvWorkerNum).c_str(), nullptr, kBase);
