...
#ifndef LUOJIANET_MS_CCSRC_PS_WORKER_H_
#define LUOJIANET_MS_CCSRC_PS_WORKER_H_
...
class BACKEND_EXPORT Worker {
 ...
 private:
  ...
  void PrepareSparseGradient(const size_t begin, const size_t end, const luojianet_ms::HashSet<int> &distinct_ids,
  ...
  luojianet_ms::HashMap<Key, int64_t> key_to_server_id_;
  luojianet_ms::HashMap<Key, size_t> embedding_row_cnt_;

  luojianet_ms::HashMap<Key, std::shared_ptr<std::vector<EmbeddingTableShardMetadata>>> embedding_table_ranges_;
};
}  // namespace ps
}  // namespace luojianet_ms
#endif  // LUOJIANET_MS_CCSRC_PS_WORKER_H_
