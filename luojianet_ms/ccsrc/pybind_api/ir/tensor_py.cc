...
REGISTER_PYBIND_DEFINE(Tensor, ([](const py::module *m) {
                         ...
                         (void)py::class_<Tensor, MetaTensor, std::shared_ptr<Tensor>>(*m, "Tensor")
                           ...
                           .def_property_readonly("_dtype", &Tensor::Dtype, R"mydelimiter(
                             ...
                             Examples:
                                 >>> data = luojianet_ms.Tensor(np.ones((2, 1), np.int32))
                                 >>> data.dtype
                                 Int32
                             )mydelimiter")
                           .def_property_readonly("_shape", TensorPy::GetPyTupleShape, R"mydelimiter(
                             ...
                             Examples:
                                 >>> data = luojianet_ms.Tensor(np.ones((3, 3)))
                                 >>> data.shape()
                                 (3, 3)
                             )mydelimiter")
                           .def_property_readonly("_size", &Tensor::DataSize, R"mydelimiter(
                             ...
                             Examples:
                                 >>> data = luojianet_ms.Tensor(np.ones((2, 3)))
                                 >>> data.size
                                 6
                             )mydelimiter")
                           ...
                           .def("from_numpy", TensorPy::MakeTensorOfNumpy, R"mydelimiter(
                             ...
                             Examples:
                                 >>> a = np.ones((2, 3))
                                 >>> t = luojianet_ms.Tensor.from_numpy(a)
                             )mydelimiter")
                           ...
                           .def("assign_value_cpp", &Tensor::AssignValue, R"mydelimiter(
                             ...
                             Arg:
                                 value (:class:`luojianet_ms.tensor`): The value tensor.

                             Examples:
                                 >>> data = luojianet_ms.Tensor(np.ones((1, 2), np.float32))
                                 >>> data2 = luojianet_ms.Tensor(np.ones((2, 2), np.float32))
                           ...
                           .def("set_dtype", &Tensor::SetDtype, R"mydelimiter(
                              ...
                              Arg:
                                  dtype (:class:`luojianet_ms.dtype`): The type of output tensor.

                              Examples:
                                  >>> data = luojianet_ms.Tensor(np.ones((1, 2), np.float32))
                                  >>> data.set_dtype(luojianet_ms.int32)
                                  luojianet_ms.int32
                              )mydelimiter")
