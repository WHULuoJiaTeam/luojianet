...
bool InitRandomNormal(std::vector<int64_t> out_shape, int64_t seed, int64_t seed2, const py::object &output_tensor) {
  ...
  auto temp = py::cast<std::shared_ptr<luojianet_ms::tensor::Tensor>>(output_tensor);
  ...
  luojianet_ms::PhiloxGenerator generator = luojianet_ms::PhiloxGenerator(seed, seed2);
  float *offset_ptr = nullptr;
  if (thread_num != 1) {
    for (uint32_t i = 0; i < thread_num - 1; i++) {
      offset_ptr = start_ptr + batchSize * i;
      threads[i] = std::thread(luojianet_ms::FillRandoms<luojianet_ms::NormalDistribution<luojianet_ms::PhiloxGenerator, float>>,
                               generator, offset_ptr, batchSize, i);
    }
    offset_ptr = start_ptr + batchSize * (thread_num - 1);
    threads[thread_num - 1] =
      std::thread(luojianet_ms::FillRandoms<luojianet_ms::NormalDistribution<luojianet_ms::PhiloxGenerator, float>>, generator,
                  offset_ptr, total_count - (thread_num - 1) * batchSize, thread_num - 1);
  } else {
    threads[0] = std::thread(luojianet_ms::FillRandoms<luojianet_ms::NormalDistribution<luojianet_ms::PhiloxGenerator, float>>,
