...
// which is copyright Hamilton 2009-2014. http://www.apache.org/licenses/LICENSE-2.0.
...
//
// The norm of a Vector2 is its "length", the distance along it.
class Vector2
{
 public:
	//
 ...
 public:
	...
	//               as original.
	//
	Vector2(const Vector2& original) : x(original.x), y(original.y) {}
	...
	//  Side Effect: N/A
	//
	bool isAllComponentsLessThanOrEqual(const Vector2& other) const {
		assert(isFinite());
		assert(other.isFinite());
	...
	//  Side Effect: N/A
	//
	bool isAllComponentsGreaterThan(const Vector2& other) const {
		assert(isFinite());
		assert(other.isFinite());
	...
	//  Side Effect: N/A
	//
	bool isAllComponentsGreaterThanOrEqual(const Vector2& other) const {
		assert(isFinite());
		assert(other.isFinite());
	...
	//  Side Effect: N/A
	//
	bool isAllComponentsGreaterThanOrEqual(double value) const {
		assert(isFinite());
	...
	Vector2 componentRatioSafe(const Vector2& other) {
		...
		return Vector2((other.x != 0.0) ? (x / other.x) : x, (other.y != 0.0) ? (y / other.y) : y);
	}
	...
	//  Side Effect: N/A
	//
	bool isDistanceLessThan(const Vector2& other, double distance) const {
		return (getDistanceSquared(other) < distance * distance);
	}
	...
	//  Side Effect: N/A
	//
	bool isDistanceGreaterThan(const Vector2& other, double distance) const {
		return (getDistanceSquared(other) > distance * distance);
	}
	...
	Vector2 getRotated(double radians) const {
		...
		return Vector2(cos_angle * x - sin_angle * y, sin_angle * x + cos_angle * y);
	}
	...
	//  Side Effect: N/A
	//
	static Vector2 getClosestPointOnLine(const Vector2& l1, const Vector2& l2, const Vector2& p, bool bounded);
};
