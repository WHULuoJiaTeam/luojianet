...
class SoftmaxCrossEntropyLoss(nn.Module):
    def __init__(self, num_classes=21, ignore_label=255):
        ...
        self.cast = P.Cast()
        self.crit = nn.SoftmaxCrossEntropyWithLogits()
    ...
    def call(self, logit, label):
        label_int = self.cast(label, mstype.int32)
        label_int = self.reshape(label_int, (-1,))
        logit = self.transpose(logit, (0, 2, 3, 1))
        logit = self.reshape(logit, (-1, self.num_classes))
        weight = self.not_equal(label_int, self.ignore_label)
        weight = self.cast(weight, mstype.float32)
        one_hot_label = self.one_hot(label_int, self.num_classes, self.on_value, self.off_value)
        loss = self.crit(logit, one_hot_label)
        loss = self.mul(weight, loss)
        loss = self.div(self.sum(loss), self.sum(weight))
        return loss
...
def test_net():
    ...
    model = DeepLabV3Plus(phase='train', num_classes=args.num_classes,
                          output_stride=args.output_stride, freeze_bn=args.freeze_bn)
    print(list(model.name_cells()))
    image = Tensor(np.random.rand(2, 3, 513, 513), mstype.float32)
    label = Tensor(np.array([0, 3, 6]), mstype.int32)
