...
void write_pixel(const double& pixelValue, const GDALDataType& gdalType, const int& gdalChannels,
								 Mat& image, const int& row, const int& col, const int& channel) {
	// convert the pixel
...
void write_ctable_pixel(const double& pixelValue, const GDALDataType& gdalType, GDALColorTable const* gdalColorTable,
												Mat& image, const int& y, const int& x, const int& c) {
...
Mat GDAL2CV::gdal_read(const string& filename, int xStart, int yStart, int xWidth, int yWidth) {
	...
	Mat img(yWidth, xWidth, dataType, Scalar::all(0.f));
