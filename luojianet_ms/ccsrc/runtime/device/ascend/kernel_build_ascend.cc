...
void InsertAtomicOpForNormalOp(const luojianet_ms::CNodePtr &pre_node, CleanOpsMap *clean_ops) {
  MS_EXCEPTION_IF_NULL(pre_node);
  MS_EXCEPTION_IF_NULL(clean_ops);
...
std::map<AnfNodePtr, std::vector<size_t>> GetCommunicationOpInputInfo(const std::vector<CNodePtr> &kernels) {
  ...
  for (auto &kernel : kernels) {
    ...
    if (luojianet_ms::session::AnfRuntimeAlgorithm::IsCommunicationOp(kernel)) {
