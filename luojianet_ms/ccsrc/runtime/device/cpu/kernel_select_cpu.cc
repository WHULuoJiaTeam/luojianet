...
void KernelNotSupportException(const AnfNodePtr &kernel_node, const std::vector<TypeId> &input_types,
  ...
  operator_info
    << "is not supported. This error means the current input type is not supported, please refer to the LuoJiaNet "
...
void SetKernelInfo(const CNodePtr &kernel_node) {
  ...
  if (IsPrimitiveCNode(kernel_node, prim::kPrimCustom)) {
    ...
    if (luojianet_ms::kernel::OpLib::FindOp(op_name, kernel::OpImplyType::kCPU) == nullptr) {
      MS_LOG(WARNING) << "Not find operator information for Custom operator[" << op_name << "]. "
                      ...
                      << "please refer to 'luojianet_ms.ops.Custom' at https://www.luojianet_ms.cn.";
  ...
  if (kernel_attrs.empty() || (kernel_attrs[0].GetInputSize() == 0 && kernel_attrs[0].GetOutputSize() == 0)) {
    ...
    auto op_info_ptr = luojianet_ms::kernel::OpLib::FindOp(op_name, kernel::OpImplyType::kCPU);
    if (op_info_ptr == nullptr) {
      MS_LOG(EXCEPTION) << "Not find op[" << op_name << "] in cpu. For more details, "
                        << "please refer to the list of supported cpu operations at https://www.luojianet_ms.cn.";
    }
    kernel_attrs.clear();
