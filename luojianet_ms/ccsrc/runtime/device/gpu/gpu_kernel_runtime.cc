...
namespace device {
namespace gpu {
using luojianet_ms::device::memswap::MemSwapInfoSet;
using luojianet_ms::device::memswap::MemSwapManager;
using luojianet_ms::device::memswap::SwapKind;
...
void LoadKernelData(Debugger *debugger, const CNodePtr &kernel,
                    const std::vector<luojianet_ms::kernel::AddressPtr> &kernel_inputs,
                    const std::vector<luojianet_ms::kernel::AddressPtr> &kernel_workspaces,
                    const std::vector<luojianet_ms::kernel::AddressPtr> &kernel_outputs, int exec_order, void *stream_ptr,
...
bool GPUKernelRuntime::IsGraphOutput(const session::KernelGraph *graph, const luojianet_ms::AnfNodePtr &kernel) const {
  MS_EXCEPTION_IF_NULL(graph);
  auto graph_id = graph->graph_id();
...
bool GPUKernelRuntime::LaunchKernelDynamic(const session::KernelGraph *graph, bool mock, bool profiling) {
  ...
  (void)luojianet_ms::RDR::RecordMemAddressInfo(SubModuleId::SM_KERNEL, name);
#endif
  ...
  for (const auto &kernel : kernels) {
    ...
    (void)luojianet_ms::RDR::UpdateMemAddress(SubModuleId::SM_KERNEL, name, op_name, mem_info);
#endif
    if (!mock) {
...
bool GPUKernelRuntime::AllocKernelDynamicRes(const luojianet_ms::kernel::KernelMod &kernel_mod,
                                             const luojianet_ms::AnfNodePtr &kernel, AddressPtrList *kernel_inputs,
...
bool GPUKernelRuntime::AllocKernelInputDynamicRes(const luojianet_ms::AnfNodePtr &kernel, AddressPtrList *kernel_inputs,
                                                  bool mock) {
  MS_EXCEPTION_IF_NULL(kernel);
...
bool GPUKernelRuntime::AllocKernelOutputDynamicRes(const luojianet_ms::kernel::KernelMod &kernel_mod,
                                                   const luojianet_ms::AnfNodePtr &kernel, AddressPtrList *kernel_outputs,
                                                   bool mock) {
  MS_EXCEPTION_IF_NULL(kernel);
...
bool GPUKernelRuntime::AllocKernelWorkspaceDynamicRes(const luojianet_ms::kernel::KernelMod &kernel_mod,
                                                      const luojianet_ms::AnfNodePtr &kernel,
...
void GPUKernelRuntime::AllocCommunicationOpInputDynamicRes(const luojianet_ms::AnfNodePtr &kernel) {
  MS_EXCEPTION_IF_NULL(kernel);
  MS_EXCEPTION_IF_NULL(mem_reuse_util_);
...
void GPUKernelRuntime::AllocCommunicationOpOutputDynamicRes(const luojianet_ms::AnfNodePtr &kernel) {
  MS_EXCEPTION_IF_NULL(kernel);
  bool is_need_alloc_memory = false;
  bool is_need_free_memory = false;
  size_t total_size = 0;
...
void GPUKernelRuntime::FreeKernelDynamicRes(const luojianet_ms::AnfNodePtr &kernel) {
  MS_EXCEPTION_IF_NULL(kernel);
  MS_EXCEPTION_IF_NULL(mem_manager_);
  MS_EXCEPTION_IF_NULL(mem_reuse_util_);
