...
void MemoryManager::MallocSomasDynamicMem(const session::KernelGraph &graph) {
  ...
  (void)luojianet_ms::RDR::RecordString(module, name, somas_reuse_util_ptr_->SomasInfo());

  name = "somas_mem_info." + std::to_string(graph.graph_id());
  (void)luojianet_ms::RDR::RecordString(module, name, somas_reuse_util_ptr_->SomasMemory());
#endif
