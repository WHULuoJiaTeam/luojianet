...
#ifndef LUOJIANET_MS_CCSRC_RUNTIME_DEVICE_TENSOR_ARRAY_H_
#define LUOJIANET_MS_CCSRC_RUNTIME_DEVICE_TENSOR_ARRAY_H_
...
class TensorArray {
 public:
  ...
  virtual bool Write(const int64_t index, const luojianet_ms::kernel::AddressPtr &dev_value);

  // Function Read() can get the tensors in the scope of tensors_.
  virtual luojianet_ms::kernel::AddressPtr Read(const int64_t index);
 ...
 protected:
  ...
  std::vector<luojianet_ms::kernel::AddressPtr> tensors_;
};
using TensorArrayPtr = std::shared_ptr<TensorArray>;
}  // namespace device
}  // namespace luojianet_ms

#endif  // LUOJIANET_MS_CCSRC_RUNTIME_DEVICE_TENSOR_ARRAY_H_
