...
using GroupNameToCommuNodes =
  luojianet_ms::HashMap<std::string, std::pair<std::vector<CNodePtr>, std::vector<KernelGraphPtr>>>;
...
class ControlNodeParser {
 ...
 private:
  ...
  // stack to d.
  luojianet_ms::HashMap<AnfNodePtr, size_t> node_to_level_;
  ...
  luojianet_ms::HashMap<FuncGraphPtr, std::set<KernelGraphGroupInfoPtr>> func_graph_to_first_kernel_graphs_;
