...
enum OpMergeMode {
  ...
  OP_MERGE_CONV2D_TRANSPOSE = 7,     // indicate `LuoJiaNET ConvTranspose + BiasAdd` --> `ONNX ConvTranspose`
  OP_MERGE_CONCAT = 8,               // indicate `LuoJiaNET MakeTuple + Concat` --> `ONNX Concat`
};
...
/*
  See:
    - luojianet_ms/ccsrc/backend/kernel_compiler/cpu/stridedslice_cpu_kernel.cc
    - luojianet_ms/ccsrc/backend/kernel_compiler/common_utils.cc
 */
...
// Based on luojianet_ms/ccsrc/backend/kernel_compiler/cpu/boundingbox_decode_cpu_kernel.cc
...
/*
  Based on luojianet_ms-project/luojianet_ms/ccsrc/backend/kernel_compiler/cpu/roi_align_cpu_kernel.cc
  Notes:
...
void OnnxExporter::ExportPrimTensorCopySlices(const FuncGraphPtr &func_graph, const CNodePtr &node,
  ...
  // LuoJiaNET only allows contuguous slices of memory
