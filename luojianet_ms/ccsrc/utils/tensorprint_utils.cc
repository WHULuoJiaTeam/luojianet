...
bool PrintTensorToString(const char *str_data_ptr, luojianet_ms::tensor::Tensor *const print_tensor,
...
bool ConvertDataset2Tensor(acltdtDataset *acl_dataset) {
  ...
  for (size_t i = 0; i < acl_dataset_size; i++) {
    ...
    if (acl_data_type == ACL_STRING) {
      ...
      auto type_id = type_iter->second;
      luojianet_ms::tensor::Tensor print_tensor(type_id, tensorShape);
