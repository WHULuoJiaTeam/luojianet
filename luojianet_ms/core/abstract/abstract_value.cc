...
namespace abstract {
using luojianet_ms::common::IsEqual;

AnfNodePtr GetTraceNode(const AbstractBasePtr &abs) {
  AnfNodePtr node = nullptr;
  if (luojianet_ms::abstract::AbstractBase::trace_node_provider_ != nullptr) {
    luojianet_ms::abstract::AbstractBase::trace_node_provider_(&node);
  }
  return node;
}
...
ValuePtr AbstractClass::RealBuildValue() const {
  ...
  luojianet_ms::HashMap<std::string, ValuePtr> attributes_value_map;
  for (const auto &attr : attributes_) {
    MS_EXCEPTION_IF_NULL(attr.second);
