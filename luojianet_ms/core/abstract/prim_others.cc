...
inline void CheckSparseShape(ShapeVector sparse_shp, ShapeVector dense_shp) {
  ...
  int dlen = luojianet_ms::SizeToInt(sparse_shp.size()) - luojianet_ms::SizeToInt(dense_shp.size());
  if (dlen < 0) {
    MS_EXCEPTION(luojianet_ms::ValueError) << "Currently, only support dense tensor broadcast to sparse tensor, "
  ...
  for (size_t i = 0; i < sparse_shp.size(); i++) {
    ...
    if (i < kCSRMulBatchPos) {
      if (d != s && d != 1) {
        MS_EXCEPTION(luojianet_ms::ValueError) << "Dense shape cannot broadcast to sparse shape.";
      }
    } else {
      if (d != s) {
        MS_EXCEPTION(luojianet_ms::ValueError)
...
inline void CheckSparseShape(const size_t shape_size, const size_t expected_dim, const std::string &arg_name) {
  if (shape_size != expected_dim) {
    MS_EXCEPTION(luojianet_ms::ValueError) << arg_name << " must be a " << expected_dim
                                        << "-dimensional tensor, but got a " << shape_size << "-dimensional tensor.";
  }
}
inline void CheckSparseIndicesDtype(const luojianet_ms::TypePtr data_type, const std::string &arg_name) {
  if (!(data_type->equal(luojianet_ms::kInt16) || data_type->equal(luojianet_ms::kInt32) ||
        data_type->equal(luojianet_ms::kInt64))) {
    MS_EXCEPTION(luojianet_ms::TypeError) << "The dtype of " << arg_name << " must be Int16 or Int32 or Int64, but got "
                                       << data_type->ToString() << ".";
  }
}
inline void CheckSparseIndicesDtypeInt32(const luojianet_ms::TypePtr data_type, const std::string &arg_name) {
  if (!data_type->equal(luojianet_ms::kInt32)) {
    MS_EXCEPTION(luojianet_ms::TypeError) << "The dtype of " << arg_name << " only support Int32 for now, but got "
                                       << data_type->ToString() << ".";
  }
}
}  // namespace
