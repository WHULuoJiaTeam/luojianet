...
#ifndef LUOJIANET_MS_CORE_IR_ANF_H_
#define LUOJIANET_MS_CORE_IR_ANF_H_
...
// A LuoJiaNet ANF IR defined here.
// with BNF followed:
...
using PrimitiveSet = luojianet_ms::HashSet<PrimitivePtr, PrimitiveHasher, PrimitiveEqual>;
...
class MS_CORE_API CNode final : public AnfNode, public EffectInfoHolder {
 public:
  ...
  const luojianet_ms::HashMap<std::string, ValuePtr> &primal_attrs() const { return primal_attrs_; }

  /// \brief Set the primal attributes of this CNode.
  ///
  /// \param[in] attrs The primal attributes.
  void set_primal_attrs(const luojianet_ms::HashMap<std::string, ValuePtr> &attrs) {
...
using TaggedNodeMap = luojianet_ms::HashMap<AnfNodePtr, size_t>;
...
#endif  // LUOJIANET_MS_CORE_IR_ANF_H_
