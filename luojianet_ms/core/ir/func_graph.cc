...
std::string FuncGraph::ToString() const {
  ...
  buffer << luojianet_ms::label_manage::Label(debug_info);
...
const luojianet_ms::HashMap<AnfNodePtr, int> &FuncGraph::j_value_nodes() const { return j_value_nodes_; }
...
std::list<CNodePtr> FuncGraph::GetOrderedCnodes() {
  ...
  auto nodes = luojianet_ms::TopoSort(return_node(), SuccDepends, BelongSameGraph);
...
bool FuncGraph::ContainMultiTarget() const {
  ...
  for (auto &g : graphs) {
    auto nodes = luojianet_ms::TopoSort(g->get_return());
    if (luojianet_ms::ContainMultiTarget(nodes)) {
      return true;
    }
  }
  return false;
...
std::vector<AnfNodePtr> api::FuncGraph::TopoSort(const AnfNodePtr &node) { return luojianet_ms::TopoSort(node); }

// Create an api::FuncGraph instance.
api::FuncGraphPtr api::FuncGraph::Create() { return std::make_shared<luojianet_ms::FuncGraph>(); }

AnfNodePtr api::FuncGraph::MakeValueNode(const api::FuncGraphPtr &func_graph) {
  auto fg = std::dynamic_pointer_cast<luojianet_ms::FuncGraph>(func_graph);
  return NewValueNode(fg);
}

api::FuncGraphPtr api::FuncGraph::GetFuncGraphFromAnfNode(const AnfNodePtr &input) {
  auto fg = GetValueNode<luojianet_ms::FuncGraphPtr>(input);
  return fg;
}
