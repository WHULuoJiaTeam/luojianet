...
api::FuncGraphManagerPtr api::FuncGraphManager::Manage(const api::FuncGraphPtr &func_graph, bool manage) {
  return luojianet_ms::Manage(std::dynamic_pointer_cast<luojianet_ms::FuncGraph>(func_graph), manage);
}
...
FuncGraphSetPtr FuncGraphParentsTotalComputer::SeekParents(
  const FuncGraphPtr &fg, luojianet_ms::HashMap<FuncGraphPtr, FuncGraphSetPtr> *seen_fgs) {
...
void FuncGraphParentsTotalComputer::RealRecompute(FuncGraphPtr fg) {
  MS_EXCEPTION_IF_NULL(fg);
  luojianet_ms::HashMap<FuncGraphPtr, FuncGraphSetPtr> seen_fgs;
  fg->seen_ = 1;
