...
FuncGraphPtr MetaFuncGraph::GenerateFuncGraph(const abstract::AbstractBasePtrList &args_spec_list) {
  ...
  if (iter == cache_.end()) {
    ...
    MS_LOG(INFO) << "MetaFuncgraph: cache miss for types: " << luojianet_ms::ToString(args_spec_list)
                 << ", g: " << fg->ToString();
    cache_[types] = fg;
    return fg;
  } else {
    MS_LOG(DEBUG) << "MetaFuncgraph: cache hit for types: " << luojianet_ms::ToString(args_spec_list)
