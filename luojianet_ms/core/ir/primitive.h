...
#ifndef LUOJIANET_MS_CORE_IR_PRIMITIVE_H_
#define LUOJIANET_MS_CORE_IR_PRIMITIVE_H_
...
/// \brief Primitive defines a operator primitive of LuoJiaNet.
class MS_CORE_API Primitive : public Named {
 public:
  ...
  Primitive(const std::string &name, const luojianet_ms::HashMap<std::string, ValuePtr> &attrs);
  ...
  /// \return The primitive to which attribute has been added.
  Primitive &SetAttrs(const luojianet_ms::HashMap<std::string, ValuePtr> &attrs) {
    for (auto &attr : attrs) {
  ...
  const luojianet_ms::HashMap<std::string, ValuePtr> &attrs() const { return attrs_; }
  /// \brief Get the attributes added in LuoJiaNet renormalize stage.
  ///
  /// \return Attributes which have been added in LuoJiaNet renormalize stage.
  const luojianet_ms::HashMap<std::string, ValuePtr> &evaluate_added_attrs() const { return evaluate_added_attrs_; }
  /// \brief Use add attribute using a map,all elements of the map will be added in the primitive's attribute map.
  ///
  /// \param[in] attrs The attribute map needs to be added in the primitive attribute.
  void set_evaluate_added_attrs(const luojianet_ms::HashMap<std::string, ValuePtr> &attrs) {
    for (auto &attr : attrs) {
 ...
 protected:
  ...
  luojianet_ms::HashMap<std::string, ValuePtr> evaluate_added_attrs_;
...
#endif  // LUOJIANET_MS_CORE_IR_PRIMITIVE_H_
