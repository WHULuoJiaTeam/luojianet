...
static luojianet_ms::HashMap<int, TypeId> kDefaultValueSwitchMap{
...
std::shared_ptr<T> ParserAttr(const std::string &str, const luojianet_ms::HashMap<string, P> &kv) {
...
template <typename T>
std::shared_ptr<T> ParserScalarAttrValue(const std::string &attr_name, const luojianet_ms::HashMap<string, ValuePtr> &kv) {
...
std::shared_ptr<abstract::AbstractTuple> ParserAttrShape(
  const std::string &attr_name, const luojianet_ms::HashMap<string, abstract::AbstractBasePtr> &kv) {
  std::string str = attr_name;
  auto replace = [&](const string &orgStr, const string &newStr) {
    std::string::size_type pos(0);
    while ((pos = str.find(orgStr)) != std::string::npos) {
...
bool MSANFModelParser::SetNodeAbstractFromAttrProto(const mind_ir::AttributeProto &attr_proto,
  ...
  luojianet_ms::HashMap<std::string, abstract::AbstractBasePtr> kv;
...
void MSANFModelParser::ObtainCNodeAttrInScalarForm(const mind_ir::AttributeProto &attr_proto,
                                                   luojianet_ms::HashMap<std::string, ValuePtr> *multi_value_map) {
  string name;
...
luojianet_ms::HashMap<std::string, abstract::AbstractBasePtr> MSANFModelParser::GetAbstractForNode(
  const mind_ir::AttributeProto &attr_proto) {
  luojianet_ms::HashMap<std::string, abstract::AbstractBasePtr> kv;
  for (int i = 0; i < attr_proto.tensors_size(); ++i) {
