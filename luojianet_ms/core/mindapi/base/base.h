...
#ifndef LUOJIANET_MS_CORE_MINDAPI_BASE_BASE_H_
#define LUOJIANET_MS_CORE_MINDAPI_BASE_BASE_H_
...
class MIND_API Base {
 public:
  ...
  explicit Base(const std::shared_ptr<luojianet_ms::Base> &impl);
  ...
  const std::shared_ptr<luojianet_ms::Base> &impl() const { return impl_; }
 ...
 protected:
  ...
  const std::shared_ptr<luojianet_ms::Base> impl_;
};

#define MIND_API_BASE_MEMBER(current_class)                             \
  explicit current_class(const std::shared_ptr<luojianet_ms::Base> &impl); \
  ~current_class() override = default;                                  \
  static uint32_t ClassId()

using BasePtr = SharedPtr<Base>;
}  // namespace luojianet_ms::api
#endif  // LUOJIANET_MS_CORE_MINDAPI_BASE_BASE_H_
