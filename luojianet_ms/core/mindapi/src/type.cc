...
using TensorTypeImpl = luojianet_ms::TensorType;
...
TypePtr Type::GetType(TypeId id) {
  auto type_impl = luojianet_ms::TypeIdToType(id);
  return ToWrapper<Type>(type_impl);
}

size_t Type::GetSize(TypeId id) { return luojianet_ms::abstract::TypeIdSize(id); }
