...
#ifndef LUOJIANET_MS_CORE_MINDRT_INCLUDE_ACTOR_ACTOR_H
#define LUOJIANET_MS_CORE_MINDRT_INCLUDE_ACTOR_ACTOR_H
...
class MS_CORE_API ActorBase {
 ...
 protected:
  ...
  template <typename T>
  void Receive(const std::string &msgName, void (T::*method)(luojianet_ms::AID, std::string &&, std::string &&)) {
  ...
  // register the message handle
  template <typename T>
  void Receive(const std::string &msgName, void (T::*method)(const luojianet_ms::AID &, std::string &&, std::string &&)) {
  ...
  void ReceiveUdp(const std::string &msgName,
                  void (T::*method)(const luojianet_ms::AID &, std::string &&, std::string &&)) {
 ...
 private:
  ...
  template <typename T>
  static void BehaviorBase(T *t, void (T::*method)(const luojianet_ms::AID &, std::string &&, std::string &&),
  ...
  // register the message handle. It will be discarded.
  template <typename T>
  static void BehaviorBase1(T *t, void (T::*method)(luojianet_ms::AID, std::string &&, std::string &&),
                            const std::unique_ptr<MessageBase> &msg) {
    MINDRT_OOM_EXIT(msg);
    if (msg->type != MessageBase::Type::KMSG) {
      MS_LOG(ERROR) << "Drop non-tcp message:  from:" << std::string(msg->from).c_str()
  ...
  template <typename T>
  static void BehaviorBaseForUdp(T *t, void (T::*method)(const luojianet_ms::AID &, std::string &&, std::string &&),
                                 const std::unique_ptr<MessageBase> &msg) {
    MINDRT_OOM_EXIT(msg);
