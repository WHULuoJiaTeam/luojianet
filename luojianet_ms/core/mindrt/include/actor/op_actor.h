...
#ifndef LUOJIANET_MS_CORE_MINDRT_INCLUDE_ACTOR_OP_ACTOR_H
#define LUOJIANET_MS_CORE_MINDRT_INCLUDE_ACTOR_OP_ACTOR_H
...
class OpActor : public ActorBase {
 ...
 protected:
  // The op data.
  luojianet_ms::HashMap<int, std::vector<OpData<T> *>> input_op_datas_;
  std::vector<DataArrowPtr> output_data_arrows_;

  // The op controls.
  luojianet_ms::HashMap<int, std::vector<AID *>> input_op_controls_;
...
Future<std::list<int>> MindrtAsyncRun(const std::vector<OpDataPtr<T>> &input_data, OpContext<T> *context) {
  ...
  Future<std::list<int>> collect = luojianet_ms::Collect<int>(futures);

  for (auto data : input_data) {
    Async(data->op_id_, &luojianet_ms::OpActor<T>::RunOpData, data.get(), context);
  }
...
#endif  // LUOJIANET_MS_CORE_MINDRT_INCLUDE_ACTOR_OP_ACTOR_H
