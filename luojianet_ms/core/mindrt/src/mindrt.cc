...
int MindrtInitializeC(const struct MindrtConfig *config) {
  ...
  luojianet_ms::SetHttpKmsgFlag(config->httpKmsgFlag);

  return luojianet_ms::Initialize(std::string(config->tcpUrl), std::string(config->tcpUrlAdv), std::string(config->udpUrl),
                               std::string(config->udpUrlAdv), config->threadCount);
}

void MindrtFinalizeC() { luojianet_ms::Finalize(); }
}
...
void TerminateAll() { luojianet_ms::ActorMgr::GetActorMgrRef()->TerminateAll(); }
...
void Finalize() {
  ...
  luojianet_ms::ActorMgr::GetActorMgrRef()->Finalize();
...
void SetDelegate(const std::string &delegate) { luojianet_ms::ActorMgr::GetActorMgrRef()->SetDelegate(delegate); }
