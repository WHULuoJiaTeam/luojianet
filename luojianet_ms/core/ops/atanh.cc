...
abstract::ShapePtr AtanhInferShape(const PrimitivePtr &primitive, const std::vector<AbstractBasePtr> &input_args) {
  ...
  (void)CheckAndConvertUtils::CheckInteger("Atanh input number", SizeToLong(input_args.size()), kEqual, input_num,
                                           prim_name);
  (void)CheckAndConvertUtils::CheckArgs<abstract::AbstractTensor>(prim_name, input_args, 0);
  auto x = input_args[0]->BuildShape();
  const int64_t max_dim = 8;
  auto in_shape = CheckAndConvertUtils::ConvertShapePtrToShapeMap(input_args[0]->BuildShape())[kShape];
  (void)CheckAndConvertUtils::CheckInteger("The dimension of Atanh input", SizeToLong(in_shape.size()), kLessThan,
                                           max_dim, prim_name);
...
AbstractBasePtr AtanhInfer(const abstract::AnalysisEnginePtr &, const PrimitivePtr &primitive,
  ...
  auto shape = AtanhInferShape(primitive, input_args);
  return abstract::MakeAbstract(shape, type);
}
REGISTER_PRIMITIVE_EVAL_IMPL(Atanh, prim::kPrimAtanh, AtanhInfer, nullptr, true);
}  // namespace ops
}  // namespace luojianet_ms
