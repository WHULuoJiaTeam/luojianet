...
#ifndef LUOJIANET_MS_CORE_OPS_CEIL_H_
#define LUOJIANET_MS_CORE_OPS_CEIL_H_
#include <vector>
#include <memory>
...
/// Refer to Python API @ref luojianet_ms.ops.Ceil for more details.
class MIND_API Ceil : public BaseOperator {
 public:
  MIND_API_BASE_MEMBER(Ceil);
  /// \brief Constructor.
  Ceil() : BaseOperator(kNameCeil) { InitIOName({"x"}, {"y"}); }
  /// \brief Init. Refer to the parameters of Python API @ref luojianet_ms.ops.Ceil for the inputs.
  void Init() const {}
};
...
#endif  // LUOJIANET_MS_CORE_OPS_CEIL_H_
