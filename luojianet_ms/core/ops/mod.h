...
#ifndef LUOJIANET_MS_CORE_OPS_MOD_H_
#define LUOJIANET_MS_CORE_OPS_MOD_H_
#include <vector>
#include <memory>
...
/// Refer to Python API @ref luojianet_ms.ops.Mod for more details.
class MIND_API Mod : public BaseOperator {
 public:
  MIND_API_BASE_MEMBER(Mod);
  /// \brief Constructor.
  Mod() : BaseOperator(kNameMod) { InitIOName({"x", "y"}, {"output"}); }
  /// \brief Init. Refer to the parameters of Python API @ref luojianet_ms.ops.Mod for the inputs.
  void Init() const {}
};
...
#endif  // LUOJIANET_MS_CORE_OPS_MOD_H_
