...
MS_CORE_API void common_log_init(void) {
  ...
  if (luojianet_ms::GetEnv("GLOG_v").empty()) {
    FLAGS_v = static_cast<int>(luojianet_ms::WARNING);
  }
...
#ifndef BUILD_LITE_INFERENCE
      const std::string rank_id = luojianet_ms::GetEnv("RANK_ID");
      const std::string gpu_rank_id = luojianet_ms::GetEnv("OMPI_COMM_WORLD_RANK");
      std::string rank = "0";
...
#else
      FLAGS_log_dir = luojianet_ms::GetEnv("GLOG_log_dir");
#endif
    }
  }
...
#if defined(_WIN32) || defined(_WIN64) || defined(__APPLE__)
#ifdef _MSC_VER
void luojianet_ms_log_init(void) {
#else
__attribute__((constructor)) void luojianet_ms_log_init(void) {
#endif
#else
void luojianet_ms_log_init(void) {
#endif
