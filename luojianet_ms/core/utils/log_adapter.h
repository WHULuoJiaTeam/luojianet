...
#else
#define SUBMODULE_ID luojianet_ms::SubModuleId::SM_LITE
#endif
#endif
...
#define MS_LOG_TRY_CATCH_SCOPE luojianet_ms::TryCatchGuard luojianet_ms_log_try_catch_guard
#endif
...
#define MSLOG_IF(level, condition, excp_type)                                                                          \
  !(condition) ? void(0)                                                                                               \
               : luojianet_ms::LogWriter(luojianet_ms::LocationInfo(FILE_NAME, __LINE__, __FUNCTION__), level, SUBMODULE_ID, \
                                      excp_type) < luojianet_ms::LogStream()
...
#else
#define MSLOG_THROW(excp_type)                                                                                     \
  luojianet_ms::LogWriter(luojianet_ms::LocationInfo(FILE_NAME, __LINE__, __FUNCTION__), luojianet_ms::ERROR, SUBMODULE_ID, \
                       excp_type) < luojianet_ms::LogStream()
#endif

inline bool IS_OUTPUT_ON(enum MsLogLevel level) noexcept(true) {
  return (static_cast<int>(level) >= luojianet_ms::g_ms_submodule_log_levels[SUBMODULE_ID] &&
          static_cast<int>(level) <= static_cast<int>(luojianet_ms::this_thread_max_log_level));
}
