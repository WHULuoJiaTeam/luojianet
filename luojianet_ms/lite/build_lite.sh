...
check_Hi35xx() {
  if [[ "X${HI35XX_SDK_PATH}" == "X" ]]; then
    ...
    cp -r ${HI35XX_SDK_PATH}/third_patry ${BASEPATH}/luojianet_ms/lite/tools/benchmark/nnie/
  fi
}

get_version() {
    VERSION_MAJOR=$(grep "const int ms_version_major =" ${BASEPATH}/luojianet_ms/lite/include/version.h | tr -dc "[0-9]")
    VERSION_MINOR=$(grep "const int ms_version_minor =" ${BASEPATH}/luojianet_ms/lite/include/version.h | tr -dc "[0-9]")
    VERSION_REVISION=$(grep "const int ms_version_revision =" ${BASEPATH}/luojianet_ms/lite/include/version.h | tr -dc "[0-9]")
...
build_lite_x86_64_jni_and_jar() {
    ...
    local train_so=$pkg_name/runtime/lib/libluojianet-lite-train.so
    if [ ! -f "$train_so" ]; then
        echo "not exist"
        is_train=off
    fi
    ...
    cp ./libluojianet-lite-jni.so ${LITE_JAVA_PATH}/java/linux_x86/libs/
    cp ./libluojianet-lite-jni.so ${LITE_JAVA_PATH}/native/libs/linux_x86/
    cp ./libluojianet-lite-jni.so ${BASEPATH}/output/tmp/${pkg_name}/runtime/lib/
    if [[ "X$is_train" = "Xon" ]]; then
        cp ./libluojianet-lite-train-jni.so ${LITE_JAVA_PATH}/java/linux_x86/libs/
        cp ./libluojianet-lite-train-jni.so ${LITE_JAVA_PATH}/native/libs/linux_x86/
        cp ./libluojianet-lite-train-jni.so ${BASEPATH}/output/tmp/${pkg_name}/runtime/lib/
    fi
...
build_lite() {
    ...
    echo "cmake ${LITE_CMAKE_ARGS} -DBUILD_FIRST=ON ${BASEPATH}/luojianet_ms/lite"
    cmake ${LITE_CMAKE_ARGS} -DBUILD_FIRST=ON "${BASEPATH}/luojianet_ms/lite"

    if [[ "$(uname)" == "Darwin" && "${local_lite_platform}" != "x86_64" ]]; then
        xcodebuild ONLY_ACTIVE_ARCH=NO -configuration Release -scheme luojianet_ms-lite_static -target luojianet_ms-lite_static -sdk iphoneos -quiet
    elif [[ "$(uname)" == "Darwin" && "${local_lite_platform}" == "x86_64" ]]; then
        xcodebuild ONLY_ACTIVE_ARCH=NO -configuration Release -scheme luojianet_ms-lite_static -target luojianet_ms-lite_static -sdk iphonesimulator -quiet
    else
      make -j$THREAD_NUM && make install
      cp -r ${BASEPATH}/output/tmp/luojianet_ms*/runtime ${BASEPATH}/luojianet_ms/lite/tools/benchmark
      cmake ${LITE_CMAKE_ARGS} -DBUILD_FIRST=off "${BASEPATH}/luojianet_ms/lite"
        ...
        if [[ "${local_lite_platform}" == "x86_64" ]]; then
          if [[ "${MSLITE_ENABLE_TESTCASES}" == "ON" || "${MSLITE_ENABLE_TESTCASES}" == "on" ]]; then
            ...
            if [[ ! "${MSLITE_ENABLE_CONVERTER}" || "${MSLITE_ENABLE_CONVERTER}"  == "ON" || "${MSLITE_ENABLE_CONVERTER}"  == "on" ]]; then
              cp ${BASEPATH}/output/tmp/luojianet_ms-lite*/tools/converter/lib/*.so* ${BASEPATH}/luojianet_ms/lite/test/do_test || true
            fi
            cp ${BASEPATH}/output/tmp/luojianet_ms-lite*/runtime/lib/*.so* ${BASEPATH}/luojianet_ms/lite/test/do_test || true
            if [[ ! "${MSLITE_ENABLE_TRAIN}" || "${MSLITE_ENABLE_TRAIN}"  == "ON" || "${MSLITE_ENABLE_TRAIN}"  == "on" ]]; then
              cp ${BASEPATH}/output/tmp/luojianet_ms-lite*/runtime/third_party/libjpeg-turbo/lib/*.so* ${BASEPATH}/luojianet_ms/lite/test/do_test || true
            fi
          fi
        fi
...
build_lite_arm64_and_jni() {
    ...
    cp ./${pkg_name}/runtime/lib/libluojianet-lite.so ${LITE_JAVA_PATH}/java/app/libs/arm64-v8a/
    cp ./${pkg_name}/runtime/lib/libluojianet-lite.so ${LITE_JAVA_PATH}/native/libs/arm64-v8a/
    local train_so=$pkg_name/runtime/lib/libluojianet-lite-train.so
    if [ ! -f "$train_so" ]; then
        echo "not exist"
        is_train=off
    ...
    cp ./libluojianet-lite-jni.so ${LITE_JAVA_PATH}/java/app/libs/arm64-v8a/
    cp ./libluojianet-lite-jni.so ${LITE_JAVA_PATH}/native/libs/arm64-v8a/
    if [[ "X$is_train" = "Xon" ]]; then
      cp ./libluojianet-lite-train-jni.so ${LITE_JAVA_PATH}/java/app/libs/arm64-v8a/
      cp ./libluojianet-lite-train-jni.so ${LITE_JAVA_PATH}/native/libs/arm64-v8a/
    fi
}
...
build_lite_arm32_and_jni() {
    ...
    cp ./${pkg_name}/runtime/lib/libluojianet-lite.so ${LITE_JAVA_PATH}/java/app/libs/armeabi-v7a/
    cp ./${pkg_name}/runtime/lib/libluojianet-lite.so ${LITE_JAVA_PATH}/native/libs/armeabi-v7a/
    local train_so=$pkg_name/runtime/lib/libluojianet-lite-train.so
    if [ ! -f "$train_so" ]; then
        echo "not exist"
        is_train=off
    ...
    cp ./libluojianet-lite-jni.so ${LITE_JAVA_PATH}/java/app/libs/armeabi-v7a/
    cp ./libluojianet-lite-jni.so ${LITE_JAVA_PATH}/native/libs/armeabi-v7a/
    if [[ "X$is_train" = "Xon" ]]; then
      cp ./libluojianet-lite-train-jni.so ${LITE_JAVA_PATH}/java/app/libs/armeabi-v7a/
      cp ./libluojianet-lite-train-jni.so ${LITE_JAVA_PATH}/native/libs/armeabi-v7a/
    fi
}
...
build_aar() {
    ...
    # build java fl_client
    local is_train=on
    local train_so=${LITE_JAVA_PATH}/java/app/libs/arm64-v8a/libluojianet-lite-train.so
    if [ ! -f "$train_so" ]; then
        echo "not exist"
        is_train=off
    fi
    ...
    zip -r ${BASEPATH}/output/luojianet_ms-lite-maven-${VERSION_STR}.zip luojianet_ms
    cd ${BASEPATH}/output
    sha256sum luojianet_ms-lite-maven-${VERSION_STR}.zip > luojianet_ms-lite-maven-${VERSION_STR}.zip.sha256
}
