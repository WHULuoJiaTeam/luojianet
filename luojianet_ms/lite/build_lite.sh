...
check_Hi35xx() {
  if [[ "X${HI35XX_SDK_PATH}" == "X" ]]; then
    ...
    cp -r ${HI35XX_SDK_PATH}/third_patry ${BASEPATH}/luojianet_ms/lite/providers/nnie
  fi
}
...
write_commit_file() {
    ...
    echo ${COMMIT_STR} > "${BASEPATH}/luojianet_ms/lite/build/.commit_id"
}
...
build_lite_x86_64_jni_and_jar() {
    ...
    local is_train=on
    cd ${INSTALL_PREFIX}/
    local pkg_name=luojianet_ms-lite-${VERSION_STR}-linux-x64
    ...
    local train_so=$pkg_name/runtime/lib/libluojianet_ms-lite-train.so
    if [ ! -f "$train_so" ]; then
        echo "not exist"
        is_train=off
    fi
    ...
    # build jni so
    cd ${BASEPATH}/luojianet_ms/lite/build
    ...
    make -j$THREAD_NUM
    if [[ $? -ne 0 ]]; then
        echo "---------------- luojianet_ms lite: build jni x86_64 failed----------------"
        exit 1
    fi
    rm -f ${LITE_JAVA_PATH}/src/main/resources/com/luojianet_ms/lite/linux_x86_64/*.so*
    cp ./libluojianet_ms-lite-jni.so ${LITE_JAVA_PATH}/java/linux_x86/libs/
    cp ./libluojianet_ms-lite-jni.so ${LITE_JAVA_PATH}/native/libs/linux_x86/
    cp ./libluojianet_ms-lite-jni.so ${INSTALL_PREFIX}/${pkg_name}/runtime/lib/
    cp ./libluojianet_ms-lite-jni.so ${LITE_JAVA_PATH}/src/main/resources/com/luojianet_ms/lite/linux_x86_64/
    cp ${BASEPATH}/output/tmp/${pkg_name}/runtime/lib/libluojianet_ms-lite.so ${LITE_JAVA_PATH}/src/main/resources/com/luojianet_ms/lite/linux_x86_64/
    if [ -f "${BASEPATH}/output/tmp/${pkg_name}/runtime/third_party/glog/libglog.so.0" ]; then
      cp ${BASEPATH}/output/tmp/${pkg_name}/runtime/third_party/glog/libglog.so* ${LITE_JAVA_PATH}/src/main/resources/com/luojianet_ms/lite/linux_x86_64/libglog.so
    fi
    if [[ "X$is_train" = "Xon" ]]; then
      cp ./libluojianet_ms-lite-train-jni.so ${LITE_JAVA_PATH}/java/linux_x86/libs/
      cp ./libluojianet_ms-lite-train-jni.so ${LITE_JAVA_PATH}/native/libs/linux_x86/
      cp ./libluojianet_ms-lite-train-jni.so ${INSTALL_PREFIX}/${pkg_name}/runtime/lib/
    fi
    ...
    cp ${LITE_JAVA_PATH}/java/common/build/libs/luojianet_ms-lite-java-common.jar ${LITE_JAVA_PATH}/java/linux_x86/libs/
    ...
    if [[ "X$is_train" = "Xon" ]]; then
        ...
        cp ${LITE_JAVA_PATH}/java/fl_client/build/libs/jarX86/luojianet_ms-lite-java-flclient.jar ${INSTALL_PREFIX}/${pkg_name}/runtime/lib/
        rm -rf ${LITE_JAVA_PATH}/java/fl_client/.gradle ${LITE_JAVA_PATH}/java/fl_client/src/main/java/luojianet_ms
    fi
...
build_lite_aarch64_jni_and_jar() {
    ...
    local pkg_name=luojianet_ms-lite-${VERSION_STR}-linux-aarch64
    ...
    make -j$THREAD_NUM
    if [[ $? -ne 0 ]]; then
        echo "---------------- luojianet_ms lite: build jni arm64 failed----------------"
        exit 1
    fi
    rm -f ${LITE_JAVA_PATH}/src/main/resources/com/luojianet_ms/lite/linux_aarch64/*.so*
    cp ./libluojianet_ms-lite-jni.so ${LITE_JAVA_PATH}/java/linux_aarch64/libs/
    cp ./libluojianet_ms-lite-jni.so ${LITE_JAVA_PATH}/native/libs/linux_aarch64/
    cp ./libluojianet_ms-lite-jni.so ${LITE_JAVA_PATH}/src/main/resources/com/luojianet_ms/lite/linux_aarch64/
    cp ./libluojianet_ms-lite-jni.so ${BASEPATH}/output/tmp/${pkg_name}/runtime/lib/
    cp ${BASEPATH}/output/tmp/${pkg_name}/runtime/lib/libluojianet_ms-lite.so ${LITE_JAVA_PATH}/src/main/resources/com/luojianet_ms/lite/linux_aarch64/
    if [ -f "${BASEPATH}/output/tmp/${pkg_name}/runtime/third_party/glog/libglog.so.0" ]; then
      cp ${BASEPATH}/output/tmp/${pkg_name}/runtime/third_party/glog/libglog.so* ${LITE_JAVA_PATH}/src/main/resources/com/luojianet_ms/lite/linux_aarch64/libglog.so
    fi
    if [[ "X$is_train" = "Xon" ]]; then
      cp ./libluojianet_ms-lite-train-jni.so ${LITE_JAVA_PATH}/java/linux_aarch64/libs/
      cp ./libluojianet_ms-lite-train-jni.so ${LITE_JAVA_PATH}/native/libs/linux_aarch64/
      cp ./libluojianet_ms-lite-train-jni.so ${BASEPATH}/output/tmp/${pkg_name}/runtime/lib/
    fi
    ...
    ${gradle_command} clean -p ${LITE_JAVA_PATH}/java/common
    ${gradle_command} build -p ${LITE_JAVA_PATH}/java/common
    cp ${LITE_JAVA_PATH}/java/common/build/libs/luojianet_ms-lite-java-common.jar ${LITE_JAVA_PATH}/java/linux_aarch64/libs/
    ...
    if [[ "X$is_train" = "Xon" ]]; then
      ...
      ${gradle_command} clearJar -p ${LITE_JAVA_PATH}/java/fl_client
      ${gradle_command} flReleaseJarX86 --rerun-tasks -p ${LITE_JAVA_PATH}/java/fl_client
      cp ${LITE_JAVA_PATH}/java/fl_client/build/libs/jarX86/luojianet_ms-lite-java-flclient.jar ${BASEPATH}/output/tmp/${pkg_name}/runtime/lib/
      rm -rf ${LITE_JAVA_PATH}/java/fl_client/.gradle ${LITE_JAVA_PATH}/java/fl_client/src/main/java/luojianet_ms
    fi
...
build_lite() {
    ...
    echo "============ Start building LUOJIANET_MS Lite ${VERSION_STR} ============"
    local local_lite_platform=${LITE_PLATFORM}
    if [[ "${LITE_ENABLE_AAR}" == "on" ]]; then
        local_lite_platform=$1
        mkdir -pv ${BASEPATH}/luojianet_ms/lite/build/java
        cd ${BASEPATH}/luojianet_ms/lite/build/
        [ -n "${BASEPATH}" ] && find . -maxdepth 1 | grep -v java | grep '/' | xargs -I {} rm -rf {}
    else
        if [[ "${INC_BUILD}" == "off" ]]; then
            [ -n "${BASEPATH}" ] && rm -rf ${BASEPATH}/luojianet_ms/lite/build
        fi
        mkdir -pv ${BASEPATH}/luojianet_ms/lite/build
    fi
    cd ${BASEPATH}/luojianet_ms/lite/build
    write_commit_file
    ...
    if [[ "${local_lite_platform}" == "arm32" ]]; then
      ...
      if [ "$(uname)" == "Darwin" ]; then
        pkg_name=luojianet_ms-lite-${VERSION_STR}-ios-aarch32
        LITE_CMAKE_ARGS="${LITE_CMAKE_ARGS} -DARCHS=armv7;armv7s"
      elif [[ "${TOOLCHAIN_NAME}" == "ohos-lite" ]]; then
        CMAKE_TOOLCHAIN_FILE=${BASEPATH}/luojianet_ms/lite/cmake/ohos-lite.toolchain.cmake
        ...
        CMAKE_TOOLCHAIN_FILE=${BASEPATH}/luojianet_ms/lite/cmake/himix200.toolchain.cmake
      ...
      if [ "$(uname)" == "Darwin" ]; then
        pkg_name=luojianet_ms-lite-${VERSION_STR}-ios-aarch64
        LITE_CMAKE_ARGS="${LITE_CMAKE_ARGS} -DARCHS=arm64"
        LITE_CMAKE_ARGS="${LITE_CMAKE_ARGS} -DMSLITE_ENABLE_FP16=on"
      elif [[ "${TOOLCHAIN_NAME}" == "himix100" ]]; then
        CMAKE_TOOLCHAIN_FILE=${BASEPATH}/luojianet_ms/lite/cmake/himix100.toolchain.cmake
        LITE_CMAKE_ARGS="${LITE_CMAKE_ARGS} -DTOOLCHAIN_NAME=himix100"
        LITE_CMAKE_ARGS="${LITE_CMAKE_ARGS} -DMSLITE_MINDDATA_IMPLEMENT=off"
        LITE_CMAKE_ARGS="${LITE_CMAKE_ARGS} -DMSLITE_ENABLE_FP16=off -DMSLITE_ENABLE_TRAIN=off -DMSLITE_GPU_BACKEND=off"
      elif [[ "${TOOLCHAIN_NAME}" == "mix210" ]]; then
        CMAKE_TOOLCHAIN_FILE=${BASEPATH}/luojianet_ms/lite/cmake/mix210.toolchain.cmake
      ...
      if [ "$(uname)" == "Darwin" ]; then
         pkg_name=luojianet_ms-lite-${VERSION_STR}-ios-simulator
         CMAKE_TOOLCHAIN_FILE=${BASEPATH}/cmake/lite_ios.cmake
         LITE_CMAKE_ARGS=`echo $LITE_CMAKE_ARGS | sed 's/-DCMAKE_BUILD_TYPE=Debug/-DCMAKE_BUILD_TYPE=Release/g'`
        ...
        if [[ "${machine}" == "aarch64" ]]; then
          echo "Use the '-I arm64' command when compiling LUOJIANET_MS Lite on an aarch64 architecture system."
          exit 1
        fi
        LITE_CMAKE_ARGS="${LITE_CMAKE_ARGS} -DMSLITE_MINDDATA_IMPLEMENT=lite_cv"
    ...
    echo "cmake ${LITE_CMAKE_ARGS} ${BASEPATH}/luojianet_ms/lite"
    cmake ${LITE_CMAKE_ARGS} "${BASEPATH}/luojianet_ms/lite"

    if [[ "$(uname)" == "Darwin" && "${local_lite_platform}" != "x86_64" ]]; then
        xcodebuild ONLY_ACTIVE_ARCH=NO -configuration Release -scheme luojianet_ms-lite_static -target luojianet_ms-lite_static -sdk iphoneos -quiet -UseModernBuildSystem=YES
    elif [[ "$(uname)" == "Darwin" && "${local_lite_platform}" == "x86_64" ]]; then
        xcodebuild ONLY_ACTIVE_ARCH=NO -configuration Release -scheme luojianet_ms-lite_static -target luojianet_ms-lite_static -sdk iphonesimulator -quiet -UseModernBuildSystem=YES
    else
      ...
      if [[ "X$MSLITE_COMPILE_TWICE" == "XON" ]]; then
        ...
        if [[ "X$MSLITE_ENABLE_TOOLS" != "XOFF" ]]; then
          ...
          cp -r ${INSTALL_PREFIX}/luojianet_ms*/runtime ${BASEPATH}/luojianet_ms/lite/providers
          echo "cmake ${LITE_CMAKE_ARGS} ${BASEPATH}/luojianet_ms/lite"
          cmake ${LITE_CMAKE_ARGS} "${BASEPATH}/luojianet_ms/lite"
          cmake --build "${BASEPATH}/luojianet_ms/lite/build" --target benchmark -j$THREAD_NUM
          make install
        fi
      fi
      make package
    ...
    if [[ $? -ne 0 ]]; then
        echo "---------------- luojianet_ms lite: build failed ----------------"
        exit 1
    else
        if [ "$(uname)" == "Darwin" ]; then
          mkdir -p ${BASEPATH}/output
          cp -r ${BASEPATH}/luojianet_ms/lite/build/src/Release-*/luojianet_ms-lite.framework ${BASEPATH}/output/luojianet_ms-lite.framework
          cd ${BASEPATH}/output
          tar -zcvf ${pkg_name}.tar.gz luojianet_ms-lite.framework/
          sha256sum ${pkg_name}.tar.gz > ${pkg_name}.tar.gz.sha256
          rm -r luojianet_ms-lite.framework
        else
        ...
        if [[ "${local_lite_platform}" == "x86_64" ]]; then
          if [[ "${MSLITE_ENABLE_TESTCASES}" == "ON" || "${MSLITE_ENABLE_TESTCASES}" == "on" ]]; then
            mkdir -pv ${BASEPATH}/luojianet_ms/lite/test/do_test || true
            if [[ ! "${MSLITE_ENABLE_CONVERTER}" || "${MSLITE_ENABLE_CONVERTER}"  == "ON" || "${MSLITE_ENABLE_CONVERTER}"  == "on" ]]; then
              cp ${INSTALL_PREFIX}/luojianet_ms-lite*/tools/converter/lib/*.so* ${BASEPATH}/luojianet_ms/lite/test/do_test || true
            fi
            cp ${INSTALL_PREFIX}/luojianet_ms-lite*/runtime/lib/*.so* ${BASEPATH}/luojianet_ms/lite/test/do_test || true
            if [[ ! "${MSLITE_ENABLE_TRAIN}" || "${MSLITE_ENABLE_TRAIN}"  == "ON" || "${MSLITE_ENABLE_TRAIN}"  == "on" ]]; then
              cp ${INSTALL_PREFIX}/luojianet_ms-lite*/runtime/third_party/libjpeg-turbo/lib/*.so* ${BASEPATH}/luojianet_ms/lite/test/do_test || true
            fi
          fi
        fi
        ...
        if [[ "X$MSLITE_REGISTRY_DEVICE" != "X" ]] && [[ "${MSLITE_REGISTRY_DEVICE}" != "SD3403" ]]; then
          local compile_nnie_script=${BASEPATH}/luojianet_ms/lite/tools/providers/NNIE/Hi3516D/compile_nnie.sh
          cd ${BASEPATH}/../
          if [[ "${local_lite_platform}" == "x86_64" ]]; then
        ...
        echo "---------------- luojianet_ms lite: build success ----------------"
    fi
}
...
build_lite_arm64_and_jni() {
    ...
    # copy arm64 so
    local is_train=on
    local pkg_name=luojianet_ms-lite-${VERSION_STR}-android-aarch64
    cd "${BASEPATH}/luojianet_ms/lite/build"
    ...
    cp ./${pkg_name}/runtime/lib/libluojianet_ms-lite.so ${LITE_JAVA_PATH}/java/app/libs/arm64-v8a/
    cp ./${pkg_name}/runtime/lib/libluojianet_ms-lite.so ${LITE_JAVA_PATH}/native/libs/arm64-v8a/
    local train_so=$pkg_name/runtime/lib/libluojianet_ms-lite-train.so
    if [ ! -f "$train_so" ]; then
        echo "not exist"
        is_train=off
    ...
    cp ./libluojianet_ms-lite-jni.so ${LITE_JAVA_PATH}/java/app/libs/arm64-v8a/
    cp ./libluojianet_ms-lite-jni.so ${LITE_JAVA_PATH}/native/libs/arm64-v8a/
    if [[ "X$is_train" = "Xon" ]]; then
      cp ./libluojianet_ms-lite-train-jni.so ${LITE_JAVA_PATH}/java/app/libs/arm64-v8a/
      cp ./libluojianet_ms-lite-train-jni.so ${LITE_JAVA_PATH}/native/libs/arm64-v8a/
    fi
}
...
build_lite_arm32_and_jni() {
    ...
    # copy arm32 so
    local is_train=on
    local pkg_name=luojianet_ms-lite-${VERSION_STR}-android-aarch32
    cd "${BASEPATH}/luojianet_ms/lite/build"
    ...
    cp ./${pkg_name}/runtime/lib/libluojianet_ms-lite.so ${LITE_JAVA_PATH}/java/app/libs/armeabi-v7a/
    cp ./${pkg_name}/runtime/lib/libluojianet_ms-lite.so ${LITE_JAVA_PATH}/native/libs/armeabi-v7a/
    local train_so=$pkg_name/runtime/lib/libluojianet_ms-lite-train.so
    if [ ! -f "$train_so" ]; then
        echo "not exist"
        is_train=off
    ...
    make -j$THREAD_NUM
    if [[ $? -ne 0 ]]; then
        echo "---------------- luojianet_ms lite: build jni arm32 failed----------------"
        exit 1
    fi
    cp ./libluojianet_ms-lite-jni.so ${LITE_JAVA_PATH}/java/app/libs/armeabi-v7a/
    cp ./libluojianet_ms-lite-jni.so ${LITE_JAVA_PATH}/native/libs/armeabi-v7a/
    if [[ "X$is_train" = "Xon" ]]; then
      cp ./libluojianet_ms-lite-train-jni.so ${LITE_JAVA_PATH}/java/app/libs/armeabi-v7a/
      cp ./libluojianet_ms-lite-train-jni.so ${LITE_JAVA_PATH}/native/libs/armeabi-v7a/
    fi
}
...
build_aar() {
    ...
    # build java fl_client
    local is_train=on
    local train_so=${LITE_JAVA_PATH}/java/app/libs/arm64-v8a/libluojianet_ms-lite-train.so
    if [ ! -f "$train_so" ]; then
        echo "not exist"
        is_train=off
    fi
    if [[ "X$is_train" = "Xon" ]]; then
        ...
        cp ${LITE_JAVA_PATH}/java/fl_client/build/libs/jarAAR/luojianet_ms-lite-java-flclient.jar ${LITE_JAVA_PATH}/java/app/libs
        rm -rf ${LITE_JAVA_PATH}/java/fl_client/.gradle ${LITE_JAVA_PATH}/java/fl_client/src/main/java/luojianet_ms
    fi

    cp ${LITE_JAVA_PATH}/java/common/build/libs/luojianet_ms-lite-java-common.jar ${LITE_JAVA_PATH}/java/app/libs
    cp ${LITE_JAVA_PATH}/build/libs/luojianet_ms-lite-java.jar ${LITE_JAVA_PATH}/java/app/libs
    ...
    if [ -e "${minddata_so}" ]; then
      cp ${LITE_JAVA_PATH}/java/app/build/outputs/aar/luojianet_ms-lite.aar ${BASEPATH}/output/luojianet_ms-lite-full-${VERSION_STR}.aar
      sha256sum ${BASEPATH}/output/luojianet_ms-lite-full-${VERSION_STR}.aar > ${BASEPATH}/output/luojianet_ms-lite-full-${VERSION_STR}.aar.sha256
    else
      cp ${LITE_JAVA_PATH}/java/app/build/outputs/aar/luojianet_ms-lite.aar ${BASEPATH}/output/luojianet_ms-lite-${VERSION_STR}.aar
      sha256sum ${BASEPATH}/output/luojianet_ms-lite-${VERSION_STR}.aar > ${BASEPATH}/output/luojianet_ms-lite-${VERSION_STR}.aar.sha256
    fi
}
...
build_lite_x86_64_aarch64_jar()
{
  ...
  # unzip tar.gz, extract native libs(libluojianet_ms-lite.so,libluojianet_ms-lite-jni.so)
  ...
  local LITE_JAVA_NATIVE_RESOURCE_PATH=${LITE_JAVA_PATH}/src/main/resources/com/luojianet_ms/lite
  ...
  cp ${x86_64_base_path}/tmp/${x86_64_package_name}/runtime/lib/libluojianet_ms-lite.so ${LITE_JAVA_NATIVE_RESOURCE_PATH}/linux_x86_64/
  cp ${x86_64_base_path}/tmp/${x86_64_package_name}/runtime/lib/libluojianet_ms-lite-jni.so ${LITE_JAVA_NATIVE_RESOURCE_PATH}/linux_x86_64/
  cp ${aarch64_base_path}/tmp/${aarch64_package_name}/runtime/lib/libluojianet_ms-lite.so ${LITE_JAVA_NATIVE_RESOURCE_PATH}/linux_aarch64/
  cp ${aarch64_base_path}/tmp/${aarch64_package_name}/runtime/lib/libluojianet_ms-lite-jni.so ${LITE_JAVA_NATIVE_RESOURCE_PATH}/linux_aarch64/
  ...
  cp ${LITE_JAVA_PATH}/build/lib/jar/luojianet_ms-lite-java.jar ${x86_64_base_path}/tmp/${x86_64_package_name}/runtime/lib/ -f
  cp ${LITE_JAVA_PATH}/build/lib/jar/luojianet_ms-lite-java.jar ${aarch64_base_path}/tmp/${aarch64_package_name}/runtime/lib/ -f
...
LITE_JAVA_PATH=${BASEPATH}/luojianet_ms/lite/java
