...
display_usage()
{
  echo "Usage: prepare.sh [-d luojianet_ms_docker] [-c model_config_file] [-i] [-T]"
  echo "Options:"
  echo "    -d docker where luojianet_ms is installed. If no docker is provided script will use local python"
...
if [ -z "${DOCKER}" ]; then
    echo "LUOJIANET_MS docker was not provided, attempting to run locally"
fi
...
while read line; do
    ...
    if [ $? = 0 ]; then
      export_result='export luojianet_ms '${model_name}${suffix}'_export pass';echo ${export_result} >> ${export_result_file}
    else
      export_result='export luojianet_ms '${model_name}${suffix}'_export failed';echo ${export_result} >> ${export_result_file}
    fi
done < ${CONFIG_FILE}
