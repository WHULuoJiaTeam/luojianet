...
第一步：下载LUOJIANET_MS源码(https://gitee.com/luojianet_ms/luojianet_ms)，在项目根目录使用命令

```bash
bash build.sh -I x86_64 -j128
```

编译LUOJIANET_MS，在项目根目录的output目录会生成LUOJIANET_MS tar 包。

第二步：进入luojianet_ms/luojianet_ms/lite/examples/quick_start_micro/mnist_x86 目录，执行脚本

```bash
bash mnist.sh -g on -r ${dir}/luojianet_ms-lite-${VERSION_STR}-linux-x64.tar.gz
```
...
```text
 ...
 └── tools
     └── codegen # 代码生成工具
         ...
         │   └── wrapper
         ├── lib
         │   └── libwrapper.a       # LUOJIANET_MS Lite CodeGen生成代码依赖的部分算子静态库
         └── third_party
             ├── include
...
**模型编译体验**第二步中会先下载[MNIST分类网络](https://download.luojianet_ms.cn/model_zoo/official/lite/quick_start/micro/mnist.tar.gz)，  模型及输入输出标杆数据解压在 quick_start_micro/models 目录下。
...
> 关于codegen的更多使用命令说明，可参见[codegen使用说明](https://www.luojianet_ms.cn/lite/docs/zh-CN/r1.7/use/micro.html#id4)。

### 部署应用

接下来介绍如何构建LUOJIANET_MS Lite Micro生成的模型推理代码工程，并在x86平台完成部署。
...
1. **生成代码工程说明**
   ...
   进入`luojianet_ms/luojianet_ms/lite/example/quick_start_micro/mnist_x86`目录中,生成代码工程目录说明：
...
详情代码可以参见[examples/quick_start_micro/mnist_x86](https://gitee.com/luojianet_ms/luojianet_ms/tree/master/luojianet_ms/lite/examples/quick_start_micro/mnist_x86)下的示例代码示例：
...
## 更多详情
### [Android平台编译部署](https://gitee.com/luojianet_ms/luojianet_ms/blob/master/luojianet_ms/lite/examples/quick_start_micro/mobilenetv2_arm64/README.md)

### [Arm&nbsp;Cortex-M平台编译部署](https://www.luojianet_ms.cn/lite/docs/zh-CN/r1.7/use/micro.html)
