...
std::vector<luojianet_ms::MSTensor> GenerateInputDataWithRandom(std::vector<luojianet_ms::MSTensor> model_inputs) {
  ...
  auto new_tensor = luojianet_ms::MSTensor::CreateTensor(tensor_name, model_inputs[0].DataType(), shape, input_data, size);
...
int QuickStart(int argc, const char **argv) {
  ...
  // Create model
  auto model_runner = new (std::nothrow) luojianet_ms::ModelParallelRunner();
