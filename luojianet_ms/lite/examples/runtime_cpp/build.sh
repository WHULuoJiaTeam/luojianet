...
checkopts()
{
  ...
  LUOJIANET_MS_LITE_DOWNLOAD_URL="https://ms-release.obs.cn-north-4.myhuaweicloud.com/${VERSION_STR}/LUOJIANET_MS/lite/release/android/gpu/${LUOJIANET_MS_FILE}"
  # Process the options
    ...
    case "${opt}" in
      e)
        ...
        if [[ "X${DEVICE}" == "Xgpu" ]]; then
          ...
          LUOJIANET_MS_FILE_NAME="luojianet_ms-lite-${VERSION_STR}-android-aarch64"
          LUOJIANET_MS_LITE_DOWNLOAD_URL="https://ms-release.obs.cn-north-4.myhuaweicloud.com/${VERSION_STR}/LUOJIANET_MS/lite/release/android/npu/${LUOJIANET_MS_FILE}"
          SUPPORT_NPU="on"
        else
...
cp -r ${BASEPATH}/build/${LUOJIANET_MS_FILE_NAME}/runtime/lib/libluojianet_ms-lite.a ${BASEPATH}/lib
