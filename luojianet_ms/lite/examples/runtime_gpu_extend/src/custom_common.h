...
#ifndef LUOJIANET_MS_LITE_EXAMPLES_RUNTIME_GPU_EXTEND_SRC_CUSTOM_COMMON_H
#define LUOJIANET_MS_LITE_EXAMPLES_RUNTIME_GPU_EXTEND_SRC_CUSTOM_COMMON_H
...
int CheckInputs(const std::vector<luojianet_ms::MSTensor> &inputs);

// versify that the outputs' shape is inferred successfully when running current node.
int CheckOutputs(const std::vector<luojianet_ms::MSTensor> &inputs);
void PackNHWCToNHWC4(void *src, void *dst, bool src_is_fp16, bool dst_is_fp16, const GpuTensorInfo &tensor,
                     luojianet_ms::DataType data_type = luojianet_ms::DataType::kNumberTypeFloat32);
}  // namespace custom_common
}  // namespace luojianet_ms
#endif  // LUOJIANET_MS_LITE_EXAMPLES_RUNTIME_GPU_EXTEND_SRC_CUSTOM_COMMON_H
