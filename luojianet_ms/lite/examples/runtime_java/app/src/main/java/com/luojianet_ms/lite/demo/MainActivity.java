...
import com.luojianet_ms.MSTensor;
import com.luojianet_ms.Model;
import com.luojianet_ms.config.CpuBindMode;
import com.luojianet_ms.config.DataType;
import com.luojianet_ms.config.MSContext;
import com.luojianet_ms.config.ModelType;
...
public class MainActivity extends AppCompatActivity {
    ...
    private MSContext createCPUConfig() {
        ...
        boolean ret = context.addDeviceInfo(com.luojianet_ms.config.DeviceType.DT_CPU, false, 0);
        if (!ret) {
    ...
    private MSContext createGPUConfig() {
        ...
        boolean ret = context.addDeviceInfo(com.luojianet_ms.config.DeviceType.DT_GPU, true, 0);
        if (!ret) {
    ...
    private Model createLiteModel(String filePath, boolean isResize) {
        ...
        // Create the LUOJIANET_MS lite model.
    ...
    private boolean runInference(Model model) {
        ...
        if (!ret) {
            Log.e(TAG, "LUOJIANET_MS Lite run failed.");
            return false;
        }
        ...
        btn_run.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (model1Finish && model1Compile) {
                    ...
                    Toast.makeText(getApplicationContext(), "LUOJIANET_MS Lite is running...",
                            Toast.LENGTH_SHORT).show();
                }
            }
        });
