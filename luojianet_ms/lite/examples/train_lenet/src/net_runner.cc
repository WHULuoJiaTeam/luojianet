...
using luojianet_ms::lite::AccuracyMetrics;
using luojianet_ms::lite::Model;
using luojianet_ms::session::TrainLoopCallBack;
using luojianet_ms::session::TrainLoopCallBackData;
...
class Rescaler : public luojianet_ms::session::TrainLoopCallBack {
 public:
  ...
  void StepBegin(const luojianet_ms::session::TrainLoopCallBackData &cb_data) override {
...
class Measurement : public luojianet_ms::session::TrainLoopCallBack {
 public:
  ...
  void EpochBegin(const luojianet_ms::session::TrainLoopCallBackData &cb_data) override {
  ...
  int EpochEnd(const luojianet_ms::session::TrainLoopCallBackData &cb_data) override {
    ...
    time_avg_ += time;
    return luojianet_ms::session::RET_CONTINUE;
  }
  void End(const luojianet_ms::session::TrainLoopCallBackData &cb_data) override {
    if (epochs_ > 0) {
...
bool after_callback(const std::vector<luojianet_ms::tensor::MSTensor *> &after_inputs,
                    const std::vector<luojianet_ms::tensor::MSTensor *> &after_outputs,
                    const luojianet_ms::CallBackParam &call_param) {
  ...
  for (size_t i = 0; i < after_inputs.size(); i++) {
    ...
    if (after_inputs.at(i)->data_type() == luojianet_ms::kNumberTypeInt32) {
...
void NetRunner::InitAndFigureInputs() {
  luojianet_ms::lite::Context context;
  context.device_list_[0].device_info_.cpu_device_info_.cpu_bind_mode_ = luojianet_ms::lite::NO_BIND;
  ...
  luojianet_ms::lite::TrainCfg train_cfg;
  session_ = luojianet_ms::session::TrainSession::CreateTrainSession(ms_file_, &context, true, &train_cfg);
  MS_ASSERT(session_ != nullptr);

  session_->SetupVirtualBatch(virtual_batch_);
  loop_ = luojianet_ms::session::TrainLoop::CreateTrainLoop(session_);
...
int NetRunner::TrainLoop() {
  luojianet_ms::lite::LossMonitor lm(kPrintTimes);
  luojianet_ms::lite::ClassificationTrainAccuracyMonitor am(1);

  luojianet_ms::lite::CkptSaver cs(kSaveSteps, std::string("lenet"));
  Rescaler rescale(kScalePoint);
  ...
  if (virtual_batch_ > 0) {
    ...
    struct luojianet_ms::lite::StepLRLambda step_lr_lambda(1, kGammaFactor);
    luojianet_ms::lite::LRScheduler step_lr_sched(luojianet_ms::lite::StepLRLambda, static_cast<void *>(&step_lr_lambda), 1);
