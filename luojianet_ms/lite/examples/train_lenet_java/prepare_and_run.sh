...
display_usage()
{
  echo -e "\nUsage: prepare_and_run.sh -D dataset_path [-d luojianet_ms_docker] [-r release.tar.gz] [-m mindir]\n"
}
...
cd model/ || exit 1
MSLITE_LINUX=$(ls -d ${BASEPATH}/build/luojianet_ms-lite-*-linux-x64)
...
java -Djava.library.path=../lib/ -classpath .:./train_lenet_java.jar:../lib/luojianet_ms-lite-java.jar  com.luojianet_ms.lite.train_lenet.Main ../model/lenet_tod.ms $MNIST_DATA_PATH 1
cd -
END=$(date +%s.%N)
