...
- 服务端
    - [LuoJiaNet Framework](https://www.luojianet_ms.cn/install/en) - 建议使用安装docker环境
    - [LuoJiaNet ToD Download](https://www.luojianet_ms.cn/lite/docs/zh-CN/master/use/downloads.html)
    - [LuoJiaNet ToD Build](https://www.luojianet_ms.cn/lite/docs/zh-CN/master/use/build.html)
...
其中，`DATASET_PATH`是数据集路径；`LUOJIANET_MS_DOCKER`是运行LuoJiaNet的docker镜像，如果没有使用docker环境，则使用本地运行；`REALEASE.tar.gz`为端侧运行时训练工具压缩包绝对路径；`-t`选项为设备处理器架构，默认为`arm64`，如果输入`x86`则本地运行。注意：若在不同平台执行训练，需在先执行脚本前运行`make clean`指令。
...
脚本`prepare_model.sh`会基于LuoJiaNet架构将Python模型转换为`lenet_tod.mindir`模型；然后，使用LuoJiaNet ToD 模型转换工具将`lenet_tod.mindir`文件转换为`lenet_tod.ms`文件。如果没有docker环境，则本地执行转换。

## 模型训练

首先编译`/src`文件夹中训练代码源码，生成的二进制文件在`./bin`目录下；然后将`transfer_learning_tod.ms`模型文件、训练脚本、LuoJiaNet ToD库文件、编译生成的`/bin`目录和预处理后的`Places`数据集拷贝到`package`文件夹；最后使用`adb`工具将`package`文件夹传输至设备端并执行训练。
