...
if [ "$?" -ne 0 ]; then
  ...
  then
    tar -xzf ../../../../../output/luojianet_ms-lite-*-linux-x64.tar.gz --strip-components 4 --wildcards --no-anchored converter_lite *so.* *.so
    if [ -f ./converter_lite ]; then
      CONVERTER=./converter_lite
    else
      echo "converter_lite could not be found in LUOJIANET_MS build directory nor in system path"
      exit 1
    fi
  else
    CONVERTER=converter_lite
