...
if [ ! -f "efficient_net_b0.ckpt" ]; then
  ...
  wget https://download.luojianet_ms.cn/model_zoo/official/lite/efficient_net/efficient_net_b0.ckpt
fi
...
if [ ! -f "$CONVERTER" ]; then
  ...
  then
    tar -xzf ../../../../../output/luojianet_ms-lite-*-linux-x64.tar.gz --strip-components 4 --wildcards --no-anchored converter_lite libglog.so.0 libmslite_converter_plugin.so
    if [ -f ./converter_lite ]; then
      CONVERTER=./converter_lite
    else
      echo "converter_lite could not be found in LuoJiaNet build directory nor in system path"
      exit 1
    fi
  else
    CONVERTER=converter_lite
