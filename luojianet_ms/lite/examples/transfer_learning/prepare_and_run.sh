...
display_usage()
{
  echo -e "\nUsage: prepare_and_run.sh -D dataset_path [-d luojianet_ms_docker] [-r release.tar.gz] [-t arm64|x86] [-i device_id] [-o]\n"
}
...
if [ "$TARBALL" == "" ]; then
  if [ "${TARGET}" == "arm64" ]; then
    file=$(ls ../../../../output/luojianet_ms-lite-*-android-aarch64.tar.gz)
  else
    file=$(ls ../../../../output/luojianet_ms-lite-*-linux-x64.tar.gz)
  fi
  if [[ ${file} != "" ]] && [[ -f ${file} ]]; then
    TARBALL=${file}
  else
...
# Copy the shared LUOJIANET_MS ToD library
tar -xzf ${TARBALL}
mv luojianet_ms-*/runtime/lib ${PACKAGE}/
mv luojianet_ms-*/runtime/third_party/libjpeg-turbo/lib/* ${PACKAGE}/lib/
if [ "${TARGET}" == "arm64" ]; then
  tar -xzf ${TARBALL} --wildcards --no-anchored hiai_ddk
  mv luojianet_ms-*/runtime/third_party/hiai_ddk/lib/* ${PACKAGE}/lib/
fi
