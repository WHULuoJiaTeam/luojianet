...
void NetRunner::InitAndFigureInputs() {
  ...
  session_ = luojianet_ms::session::TrainSession::CreateTransferSession(ms_backbone_file_, ms_head_file_, &context);
...
luojianet_ms::tensor::MSTensor *NetRunner::SearchOutputsForSize(size_t size) const {
...
float NetRunner::CalculateAccuracy(const std::vector<DataLabelTuple> &dataset,
                                   luojianet_ms::session::LiteSession *session) const {
  float accuracy = 0.0;
...
int NetRunner::Main() {
  ...
  if (!save_inference_.empty()) {
    int status = session_->Export(save_inference_, luojianet_ms::lite::MT_INFERENCE);
    if (status != luojianet_ms::lite::RET_OK) {
