...
- 服务器或个人笔记本
    - [LuoJiaNet Framework](https://www.luojianet_ms.cn/install): 建议使用Docker安装
...
将`lenet_tod.ms`模型文件、训练脚本、LuoJiaNet ToD库文件和`MNIST`数据集拷贝到`package`文件夹。`/src`文件夹中代码将会被编译成arm64架构版本，生成的二进制文件会被拷贝至`package`文件夹。最后使用`adb`工具将`package`文件夹传输至设备端，并执行训练。
