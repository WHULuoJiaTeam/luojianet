...
- 服务器或个人笔记本
    - [LUOJIANET_MS Framework](https://www.luojianet_ms.cn/install): 建议使用Docker安装
...
安装完毕，在`./luojianet_ms/luojianet_ms/lite/examples/train_lenet`目录下执行脚本，命令如下：
...
将`lenet_tod.ms`模型文件、训练脚本、LUOJIANET_MS ToD库文件和`MNIST`数据集拷贝到`package`文件夹。`/src`文件夹中代码将会被编译成arm64架构版本，生成的二进制文件会被拷贝至`package`文件夹。最后使用`adb`工具将`package`文件夹传输至设备端，并执行训练。
