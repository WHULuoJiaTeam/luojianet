...
int main(int argc, char **argv) {
  ...
  auto status = luojianet_ms::Serialization::Load(infer_model_fn, luojianet_ms::kMindIR, &graph);
  ...
  luojianet_ms::Model model;
  status = model.Build(luojianet_ms::GraphCell(graph), context);
  if (status != luojianet_ms::kSuccess) {
  ...
  luojianet_ms::MSTensor *input_tensor = inputs.at(0).Clone();
