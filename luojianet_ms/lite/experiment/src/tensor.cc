...
Tensor::Tensor(const TypeId data_type, std::vector<int> shape, const luojianet_ms::Format &format, Category category)
...
int32_t Tensor::Batch() const {
  ...
  switch (this->format_) {
    case luojianet_ms::NHWC:
    case luojianet_ms::NHWC4:
    case luojianet_ms::NCHW:
    case luojianet_ms::NC4HW4:
    case luojianet_ms::KCHW:
    case luojianet_ms::KHWC:
    case luojianet_ms::NC:
    case luojianet_ms::NC4:
      return this->shape_[0];
    case luojianet_ms::HWCK:
    case luojianet_ms::CHWK:
      return this->shape_[3];
    case luojianet_ms::HWKC:
      return this->shape_[2];
    case luojianet_ms::CKHW:
...
int32_t Tensor::Height() const {
  ...
  switch (this->format_) {
    ...
    case luojianet_ms::HWCK:
    case luojianet_ms::HWKC:
    case luojianet_ms::HW:
    case luojianet_ms::HW4:
      return this->shape_[0];
    default:
      MS_LOG(ERROR) << "Unsupported format: " << EnumNameFormat(static_cast<schema::Format>(this->format_));
      return RET_ERROR;
...
size_t Tensor::Size() const {
  ...
  auto element_num = (format_ == luojianet_ms::NC4HW4 || format_ == luojianet_ms::NHWC4) ? ElementsC4Num() : ElementsNum();
  if (element_num < 0) {
