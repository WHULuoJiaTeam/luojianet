...
#ifndef LUOJIANET_MS_LITE_INCLUDE_REGISTRY_PASS_REGISTRY_H_
#define LUOJIANET_MS_LITE_INCLUDE_REGISTRY_PASS_REGISTRY_H_
...
#define REG_PASS(name, pass) \
  static luojianet_ms::registry::PassRegistry g_##name##PassReg(#name, std::make_shared<pass>());

/// \brief Defined assigning macro to assign Passes, which called by user directly.
///
/// \param[in] position Define the place where assigned passes will run.
/// \param[in] names Define the names of the passes.
#define REG_SCHEDULED_PASS(position, names) static luojianet_ms::registry::PassRegistry g_##position(position, names);
}  // namespace registry
}  // namespace luojianet_ms

#endif  // LUOJIANET_MS_LITE_INCLUDE_REGISTRY_PASS_REGISTRY_H_
