 ...
 */
#ifndef LUOJIANET_MS_LITE_INCLUDE_TRAIN_TRAIN_LOOP_H_
#define LUOJIANET_MS_LITE_INCLUDE_TRAIN_TRAIN_LOOP_H_
#include <vector>
#include <string>
#include <tuple>
...
namespace dataset {
class Dataset;
using MSTensorVec = std::vector<luojianet_ms::MSTensor>;
}  // namespace dataset
...
class TrainLoop {
 public:
  ...
  ///
  /// \return Pointer of LUOJIANET_MS Lite TrainLoop
  ...
  ///
  /// \return 0 on success or -1 in case of error
  virtual int Init(std::vector<luojianet_ms::session::Metrics *> metrics) = 0;

  /// \brief Accessor to TrainLoop metric objects
  ///
  /// \return vector of metrics
  virtual std::vector<luojianet_ms::session::Metrics *> GetMetrics() = 0;
  ...
  ///
  /// \return 0 on success or -1 in case of error
  virtual int Train(int epochs, luojianet_ms::dataset::Dataset *dataset, std::vector<TrainLoopCallBack *> cbs,
  ...
  ///
  /// \return 0 on success or -1 in case of error
  virtual int Eval(luojianet_ms::dataset::Dataset *dataset, std::vector<TrainLoopCallBack *> cbs, LoadDataFunc load_func,
                   int max_steps) = 0;
};
}  // namespace session
}  // namespace luojianet_ms
#endif  // LUOJIANET_MS_LITE_INCLUDE_TRAIN_TRAIN_LOOP_H_
