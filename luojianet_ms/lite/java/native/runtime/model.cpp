...
extern "C" JNIEXPORT jlong JNICALL Java_com_luojianet_ms_lite_Model_loadModel(JNIEnv *env, jobject thiz, jobject buffer) {
  if (buffer == nullptr) {
  ...
  auto model = luojianet_ms::lite::Model::Import(model_buffer, buffer_len);
  if (model == nullptr) {
...
extern "C" JNIEXPORT jlong JNICALL Java_com_luojianet_ms_lite_Model_loadModelByPath(JNIEnv *env, jobject thiz,
                                                                                 jstring model_path) {
  ...
  ifs.read(buf, size);
  ifs.close();
  auto model = luojianet_ms::lite::Model::Import(buf, size);
  delete[] buf;
  if (model == nullptr) {
    MS_LOGE("Import model failed");
    return reinterpret_cast<jlong>(nullptr);
...
extern "C" JNIEXPORT void JNICALL Java_com_luojianet_ms_lite_Model_free(JNIEnv *env, jobject thiz, jlong model_ptr) {
  auto *pointer = reinterpret_cast<void *>(model_ptr);
  if (pointer == nullptr) {
    MS_LOGE("Model pointer from java is nullptr");
    return;
  }
  auto *lite_model_ptr = static_cast<luojianet_ms::lite::Model *>(pointer);
  delete (lite_model_ptr);
}

extern "C" JNIEXPORT void JNICALL Java_com_luojianet_ms_lite_Model_freeBuffer(JNIEnv *env, jobject thiz, jlong model_ptr) {
  auto *pointer = reinterpret_cast<void *>(model_ptr);
  if (pointer == nullptr) {
    MS_LOGE("Model pointer from java is nullptr");
    return;
