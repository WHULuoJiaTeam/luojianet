...
extern "C" JNIEXPORT jlong JNICALL Java_com_luojianet_ms_ModelParallelRunner_init(JNIEnv *env, jobject thiz,
                                                                               jstring model_path,
                                                                               jlong runner_config_ptr) {
  auto runner = new (std::nothrow) luojianet_ms::ModelParallelRunner();
  if (runner == nullptr) {
  ...
  if (runner_config_ptr == 0L) {
    ...
    auto *c_runner_config = reinterpret_cast<luojianet_ms::RunnerConfig *>(runner_config_ptr);
    ...
    auto copy_context = std::make_shared<luojianet_ms::Context>();
    ...
    for (size_t i = 0; i < origin_device_list.size(); i++) {
      ...
      if (origin_device->GetDeviceType() == luojianet_ms::kCPU) {
        auto origin_device_info = origin_device->Cast<luojianet_ms::CPUDeviceInfo>();
        auto copy_device_info = std::make_shared<luojianet_ms::CPUDeviceInfo>();
      ...
      } else if (origin_device->GetDeviceType() == luojianet_ms::kGPU) {
        auto origin_device_info = origin_device->Cast<luojianet_ms::GPUDeviceInfo>();
        auto copy_device_info = std::make_shared<luojianet_ms::GPUDeviceInfo>();
        if (copy_device_info == nullptr) {
          MS_LOGE("new copy_device_info is nullptr.");
          delete runner;
          return (jlong) nullptr;
    ...
    auto runner_config = std::make_shared<luojianet_ms::RunnerConfig>();
...
jobject GetParallelInOrOutTensors(JNIEnv *env, jobject thiz, jlong model_parallel_runner_ptr, bool is_input) {
  ...
  auto *pointer = reinterpret_cast<luojianet_ms::ModelParallelRunner *>(model_parallel_runner_ptr);
  if (pointer == nullptr) {
...
extern "C" JNIEXPORT jobject JNICALL Java_com_luojianet_ms_ModelParallelRunner_getInputs(JNIEnv *env, jobject thiz,
                                                                                      jlong model_parallel_runner_ptr) {
  return GetParallelInOrOutTensors(env, thiz, model_parallel_runner_ptr, true);
}

extern "C" JNIEXPORT jobject JNICALL
Java_com_luojianet_ms_ModelParallelRunner_getOutputs(JNIEnv *env, jobject thiz, jlong model_parallel_runner_ptr) {
  return GetParallelInOrOutTensors(env, thiz, model_parallel_runner_ptr, false);
}

extern "C" JNIEXPORT jobject JNICALL Java_com_luojianet_ms_ModelParallelRunner_predict(JNIEnv *env, jobject thiz,
                                                                                    jlong model_parallel_runner_ptr,
                                                                                    jlongArray inputs) {
  jclass array_list = env->FindClass("java/util/ArrayList");
  jmethodID array_list_construct = env->GetMethodID(array_list, "<init>", "()V");
...
extern "C" JNIEXPORT void JNICALL Java_com_luojianet_ms_ModelParallelRunner_free(JNIEnv *env, jobject thiz,
                                                                              jlong model_parallel_runner_ptr) {
  auto *pointer = reinterpret_cast<luojianet_ms::ModelParallelRunner *>(model_parallel_runner_ptr);
  if (pointer == nullptr) {
