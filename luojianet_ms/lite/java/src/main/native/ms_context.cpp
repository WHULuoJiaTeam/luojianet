...
extern "C" JNIEXPORT jlong JNICALL Java_com_luojianet_ms_config_MSContext_createMSContext(JNIEnv *env, jobject thiz,
                                                                                       jint thread_num,
                                                                                       jint cpu_bind_mode,
                                                                                       jboolean enable_parallel) {
  auto *context = new (std::nothrow) luojianet_ms::Context();
  if (context == nullptr) {
...
extern "C" JNIEXPORT jlong JNICALL Java_com_luojianet_ms_config_MSContext_createDefaultMSContext(JNIEnv *env,
                                                                                              jobject thiz) {
  auto *context = new (std::nothrow) luojianet_ms::Context();
  if (context == nullptr) {
    MS_LOGE("new Context fail!");
...
extern "C" JNIEXPORT jboolean JNICALL Java_com_luojianet_ms_config_MSContext_addDeviceInfo(
  ...
  auto *c_context_ptr = static_cast<luojianet_ms::Context *>(pointer);
  auto &device_list = c_context_ptr->MutableDeviceInfo();
  switch (device_type) {
    case 0: {
      auto cpu_device_info = std::make_shared<luojianet_ms::CPUDeviceInfo>();
    ...
    case 1:  // DT_GPU
    {
      auto gpu_device_info = std::make_shared<luojianet_ms::GPUDeviceInfo>();
    ...
    {
      auto npu_device_info = std::make_shared<luojianet_ms::KirinNPUDeviceInfo>();
...
extern "C" JNIEXPORT void JNICALL Java_com_luojianet_ms_config_MSContext_free(JNIEnv *env, jobject thiz,
                                                                           jlong context_ptr) {
  auto *pointer = reinterpret_cast<void *>(context_ptr);
  if (pointer == nullptr) {
    MS_LOGE("Context pointer from java is nullptr");
