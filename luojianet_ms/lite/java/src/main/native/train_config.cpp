...
extern "C" JNIEXPORT jlong JNICALL Java_com_luojianet_ms_config_TrainCfg_createTrainCfg(JNIEnv *env, jobject thiz,
                                                                                     jstring loss_name,
                                                                                     jint optimizationLevel,
                                                                                     jboolean accmulateGrads) {
  luojianet_ms::OptimizationLevel ol;
  switch (optimizationLevel) {
    case 0:
      ol = luojianet_ms::OptimizationLevel::kO0;
      break;
    case kOLKeepBatchNorm:
      ol = luojianet_ms::OptimizationLevel::kO2;
      break;
    case kOLNotKeepBatchNorm:
      ol = luojianet_ms::OptimizationLevel::kO3;
      break;
    case kOLAuto:
      ol = luojianet_ms::OptimizationLevel::kAuto;
      break;
    default:
      MS_LOGE("Invalid optimization_type : %d", optimizationLevel);
      return (jlong) nullptr;
  }
  auto *traincfg_ptr = new (std::nothrow) luojianet_ms::TrainCfg();
...
extern "C" JNIEXPORT jlong JNICALL Java_com_luojianet_ms_config_TrainCfg_addMixPrecisionCfg(JNIEnv *env, jobject thiz,
                                                                                         jlong train_cfg_ptr,
                                                                                         jboolean dynamic_loss_scale,
                                                                                         jfloat loss_scale,
                                                                                         jint thresholdIterNum) {
  luojianet_ms::MixPrecisionCfg mix_precision_cfg;
  ...
  auto *c_train_cfg_ptr = static_cast<luojianet_ms::TrainCfg *>(pointer);
  c_train_cfg_ptr->mix_precision_cfg_ = mix_precision_cfg;
  return jboolean(true);
}

extern "C" JNIEXPORT void JNICALL Java_com_luojianet_ms_config_TrainCfg_free(JNIEnv *env, jobject thiz,
                                                                          jlong train_cfg_ptr) {
  auto *pointer = reinterpret_cast<void *>(train_cfg_ptr);
  if (pointer == nullptr) {
    MS_LOGE("Context pointer from java is nullptr");
  ...
  auto *c_cfg_ptr = static_cast<luojianet_ms::TrainCfg *>(pointer);
  delete (c_cfg_ptr);
}
