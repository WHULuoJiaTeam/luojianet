...
#ifndef LUOJIANET_MS_LITE_MICRO_LIBRARY_SOURCE_SESSION_H_
#define LUOJIANET_MS_LITE_MICRO_LIBRARY_SOURCE_SESSION_H_
...
#define MS_NULLPTR_IF_ERROR(ptr)            \
  do {                                      \
    if ((ptr) != luojianet_ms::lite::RET_OK) { \
      return nullptr;                       \
    }                                       \
  } while (0)
...
class LiteSession : public session::LiteSession {
 public:
  ...
  luojianet_ms::tensor::MSTensor *GetInputsByTensorName(const String &tensor_name) const override { return nullptr; }
  ...
  luojianet_ms::tensor::MSTensor *GetOutputByTensorName(const String &tensor_name) const override;
...
#endif  // LUOJIANET_MS_LITE_MICRO_LIBRARY_SOURCE_SESSION_H_
...
luojianet_ms::tensor::MSTensor *LiteSession::GetOutputByTensorName(const String &tensor_name) const {
  for (const auto &output : outputs_) {
