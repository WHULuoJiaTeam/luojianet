...
static luojianet_ms::ThreadPool *g_pool = nullptr;

int CreateThreadPool(int thread_num) {
  g_pool = luojianet_ms::ThreadPool::CreateThreadPool(thread_num);
  if (g_pool == nullptr) {
    return luojianet_ms::THREAD_ERROR;
  }
  return luojianet_ms::THREAD_OK;
}
...
int SetCoreAffinity(int bind_mode) {
  ...
  return g_pool->SetCpuAffinity(static_cast<luojianet_ms::BindMode>(bind_mode));
}
