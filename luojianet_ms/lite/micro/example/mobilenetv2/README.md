...
本教程以MobileNetv2在安卓手机编译部署为例，使用用户快速了解codegen在安卓平台生成代码、工程构建以及部署的一系列流程。关于converter、codegen的获取以及详细参数介绍可参考luojianet的[编译构建介绍](https://www.luojianet_ms.cn/lite/docs/zh-CN/master/use/build.html)。
...
进入`luojianet_ms/luojianet_ms/lite/micro/examples/mobilenetv2`目录执行脚本`mobilenetv2.sh`自动生成模型推理代码并编译工程目录

```
bash mobilenetv2.sh
```

codegen编译[MobileNetv2模型](https://download.luojianet_ms.cn/model_zoo/official/lite/mobilenetv2_imagenet/r1.2/mobilenetv2.ms)，生成对应的模型推理代码。具体命令如下：

```bash
./codegen --codePath=. --modelPath=mobilenetv2.ms --target=ARM64
```

关于codegen的更多使用命令说明，可参见[codegen工具的详细介绍](https://www.luojianet_ms.cn/lite/docs/zh-CN/master/use/downloads.html)
...
在编译此工程之前需要预先获取安卓平台对应的[Release包](https://www.luojianet_ms.cn/lite/docs/zh-CN/master/use/downloads.html)。
...
│   ├── include     # 推理框架头文件
│   ├── lib         # 推理框架库
│   │   ├── libluojianet-lite.a  # LuoJiaNet Lite推理框架的静态库
│   │   └── libluojianet-lite.so # LuoJiaNet Lite推理框架的动态库
│   ├── minddata    # 图像处理库
│   │   ├── include
│   │   └── lib
...
### [Linux_x86_64编译部署](https://www.luojianet_ms.cn/lite/docs/zh-CN/master/use/micro.html)

### [STM32F746编译部署](https://gitee.com/luojianet_ms/luojianet_ms/tree/master/luojianet_ms/lite/micro/example/mnist_stm32f746)
