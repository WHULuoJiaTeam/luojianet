...
using luojianet_ms::dataset::Path;
using luojianet_ms::dataset::Tensor;

using TensorOperation = luojianet_ms::dataset::TensorOperation;
using RotateOperation = luojianet_ms::dataset::vision::RotateOperation;

using luojianet_ms::LogStream;
using luojianet_ms::MsLogLevel::DEBUG;
...
class MDToDApi {
 public:
  std::shared_ptr<luojianet_ms::dataset::AlbumOp> _iter;
...
extern "C" MDToDApi *MDToDApi_createPipeLine(MDToDConf_t MDConf) {
  ...
  if (std::find(column_names.begin(), column_names.end(), "image") != column_names.end()) {
    ...
    if ((MDConf.ResizeSizeWH[0] != 0) && (MDConf.ResizeSizeWH[1] != 0)) {
      std::shared_ptr<TensorOperation> resize_op =
        luojianet_ms::dataset::vision::Resize({MDConf.ResizeSizeWH[0], MDConf.ResizeSizeWH[1]});
      MS_LOG(INFO) << "Push back resize";
      mapOperations.push_back(resize_op);
    }
    if (MDConf.fixOrientation == 1) {
      std::shared_ptr<TensorOperation> rotate_op = luojianet_ms::dataset::vision::Rotate();
    ...
    if ((MDConf.CropSizeWH[0] != 0) && (MDConf.CropSizeWH[1] != 0)) {
      ...
      std::shared_ptr<TensorOperation> center_crop_op = luojianet_ms::dataset::vision::CenterCrop(Crop);
  ...
  std::shared_ptr<luojianet_ms::dataset::AlbumOp> iter = nullptr;
  const std::set<std::string> exts = {};
  if (MDConf.fileid > -1) {
    // read specific image using SequentialSampler witn
    iter =
      std::make_shared<luojianet_ms::dataset::AlbumOp>(folder_path, true, schema_file, column_names, exts, MDConf.fileid);
  } else {
    iter = std::make_shared<luojianet_ms::dataset::AlbumOp>(folder_path, true, schema_file, column_names, exts);
  }
...
void GetTensorToBuff(std::unordered_map<std::string, std::shared_ptr<Tensor>> row, std::string columnName,
  ...
  if (column != NULL) {
    ...
    if (column->shape()[firstDim] > 0) {
      if (luojianet_ms::dataset::DataType::DE_STRING == column->type()) {
        std::string str;
        ...
        luojianet_ms::dataset::DataHelper dh;
        resBuff->DataSize =
  ...
  if (row.size() != 0 && ret) {
    ...
    if ((pMDToDApi->_augs).size() > 0) {
      ...
      for (int i = 0; i < (pMDToDApi->_augs).size(); i++) {
        ...
        luojianet_ms::MSTensor image(std::make_shared<luojianet_ms::dataset::DETensor>(row["image"]));
        (void)luojianet_ms::dataset::Execute((pMDToDApi->_augs)[i])(image, &image);
        luojianet_ms::dataset::Tensor::CreateFromMemory(
          luojianet_ms::dataset::TensorShape(image.Shape()),
          luojianet_ms::dataset::MSTypeToDEType(static_cast<luojianet_ms::TypeId>(image.DataType())),
