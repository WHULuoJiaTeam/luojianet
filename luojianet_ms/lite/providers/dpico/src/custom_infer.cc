...
Status SetOutputFormat(const std::map<std::string, std::string> &attrs, std::vector<luojianet_ms::MSTensor> *outputs) {
  if (outputs == nullptr) {
    MS_LOG(ERROR) << "the function input parameter is nullptr.";
    return kLiteError;
  }
...
Status CustomInterface::InferShapeJudge(std::vector<luojianet_ms::MSTensor> *inputs,
...
Status CustomInterface::InferRecurrentTwoOutputProcess(const luojianet_ms::schema::Primitive *primitive,
...
Status CustomInterface::Infer(std::vector<luojianet_ms::MSTensor> *inputs, std::vector<luojianet_ms::MSTensor> *outputs,
                              const luojianet_ms::schema::Primitive *primitive, const kernel::Kernel *kernel) {
  ...
  if (type != luojianet_ms::schema::PrimitiveType_Custom) {
