...
#ifndef LUOJIANET_MS_LITE_SRC_RUNTIME_AGENT_NNIE_NNIE_COMMON_H_
#define LUOJIANET_MS_LITE_SRC_RUNTIME_AGENT_NNIE_NNIE_COMMON_H_
#include <iostream>
#include <string>
#include <vector>
...
int NnieCommCreate(NnieRunCfg *nnie_run_cfg, char *model_buf, int size, const std::vector<luojianet_ms::MSTensor> &inputs);

size_t GetFillIndex(const std::vector<luojianet_ms::MSTensor> &inputs, size_t input_size, const HI_CHAR *name);

void NnieCommDelete(NnieParam *pstNnieParamm, NnieModel *nnie_model);

int NnieCommRun(NnieRunCfg *nnie_run_cfg, bool run_box);

int NnieCommFillData(NnieRunCfg *nnie_run_cfg, void *data, luojianet_ms::DataType dtype, int64_t *shape, int size, int id);

int NnieCommGetOutputData(NnieRunCfg *nnie_run_cfg, float *data, int64_t *shape, int size, int tensor_index);
}  // namespace nnie
}  // namespace luojianet_ms
#endif  // LUOJIANET_MS_LITE_SRC_RUNTIME_AGENT_NNIE_NNIE_COMMON_H_
