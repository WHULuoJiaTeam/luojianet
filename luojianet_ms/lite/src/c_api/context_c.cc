...
MSContextHandle MSContextCreate() {
  auto impl = new (std::nothrow) luojianet_ms::ContextC;
  if (impl == nullptr) {
...
void MSContextDestroy(MSContextHandle *context) {
  if (context != nullptr && *context != nullptr) {
    auto impl = static_cast<luojianet_ms::ContextC *>(*context);
    delete impl;
    *context = nullptr;
  }
}
...
void MSContextAddDeviceInfo(MSContextHandle context, MSDeviceInfoHandle device_info) {
  ...
  auto impl = static_cast<luojianet_ms::ContextC *>(context);
  std::shared_ptr<luojianet_ms::DeviceInfoC> device(static_cast<luojianet_ms::DeviceInfoC *>(device_info));
  impl->device_info_list.push_back(device);
}

// ================ DeviceInfo ================
MSDeviceInfoHandle MSDeviceInfoCreate(MSDeviceType device_type) {
  luojianet_ms::DeviceInfoC *impl = new (std::nothrow) luojianet_ms::DeviceInfoC;
  if (impl == nullptr) {
    MS_LOG(ERROR) << "memory allocation failed.";
    return nullptr;
  }
...
void MSDeviceInfoDestroy(MSDeviceInfoHandle *device_info) {
  if (device_info != nullptr && *device_info != nullptr) {
    auto impl = static_cast<luojianet_ms::DeviceInfoC *>(*device_info);
    delete impl;
    *device_info = nullptr;
  }
}
