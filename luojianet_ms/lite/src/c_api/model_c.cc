...
class ModelC {
 ...
 private:
  ...
  std::map<luojianet_ms::tensor::MSTensor *, MSTensor::Impl *> tensor_map_;
  ...
  MSTensor::Impl *TensorToTensorImpl(luojianet_ms::tensor::MSTensor *tensor);
};
...
Status ModelC::Predict(const MSTensorHandle *inputs, size_t input_num, MSTensorHandle **outputs, size_t *output_num,
  ...
  for (size_t i = 0; i < input_num; i++) {
    ...
    auto user_input = static_cast<luojianet_ms::MSTensor::Impl *>(inputs[i]);
...
Status ModelC::RunGraph(const MSKernelCallBackC &before, const MSKernelCallBackC &after) {
  ...
  auto before_call_back = [&](const std::vector<luojianet_ms::tensor::MSTensor *> &before_inputs,
                              const std::vector<luojianet_ms::tensor::MSTensor *> &before_outputs,
                              const CallBackParam &call_param) {
    std::vector<luojianet_ms::MSTensor::Impl> inputs_impl;
    std::vector<luojianet_ms::MSTensor::Impl> outputs_impl;
  ...
  auto after_call_back = [&](const std::vector<luojianet_ms::tensor::MSTensor *> &after_inputs,
...
MSTensor::Impl *ModelC::TensorToTensorImpl(luojianet_ms::tensor::MSTensor *tensor) {
...
MSTensor::Impl **ModelC::GetOutputs(size_t *output_num) {
  ...
  *output_num = outputs.size();
  ...
  std::transform(outputs.begin(), outputs.end(), std::back_inserter(outputs_),
                 [&](std::unordered_map<std::string, luojianet_ms::tensor::MSTensor *>::value_type iter) {
...
MSModelHandle MSModelCreate() {
  auto impl = new (std::nothrow) luojianet_ms::ModelC();
  if (impl == nullptr) {
...
void MSModelDestroy(MSModelHandle *model) {
  if (model != nullptr && *model != nullptr) {
    auto impl = static_cast<luojianet_ms::ModelC *>(*model);
    delete impl;
    *model = nullptr;
  }
}
...
MSStatus MSModelBuild(MSModelHandle model, const void *model_data, size_t data_size, MSModelType model_type,
  ...
  luojianet_ms::ContextC *context = static_cast<luojianet_ms::ContextC *>(model_context);
  auto impl = static_cast<luojianet_ms::ModelC *>(model);
  auto ret = impl->Build(model_data, data_size, static_cast<luojianet_ms::ModelType>(model_type), context);
...
MSStatus MSModelBuildFromFile(MSModelHandle model, const char *model_path, MSModelType model_type,
  ...
  luojianet_ms::ContextC *context = static_cast<luojianet_ms::ContextC *>(model_context);
  auto impl = static_cast<luojianet_ms::ModelC *>(model);
  auto ret = impl->Build(model_path, static_cast<luojianet_ms::ModelType>(model_type), context);
  return static_cast<MSStatus>(ret.StatusCode());
}
...
MSStatus MSModelResize(MSModelHandle model, const MSTensorHandleArray inputs, MSShapeInfo *shape_infos,
  ...
  std::vector<luojianet_ms::MSTensor::Impl *> vec_inputs;
  std::transform(inputs.handle_list, inputs.handle_list + inputs.handle_num, std::back_inserter(vec_inputs),
                 [](MSTensorHandle value) { return static_cast<luojianet_ms::MSTensor::Impl *>(value); });
