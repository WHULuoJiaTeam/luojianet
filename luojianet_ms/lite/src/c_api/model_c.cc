...
class ModelC {
 ...
 private:
  ...
  std::map<luojianet_ms::tensor::MSTensor *, LiteTensorImpl *> tensor_map_;
  ...
  LiteTensorImpl *TensorToTensorImpl(luojianet_ms::tensor::MSTensor *tensor);
};
...
Status ModelC::RunGraph(const MSKernelCallBackC &before, const MSKernelCallBackC &after) {
  ...
  if (after != nullptr) {
    after_call_back = [&](const std::vector<luojianet_ms::tensor::MSTensor *> &after_inputs,
...
LiteTensorImpl *ModelC::TensorToTensorImpl(luojianet_ms::tensor::MSTensor *tensor) {
...
MSStatus MSModelResize(MSModelHandle model, const MSTensorHandleArray inputs, MSShapeInfo *shape_infos,
  ...
  std::vector<luojianet_ms::LiteTensorImpl *> vec_inputs;
  std::transform(inputs.handle_list, inputs.handle_list + inputs.handle_num, std::back_inserter(vec_inputs),
                 [](MSTensorHandle value) { return static_cast<luojianet_ms::LiteTensorImpl *>(value); });
