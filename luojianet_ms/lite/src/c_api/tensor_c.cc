...
MSTensorHandle MSTensorCreate(const char *name, MSDataType type, const int64_t *shape, size_t shape_num,
  ...
  auto lite_tensor =
    luojianet_ms::lite::Tensor::CreateTensor(name, static_cast<luojianet_ms::TypeId>(type), vec_shape, data, data_len);
  auto impl = new (std::nothrow) luojianet_ms::LiteTensorImpl(lite_tensor);
...
void MSTensorDestroy(MSTensorHandle *tensor) {
  if (tensor != nullptr && *tensor != nullptr) {
    auto impl = static_cast<luojianet_ms::LiteTensorImpl *>(*tensor);
    delete impl;
    *tensor = nullptr;
  }
}
...
MSTensorHandle MSTensorClone(MSTensorHandle tensor) {
  ...
  auto impl = static_cast<luojianet_ms::LiteTensorImpl *>(tensor);
  auto lite_tensor = static_cast<luojianet_ms::lite::Tensor *>(impl->lite_tensor());
  auto clone = luojianet_ms::lite::Tensor::CopyTensor(*lite_tensor, true, lite_tensor->allocator());
  if (clone == nullptr) {
    MS_LOG(ERROR) << "Failed to allocate tensor.";
    return nullptr;
  }
  auto clone_impl = new (std::nothrow) luojianet_ms::LiteTensorImpl(clone);
...
void MSTensorSetDataType(MSTensorHandle tensor, MSDataType type) {
  ...
  auto impl = static_cast<luojianet_ms::LiteTensorImpl *>(tensor);
  impl->SetDataType(static_cast<luojianet_ms::DataType>(type));
}
...
void MSTensorSetFormat(MSTensorHandle tensor, MSFormat format) {
  ...
  auto impl = static_cast<luojianet_ms::LiteTensorImpl *>(tensor);
  return impl->SetFormat(static_cast<luojianet_ms::Format>(format));
}
