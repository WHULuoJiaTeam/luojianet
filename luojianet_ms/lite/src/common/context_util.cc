...
std::shared_ptr<luojianet_ms::CPUDeviceInfo> CPUDeviceInfoFromCPUDeviceContext(const lite::DeviceContext &cpu_context) {
  if (cpu_context.device_type_ != DT_CPU) {
    MS_LOG(ERROR) << "function input parameter is not cpu context.";
    return nullptr;
  }
  auto cpu_info = std::make_shared<luojianet_ms::CPUDeviceInfo>();
...
std::shared_ptr<luojianet_ms::GPUDeviceInfo> GPUDeviceInfoFromGPUDeviceContext(const lite::DeviceContext &gpu_context) {
  if (gpu_context.device_type_ != DT_GPU) {
    MS_LOG(ERROR) << "function input parameter is not gpu context.";
    return nullptr;
  }
  auto gpu_info = std::make_shared<luojianet_ms::GPUDeviceInfo>();
...
std::shared_ptr<luojianet_ms::KirinNPUDeviceInfo> NPUDeviceInfoFromNPUDeviceContext(
  ...
  auto npu_info = std::make_shared<luojianet_ms::KirinNPUDeviceInfo>();
...
std::shared_ptr<luojianet_ms::AscendDeviceInfo> AscendDeviceInfoFromAscendDeviceContext(
  ...
  auto ascend_info = std::make_shared<luojianet_ms::AscendDeviceInfo>();
...
luojianet_ms::Context *MSContextFromContext(const lite::Context *context) {
  if (context == nullptr) {
    MS_LOG(ERROR) << "context is nullptr";
    return nullptr;
  }
  auto ms_context = new (std::nothrow) luojianet_ms::Context();
  ...
  std::map<DeviceType, std::function<std::shared_ptr<luojianet_ms::DeviceInfoContext>(const lite::DeviceContext &)>>
