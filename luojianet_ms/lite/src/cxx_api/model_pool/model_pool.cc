...
Status ModelPool::SetDefaultOptimalModelNum(const std::shared_ptr<luojianet_ms::Context> &context) {
...
Status ModelPool::InitDefaultContext(const std::shared_ptr<luojianet_ms::Context> &context) {
...
Status ModelPool::ConcatPredictOutput(std::vector<std::vector<MSTensor>> *outputs, std::vector<MSTensor> *new_outputs,
  ...
  for (size_t i = 0; i < outputs->at(0).size(); i++) {
    ...
    auto new_tensor = luojianet_ms::MSTensor::CreateTensor(outputs->at(0)[i].Name(), outputs->at(0)[i].DataType(),
