 ...
 */
#ifndef LUOJIANET_MS_LITE_SRC_DELEGATE_DELEGATE_UTILS
#define LUOJIANET_MS_LITE_SRC_DELEGATE_DELEGATE_UTILS
#include <vector>
...
bool IsSubGraphInputTensor(const std::vector<luojianet_ms::MSTensor> &inputs, luojianet_ms::MSTensor input);

template <typename T>
std::vector<luojianet_ms::MSTensor> GetGraphInTensors(std::vector<T *> ops, std::vector<size_t> *input_index) {
  std::vector<luojianet_ms::MSTensor> inputs;
  auto is_op_output = [&](luojianet_ms::MSTensor tensor) -> bool {
    for (auto op : ops) {
...
template <typename T>
std::vector<luojianet_ms::MSTensor> GetGraphOutTensors(const std::vector<T *> &ops) {
  std::vector<luojianet_ms::MSTensor> outputs;
  auto is_op_input = [&](const luojianet_ms::MSTensor tensor) -> bool {
    for (auto op : ops) {
...
template <typename T>
std::vector<luojianet_ms::MSTensor> GraphInTensors(const std::vector<T *> &ops, DelegateModel<schema::Primitive> *model,
                                                KernelIter from, KernelIter end) {
  auto in_tensors = GetGraphInTensors(ops, nullptr);
  std::vector<luojianet_ms::MSTensor> all_in_tensors;
  for (auto op : ops) {
    for (auto in_tensor : op->inputs()) {
...
template <typename T>
std::vector<luojianet_ms::MSTensor> GraphOutTensors(const std::vector<T *> &ops, DelegateModel<schema::Primitive> *model,
                                                 KernelIter from, KernelIter end) {
  auto out_tensors = GetGraphOutTensors(ops);
  std::vector<luojianet_ms::MSTensor> all_out_tensors;
  for (auto op : ops) {
    for (auto out_tensor : op->outputs()) {
...
template <typename T>
int GetGraphInOutOps(const std::vector<luojianet_ms::MSTensor> &inputs, const std::vector<luojianet_ms::MSTensor> &outputs,
...
#endif  // LUOJIANET_MS_LITE_SRC_DELEGATE_DELEGATE_UTILS
