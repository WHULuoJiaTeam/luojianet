...
Status NPUDelegate::Init() {
  ...
  if (!npu_manager_->IsSupportNPU()) {
    ...
    free(npu_manager_);
    npu_manager_ = nullptr;
    return luojianet_ms::kLiteNotSupport;
  }
...
Status NPUDelegate::Build(DelegateModel<schema::Primitive> *model) {
  ...
  if (ret != RET_OK) {
    ...
    return luojianet_ms::kLiteError;
  }
  return luojianet_ms::kSuccess;
}
...
NPUOp *NPUDelegate::GetOP(kernel::Kernel *kernel, const schema::Primitive *primitive) {
  ...
  for (int i = 0; i < kernel->inputs().size(); i++) {
    luojianet_ms::MSTensor tensor = kernel->inputs()[i];
  ...
  for (int i = 0; i < kernel->outputs().size(); i++) {
    luojianet_ms::MSTensor tensor = kernel->outputs()[i];
...
std::vector<luojianet_ms::MSTensor> GraphOutTensors(const std::vector<NPUOp *> &ops,
