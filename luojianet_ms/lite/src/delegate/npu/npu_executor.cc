...
bool IsSameShapeTensor(luojianet_ms::MSTensor tensor, const std::shared_ptr<hiai::AiTensor> &npu_tensor) {
...
int NPUExecutor::Run(const std::vector<luojianet_ms::MSTensor> &in_tensors,
                     const std::vector<luojianet_ms::MSTensor> &valid_out_tensors,
  ...
  for (size_t i = 0; i < npu_output_tensors_.size(); ++i) {
    luojianet_ms::MSTensor out_tensor = all_out_tensors[i];
