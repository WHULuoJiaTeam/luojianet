...
int ArithmeticNPUOp::IsSupport(const schema::Primitive *primitive, const std::vector<luojianet_ms::MSTensor> &in_tensors,
  ...
  if (type_ == luojianet_ms::schema::PrimitiveType_Less && in_shape_0.size() == 1) {
    MS_LOG(WARNING) << name_ << " dose not support 1d input.";
    return RET_NOT_SUPPORT;
  }
  if (type_ == luojianet_ms::schema::PrimitiveType_Equal && in_shape_0.size() == ARITHMETIC_INPUT_NUM) {
