...
int ArithmeticNPUOp::IsSupport(const schema::Primitive *primitive, const std::vector<luojianet_ms::MSTensor> &in_tensors,
  ...
  if (type == luojianet_ms::schema::PrimitiveType_Less && in_tensors[0].Shape().size() == 1) {
    MS_LOG(WARNING) << name_ << " not support input 1d";
    return RET_NOT_SUPPORT;
  }
  if (type == luojianet_ms::schema::PrimitiveType_Equal && in_tensors[0].Shape().size() == ARITHMETIC_INPUT_NUM) {
...
int ArithmeticNPUOp::Init(const schema::Primitive *primitive, const std::vector<luojianet_ms::MSTensor> &in_tensors,
                          const std::vector<luojianet_ms::MSTensor> &out_tensors) {
  switch (type_) {
...
int ArithmeticNPUOp::SetNPUInputs(const std::vector<luojianet_ms::MSTensor> &in_tensors,
                                  const std::vector<luojianet_ms::MSTensor> &out_tensors,
...
int ArithmeticNPUOp::SetNPUInputs(
  const std::vector<luojianet_ms::MSTensor> &in_tensors, const std::vector<luojianet_ms::MSTensor> &out_tensors,
  const std::vector<ge::Operator *> &npu_inputs,
