...
#ifndef LUOJIANET_MS_LITE_SRC_RUNTIME_DELEGATE_NPU_OP_NPU_OP_
#define LUOJIANET_MS_LITE_SRC_RUNTIME_DELEGATE_NPU_OP_NPU_OP_
#include <utility>
#include <vector>
#include <string>
#include <set>
...
using luojianet_ms::lite::RET_NOT_SUPPORT;
...
class NPUOp {
 public:
  NPUOp(const schema::Primitive *primitive, const std::vector<luojianet_ms::MSTensor> &in_tensors,
  ...
  virtual int IsSupport(const schema::Primitive *primitive, const std::vector<luojianet_ms::MSTensor> &in_tensors,
                        const std::vector<luojianet_ms::MSTensor> &out_tensors) {
    return RET_ERROR;
  }

  virtual int Init(const schema::Primitive *primitive, const std::vector<luojianet_ms::MSTensor> &in_tensors,
                   const std::vector<luojianet_ms::MSTensor> &out_tensors) {
    return RET_ERROR;
  }

  virtual int SetNPUInputs(const std::vector<luojianet_ms::MSTensor> &in_tensors,
                           const std::vector<luojianet_ms::MSTensor> &out_tensors,
  ...
  void set_inputs(const std::vector<luojianet_ms::MSTensor> &in_tensors) { this->inputs_ = in_tensors; }

  void set_input(luojianet_ms::MSTensor in_tensor, int index) {
    MS_ASSERT(index < inputs_.size());
    this->inputs_[index] = in_tensor;
  }

  void set_outputs(const std::vector<luojianet_ms::MSTensor> &out_tensors) { this->outputs_ = out_tensors; }
...
typedef NPUOp *(*NPUGetOp)(const schema::Primitive *primitive, const std::vector<luojianet_ms::MSTensor> &in_tensors,
                           const std::vector<luojianet_ms::MSTensor> &out_tensors, std::string name);

template <class T>
NPUOp *GetNPUOp(const schema::Primitive *primitive, const std::vector<luojianet_ms::MSTensor> &in_tensors,
                const std::vector<luojianet_ms::MSTensor> &out_tensors, std::string name) {
...
#endif  // LUOJIANET_MS_LITE_SRC_RUNTIME_DELEGATE_NPU_OP_NPU_OP_
