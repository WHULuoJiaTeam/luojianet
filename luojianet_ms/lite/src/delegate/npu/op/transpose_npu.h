...
#ifndef LUOJIANET_MS_LITE_SRC_RUNTIME_DELEGATE_NPU_OP_TRANSPOSE_NPU_H_
#define LUOJIANET_MS_LITE_SRC_RUNTIME_DELEGATE_NPU_OP_TRANSPOSE_NPU_H_
#include <vector>
#include <string>
...
class TransposeNPUOp : public NPUOp {
 public:
  TransposeNPUOp(const std::vector<luojianet_ms::MSTensor> &in_tensors,
                 const std::vector<luojianet_ms::MSTensor> &out_tensors, std::vector<int> perm, std::string name)
      : NPUOp(nullptr, in_tensors, out_tensors, name) {
    perm_ = perm;
    type_ = schema::PrimitiveType_Transpose;
  }

  TransposeNPUOp(const schema::Primitive *primitive, const std::vector<luojianet_ms::MSTensor> &in_tensors,
...
#endif  // LUOJIANET_MS_LITE_SRC_RUNTIME_DELEGATE_NPU_OP_TRANSPOSE_NPU_H_
