...
bool CheckFusion(NPUOp *cur_op, const std::vector<luojianet_ms::MSTensor> &graph_outputs) {
...
int UpdatePreTensors(NPUOp *cur_op) {
  ...
  for (auto in_op : cur_op->in_ops()) {
    ...
    luojianet_ms::MSTensor cur_tensor;
...
int UpdatePostTensors(NPUOp *cur_op) {
  luojianet_ms::MSTensor new_post_input;
  for (auto out_op : cur_op->out_ops()) {
