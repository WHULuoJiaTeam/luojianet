...
int NPUInsertTransformPass::InsertNode(NPUOp *op, NPUOp *post_op, size_t post_input_index,
  ...
  std::string op_name;
  std::vector<luojianet_ms::MSTensor> in_tensors;
  ...
  for (auto i = 0; i < in_tensors.size(); ++i) {
    ...
    auto nh2nc_tensor =
      luojianet_ms::MSTensor::CreateTensor(nh2nc_name + "/output0", in_tensor.DataType(), nchw_shape, nullptr, 0);
    ...
    auto nc2nh_tensor =
      luojianet_ms::MSTensor::CreateTensor(nc2nh_name + "/output0", in_tensor.DataType(), nhwc_shape, nullptr, 0);
