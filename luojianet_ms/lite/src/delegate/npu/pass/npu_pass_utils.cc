...
NPUOp *NPUPassUtils::CreateNchw2NhwcOp(const std::vector<luojianet_ms::MSTensor> &in_tensors,
...
NPUOp *NPUPassUtils::CreateNhwc2NchwOp(const std::vector<luojianet_ms::MSTensor> &in_tensors,
                                       const std::vector<luojianet_ms::MSTensor> &out_tensors, const std::string &name) {
...
void NPUPassUtils::UpdateNC2NHTransNodePostOp(NPUOp *op, NPUOp *trans_op, NPUOp *post_op) {
  ...
  auto post_in_tensors = post_op->inputs();
  luojianet_ms::MSTensor old_in_tensor;
...
NPUOp *NPUPassUtils::OpInputFromOp(NPUOp *op, luojianet_ms::MSTensor in_tensor) {
...
std::vector<luojianet_ms::MSTensor> NPUPassUtils::GetNonConstInputs(NPUOp *op) {
  if (op == nullptr) {
    return std::vector<luojianet_ms::MSTensor>{};
  }
  auto type = op->type();
  auto it = nodes2const_index.find(type);
  if (it != nodes2const_index.end()) {
    auto const_input_indices = it->second;
    std::vector<luojianet_ms::MSTensor> non_const_in_tensors;
