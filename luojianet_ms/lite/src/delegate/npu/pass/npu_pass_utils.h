...
#ifndef LUOJIANET_MS_LITE_SRC_RUNTIME_DELEGATE_NPU_PASS_NPU_PASS_UTILS_H_
#define LUOJIANET_MS_LITE_SRC_RUNTIME_DELEGATE_NPU_PASS_NPU_PASS_UTILS_H_
#include <vector>
#include <set>
#include <string>
...
class NPUPassUtils {
 public:
  static NPUOp *CreateNchw2NhwcOp(const std::vector<luojianet_ms::MSTensor> &in_tensors,
                                  const std::vector<luojianet_ms::MSTensor> &out_tensors, const std::string &name);

  static NPUOp *CreateNhwc2NchwOp(const std::vector<luojianet_ms::MSTensor> &in_tensors,
                                  const std::vector<luojianet_ms::MSTensor> &out_tensors, const std::string &name);
  ...
  static NPUOp *OpInputFromOp(NPUOp *op, luojianet_ms::MSTensor in_tensor);
  static std::vector<luojianet_ms::MSTensor> GetNonConstInputs(NPUOp *op);
  static bool Scale4dCase(NPUOp *op);
};
}  // namespace luojianet_ms
#endif  // LUOJIANET_MS_LITE_SRC_RUNTIME_DELEGATE_NPU_PASS_NPU_PASS_UTILS_H_
