...
std::set<luojianet_ms::schema::PrimitiveType> nchw_nodes = {
...
int NPUTransformPass::InsertPreNodes(NPUOp *op, std::vector<NPUOp *> *trans_ops) {
  ...
  if (is_input_op || nchw_nodes.find((*it)->type()) == nchw_nodes.end()) {
    ...
    auto tensor =
      luojianet_ms::MSTensor::CreateTensor(name + "/output0", op->inputs()[0].DataType(), nchw_shape, nullptr, 0);
    if (tensor == nullptr) {
      MS_LOG(ERROR) << "New nchw tensor failed when inserting pre nhwc2nchw op.";
      return RET_ERROR;
    }
    std::vector<luojianet_ms::MSTensor> pre_trans_outputs = {*tensor};
...
int NPUTransformPass::InsertPostNodes(NPUOp *op, std::vector<NPUOp *> *trans_ops,
                                      std::vector<luojianet_ms::MSTensor> graph_outputs) {
  ...
  std::vector<int64_t> nchw_shape = {nhwc_shape[NHWC_N], nhwc_shape[NHWC_C], nhwc_shape[NHWC_H], nhwc_shape[NHWC_W]};
  auto nc2nh_tensor =
    luojianet_ms::MSTensor::CreateTensor(name + "/input0", op->outputs()[0].DataType(), nchw_shape, nullptr, 0);
  ...
  if (is_output_op) {
    std::vector<luojianet_ms::MSTensor> nc2nh_outputs{op->outputs().at(0)};
  ...
  for (auto i = 0; i < post_insert_ops.size(); ++i) {
    ...
    std::vector<luojianet_ms::MSTensor> nc2nh_outputs{};

    auto origin_out_tensor = op->outputs().at(0);
    auto out_tensor_name = op->name() + "_post_trans" + "_Nchw2Nhwc_" + std::to_string(i) + "_out_tensor";
    auto out_tensor = luojianet_ms::MSTensor::CreateTensor(out_tensor_name, origin_out_tensor.DataType(),
...
int NPUTransformPass::Run(NPUGraph *subgraph) {
  ...
  for (size_t i = 0; i < all_ops_->size();) {
    ...
    if (op->type() == schema::PrimitiveType_InstanceNorm && op->inputs().front().format() == luojianet_ms::Format::NCHW) {
      i++;
      continue;
    }
