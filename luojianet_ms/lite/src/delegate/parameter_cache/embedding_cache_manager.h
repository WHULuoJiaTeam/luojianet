...
#ifndef LUOJIANET_MS_LITE_EMBEDDING_CACHE_MANAGER_H_
#define LUOJIANET_MS_LITE_EMBEDDING_CACHE_MANAGER_H_
#include <memory>
#include <map>
#include <string>
#include <vector>
...
class EmbeddingCacheManager {
 public:
  ...
  bool IsCacheTensor(luojianet_ms::MSTensor tensor);
  int CacheHandle(const std::string &tensor_name, luojianet_ms::MSTensor model_input_tensor, void *device_addr);
  Status SetDeviceCacheAddr(const std::string &tensor_name, void *device_mem_addr, size_t size);
  std::vector<int64_t> GetCacheShape(luojianet_ms::MSTensor tensor);
  size_t GetCacheDataSize(luojianet_ms::MSTensor tensor);
...
#endif  // LUOJIANET_MS_LITE_EMBEDDING_CACHE_MANAGER_H_
