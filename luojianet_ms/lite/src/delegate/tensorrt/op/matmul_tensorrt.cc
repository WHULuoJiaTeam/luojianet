...
int MatMulTensorRT::IsSupport(const luojianet_ms::schema::Primitive *primitive,
...
bool MatMulTensorRT::RunOptPlugin() {
  if (quant_type_ == schema::QuantType_QUANT_NONE && in_tensors_[0].Shape().size() == DIMENSION_2D &&
