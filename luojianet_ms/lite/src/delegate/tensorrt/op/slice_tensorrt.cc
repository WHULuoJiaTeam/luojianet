...
int SliceTensorRT::IsSupport(const luojianet_ms::schema::Primitive *primitive,
...
int SliceTensorRT::ConvertParamsDims() {
  const luojianet_ms::MSTensor &begin = in_tensors_[BEGIN_INDEX];
  const luojianet_ms::MSTensor &stride = in_tensors_[strides_index_];
