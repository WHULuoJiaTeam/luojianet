...
void *TensorRTAllocator::MallocDeviceMem(const luojianet_ms::MSTensor &host_tensor, size_t size) {
  if (host_tensor == NULL) {
    return nullptr;
  }
...
int TensorRTAllocator::SyncMemInHostAndDevice(luojianet_ms::MSTensor host_tensor, const std::string &device_tensor_name,
