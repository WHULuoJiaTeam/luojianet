...
class InnerKernel : public Kernel {
 public:
  ...
  void set_inputs(const std::vector<luojianet_ms::tensor::MSTensor *> &in_tensors) {
    this->in_tensors_.resize(in_tensors.size());
    (void)std::transform(in_tensors.begin(), in_tensors.end(), in_tensors_.begin(),
                         [](luojianet_ms::tensor::MSTensor *tensor) { return static_cast<lite::Tensor *>(tensor); });
  }

  void set_outputs(const std::vector<luojianet_ms::tensor::MSTensor *> &out_tensors) {
  ...
  const std::vector<luojianet_ms::MSTensor> &inputs() override {
    if (inputs_.empty()) {
      std::transform(in_tensors_.begin(), in_tensors_.end(), std::back_inserter(inputs_), [](lite::Tensor *tensor) {
        return luojianet_ms::MSTensor(std::make_shared<luojianet_ms::MSTensor::Impl>(tensor));
      });
    }
    return inputs_;
  }
