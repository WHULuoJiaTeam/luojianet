...
int LiteOpActor::ResizeGraphInput(const std::vector<luojianet_ms::tensor::MSTensor *> &inputs,
...
void LiteOpActor::AsyncOutput(OpContext<Tensor> *context) {
  for (size_t i = 0; i < output_data_arrows_.size(); i++) {
    ...
    Async(output_data_arrows_[i]->to_op_id_, &luojianet_ms::OpActor<Tensor>::RunOpData, data.get(), context);
  }
}
...
std::vector<std::shared_ptr<LiteOpActor>> CreateOpActor(const std::vector<kernel::LiteKernel *> &kernels,
  ...
  for (auto &actor : actors) {
    auto aid = luojianet_ms::Spawn(actor);
  }
  return actors;
}

int MindrtInit() { return luojianet_ms::Initialize("", "", "", ""); }

void MindrtTerminate(const std::vector<std::shared_ptr<LiteOpActor>> &actor_list) {
  for (const auto &actor : actor_list) {
    luojianet_ms::Terminate(actor->GetAID());
  }
}
}  // namespace luojianet_ms::lite
