...
#ifndef LUOJIANET_MS_LITE_SRC_LITE_MODEL_H_
#define LUOJIANET_MS_LITE_SRC_LITE_MODEL_H_
...
class LiteModel : public Model {
 ...
 private:
  ...
  bool CheckQuantAllInit(const flatbuffers::Vector<flatbuffers::Offset<luojianet_ms::schema::QuantParam>> *quant_params);
  ...
  bool ConvertTensors(const T &meta_graph) {
    ...
    for (uint32_t i = 0; i < tensor_count; ++i) {
      ...
      this->all_tensors_.push_back(const_cast<luojianet_ms::schema::Tensor *>(tensor));
    }
    return true;
  }
...
#endif  // LUOJIANET_MS_LITE_SRC_LITE_MODEL_H_
