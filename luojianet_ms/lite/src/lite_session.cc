...
#ifdef USE_GLOG
extern "C" {
extern void luojianet_ms_log_init();
}
#endif
namespace lite {
namespace {
...
#ifdef _MSC_VER
  luojianet_ms::luojianet_ms_log_init();
#endif
#else
  luojianet_ms::luojianet_ms_log_init();
#endif
...
lite::Tensor *LiteSession::ConvertTensor(const schema::Tensor &src_tensor) {
  ...
  if (TypeId(data_type) == kObjectTypeTensorType) {
    ...
    dst_tensor = new (std::nothrow)
      Tensor(TypeId(data_type), shape, static_cast<luojianet_ms::Format>(src_tensor.format()), src_category);
  }
...
std::vector<luojianet_ms::tensor::MSTensor *> LiteSession::GetInputs() const { return this->input_vec_; }
...
int LiteSession::DelegateInit() {
  ...
  if (delegate_ != nullptr) {
    ...
    if (delegate_ret == luojianet_ms::kLiteNotSupport) {
      MS_LOG(DEBUG) << "Delegate is unsupported";
      delegate_.reset();
      delegate_ = nullptr;
    } else if (delegate_ret == luojianet_ms::kSuccess) {
...
luojianet_ms::tensor::MSTensor *LiteSession::GetInputsByTensorName(const std::string &name) const {
...
std::vector<luojianet_ms::tensor::MSTensor *> LiteSession::GetOutputsByNodeName(const std::string &node_name) const {
  auto ret = output_node_map_.find(node_name);
  if (ret == output_node_map_.end()) {
    MS_LOG(WARNING) << "Node  " << node_name << " is not an output node";
    std::vector<luojianet_ms::tensor::MSTensor *> empty_ret;
    return empty_ret;
  }
  return ret->second;
}
...
luojianet_ms::tensor::MSTensor *LiteSession::GetOutputByTensorName(const std::string &tensor_name) const {
...
std::unordered_map<std::string, luojianet_ms::tensor::MSTensor *> LiteSession::GetOutputs() const {
...
int LiteSession::ResizeInputs(const std::vector<luojianet_ms::tensor::MSTensor *> &inputs,
...
int LiteSession::Resize(const std::vector<luojianet_ms::tensor::MSTensor *> &inputs,
                        const std::vector<std::vector<int>> &dims) {
  bool expected = false;
  if (!is_running_.compare_exchange_strong(expected, true)) {
    MS_LOG(ERROR) << "Not support multi-threading";
...
session::LiteSession *session::LiteSession::CreateSession(const lite::Context *context) {
  ...
  luojianet_ms::lite::InnerContext *inner_context = new (std::nothrow) luojianet_ms::lite::InnerContext(context);
...
session::LiteSession *session::LiteSession::CreateSession(const char *model_buf, size_t size,
  ...
  auto ret = reinterpret_cast<lite::LiteSession *>(session)->LoadModelAndCompileByBuf(
    model_buf, luojianet_ms::ModelType::kMindIR_Lite, size);
  if (ret != RET_OK) {
    MS_LOG(ERROR) << "Init session failed";
    delete session;
    return nullptr;
...
session::LiteSession *lite::LiteSession::CreateSession(const std::string &model_path, const lite::Context *context) {
  ...
  auto ret = reinterpret_cast<lite::LiteSession *>(session)->LoadModelAndCompileByPath(
    model_path, luojianet_ms::ModelType::kMindIR_Lite);
  if (ret != RET_OK) {
    MS_LOG(ERROR) << "Init session failed";
    delete session;
    return nullptr;
...
luojianet_ms::ModelType lite::LiteSession::LoadModelByBuff(const char *model_buf, const size_t &buf_size, char **lite_buf,
                                                        size_t *size, luojianet_ms::ModelType model_type) {
  if (model_type == luojianet_ms::ModelType::kMindIR_Lite) {
    *size = buf_size;
    *lite_buf = const_cast<char *>(model_buf);
    return luojianet_ms::ModelType::kMindIR_Lite;
  }

  if (model_type != luojianet_ms::ModelType::kMindIR) {
    return luojianet_ms::ModelType::kUnknownType;
  }
  ...
  return luojianet_ms::ModelType::kMindIR;
}

luojianet_ms::ModelType lite::LiteSession::LoadModelByBuff(const char *model_buf, const size_t &buf_size, char **lite_buf,
                                                        size_t *size, luojianet_ms::ModelType model_type,
                                                        const std::shared_ptr<luojianet_ms::Context> &ms_context) {
  if (model_type == luojianet_ms::ModelType::kMindIR_Lite) {
    *size = buf_size;
    *lite_buf = const_cast<char *>(model_buf);
    return luojianet_ms::ModelType::kMindIR_Lite;
...
const char *lite::LiteSession::LoadModelByPath(const std::string &file, luojianet_ms::ModelType model_type, size_t *size) {
  size_t buf_size;
  ...
  if (buf_model_type == luojianet_ms::ModelType::kUnknownType || lite_buf == nullptr) {
    return nullptr;
  }
  if (buf_model_type == luojianet_ms::ModelType::kMindIR) {
    delete[] model_buf;
    model_buf = nullptr;
  }
#ifdef SHARING_MODEL_WEIGHT
...
int lite::LiteSession::LoadModelAndCompileByBuf(const char *model_buf, luojianet_ms::ModelType model_type,
...
int lite::LiteSession::LoadModelAndCompileByPath(const std::string &model_path, luojianet_ms::ModelType model_type) {
  size_t model_size;
