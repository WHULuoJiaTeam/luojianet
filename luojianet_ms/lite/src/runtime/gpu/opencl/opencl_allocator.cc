...
int OpenCLAllocator::CreateImage2D(size_t size, const ImageSize &img_size, void *data, size_t flags, bool is_map,
  ...
  if (*image == nullptr) {
    delete *buffer;
    *buffer = nullptr;
    MS_LOG(ERROR) << "Create OpenCL Image2D failed! (ERROR CODE: " << luojianet_ms::kernel::CLErrorCode(ret) << ")";
    return RET_ERROR;
  }
  if (ret != CL_SUCCESS) {
    delete *buffer;
    ...
    *buffer = nullptr;
    *image = nullptr;
    MS_LOG(ERROR) << "Create OpenCL Image2D  (ERROR CODE: " << luojianet_ms::kernel::CLErrorCode(ret) << ")";
    return RET_ERROR;
  }
