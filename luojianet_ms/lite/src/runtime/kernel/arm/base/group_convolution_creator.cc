...
lite::Tensor *CreateConstTensor(const lite::Tensor *tensor, const std::vector<int> &shape, const int index) {
  auto new_tensor =
    new (std::nothrow) lite::Tensor(tensor->data_type(), shape, luojianet_ms::NHWC, lite::Category::CONST_TENSOR);
...
int GroupConvCreator::NewInputTensor(std::vector<lite::Tensor *> *tensors) {
  auto in_tensor = CreateVarTensor({input_shape_, luojianet_ms::NHWC, data_type_, lite::Category::VAR, true}, infered_);
