...
using luojianet_ms::lite::RET_FORMAT_ERR;
using luojianet_ms::lite::RET_OK;
using luojianet_ms::lite::RET_PARAM_INVALID;
using luojianet_ms::schema::PrimitiveType_DepthToSpace;
...
int DepthToSpaceCPUKernel::ReSize() {
  if (in_tensors_[0]->format() != luojianet_ms::NHWC) {
...
int DepthToSpaceCPUKernel::Run() {
  ...
  if (input->format() == luojianet_ms::NHWC) {
