...
using luojianet_ms::schema::PrimitiveType_SkipGram;
...
int SkipGramCPUKernel::Run() {
  ...
  StringPack sentence = luojianet_ms::lite::ParseTensorBuffer(in_tensors_.at(0)).at(0);
  ...
  auto ret = luojianet_ms::lite::WriteSeperatedStringsToTensor(out_tensors_.at(0), result);
  return ret;
}
