...
STATUS AclOptionsParser::ParseAclOptions(const luojianet_ms::Context *ctx, AclModelOptions *acl_options) {
  CHECK_NULL_RETURN(ctx);
  CHECK_NULL_RETURN(acl_options);

  auto context = const_cast<luojianet_ms::Context *>(ctx);
...
STATUS AclOptionsParser::ParseOptions(const std::shared_ptr<DeviceInfoContext> &device_info,
  ...
  auto ascend_info = device_info->Cast<luojianet_ms::AscendDeviceInfo>();
