...
CustomAscendKernel::CustomAscendKernel(const std::vector<luojianet_ms::MSTensor> &inputs,
...
STATUS CustomAscendKernel::ProcDynamicInput(std::vector<luojianet_ms::MSTensor> *inputs) {
  ...
  if (!acl_options_.batch_size.empty()) {
    ...
    luojianet_ms::MSTensor batch_size_input("batch", DataType::kNumberTypeInt32, {1}, batch_size, sizeof(int32_t));
  ...
  if (!acl_options_.image_size.empty()) {
    ...
    luojianet_ms::MSTensor image_size_input("batch", DataType::kNumberTypeInt32, {2}, image_size,
...
STATUS CustomAscendKernel::GetRealBatchSize(std::vector<luojianet_ms::MSTensor> *inputs, int32_t *batch_size) {
...
STATUS CustomAscendKernel::GetRealImageSize(std::vector<luojianet_ms::MSTensor> *inputs, int32_t *image_size,
                                            int32_t num) {
  ...
  uint64_t height;
  uint64_t width;
  if (format == luojianet_ms::Format::NHWC) {
...
STATUS CustomAscendKernel::Execute() {
  ...
  std::vector<luojianet_ms::MSTensor> inputs(inputs_.begin(), inputs_.end() - 1);
...
std::shared_ptr<kernel::Kernel> CustomCreateKernel(const std::vector<luojianet_ms::MSTensor> &inputs,
                                                   const std::vector<luojianet_ms::MSTensor> &outputs,
                                                   const schema::Primitive *primitive, const luojianet_ms::Context *ctx) {
