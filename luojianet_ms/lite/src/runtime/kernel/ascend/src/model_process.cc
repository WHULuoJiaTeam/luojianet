...
STATUS ModelProcess::SetBatchSize(const std::vector<luojianet_ms::MSTensor> &inputs) {
...
STATUS ModelProcess::SetImageSize(const std::vector<luojianet_ms::MSTensor> &inputs) {
  for (size_t i = 0; i < inputs.size(); i++) {
    input_infos_[i].buffer_size = inputs[i].DataSize();
  }
...
STATUS ModelProcess::CheckTensorByTensorInfo(const std::vector<luojianet_ms::MSTensor> &tensor,
...
STATUS ModelProcess::ProcDynamicShape(const std::vector<luojianet_ms::MSTensor> &inputs) {
  if (!IsDynamicShape()) {
...
STATUS ModelProcess::CheckAndInitInput(const std::vector<luojianet_ms::MSTensor> &inputs) {
  aclError ret;
...
STATUS ModelProcess::SortTensorInfoByName(const std::vector<luojianet_ms::MSTensor> &tensor,
...
STATUS ModelProcess::PredictFromHost(const std::vector<luojianet_ms::MSTensor> &inputs,
...
STATUS ModelProcess::GetOutputs(std::vector<luojianet_ms::MSTensor> *outputs) {
  if (outputs == nullptr) {
...
STATUS ModelProcess::ConstructTensor(std::vector<luojianet_ms::MSTensor> *outputs) {
  if (outputs == nullptr) {
    MS_LOG(ERROR) << "Ms tensor output is nullptr.";
    return lite::RET_ERROR;
  }
