...
#ifndef LUOJIANET_MS_LITE_SRC_RUNTIME_AGENT_ACL_MODEL_PROCESS_H_
#define LUOJIANET_MS_LITE_SRC_RUNTIME_AGENT_ACL_MODEL_PROCESS_H_
...
class ModelProcess {
 public:
  ...
  STATUS UnLoad();
  STATUS PredictFromHost(const std::vector<luojianet_ms::MSTensor> &inputs, std::vector<luojianet_ms::MSTensor> *outputs);
 ...
 private:
  ...
  STATUS CheckAndInitInput(const std::vector<luojianet_ms::MSTensor> &inputs);
  STATUS SortTensorInfoByName(const std::vector<luojianet_ms::MSTensor> &tensor, std::vector<AclTensorInfo> *tensor_info);
  STATUS CheckTensorByTensorInfo(const std::vector<luojianet_ms::MSTensor> &tensor,
                                 const std::vector<AclTensorInfo> &tensor_info);
  STATUS GetOutputs(std::vector<luojianet_ms::MSTensor> *outputs);
  STATUS ConstructTensor(std::vector<luojianet_ms::MSTensor> *outputs);
  STATUS SetBatchSize(const std::vector<luojianet_ms::MSTensor> &inputs);
  STATUS SetImageSize(const std::vector<luojianet_ms::MSTensor> &inputs);
  STATUS InitInputsBuffer();
  STATUS InitOutputsBuffer();
  STATUS ResetOutputSize();
  STATUS ProcDynamicShape(const std::vector<luojianet_ms::MSTensor> &inputs);
...
#endif  // LUOJIANET_MS_LITE_SRC_RUNTIME_AGENT_ACL_MODEL_PROCESS_H_
