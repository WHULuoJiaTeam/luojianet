...
void SearchSubGraph::ConvertSubGraphToModel(std::vector<Subgraph> *sub_graphs) {
  ...
  for (Subgraph &subgraph : *sub_graphs) {
    ...
    new_partial_node->node_type_ = static_cast<int>(luojianet_ms::lite::NodeType_ValueNode);
...
void SearchSubGraph::InitSearchTensor() {
  ...
  for (size_t i = 0; i < tensors_.size(); i++) {
    ...
    luojianet_ms::schema::Tensor *src_tensor = model_->all_tensors_[i];
    if (src_tensor == nullptr) {
      continue;
    }
    auto category = TensorCategory(*src_tensor);
    if (category == luojianet_ms::lite::Category::CONST_TENSOR || category == luojianet_ms::lite::Category::CONST_SCALAR) {
