...
std::unique_ptr<schema::TensorT> TrainExport::CreateTensor(const luojianet_ms::lite::Tensor *tensor,
...
Model::Node *TrainExport::FindNode(const luojianet_ms::kernel::LiteKernel *kernel, const Model *model) {
  auto nodes = model->all_nodes_;
  auto it = std::find_if(nodes.begin(), nodes.end(),
                         [&kernel](luojianet_ms::lite::Model::Node *n) { return (kernel->name() == n->name_); });
  if (it == nodes.end()) {
    return nullptr;
  }
  return *it;
...
int TrainExport::CreateAndAddCNode(const luojianet_ms::kernel::LiteKernel *kernel, std::vector<uint32_t> inputIndex,
...
std::unique_ptr<schema::CNodeT> TrainExport::CreateCNode(const luojianet_ms::kernel::LiteKernel *kernel,
...
int TrainExport::ExportTensor(const Model *model, const std::vector<luojianet_ms::lite::Tensor *> &tensors, int offset,
  ...
  std::vector<luojianet_ms::lite::Tensor *> in_tensors;
  for (auto index : map_index) {
    auto id = index.first;
    size_t pid = id - static_cast<size_t>(offset);
    luojianet_ms::lite::Tensor *tensor = tensors.at(pid);
...
int TrainExport::ExportNet(const std::vector<luojianet_ms::kernel::LiteKernel *> &kernels,
                           const std::vector<luojianet_ms::lite::Tensor *> &tensors,
