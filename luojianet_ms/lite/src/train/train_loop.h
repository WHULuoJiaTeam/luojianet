 ...
 */
#ifndef LUOJIANET_MS_LITE_SRC_TRAIN_TRAIN_LOOP_H_
#define LUOJIANET_MS_LITE_SRC_TRAIN_TRAIN_LOOP_H_
#include <vector>
#include <string>
#include <tuple>
#include <memory>
...
class TrainLoop : virtual public session::TrainLoop {
 public:
  ...
  int Init(std::vector<luojianet_ms::session::Metrics *> metrics) override {
    metrics_ = metrics;
    return RET_OK;
  }
  ...
  std::vector<luojianet_ms::session::Metrics *> GetMetrics() override { return metrics_; }
 ...
 protected:
  ...
  int batch_size;
  std::vector<luojianet_ms::session::Metrics *> metrics_;
};
}  // namespace lite
}  // namespace luojianet_ms
#endif  // LUOJIANET_MS_LITE_SRC_TRAIN_TRAIN_LOOP_H_
