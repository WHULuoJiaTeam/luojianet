...
int TrainSession::Export(const std::string &file_name, ModelType model_type, QuantizationType quant_type,
  ...
  if (model_type > luojianet_ms::lite::MT_INFERENCE || model_type < luojianet_ms::lite::MT_TRAIN) {
    MS_LOG(ERROR) << "Export model type parameter error";
    return RET_ERROR;
  }
  if (quant_type < luojianet_ms::lite::QT_DEFAULT || quant_type > luojianet_ms::lite::QT_WEIGHT) {
