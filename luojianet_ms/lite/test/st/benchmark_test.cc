...
namespace lite {
class BenchmarkTest : public luojianet_ms::CommonTest {
 public:
  BenchmarkTest() {}
};
...
TEST_F(BenchmarkTest, mindrtParallelOffline1) {
  ...
  int status = luojianet_ms::lite::RunBenchmark(6, benchmark_argv1);
...
TEST_F(BenchmarkTest, mindrtParallelOffline2) {
  const char *benchmark_argv2[] = {"./benchmark",
                                   ...
                                   "--inDataFile=./mindrtParallel/mindrt_parallel_model.bin",
                                   "--benchmarkDataFile=./mindrtParallel/mindrt_parallel_model.out"};
  int status = luojianet_ms::lite::RunBenchmark(6, benchmark_argv2);
  ASSERT_EQ(status, lite::RET_OK);
}
...
TEST_F(BenchmarkTest, mindrtParallelOffline3) {
  const char *benchmark_argv3[] = {"./benchmark",
                                   ...
                                   "--inDataFile=./mindrtParallel/mindrt_parallel_model.bin",
                                   "--benchmarkDataFile=./mindrtParallel/mindrt_parallel_model.out"};
  int status = luojianet_ms::lite::RunBenchmark(6, benchmark_argv3);
  ASSERT_EQ(status, lite::RET_OK);
}
...
TEST_F(BenchmarkTest, runtimeConvert1) {
  ...
  int status = luojianet_ms::lite::RunBenchmark(4, benchmark_argv3);
  ASSERT_EQ(status, lite::RET_OK);
}
#endif
}  // namespace lite
