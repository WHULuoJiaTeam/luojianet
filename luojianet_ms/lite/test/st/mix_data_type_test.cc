...
class MixDataTypeTest : public luojianet_ms::CommonTest {
 public:
  MixDataTypeTest() {}
};
...
void ConstructModel(schema::MetaGraphT *meta_graph) {
  ...
  auto cos2 = std::make_unique<luojianet_ms::schema::CNodeT>();
  cos2->inputIndex = {3};
  cos2->outputIndex = {4};
  cos2->primitive = std::make_unique<luojianet_ms::schema::PrimitiveT>();
  cos2->primitive->value.type = luojianet_ms::schema::PrimitiveType_Cos;
  auto cos2_primitive = new luojianet_ms::schema::CosT;
...
TEST_F(MixDataTypeTest, mix1) {
  ...
  auto impl = std::make_shared<luojianet_ms::ModelImpl>();
  ...
  /* set input data */
  std::vector<luojianet_ms::MSTensor> inputs = impl->GetInputs();
  auto in = inputs[0];
  std::vector<float> in_float = {1.0, 2.0, 3.0, 4.0};
  memcpy(in.MutableData(), in_float.data(), in.DataSize());

  std::vector<luojianet_ms::MSTensor> outputs = impl->GetOutputs();
