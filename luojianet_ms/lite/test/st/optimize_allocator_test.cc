...
class OptAllocator : public luojianet_ms::CommonTest {
 public:
  OptAllocator() = default;
};

void CreateModel1(luojianet_ms::schema::MetaGraphT *meta_graph) {
  ...
  auto add = std::make_unique<luojianet_ms::schema::CNodeT>();
  add->inputIndex = {1, 2};
  add->outputIndex = {3};
  add->primitive = std::make_unique<luojianet_ms::schema::PrimitiveT>();
  add->primitive->value.type = luojianet_ms::schema::PrimitiveType_AddFusion;
  auto add_primitive = new luojianet_ms::schema::AddFusionT;
