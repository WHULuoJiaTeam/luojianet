...
function Run_Converter() {
    cd ${x86_path} || exit 1
    tar -zxf luojianet_ms-lite-${version}-linux-${arch}.tar.gz || exit 1
    cd ${x86_path}/luojianet_ms-lite-${version}-linux-${arch}/ || exit 1
...
model_data_path=/home/workspace/luojianet_ms_dataset/mslite
