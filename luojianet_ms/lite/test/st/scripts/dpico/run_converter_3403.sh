...
function Run_Build_x86() {
  ...
  unset JAVA_HOME
  bash ${luojianet_top_dir}/build.sh -I x86_64 -j 80
  if [ $? = 0 ]; then
    echo "build x86 for dpico success"
    cp ${luojianet_top_dir}/output/*linux-x64.tar.gz ${x86_path}
    mkdir -p ${x86_path}/lib
    cp ${luojianet_top_dir}/luojianet_ms/lite/build/_deps/opencv-4.2-for-dpico-src/lib/* ${x86_path}/lib
    cp ${luojianet_top_dir}/luojianet_ms/lite/build/_deps/protobuf-3.9-for-dpico-src/lib/* ${x86_path}/lib
    cp ${luojianet_top_dir}/luojianet_ms/lite/build/_deps/pico_mapper-src/lib/* ${x86_path}/lib
  else
...
function Run_Build_arm64() {
  export MSLITE_REGISTRY_DEVICE=SD3403
  unset JAVA_HOME
  bash ${luojianet_top_dir}/build.sh -I arm64 -j 80
  if [ $? = 0 ]; then
    echo "build arm64 for dpico success"
    cp ${luojianet_top_dir}/output/*linux-aarch64.tar.gz ${arm64_path}
  else
...
luojianet_top_dir=$(pwd)
echo ${luojianet_top_dir}
x86_path=${luojianet_top_dir}/x86_release/
rm -rf ${x86_path}
mkdir -p ${x86_path}
arm64_path=${luojianet_top_dir}/arm64_release/
rm -rf ${arm64_path}
mkdir -p ${arm64_path}
#set -e
st_dir=${luojianet_top_dir}/luojianet_ms/lite/test/st
...
chmod +x ${luojianet_top_dir}/luojianet_ms/lite/build/tools/benchmark/benchmark
cp -a ${luojianet_top_dir}/luojianet_ms/lite/build/tools/benchmark/benchmark ${benchmark_test_path}/benchmark || exit 1
...
cp -a ${arm64_path}/luojianet_ms-enterprise-lite-${version}-linux-aarch64/runtime/lib/libluojianet-lite.so ${benchmark_test_path}/libluojianet-lite.so || exit 1
cp -a ${luojianet_top_dir}/luojianet_ms/lite/build/_deps/34xx_sdk-src/lib/*so* ${benchmark_test_path} || exit 1
