...
function Run_Build_x86() {
  ...
  unset JAVA_HOME
  bash ${luojianet_ms_top_dir}/build.sh -I x86_64 -j 80
  if [ $? = 0 ]; then
    echo "build x86 for dpico success"
    cp ${luojianet_ms_top_dir}/output/*linux-x64.tar.gz ${x86_path}
    mkdir -p ${x86_path}/lib
    cp ${luojianet_ms_top_dir}/luojianet_ms/lite/build/_deps/opencv-4.2-for-dpico-src/lib/* ${x86_path}/lib
    cp ${luojianet_ms_top_dir}/luojianet_ms/lite/build/_deps/protobuf-3.9-for-dpico-src/lib/* ${x86_path}/lib
    cp ${luojianet_ms_top_dir}/luojianet_ms/lite/build/_deps/pico_mapper-src/lib/* ${x86_path}/lib
  else
...
function Run_Build_arm64() {
  export MSLITE_REGISTRY_DEVICE=SD3403
  unset JAVA_HOME
  bash ${luojianet_ms_top_dir}/build.sh -I arm64 -j 80
  if [ $? = 0 ]; then
    echo "build arm64 for dpico success"
    cp ${luojianet_ms_top_dir}/output/*linux-aarch64.tar.gz ${arm64_path}
  else
...
function Run_Converter() {
    cd ${x86_path} || exit 1
    tar -zxf luojianet_ms-enterprise-lite-${version}-linux-x64.tar.gz || exit 1
    cd ${x86_path}/luojianet_ms-enterprise-lite-${version}-linux-x64/ || exit 1
...
luojianet_ms_top_dir=$(pwd)
echo ${luojianet_ms_top_dir}
x86_path=${luojianet_ms_top_dir}/x86_release/
rm -rf ${x86_path}
mkdir -p ${x86_path}
arm64_path=${luojianet_ms_top_dir}/arm64_release/
rm -rf ${arm64_path}
mkdir -p ${arm64_path}
#set -e
st_dir=${luojianet_ms_top_dir}/luojianet_ms/lite/test/st
...
tar -zxf luojianet_ms-enterprise-lite-${version}-linux-aarch64.tar.gz || exit 1
cd ${arm64_path}/luojianet_ms-enterprise-lite-${version}-linux-aarch64/ || exit 1
chmod +x ${luojianet_ms_top_dir}/luojianet_ms/lite/build/tools/benchmark/benchmark
cp -a ${luojianet_ms_top_dir}/luojianet_ms/lite/build/tools/benchmark/benchmark ${benchmark_test_path}/benchmark || exit 1
cp -a ${arm64_path}/luojianet_ms-enterprise-lite-${version}-linux-aarch64/providers/SD3403/libdpico_acl_adapter.so ${benchmark_test_path}/libdpico_acl_adapter.so || exit 1
cp -a ${arm64_path}/luojianet_ms-enterprise-lite-${version}-linux-aarch64/runtime/lib/libluojianet_ms-lite.so ${benchmark_test_path}/libluojianet_ms-lite.so || exit 1
cp -a ${luojianet_ms_top_dir}/luojianet_ms/lite/build/_deps/34xx_sdk-src/lib/*so* ${benchmark_test_path} || exit 1
