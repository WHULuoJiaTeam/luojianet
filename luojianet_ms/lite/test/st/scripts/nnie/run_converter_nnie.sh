...
function Run_Converter() {
    ...
    while read line; do
        ...
        read -u6
        {
          ...
          echo 'plugin_path='${x86_path}'/luojianet_ms-lite-'${version}'-linux-x64/tools/converter/providers/Hi3516D/libmslite_nnie_converter.so' >> ${ms_config_file}
          ...
          echo 'benchmark_path='${x86_path}'/luojianet_ms-lite-'${version}'-linux-x64/tools/benchmark/benchmark' >> ${ms_config_file}
          ...
          if [ $? = 0 ]; then
              rm -rf ${x86_path}/luojianet_ms-lite-${version}-linux-x64/${model_name}
...
function Run_Hi3516() {
  ...
  tar -zxf luojianet_ms-lite-${version}-linux-aarch32.tar.gz || exit 1
  cd ${arm32_path}/luojianet_ms-lite-${version}-linux-aarch32 || return 1
  ...
  cp -a ./runtime/lib/libluojianet_ms-lite.so ${benchmark_test_path}/libluojianet_ms-lite.so || exit 1
