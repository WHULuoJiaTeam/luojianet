...
if [[ $backend == "arm64_tflite" ]]; then
  ...
  fp32_cfg_file_list=("$models_luojianet_train_config" "$models_posttraining_config" "$models_process_only_config"\
  ...
  fp32_cfg_file_list=("$models_tf_config" "$models_tflite_config" "$models_caffe_config" "$models_onnx_config" "$models_luojianet_config" \
                       "$models_luojianet_train_config" "$models_posttraining_config" "$models_process_only_config"\
