...
function Run_x86_codegen() {
    ...
    local CODEGEN_PATH=${x86_path}/luojianet_ms-lite-${version}-linux-x64/tools/codegen
    rm -rf $1
    mkdir -p $1
    ...
    while read line; do
        ...
        cmake -DPKG_PATH=${x86_path}/luojianet_ms-lite-${version}-linux-x64 $1/${model_name} >> $4
        make >> $4
        # 2. run benchmark
...
function Run_arm_codegen() {
    ...
    tar -zxf luojianet_ms-lite-${version}-android-${package_suffix}.tar.gz || exit 1
    local PKG_PATH=${package_path}/luojianet_ms-lite-${version}-android-${package_suffix}
    local CODEGEN_PATH=${x86_path}/luojianet_ms-lite-${version}-linux-x64/tools/codegen
