...
function Run_cropping() {
    ...
    tar -zxf luojianet_ms-lite-${version}-android-aarch64.tar.gz || exit 1
    cd luojianet_ms-lite-${version}-android-aarch64 || exit 1
    cp -a ./runtime/lib/lib* "${cropper_test_path}"/ || exit 1
    cp -r "${x86_path}"/luojianet_ms-lite-${version}-linux-x64/tools/cropper/ "${cropper_test_path}" || exit 1

    cd "${cropper_test_path}" || exit 1
    echo "${cropper_test_path}"

    ls -l libluojianet_ms-lite.a || exit 1
    ls -l -h libluojianet_ms-lite.a || exit 1
    ls -l -h libluojianet_ms-lite.so || exit 1

    echo "./cropper/cropper --packageFile=./libluojianet_ms-lite.a --configFile=./cropper/cropper_mapping_cpu.cfg --modelFile=${ms_models_path}/${model_name}.ms --outputFile=./libluojianet_ms-lite-${model_name}.a"
    ./cropper/cropper --packageFile=./libluojianet_ms-lite.a --configFile=./cropper/cropper_mapping_cpu.cfg --modelFile=${ms_models_path}/${model_name}.ms --outputFile=./libluojianet_ms-lite-${model_name}.a
    ...
    echo "after cropped:"
    ls -l libluojianet_ms-lite-${model_name}.a || exit 1
    ls -l -h libluojianet_ms-lite-${model_name}.a || exit 1
    mkdir -p mm || exit 1
    cp libluojianet_ms-lite-${model_name}.a mm/ || exit 1
    cd mm/ || exit 1
    ar -x libluojianet_ms-lite-${model_name}.a || exit 1
    ar -d libluojianet_ms-lite-${model_name}.a  *.S.o || exit 1
    cd ../ || exit 1
    echo "after ar -d libluojianet_ms-lite-${model_name}.a  *.S.o"
    ls -l mm/libluojianet_ms-lite-${model_name}.a || exit 1
    ls -l -h mm/libluojianet_ms-lite-${model_name}.a || exit 1
    ...
    -shared -Wl,-soname,libluojianet_ms-lite.so -o libluojianet_ms-lite.so \
    -Wl,--whole-archive ./mm/libluojianet_ms-lite-${model_name}.a -Wl,--no-whole-archive  \
    -llog -ldl -latomic -lm
    ...
    ls -l libluojianet_ms-lite.so  || exit 1
    ls -l -h libluojianet_ms-lite.so  || exit 1
    so_size=`ls libluojianet_ms-lite.so  -l|awk -F ' ' '{print $5}'`
