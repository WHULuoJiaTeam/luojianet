...
function Run_Converter() {
    ...
    if [[ $backend == "all" || $backend == "server_inference_x86" ]]; then
        ...
        cd ${path}/server/luojianet_ms-lite-${version}-linux-x64/ || exit 1
    fi
    if [[ $backend == "all" || $backend == "server_inference_arm" ]]; then
        tar -zxf luojianet_ms-lite-${version}-linux-aarch64.tar.gz || exit 1
        cd ${path}/server/luojianet_ms-lite-${version}-linux-aarch64/ || exit 1
    fi
