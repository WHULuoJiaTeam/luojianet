...
function Run_Converter() {
    ...
    cd ${x86_path} || exit 1
    tar -zxf ${x86_path}/luojianet_ms-lite-${version}-linux-x64.tar.gz || exit 1
...
function Run_TensorRT_Mpirun() {
  ...
  tar -zxf ${x86_path}/tensorrt/luojianet_ms-lite-${version}-linux-x64.tar.gz || exit 1
  tar -zxf luojianet_ms-lite-${version}-linux-x64.tar.gz || exit 1
  cd ${x86_path}/tensorrt/luojianet_ms-lite-${version}-linux-x64/ || exit 1
