...
function Run_Converter() {
    ...
    cd ${x86_path} || exit 1
    tar -zxf ${x86_path}/avx/luojianet_ms-lite-${version}-linux-x64.tar.gz || exit 1
...
function Run_x86() {
    # $1:framework;
    echo 'cd  '${x86_path}'/luojianet_ms-lite-'${version}'-linux-x64' >> "${run_x86_log_file}"
    cd ${x86_path}/luojianet_ms-lite-${version}-linux-x64 || exit 1
...
function Run_x86_sse() {
    ...
    tar -zxf luojianet_ms-lite-${version}-linux-x64.tar.gz || exit 1
    cd ${x86_path}/sse/luojianet_ms-lite-${version}-linux-x64 || exit 1
    export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:./runtime/lib
    cp tools/benchmark/benchmark ./ || exit 1
...
function Run_x86_avx() {
    ...
    tar -zxf luojianet_ms-lite-${version}-linux-x64.tar.gz || exit 1
    cd ${x86_path}/avx/luojianet_ms-lite-${version}-linux-x64 || exit 1
    export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:./runtime/lib
    cp tools/benchmark/benchmark ./ || exit 1
...
function Run_x86_java() {
    cd ${x86_path} || exit 1
    mkdir java || exit 1
    cp ${x86_path}/avx/luojianet_ms-lite-${version}-linux-x64.tar.gz ./java/ || exit 1
    cd ./java || exit 1
    tar -zxf luojianet_ms-lite-${version}-linux-x64.tar.gz || exit 1
    export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:${x86_path}/java/luojianet_ms-lite-${version}-linux-x64/runtime/lib:${x86_path}/java/luojianet_ms-lite-${version}-linux-x64/runtime/third_party/libjpeg-turbo/lib
    # compile benchmark
    echo "javac -cp ${x86_path}/java/luojianet_ms-lite-${version}-linux-x64/runtime/lib/luojianet_ms-lite-java.jar ${basepath}/java/src/main/java/Benchmark.java -d ."
    javac -cp ${x86_path}/java/luojianet_ms-lite-${version}-linux-x64/runtime/lib/luojianet_ms-lite-java.jar ${basepath}/java/src/main/java/Benchmark.java -d .
    ...
    while read line; do
        ...
        echo "java -classpath .:${x86_path}/java/luojianet_ms-lite-${version}-linux-x64/runtime/lib/luojianet_ms-lite-java.jar Benchmark ${ms_models_path}/${model_name}.ms '${models_path}'/input_output/input/${model_name}.ms.bin '${models_path}'/input_output/output/${model_name}.ms.out 1" >> "${run_x86_java_log_file}"
        java -classpath .:${x86_path}/java/luojianet_ms-lite-${version}-linux-x64/runtime/lib/luojianet_ms-lite-java.jar Benchmark ${ms_models_path}/${model_name}.ms ${models_path}/input_output/input/${model_name}.ms.bin ${models_path}/input_output/output/${model_name}.ms.out 1
        if [ $? = 0 ]; then
...
models_luojianet_config=${basepath}/../config/models_luojianet.cfg
models_luojianet_train_config=${basepath}/../config/models_luojianet_train.cfg
...
if [[ $backend == "x86_tflite" ]]; then
  ...
  x86_cfg_file_list=("$models_luojianet_train_config" "$models_posttraining_config" "$models_tflite_awaretraining_config" \
                     ...
                     "$models_weightquant_9bit_config" "$models_process_only_config" "$models_luojianet_config")
else
  x86_cfg_file_list=("$models_tf_config" "$models_tflite_config" "$models_caffe_config" "$models_onnx_config" "$models_luojianet_config" \
                     "$models_luojianet_train_config" "$models_posttraining_config" "$models_tflite_awaretraining_config" \
                     "$models_weightquant_0bit_config" "$models_weightquant_8bit_config" "$models_weightquant_7bit_config" \
