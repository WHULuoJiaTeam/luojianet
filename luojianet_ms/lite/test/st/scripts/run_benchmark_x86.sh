...
function Run_Converter() {
    ...
    tar -zxf luojianet_ms-lite-${version}-linux-*.tar.gz || exit 1
    cd ${x86_path}/luojianet_ms-lite-${version}-linux-*/ || exit 1
...
function Run_x86() {
    # $1:framework;
    echo 'cd  '${x86_path}'/luojianet_ms-lite-'${version}'-linux-*' >> "${run_x86_log_file}"
    cd ${x86_path}/luojianet_ms-lite-${version}-linux-*/ || exit 1
...
function Run_x86_avx512() {
    ...
    tar -zxf luojianet_ms-lite-${version}-linux-x64.tar.gz || exit 1
    cd ${x86_path}/avx512/luojianet_ms-lite-${version}-linux-x64 || exit 1
    export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:./runtime/lib:./runtime/third_party/glog
    cp tools/benchmark/benchmark ./ || exit 1
...
models_luojianet_ms_config=${basepath}/../${config_folder}/models_luojianet_ms.cfg
#models_luojianet_ms_train_config=${basepath}/../${config_folder}/models_luojianet_ms_train.cfg
...
if [[ $backend == "x86_tflite" || $backend == "x86_avx512_tflite" || $backend == "linux_arm64_tflite" ]]; then
  ...
  x86_cfg_file_list=("$models_luojianet_ms_config")
  ...
  x86_cfg_file_list=("$models_tf_config" "$models_tflite_config" "$models_caffe_config" "$models_onnx_config" "$models_luojianet_ms_config" \
