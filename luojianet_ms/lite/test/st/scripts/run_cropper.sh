...
function Run_cropper() {
    ...
    cp -a ./runtime/lib/libluojianet_ms-lite.a "${cropper_test_path}"/libluojianet_ms-lite.a || exit 1
    ...
    while read line; do
        ...
        echo "./cropper/cropper --packageFile=./libluojianet_ms-lite.a --configFile=./cropper/cropper_mapping_npu.cfg --modelFile=${ms_models_path}/${model_name}.ms --outputFile=./libluojianet_ms-lite-${model_name}.a"
         ./cropper/cropper --packageFile=./libluojianet_ms-lite.a --configFile=./cropper/cropper_mapping_npu.cfg --modelFile=${ms_models_path}/${model_name}.ms --outputFile=./libluojianet_ms-lite-${model_name}.a
       ...
       "${ANDROID_NDK}"/toolchains/llvm/prebuilt/linux-x86_64/bin/clang++ -Wl,--whole-archive ./libluojianet_ms-lite-${model_name}.a \
       ...
       -shared -Wl,-soname,libluojianet_ms-lite.so -o libluojianet_ms-lite.so -llog -ldl -latomic -lm
        ...
        adb -s ${device_id} push "${cropper_test_path}"/libluojianet_ms-lite.so /data/local/tmp/cropper_test > adb_push_log.txt

        echo "luojianet_ms run cropper: ${model_name}, accuracy limit:4" >> "${run_cropper_log_file}"
