...
docker_image=luojianet_ms_build:210301
function Run_Export(){
    ...
    # Export luojianet_ms train models:
    while read line; do
        ...
        if [ $? = 0 ]; then
            export_result='export luojianet_ms '${model_name}'_train_export pass';echo ${export_result} >> ${export_result_file}
        else
            export_result='export luojianet_ms '${model_name}'_train_export failed';echo ${export_result} >> ${export_result_file}
        fi
...
function Run_Converter() {
    # $1:cfgFileList
    cd ${x86_path} || exit 1
    tar -zxf luojianet_ms-lite-${version}-linux-x64.tar.gz || exit 1
    cd ${x86_path}/luojianet_ms-lite-${version}-linux-x64/ || exit 1
    ...
    # Convert luojianet_ms train models:
    while read line; do
        LFS=" " read -r -a line_array <<< ${line}
        ...
        read -u6
        {
          ...
          if [ $? = 0 ]; then
              converter_result='converter luojianet_ms '${model_name}' pass';echo ${converter_result} >> ${run_converter_result_file}
          else
              converter_result='converter luojianet_ms '${model_name}' failed';echo ${converter_result} >> ${run_converter_result_file}
              fail=1
          fi
...
function Run_x86() {
    # $1:cfgFileList;
    cd ${x86_path}/luojianet_ms-lite-${version}-linux-x64 || return 1
    export LD_LIBRARY_PATH=${LD_LIBRARY_PATH}:./runtime/lib:./runtime/third_party/libjpeg-turbo/lib
    # Run luojianet_ms converted train models:
    local fail=0
    while read line; do
        LFS=" " read -r -a line_array <<< ${line}
        if [[ ${line_array[0]} == \#* || ${line_array[0]} == "" ]]; then
...
function Run_arm() {
    ...
    # Unzip
    cd ${arm_path} || exit 1
    tar -zxf luojianet_ms-lite-${version_arm}-android-${process_unit}.tar.gz || exit 1

    # If build with minddata, copy the minddata related libs
    cd ${benchmark_train_test_path} || exit 1
    if [ -f ${arm_path}/luojianet_ms-lite-${version_arm}-android-${process_unit}/runtime/lib/libminddata-lite.so ]; then
        cp -a ${arm_path}/luojianet_ms-lite-${version_arm}-android-${process_unit}/runtime/third_party/libjpeg-turbo/lib/libjpeg.so* ${benchmark_train_test_path}/ || exit 1
        cp -a ${arm_path}/luojianet_ms-lite-${version_arm}-android-${process_unit}/runtime/third_party/libjpeg-turbo/lib/libturbojpeg.so* ${benchmark_train_test_path}/ || exit 1
        cp -a ${arm_path}/luojianet_ms-lite-${version_arm}-android-${process_unit}/runtime/lib/libminddata-lite.so ${benchmark_train_test_path}/libminddata-lite.so || exit 1
    fi
    cp -a ${arm_path}/luojianet_ms-lite-${version_arm}-android-${process_unit}/runtime/third_party/hiai_ddk/lib/libhiai.so ${benchmark_train_test_path}/libhiai.so || exit 1
    cp -a ${arm_path}/luojianet_ms-lite-${version_arm}-android-${process_unit}/runtime/third_party/hiai_ddk/lib/libhiai_ir.so ${benchmark_train_test_path}/libhiai_ir.so || exit 1
    cp -a ${arm_path}/luojianet_ms-lite-${version_arm}-android-${process_unit}/runtime/third_party/hiai_ddk/lib/libhiai_ir_build.so ${benchmark_train_test_path}/libhiai_ir_build.so || exit 1
    cp -a ${arm_path}/luojianet_ms-lite-${version_arm}-android-${process_unit}/runtime/lib/libluojianet_ms-lite.so ${benchmark_train_test_path}/libluojianet_ms-lite.so || exit 1
    cp -a ${arm_path}/luojianet_ms-lite-${version_arm}-android-${process_unit}/runtime/lib/libluojianet_ms-lite-train.so ${benchmark_train_test_path}/libluojianet_ms-lite-train.so || exit 1
    cp -a ${arm_path}/luojianet_ms-lite-${version_arm}-android-${process_unit}/tools/benchmark_train/benchmark_train ${benchmark_train_test_path}/benchmark_train || exit 1
...
function Run_CodeExamples() {
    fail=0
    target="x86"
    tarball_path=${x86_path}/luojianet_ms-lite-${version}-linux-x64.tar.gz
    if [[ $backend == "arm64_train" ]]; then
      target="arm64"
      tarball_path=${arm64_path}/luojianet_ms-lite-${version_arm64}-android-aarch64.tar.gz
      export ANDROID_SERIAL=${device_id}
    fi
    should_run_example "train_lenet_java"
    should_run=$?

    export PATH=${x86_path}/luojianet_ms-lite-${version}-linux-x64/tools/converter/converter/:$PATH
    export LD_LIBRARY_PATH=${LD_LIBRARY_PATH}:${x86_path}/luojianet_ms-lite-${version}-linux-x64/tools/converter/lib/:${x86_path}/luojianet_ms-lite-${version}-linux-x64/tools/converter/third_party/glog/lib
