...
7z x -r "%PACKAGE_PATH_CONVERT%\luojianet_ms-lite-*.zip"
IF NOT %errorlevel% == 0 (
    echo "Decompression of runtime tool fail!"
    SET RET_CODE=1
    goto run_eof
)
echo A | 7z x -r "%PACKAGE_PATH%\luojianet_ms-lite-*.zip"
IF NOT %errorlevel% == 0 (
    echo "Decompression of runtime tool fail!"
    SET RET_CODE=1
    goto run_eof
...
for /f %%i in ('dir /b %PACKAGE_PATH%\luojianet_ms-lite-*.zip') do set PACKAGE_NAME=%%i
