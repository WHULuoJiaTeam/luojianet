...
using MSTensor = luojianet_ms::tensor::MSTensor;
using DETensor = luojianet_ms::tensor::DETensor;
using luojianet_ms::dataset::vision::Decode;
using luojianet_ms::dataset::vision::Normalize;
using luojianet_ms::dataset::vision::Resize;
using Execute = luojianet_ms::dataset::Execute;
using Path = luojianet_ms::dataset::Path;

class MindDataTestEager : public luojianet_ms::CommonTest {
 public:
  MindDataTestEager() {}
};
...
TEST_F(MindDataTestEager, Test1) {
  ...
  while (dir_it->hasNext()) {
    ...
    luojianet_ms::dataset::Tensor::CreateFromFile(v.toString(), &de_tensor);
