...
using luojianet_ms::kernel::KernelInterface;
...
struct GpuTensorInfo {
  ...
  explicit GpuTensorInfo(const MSTensor *tensor, registry::opencl::OpenCLRuntimeWrapper *opencl_run) {
    ...
    if (tensor->DataType() == luojianet_ms::DataType::kNumberTypeFloat16) {
...
class CustomAddKernel : public kernel::Kernel {
 public:
  CustomAddKernel(const std::vector<MSTensor> &inputs, const std::vector<MSTensor> &outputs,
                  const schema::Primitive *primitive, const luojianet_ms::Context *ctx, bool fp16_enable)
  ...
  int Prepare() override {
    ...
    for (int i = 0; i < inputs_.size(); ++i) {
      ...
      if (in_tensor.IsConst()) {
        ...
        bool src_is_fp16 = in_tensor.DataType() == luojianet_ms::DataType::kNumberTypeFloat16;
        PackNHWCToNHWC4(in_tensor.MutableData(), weight.data(), src_is_fp16, fp16_enable_, in_shape,
                        in_tensor.DataType());
        DataType dtype =
          fp16_enable_ ? luojianet_ms::DataType::kNumberTypeFloat16 : luojianet_ms::DataType::kNumberTypeFloat32;
  ...
  int CheckSpecs() {
    ...
    for (int i = 0; i < inputs_.size(); ++i) {
      auto &in_tensor = inputs_.at(i);
      if (!in_tensor.IsConst()) {
        if (fp16_enable_ && in_tensor.DataType() == luojianet_ms::DataType::kNumberTypeFloat32) {
          std::cerr << "Inputs data type error, expectation kNumberTypeFloat16 but kNumberTypeFloat32.";
          return lite::RET_ERROR;
        } else if (!fp16_enable_ && in_tensor.DataType() == luojianet_ms::DataType::kNumberTypeFloat16) {
 ...
 private:
  ...
  int CheckOutputs(const std::vector<luojianet_ms::MSTensor> &outputs) {
  ...
  void PackNHWCToNHWC4(void *src, void *dst, bool src_is_fp16, bool dst_is_fp16, const GpuTensorInfo &tensor,
                       luojianet_ms::DataType data_type) {
      ...
      for (int h = 0; h < tensor.H; ++h) {
        for (int w = 0; w < tensor.W; ++w) {
          for (int c = 0; c < tensor.C; ++c, ++src_idx) {
            ...
            if (data_type == luojianet_ms::DataType::kNumberTypeInt32) {
...
class TestGPURegistryCustomOp : public luojianet_ms::CommonTest {
 public:
...
TEST_F(TestGPURegistryCustomOp, TestGPUCustomAdd) {
  ...
  // build a model
  auto model = std::make_shared<luojianet_ms::Model>();
