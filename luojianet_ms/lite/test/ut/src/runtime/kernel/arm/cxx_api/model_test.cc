...
class TestCxxApiLiteModel : public luojianet_ms::CommonTest {
 public:
...
TEST_F(TestCxxApiLiteModel, set_weights_FAILURE) {
  ...
  ASSERT_TRUE(Serialization::Load("./nets/mix_lenet_tod.ms", ModelType::kMindIR, &graph) == kSuccess);
  ASSERT_TRUE(model.Build(GraphCell(graph), context, train_cfg) == kSuccess);
  std::vector<luojianet_ms::MSTensor> changes;
  ASSERT_TRUE(model.UpdateWeights(changes) != kSuccess);
  changes.push_back(
    *MSTensor::CreateTensor("fc4.weight", luojianet_ms::DataType::kNumberTypeFloat32, {NUM_OF_CLASSES}, nullptr, 0));
  ASSERT_TRUE(model.UpdateWeights(changes) != kSuccess);
  changes.clear();
  changes.push_back(
    *MSTensor::CreateTensor("fc3.bias", luojianet_ms::DataType::kNumberTypeFloat32, {NUM_OF_CLASSES}, nullptr, 0));
  ASSERT_TRUE(model.UpdateWeights(changes) == kSuccess);
}
}  // namespace luojianet_ms
