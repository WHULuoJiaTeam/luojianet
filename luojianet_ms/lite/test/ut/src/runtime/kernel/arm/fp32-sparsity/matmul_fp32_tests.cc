...
class TestSPMMFp32 : public luojianet_ms::CommonTest {
 public:
 ...
 protected:
  ...
  std::vector<lite::Tensor *> GetInputs() {
    ...
    auto *in_tensor = new (std::nothrow) Tensor(kNumberTypeFloat, {row_, deep_}, luojianet_ms::NHWC, lite::Category::VAR);
    ...
    auto *filter_tensor =
      new (std::nothrow) Tensor(kNumberTypeFloat, {deep_, col_}, luojianet_ms::NHWC, lite::Category::CONST_TENSOR);
    ...
    auto *bias_tensor =
      new (std::nothrow) Tensor(kNumberTypeFloat, {col_}, luojianet_ms::NHWC, lite::Category::CONST_TENSOR);
...
TEST_F(TestSPMMFp32Accuracy, DenseMatmul) {
  ...
  auto out_tensor = new Tensor(kNumberTypeFloat, {}, luojianet_ms::NHWC, lite::Category::VAR);
