...
#include "luojianet_ms/lite/src/runtime/kernel/arm/base/convolution_base.h"
#include "luojianet_ms/lite/src/kernel_registry.h"

namespace luojianet_ms {
class TestConvolutionDwFp32 : public luojianet_ms::CommonTest {
 public:
...
void InitConvDwCreator(std::vector<lite::Tensor *> *inputs, std::vector<lite::Tensor *> *outputs,
  ...
  input->set_format(luojianet_ms::NHWC);
  ...
  auto weight_data = reinterpret_cast<float *>(luojianet_ms::lite::ReadFile(weight_path.c_str(), &weight_size));
  ...
  output->set_format(luojianet_ms::NHWC);
  output->MallocData();
...
TEST_F(TestConvolutionDwFp32, ConvDwFp32Accuracy) {
  ...
  auto correct_data = reinterpret_cast<float *>(luojianet_ms::lite::ReadFile(output_path.c_str(), &output_size));
