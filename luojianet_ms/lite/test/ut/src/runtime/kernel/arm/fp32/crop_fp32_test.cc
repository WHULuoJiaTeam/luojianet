...
#include "luojianet_ms/lite/src/runtime/kernel/arm/fp32/crop_fp32.h"

namespace luojianet_ms {
class CropTestFp32 : public luojianet_ms::CommonTest {
 public:
...
TEST_F(CropTestFp32, CropTest11) {
  ...
  auto in_t = new lite::Tensor(kNumberTypeFloat, in_shape, luojianet_ms::NHWC, lite::Category::CONST_TENSOR);
  in_t->MallocData();
  memcpy(in_t->MutableData(), input, sizeof(float) * in_t->ElementsNum());
  inputs.push_back(in_t);

  auto out_t = new lite::Tensor(kNumberTypeFloat, out_shape, luojianet_ms::NHWC, lite::Category::CONST_TENSOR);
  out_t->MallocData();
