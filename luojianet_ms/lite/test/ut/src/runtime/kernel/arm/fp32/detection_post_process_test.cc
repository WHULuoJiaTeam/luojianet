...
#include "luojianet_ms/lite/src/runtime/kernel/arm/fp32/detection_post_process_fp32.h"
#include "src/kernel_registry.h"
#include "src/lite_kernel.h"
#include "src/common/file_utils.h"

namespace luojianet_ms {
class TestDetectionPostProcessFp32 : public luojianet_ms::CommonTest {
 public:
...
void DetectionPostProcessTestInit(std::vector<lite::Tensor *> *inputs_, std::vector<lite::Tensor *> *outputs_,
  ...
  auto input_boxes_data =
    reinterpret_cast<float *>(luojianet_ms::lite::ReadFile(input_boxes_path.c_str(), &input_boxes_size));
  auto *input_boxes = new lite::Tensor;
  input_boxes->set_data_type(kNumberTypeFloat32);
  input_boxes->set_format(luojianet_ms::NHWC);
  ...
  auto input_scores_data =
    reinterpret_cast<float *>(luojianet_ms::lite::ReadFile(input_scores_path.c_str(), &input_scores_size));
  auto *input_scores = new lite::Tensor;
  input_scores->set_data_type(kNumberTypeFloat32);
  input_scores->set_format(luojianet_ms::NHWC);
  ...
  auto input_anchors_data =
    reinterpret_cast<uint8_t *>(luojianet_ms::lite::ReadFile(input_anchors_path.c_str(), &input_anchors_size));
  ...
  input_anchors->set_format(luojianet_ms::NHWC);
  ...
  output_boxes->set_format(luojianet_ms::NHWC);
  ...
  output_classes->set_format(luojianet_ms::NHWC);
  ...
  output_scores->set_format(luojianet_ms::NHWC);
  ...
  output_num_det->set_format(luojianet_ms::NHWC);
...
TEST_F(TestDetectionPostProcessFp32, Fast) {
  ...
  auto correct_boxes =
    reinterpret_cast<float *>(luojianet_ms::lite::ReadFile(output_boxes_path.c_str(), &output_boxes_size));
  ...
  auto correct_classes =
    reinterpret_cast<float *>(luojianet_ms::lite::ReadFile(output_classes_path.c_str(), &output_classes_size));
  ...
  auto correct_scores =
    reinterpret_cast<float *>(luojianet_ms::lite::ReadFile(output_scores_path.c_str(), &output_scores_size));
  ...
  auto correct_num_det =
    reinterpret_cast<float *>(luojianet_ms::lite::ReadFile(output_num_det_path.c_str(), &output_num_det_size));
