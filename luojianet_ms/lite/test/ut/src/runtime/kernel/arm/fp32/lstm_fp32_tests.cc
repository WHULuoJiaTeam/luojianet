...
class LstmFp32 : public luojianet_ms::CommonTest {
 public:
  LstmFp32() = default;
};
...
void InitLstmForwardCreator(std::vector<lite::Tensor *> *inputs, std::vector<lite::Tensor *> *outputs,
  ...
  weight_i->set_format(luojianet_ms::NHWC);
  ...
  weight_h->set_format(luojianet_ms::NHWC);
  ...
  bias->set_format(luojianet_ms::NHWC);
  ...
  state->set_format(luojianet_ms::NHWC);
  ...
  cell_state->set_format(luojianet_ms::NHWC);
  ...
  hidden_state->set_format(luojianet_ms::NHWC);
...
TEST_F(LstmFp32, LstmForwardFp32Accuracy) {
  ...
  // register op
  kernel::KernelKey desc = {kernel::KERNEL_ARCH::kCPU, kNumberTypeFloat32, luojianet_ms::schema::PrimitiveType_LSTM};
