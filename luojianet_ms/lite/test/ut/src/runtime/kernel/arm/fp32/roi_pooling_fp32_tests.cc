...
#include "luojianet_ms/lite/src/runtime/kernel/arm/fp32/roi_pooling_fp32.h"
#include "src/kernel_registry.h"
#include "src/lite_kernel.h"

namespace luojianet_ms {
class TestROIPoolingFp32 : public luojianet_ms::CommonTest {
 public:
  TestROIPoolingFp32() {}
};
...
int ROIPoolingTestInit(std::vector<lite::Tensor *> *inputs_, std::vector<lite::Tensor *> *outputs_, float *a_ptr,
  ...
  auto in_t = new lite::Tensor(kNumberTypeFloat, a_shape, luojianet_ms::NHWC, lite::Category::CONST_TENSOR);
  in_t->MallocData();
  memcpy(in_t->MutableData(), a_ptr, sizeof(float) * in_t->ElementsNum());
  inputs_->push_back(in_t);

  auto roi_t = new lite::Tensor(kNumberTypeFloat, b_shape, luojianet_ms::NHWC, lite::Category::CONST_TENSOR);
  roi_t->MallocData();
  memcpy(roi_t->MutableData(), b_ptr, sizeof(float) * roi_t->ElementsNum());
  inputs_->push_back(roi_t);

  auto out_t = new lite::Tensor(kNumberTypeFloat, c_shape, luojianet_ms::NHWC, lite::Category::CONST_TENSOR);
  out_t->MallocData();
