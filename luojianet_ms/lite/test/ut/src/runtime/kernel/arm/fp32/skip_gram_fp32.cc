...
using luojianet_ms::lite::StringPack;
using luojianet_ms::lite::Tensor;

class TestSkipGramFp32 : public luojianet_ms::CommonTest {
 public:
  TestSkipGramFp32() {}
};
...
void SkipGramTestInit(std::vector<Tensor *> *inputs_, std::vector<Tensor *> *outputs_,
  ...
  Tensor *in_t_first = new Tensor(kObjectTypeString, {}, luojianet_ms::NHWC, lite::Category::CONST_TENSOR);
  char sentence[] = "The quick brown fox jumps over the lazy dog";
  std::vector<StringPack> str;
  str.push_back({43, sentence});
  luojianet_ms::lite::WriteStringsToTensor(in_t_first, str);
  inputs_->push_back(in_t_first);

  Tensor *output = new Tensor(kObjectTypeString, {}, luojianet_ms::NHWC, lite::Category::CONST_TENSOR);
  outputs_->push_back(output);

  skip_gram_param->ngram_size = 3;
  skip_gram_param->max_skip_size = 2;
  skip_gram_param->include_all_ngrams = true;
  skip_gram_param->op_parameter_.type_ = luojianet_ms::schema::PrimitiveType_SkipGram;
...
TEST_F(TestSkipGramFp32, ElTest) {
  ...
  std::vector<StringPack> output = luojianet_ms::lite::ParseTensorBuffer(outputs_[0]);
