...
ArithmeticParameter *PopulateArithmeticParameter(luojianet_ms::schema::PrimitiveType type,
...
class TestArithmeticGradFp32 : public luojianet_ms::CommonTest {
 public:
...
std::vector<lite::Tensor *> GenerateTensorsForTest(const char *test, int test_id) {
  ...
  auto dy_data = reinterpret_cast<float *>(luojianet_ms::lite::ReadFile(test, &input_size));
  if (dy_data == nullptr) {
  ...
  auto x1_data = reinterpret_cast<float *>(luojianet_ms::lite::ReadFile(dx1_file, &input_size));
  if (x1_data == nullptr) {
    MS_LOG(ERROR) << "new operator failed";
    delete[] dy_data;
    delete dy_tensor;
  ...
  auto x2_data = reinterpret_cast<float *>(luojianet_ms::lite::ReadFile(dx2_file, &input_size));
  if (x2_data == nullptr) {
    MS_LOG(ERROR) << "new operator failed";
    delete[] dy_data;
    delete dy_tensor;
...
TEST_F(TestArithmeticGradFp32, TestMaximumGradBroadcastFp32) {
  ...
  auto dy_data = reinterpret_cast<float *>(luojianet_ms::lite::ReadFile(yt_path.c_str(), &input_size));
