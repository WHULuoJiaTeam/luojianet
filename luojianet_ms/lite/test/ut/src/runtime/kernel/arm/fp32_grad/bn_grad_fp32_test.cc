...
class TestBNGradFp32 : public luojianet_ms::CommonTest {
 public:
  TestBNGradFp32() {}
...
lite::Tensor *TestBNGradFp32::CreateInTensor(std::string file_name, std::vector<int> dim) {
  size_t input_size = 0;
  auto input_data = reinterpret_cast<float *>(luojianet_ms::lite::ReadFile(file_name.c_str(), &input_size));
...
TEST_F(TestBNGradFp32, BNTtrainFp32) {
  ...
  luojianet_ms::lite::InnerContext context;
