...
#include "luojianet_ms/lite/src/runtime/kernel/arm/fp32_grad/convolution.h"
#include "luojianet_ms/lite/src/runtime/kernel/arm/fp32_grad/convolution_grad_filter.h"
#include "luojianet_ms/lite/src/runtime/kernel/arm/fp32_grad/convolution_grad_input.h"
#include "nnacl/conv_parameter.h"
#include "luojianet_ms/lite/src/kernel_registry.h"

namespace luojianet_ms {
class TestConvolutionGradFp32 : public luojianet_ms::CommonTest {
 public:
...
TEST_F(TestConvolutionGradFp32, ConvFp32FilterGrad) {
  ...
  auto dy_data = reinterpret_cast<float *>(luojianet_ms::lite::ReadFile(dy_path.c_str(), &dy_size));
...
TEST_F(TestConvolutionGradFp32, ConvFp32InputGrad) {
  ...
  auto w_data = reinterpret_cast<float *>(luojianet_ms::lite::ReadFile(w_path.c_str(), &w_size));
  std::vector<int> dim_dw({32, 3, 3, 3});
...
TEST_F(TestConvolutionGradFp32, ConvGroupDilation) {
  ...
  auto x_data = reinterpret_cast<float *>(luojianet_ms::lite::ReadFile(x_path.c_str(), &x_size));
  std::vector<int> dim_x({1, 28, 28, 3});
  lite::Tensor x_tensor(TypeId::kNumberTypeFloat32, dim_x);
  ...
  auto *kernel = new luojianet_ms::kernel::ConvolutionTrainCPUKernel(reinterpret_cast<OpParameter *>(conv_param), inputs,
                                                                  outputs, &context);
  ASSERT_NE(kernel, nullptr);
  kernel->Prepare();
  kernel->AllocWorkspace();
