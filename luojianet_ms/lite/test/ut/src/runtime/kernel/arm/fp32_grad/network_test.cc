...
class NetworkTest : public luojianet_ms::CommonTest {
 public:
  NetworkTest() {}
  int32_t runNet(luojianet_ms::session::LiteSession *session, const std::string &in, const std::string &out,
                 const char *tensor_name, bool debug = false);
};

int32_t fileIterator(luojianet_ms::session::LiteSession *session, const std::string &path,
                     std::function<int32_t(luojianet_ms::session::LiteSession *session, const std::string &)> cb) {
  int32_t res = 0;
...
int32_t NetworkTest::runNet(luojianet_ms::session::LiteSession *session, const std::string &in, const std::string &out,
...
TEST_F(NetworkTest, noname) {
  ...
  auto session = luojianet_ms::session::TrainSession::CreateTrainSession(net, &context, true, &cfg);
  ASSERT_NE(session, nullptr);
...
TEST_F(NetworkTest, setname) {
  ...
  auto session = luojianet_ms::session::TrainSession::CreateTrainSession(net, &context, true, &train_cfg);
  ASSERT_NE(session, nullptr);
