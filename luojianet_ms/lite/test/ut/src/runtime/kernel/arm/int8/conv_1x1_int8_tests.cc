...
#include "luojianet_ms/lite/src/runtime/kernel/arm/int8/convolution_1x1_int8.h"
#include "src/tensor_category.h"

namespace luojianet_ms {
using lite::Tensor;
class TestConv1x1Int8 : public luojianet_ms::CommonTest {
 public:
  TestConv1x1Int8() {}
};
...
int Conv1x1Int8TestInit1_perchannel(std::vector<lite::Tensor *> *inputs_, std::vector<lite::Tensor *> *outputs_,
  ...
  Tensor *in_t = new Tensor(kNumberTypeInt8, {1, 2, 3, 4}, luojianet_ms::NHWC, lite::Category::CONST_TENSOR);
  auto in_quant_arg = new luojianet_ms::lite::LiteQuantParam();
  ...
  Tensor *weight_t = new Tensor(kNumberTypeInt8, {3, 1, 1, 4}, luojianet_ms::NHWC, lite::Category::CONST_TENSOR);
  weight_t->MallocData();
  auto weight_quant_arg1 = new luojianet_ms::lite::LiteQuantParam();
  weight_quant_arg1->zeroPoint = 66, weight_quant_arg1->scale = 0.96439215686275;
  auto weight_quant_arg2 = new luojianet_ms::lite::LiteQuantParam();
  weight_quant_arg2->zeroPoint = 33, weight_quant_arg2->scale = 0.76439215686275;
  auto weight_quant_arg3 = new luojianet_ms::lite::LiteQuantParam();
  ...
  Tensor *out_t = new Tensor(kNumberTypeInt8, {1, 2, 3, 3}, luojianet_ms::NHWC, lite::Category::CONST_TENSOR);
  out_t->MallocData();
  auto output_quant_arg = new luojianet_ms::lite::LiteQuantParam();
...
int Conv1x1Int8TestInit1(std::vector<lite::Tensor *> *inputs_, std::vector<lite::Tensor *> *outputs_,
  ...
  Tensor *weight_t = new Tensor(kNumberTypeInt8, {3, 1, 1, 4}, luojianet_ms::NHWC, lite::Category::CONST_TENSOR);
  auto weight_quant_arg = new luojianet_ms::lite::LiteQuantParam();
...
int Conv1x1Int8TestInit2(std::vector<lite::Tensor *> *inputs_, std::vector<lite::Tensor *> *outputs_,
  ...
  auto input = luojianet_ms::lite::ReadFile(input_path.c_str(), &buffer_size);
  ...
  auto weight = luojianet_ms::lite::ReadFile(weight_path.c_str(), &buffer_size);
  memcpy(weight_t->MutableData(), weight, buffer_size);
  inputs_->push_back(weight_t);
  delete[] weight;

  Tensor *bias_t = new Tensor(kNumberTypeInt32, {4}, luojianet_ms::NHWC, lite::Category::CONST_TENSOR);
  weight_t->MallocData();
  std::string bias_path = "./bias";
  auto bias = luojianet_ms::lite::ReadFile(bias_path.c_str(), &buffer_size);
  ...
  *correct = reinterpret_cast<int8_t *>(malloc(out_t->ElementsNum() * sizeof(int8_t)));
  ...
  auto output = luojianet_ms::lite::ReadFile(output_path.c_str(), &buffer_size);
