...
#include "luojianet_ms/lite/src/runtime/kernel/arm/int8/fullconnection_int8.h"
...
using lite::Tensor;
class TestFcInt8 : public luojianet_ms::CommonTest {
 public:
  TestFcInt8() {}
};
...
lite::Tensor *MakeIntTensor(int *data, int len, std::vector<int> *shape) {
  auto tensor = new lite::Tensor(kNumberTypeInt32, *shape, luojianet_ms::NHWC, lite::Category::CONST_TENSOR);
  tensor->MallocData();
