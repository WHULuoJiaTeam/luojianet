...
#include "luojianet_ms/lite/src/runtime/kernel/arm/int8/matmul_int8.h"
...
class TestMatmulInt8 : public luojianet_ms::CommonTest {
 public:
  TestMatmulInt8() {}
};
...
lite::Tensor *MakeQuantTensor(int8_t *data, int len, std::vector<int> *shape, float scale, int zp) {
  auto tensor = new lite::Tensor(kNumberTypeInt8, *shape, luojianet_ms::NHWC, lite::Category::CONST_TENSOR);
  tensor->MallocData();
  if (data) {
    auto tensor_ptr = reinterpret_cast<int8_t *>(tensor->MutableData());
    memcpy(tensor_ptr, data, len * sizeof(int8_t));
  }
  auto quant_arg = new luojianet_ms::lite::LiteQuantParam();
