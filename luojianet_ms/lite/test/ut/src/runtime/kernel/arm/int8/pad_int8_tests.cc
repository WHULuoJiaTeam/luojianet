...
using luojianet_ms::lite::LiteQuantParam;
using luojianet_ms::lite::Tensor;
class TestPadInt8 : public luojianet_ms::CommonTest {
 public:
  TestPadInt8() {}
};
...
int PadInt8TestInit1(std::vector<Tensor *> *inputs_, std::vector<Tensor *> *outputs_, PadParameter *pad_param,
                     int8_t **correct) {
  Tensor *in_t = new Tensor(kNumberTypeInt8, {3}, luojianet_ms::NHWC, lite::Category::CONST_TENSOR);
  in_t->MallocData();
  int8_t in[] = {1, 1, 1};
  ...
  Tensor *out_t = new Tensor(kNumberTypeInt8, {7}, luojianet_ms::NHWC, lite::Category::CONST_TENSOR);
  out_t->MallocData();
...
int PadInt8TestInit2(std::vector<Tensor *> *inputs_, std::vector<Tensor *> *outputs_, PadParameter *pad_param,
                     int8_t **correct) {
  Tensor *in_t = new Tensor(kNumberTypeInt8, {6, 2}, luojianet_ms::NHWC, lite::Category::VAR);
  in_t->MallocData();
  ...
  Tensor *out_t = new Tensor(kNumberTypeInt8, {10, 5}, luojianet_ms::NHWC, lite::Category::VAR);
  out_t->MallocData();
  LiteQuantParam *out_quant_arg = new LiteQuantParam();
  out_quant_arg->zeroPoint = 10, out_quant_arg->scale = 0.31228156;
  out_t->AddQuantParam(*out_quant_arg);
...
int PadInt8TestInit4(std::vector<Tensor *> *inputs_, std::vector<Tensor *> *outputs_, PadParameter *pad_param,
                     int8_t **correct) {
  Tensor *in_t = new Tensor(kNumberTypeInt8, {2, 3, 2, 1}, luojianet_ms::NHWC, lite::Category::VAR);
  in_t->MallocData();
  ...
  Tensor *out_t = new Tensor(kNumberTypeInt8, {6, 6, 4, 3}, luojianet_ms::NHWC, lite::Category::VAR);
  out_t->MallocData();
  LiteQuantParam *out_quant_arg = new LiteQuantParam();
  out_quant_arg->zeroPoint = 10, out_quant_arg->scale = 0.31228156;
  out_t->AddQuantParam(*out_quant_arg);
