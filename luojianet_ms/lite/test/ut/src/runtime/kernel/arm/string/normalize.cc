...
class TestNormalize : public luojianet_ms::CommonTest {
 public:
  TestNormalize() {}
...
void TestNormalize::NormalizeTestInit() {
  ...
  input_tensor_.set_format(luojianet_ms::NHWC);
  ...
  luojianet_ms::lite::WriteStringsToTensor(&input_tensor_, str_pack);

  output_tensor_.set_data_type(kObjectTypeString);
  output_tensor_.set_format(luojianet_ms::NHWC);
}
