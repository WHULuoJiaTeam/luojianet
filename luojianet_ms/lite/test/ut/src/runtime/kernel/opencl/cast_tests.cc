...
#include "luojianet_ms/lite/src/common/file_utils.h"
#include "luojianet_ms/lite/src/runtime/kernel/opencl/opencl_subgraph.h"
#include "luojianet_ms/lite/src/runtime/kernel/opencl/kernel/cast.h"
...
TEST_F(TestCastSelfOpenCL, Castfp32tofp16) {
  ...
  auto input_data = reinterpret_cast<float *>(luojianet_ms::lite::ReadFile(input1Ppath.c_str(), &input1_size));
  auto correctOutput =
    reinterpret_cast<float16_t *>(luojianet_ms::lite::ReadFile(correctOutputPath.c_str(), &output_size));

  MS_LOG(INFO) << " init tensors ";
  std::vector<int> shape = {1, 23, 39, 47};
  auto tensor_type = lite::Category::CONST_TENSOR;
  auto *input_tensor = new (std::nothrow) lite::Tensor(kNumberTypeFloat32, shape, luojianet_ms::NHWC, tensor_type);
  auto *output_tensor = new (std::nothrow) lite::Tensor(kNumberTypeFloat16, shape, luojianet_ms::NHWC, tensor_type);
...
TEST_F(TestCastSelfOpenCL, Castfp16tofp32) {
  ...
  auto input_data = reinterpret_cast<float16_t *>(luojianet_ms::lite::ReadFile(input1Ppath.c_str(), &input1_size));
  auto correctOutput = reinterpret_cast<float *>(luojianet_ms::lite::ReadFile(correctOutputPath.c_str(), &output_size));

  MS_LOG(INFO) << " init tensors ";
  std::vector<int> shape = {1, 23, 39, 47};
  auto tensor_type = lite::Category::CONST_TENSOR;
  auto *input_tensor = new (std::nothrow) lite::Tensor(kNumberTypeFloat16, shape, luojianet_ms::NHWC, tensor_type);
  auto *output_tensor = new (std::nothrow) lite::Tensor(kNumberTypeFloat32, shape, luojianet_ms::NHWC, tensor_type);
  if (input_tensor == nullptr || output_tensor == nullptr) {
    MS_LOG(INFO) << " new input_tensor or output_tensor failed ";
    return;
  }
