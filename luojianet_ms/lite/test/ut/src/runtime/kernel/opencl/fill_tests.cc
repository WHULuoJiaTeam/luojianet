...
#include "luojianet_ms/lite/src/runtime/kernel/opencl/kernel/fill.h"
using luojianet_ms::lite::Tensor;
using luojianet_ms::schema::PrimitiveType_Fill;
using luojianet_ms::schema::PrimitiveType_Shape;

// PrimitiveType_Fill: src/ops/populate/fill_populate.cc

namespace luojianet_ms::lite::opencl::test {
class TestFillOpenCLCI : public luojianet_ms::CommonTest {
 public:
  TestFillOpenCLCI() {}
};
...
TEST_F(TestFillOpenCLCI, Fp32testfill) {
  ...
  auto in_tensor1 = Tensor(data_type, input_shape1, luojianet_ms::NHWC, lite::Category::VAR);
  auto output_tensor = Tensor(data_type, output_shape, luojianet_ms::NHWC, lite::Category::VAR);
