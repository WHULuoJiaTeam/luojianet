...
TEST_F(TestOpenCL_LayerNorm, test1) {
  ...
  auto input_data = reinterpret_cast<float *>(luojianet_ms::lite::ReadFile(inputPpath.c_str(), &input_size));
  auto gamma_data = reinterpret_cast<float *>(luojianet_ms::lite::ReadFile(gammaPpath.c_str(), &gamma_size));
  auto beta_data = reinterpret_cast<float *>(luojianet_ms::lite::ReadFile(betaPpath.c_str(), &beta_size));
  auto output_data = reinterpret_cast<float *>(luojianet_ms::lite::ReadFile(correctOutputPath.c_str(), &output_size));
