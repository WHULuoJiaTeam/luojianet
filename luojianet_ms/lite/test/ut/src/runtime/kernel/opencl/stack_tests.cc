...
TEST_F(TestOpenCL_Stack, input2_ndim3_axis1) {
  ...
  auto input_data1 = reinterpret_cast<float *>(luojianet_ms::lite::ReadFile(input1Ppath.c_str(), &input1_size));
  auto input_data2 = reinterpret_cast<float *>(luojianet_ms::lite::ReadFile(input2Ppath.c_str(), &input2_size));
