...
class RuntimeConvert : public luojianet_ms::CommonTest {
 public:
...
TEST_F(RuntimeConvert, relu1) {
  ...
  Status build_ret = model.Build("./relu.mindir", luojianet_ms::kMindIR, context);
...
TEST_F(RuntimeConvert, relu2) {
  ...
  auto context = std::make_shared<luojianet_ms::Context>();
  context->MutableDeviceInfo().push_back(std::make_shared<luojianet_ms::CPUDeviceInfo>());
  Status build_ret = model.Build("./relu.mindir", luojianet_ms::kMindIR_Lite, context);
...
TEST_F(RuntimeConvert, relu3) {
  ...
  auto context = std::make_shared<luojianet_ms::Context>();
  context->MutableDeviceInfo().push_back(std::make_shared<luojianet_ms::CPUDeviceInfo>());
  Status build_ret = model.Build(mindir_buf, size, luojianet_ms::kMindIR, context);
  ASSERT_EQ(build_ret, Status::OK());
