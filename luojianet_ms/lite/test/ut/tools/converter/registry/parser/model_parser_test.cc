...
int ModelParserTest::BuildGraphInputs() {
  ...
  for (auto &input : inputs) {
    ...
    auto tensor_info = luojianet_ms::api::MakeShared<luojianet_ms::api::Tensor>(kNumberTypeFloat32, shape);
...
int ModelParserTest::BuildGraphNodes() {
  ...
  for (auto &node_name : model_structure_) {
    ...
    auto tensor_info = luojianet_ms::api::MakeShared<luojianet_ms::api::Tensor>(kNumberTypeFloat32, ShapeVector{});
...
int ModelParserTest::BuildGraphOutputs() {
  ...
  if (outputs.size() > 1) {
    ...
    auto return_prim = luojianet_ms::api::MakeShared<ops::Return>();
