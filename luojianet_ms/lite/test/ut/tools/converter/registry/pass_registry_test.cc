...
using luojianet_ms::converter::ConverterParameters;
using luojianet_ms::converter::kFmkTypeCaffe;
using luojianet_ms::registry::POSITION_BEGIN;
...
class PassRegistryTest : public luojianet_ms::CommonTest {
 public:
...
class Test1Fusion : public registry::PassBase {
 public:
  ...
  bool CanFusion(const CNodePtr &cnode) {
    ...
    auto primc = luojianet_ms::ops::GetOperator<ops::AddFusion>(cnode->input(0));
    if (primc == nullptr) {
      return false;
    }
    if (primc->GetAttr(ops::kActivationType) != nullptr && primc->get_activation_type() != luojianet_ms::NO_ACTIVATION) {
      return false;
    }
    ...
    for (size_t i = 1; i < cnode->size(); ++i) {
      ...
      auto add_primc = luojianet_ms::ops::GetOperator<ops::AddFusion>(input_cnode->input(0));
      if (add_primc == nullptr) {
        return false;
      }
      if (add_primc->GetAttr(ops::kActivationType) != nullptr &&
          add_primc->get_activation_type() != luojianet_ms::NO_ACTIVATION) {
        return false;
      }
      ++input_cnode_num;
      continue;
...
TEST_F(PassRegistryTest, TestRegistry) {
  ...
  auto custome_prim = luojianet_ms::ops::GetOperator<ops::Custom>(cnode_list.front()->input(0));
