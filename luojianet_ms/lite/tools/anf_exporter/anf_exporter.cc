...
#include "luojianet_ms/core/ir/primitive.h"
#include "luojianet_ms/core/ops/op_name.h"
#include "luojianet_ms/core/ops/op_utils.h"
...
using luojianet_ms::ops::PrimitiveC;
...
std::list<CNodePtr> GetOrderedCNodes(const FuncGraphPtr fg) {
  ...
  for (const auto &node : nodes) {
    auto cnode = dyn_cast<luojianet_ms::CNode>(node);
    if (cnode) {
...
int AnfExporter::SetPostTrainOutputTensorType(const std::unique_ptr<schema::MetaGraphT> &meta_graph,
                                              const std::shared_ptr<luojianet_ms::Primitive> &primitive,
  ...
  if (dst_node->quantType == schema::QuantType_QUANT_ALL) {
    if (primitive->name() != luojianet_ms::ops::kNameQuantDTypeCast) {
      first_tensor_output->dataType = kNumberTypeInt8;
    } else {
      auto primc = api::MakeShared<luojianet_ms::ops::QuantDTypeCast>(primitive);
...
int AnfExporter::SetSubGraphOutputIndex(const CNodePtr &cnode, const size_t subgraph_index,
  ...
  for (size_t i = kFirstDataIndex; i < cnode->inputs().size(); i++) {
    ...
    if (input_node == nullptr) {
      ...
      return RET_NULL_PTR;
    } else if (input_node->isa<luojianet_ms::CNode>()) {
...
int AnfExporter::ExportPartialNode(const std::unique_ptr<schema::MetaGraphT> &meta_graphT, const bool &keep_graph,
  ...
  auto prim = GetValueNode<std::shared_ptr<luojianet_ms::Primitive>>(partial_cnode->input(0));
  MS_CHECK_TRUE_MSG(prim != nullptr, RET_NULL_PTR, "GetValueNode failed");
  if (prim->name() != luojianet_ms::ops::kNamePartialFusion) {
...
std::list<CNodePtr> AnfExporter::InsertCallNode(const FuncGraphPtr &func_graph) {
  ...
  for (auto it = cnodes.begin(); it != cnodes.end();) {
    auto prim = GetValueNode<std::shared_ptr<luojianet_ms::Primitive>>((*it)->input(kPrimIndex));
    if (prim == nullptr) {
...
bool AnfExporter::CaseToContinue(const string &prim_name) {
  return prim_name == luojianet_ms::ops::kNameDepend || prim_name == luojianet_ms::ops::kNameTupleGetItem ||
         prim_name == luojianet_ms::ops::kNameMakeTuple || prim_name == "make_tuple";
}
...
int AnfExporter::Anf2Fb(const FuncGraphPtr &func_graph, const std::unique_ptr<schema::MetaGraphT> &meta_graphT,
  ...
  for (const auto &cnode : cnodes) {
    auto prim = GetValueNode<std::shared_ptr<luojianet_ms::Primitive>>(cnode->input(kPrimIndex));
    if (prim == nullptr) {
    ...
    if (opt::CheckPrimitiveType(cnode, prim::kPrimReturn)) {
      node->name = luojianet_ms::ops::kNameReturn;
    ...
    auto device_type_attr = cnode->GetAttr(luojianet_ms::ops::kDeviceType);
...
int AnfExporter::SetOpInputNode(const CNodePtr &cnode, const std::unique_ptr<schema::MetaGraphT> &meta_graphT,
  ...
  for (size_t i = 1; i < cnode->inputs().size(); i++) {
    auto input_node = cnode->input(i);
    if (input_node->isa<luojianet_ms::CNode>()) {
