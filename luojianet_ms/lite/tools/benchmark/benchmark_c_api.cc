...
using luojianet_ms::lite::GetTimeUs;
using luojianet_ms::lite::kFloatMSEC;
...
bool TimeBeforeCallback(const MSTensorHandleArray inputs, const MSTensorHandleArray outputs,
  ...
  g_op_begin_ = luojianet_ms::lite::GetTimeUs();
  return true;
}

bool TimeAfterCallback(const MSTensorHandleArray inputs, const MSTensorHandleArray outputs,
                       const MSCallBackParamC kernel_Info) {
  uint64_t opEnd = luojianet_ms::lite::GetTimeUs();
  float cost = static_cast<float>(opEnd - g_op_begin_) / luojianet_ms::lite::kFloatMSEC;
