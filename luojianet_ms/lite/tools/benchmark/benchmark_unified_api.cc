...
int BenchmarkUnifiedApi::FillGLTextureToTensor(std::map<std::string, GLuint> *gl_texture, luojianet_ms::MSTensor *tensor,
...
int BenchmarkUnifiedApi::GenerateInputData() {
  ...
  if (flags_->enable_parallel_predict_) {
    ...
    for (size_t i = 0; i < ms_inputs_for_api_.size(); i++) {
      ...
      auto new_tensor =
        luojianet_ms::MSTensor::CreateTensor(tensor_name, ms_inputs_for_api_[i].DataType(), shape, input_data, size);
...
void BenchmarkUnifiedApi::UpdateDistributionName(const std::shared_ptr<luojianet_ms::Context> &context,
                                                 std::string *name) {
...
int BenchmarkUnifiedApi::InitMSContext(const std::shared_ptr<luojianet_ms::Context> &context) {
...
#ifdef PARALLEL_INFERENCE
int BenchmarkUnifiedApi::CompareOutputForModelPool(std::vector<luojianet_ms::MSTensor> *outputs) {
  if (outputs->empty()) {
  ...
  for (size_t i = 0; i < outputs->size(); i++) {
    ...
    luojianet_ms::MSTensor tensor = outputs->at(i);
    if (tensor == nullptr) {
...
int BenchmarkUnifiedApi::CompareOutput() {
  ...
  for (const auto &calib_tensor : benchmark_data_) {
    ...
    luojianet_ms::MSTensor tensor = ms_model_.GetOutputByTensorName(tensor_name);
    if (tensor == nullptr) {
      MS_LOG(ERROR) << "Get tensor failed, tensor name: " << tensor_name;
      return RET_ERROR;
    }
    ...
    if (static_cast<int>(tensor.DataType()) == kObjectTypeString) {
      ...
      if (flags_->enable_gl_texture_) {
        ...
        auto *new_tensor = new (std::nothrow)
          MSTensor(tensor_name, luojianet_ms::DataType::kNumberTypeFloat32, tensor_shape, hostptr, data_len);
...
int BenchmarkUnifiedApi::CompareDataGetTotalBiasAndSize(const std::string &name, luojianet_ms::MSTensor *tensor,
...
int BenchmarkUnifiedApi::CompareDataGetTotalCosineDistanceAndSize(const std::string &name, luojianet_ms::MSTensor *tensor,
...
int BenchmarkUnifiedApi::ParallelInference(std::shared_ptr<luojianet_ms::Context> context) {
...
int BenchmarkUnifiedApi::RunBenchmark() {
  ...
  luojianet_ms::ModelType model_type = ModelTypeMap.at(flags_->model_type_);
...
int BenchmarkUnifiedApi::InitTimeProfilingCallbackParameter() {
  // before callback
  ms_before_call_back_ = [&](const std::vector<luojianet_ms::MSTensor> &before_inputs,
  ...
  // after callback
  ms_after_call_back_ = [&](const std::vector<luojianet_ms::MSTensor> &after_inputs,
                            const std::vector<luojianet_ms::MSTensor> &after_outputs, const MSCallBackParam &call_param) {
...
std::string DumpMSTensor(luojianet_ms::MSTensor *tensor) {
  if (tensor == nullptr) {
...
std::string GenerateOutputFileName(luojianet_ms::MSTensor *tensor, const std::string &op_name,
