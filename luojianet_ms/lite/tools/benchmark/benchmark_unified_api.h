...
class MS_API BenchmarkUnifiedApi : public BenchmarkBase {
 ...
 protected:
  ...
  int PrintInputData();
#ifdef PARALLEL_INFERENCE
  int CompareOutputForModelPool(std::vector<luojianet_ms::MSTensor> *outputs);
  void ModelParallelRunnerWarmUp(int index);
  void ModelParallelRunnerRun(int task_num, int parallel_idx);
  int ParallelInference(std::shared_ptr<luojianet_ms::Context> context);
#endif
...
#ifdef PARALLEL_INFERENCE
  std::vector<std::vector<luojianet_ms::MSTensor>> all_inputs_;
  std::vector<std::vector<luojianet_ms::MSTensor>> all_outputs_;
  std::atomic<bool> model_parallel_runner_ret_failed_{false};
  std::atomic<bool> runner_run_start_ = false;
  luojianet_ms::ModelParallelRunner model_runner_;
#endif
};
