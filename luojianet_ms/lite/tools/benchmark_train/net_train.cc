...
int NetTrain::GenerateRandomData(luojianet_ms::tensor::MSTensor *tensor) {
...
int NetTrain::GenerateInputData(std::vector<luojianet_ms::tensor::MSTensor *> *ms_inputs) {
...
int NetTrain::LoadInput(std::vector<luojianet_ms::tensor::MSTensor *> *ms_inputs) {
...
int NetTrain::ReadInputFile(std::vector<luojianet_ms::tensor::MSTensor *> *ms_inputs) {
  if (ms_inputs->empty()) {
    return RET_OK;
  }
...
int NetTrain::CompareOutput(const session::LiteSession &lite_session) {
  ...
  std::map<std::string, luojianet_ms::tensor::MSTensor *> ordered_outputs(tensors_list.begin(), tensors_list.end());
  luojianet_ms::tensor::MSTensor *tensor = nullptr;
  int i = 1;
...
std::unique_ptr<session::LiteSession> NetTrain::CreateAndRunNetworkForInference(const std::string &filename,
  ...
  auto *model = luojianet_ms::lite::Model::Import(filenamems.c_str());
  if (model == nullptr) {
...
void NetTrain::CheckSum(luojianet_ms::tensor::MSTensor *tensor, std::string node_type, int id, std::string in_out) {
...
int NetTrain::InitCallbackParameter() {
  // before callback
  before_call_back_ = [&](const std::vector<luojianet_ms::tensor::MSTensor *> &before_inputs,
                          const std::vector<luojianet_ms::tensor::MSTensor *> &before_outputs,
                          const luojianet_ms::CallBackParam &callParam) {
  ...
  // after callback
  after_call_back_ = [&](const std::vector<luojianet_ms::tensor::MSTensor *> &after_inputs,
