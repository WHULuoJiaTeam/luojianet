...
#ifndef LUOJIANET_MS_LITE_TOOLS_CONVERTER_STATISTIC_UTILS_H_
#define LUOJIANET_MS_LITE_TOOLS_CONVERTER_STATISTIC_UTILS_H_
...
inline float GetClipRate(const void *vector_a, const void *vector_b, size_t size, luojianet_ms::TypeId type_id) {
  MS_ASSERT(vector_a != nullptr);
  MS_ASSERT(vector_b != nullptr);
  if (type_id == luojianet_ms::kNumberTypeFloat32) {
    return luojianet_ms::lite::GetClipRate<float>(static_cast<const float *>(vector_a),
                                               static_cast<const float *>(vector_b), size);
  } else if (type_id == luojianet_ms::kNumberTypeInt32) {
    return luojianet_ms::lite::GetClipRate(static_cast<const int *>(vector_a), static_cast<const int *>(vector_b), size);
  } else {
...
inline float GetCosSimilarity(const void *vector_a, const void *vector_b, size_t size, luojianet_ms::TypeId type_id) {
  MS_ASSERT(vector_a != nullptr);
  MS_ASSERT(vector_b != nullptr);
  if (type_id == luojianet_ms::kNumberTypeFloat32) {
    return luojianet_ms::lite::GetCosSimilarity<float>(static_cast<const float *>(vector_a),
                                                    static_cast<const float *>(vector_b), size);
  } else if (type_id == luojianet_ms::kNumberTypeInt32) {
    return luojianet_ms::lite::GetCosSimilarity(static_cast<const int *>(vector_a), static_cast<const int *>(vector_b),
                                             size);
  } else {
    MS_LOG(ERROR) << "Unsupported data type:" << type_id;
    return 0;
...
#endif  // LUOJIANET_MS_LITE_TOOLS_CONVERTER_STATISTIC_UTILS_H_
