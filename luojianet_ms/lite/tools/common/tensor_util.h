...
#ifndef LUOJIANET_MS_LITE_TOOLS_COMMON_TENSOR_UTIL_H
#define LUOJIANET_MS_LITE_TOOLS_COMMON_TENSOR_UTIL_H
...
int GenerateRandomData(luojianet_ms::tensor::MSTensor *tensors);

int GenerateRandomData(luojianet_ms::MSTensor *tensors);
...
template <typename T>
float CompareDataByCosineDistance(const std::unordered_map<String, luojianet_ms::tensor::MSTensor *> &calib_tensors,
                                  const std::unordered_map<String, luojianet_ms::tensor::MSTensor *> &out_tensors) {
  ...
  for (const auto &calib : calib_tensors) {
    ...
    auto cos = luojianet_ms::lite::GetCosSimilarity<T>(calib_data, out_data, out_tensor->ElementsNum());
    total_cos += cos;
...
template <typename T>
float CompareData(const std::unordered_map<String, luojianet_ms::tensor::MSTensor *> &calib_tensors,
                  const std::unordered_map<String, luojianet_ms::tensor::MSTensor *> &out_tensors) {
  if (calib_tensors.empty() || out_tensors.empty()) {
    MS_LOG(ERROR) << "calib or out tenor is empty.";
    return RET_ERROR;
...
#endif  // LUOJIANET_MS_LITE_TOOLS_COMMON_TENSOR_UTIL_H
