...
Status CustomInterface::GetCustomAttr(char *buf, uint32_t buf_size, const luojianet_ms::schema::Custom *op,
...
std::shared_ptr<luojianet_ms::kernel::KernelInterface> CustomInferCreater() {
  ...
  return std::shared_ptr<luojianet_ms::kernel::KernelInterface>(infer);
}
}  // namespace lite
}  // namespace luojianet_ms
namespace luojianet_ms {
