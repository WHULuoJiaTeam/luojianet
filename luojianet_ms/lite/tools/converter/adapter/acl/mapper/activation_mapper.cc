...
STATUS ActivationMapper::Mapper(const CNodePtr &cnode) {
  static std::map<ActivationType, BaseOperatorPtr> activation_type_map = {
    {luojianet_ms::ELU, std::make_shared<ops::Elu>()},
    {luojianet_ms::GELU, std::make_shared<ops::GeLU>()},
    {luojianet_ms::RELU, std::make_shared<ops::ReLU>()},
    {luojianet_ms::RELU6, std::make_shared<ops::ReLU6>()},
    {luojianet_ms::SIGMOID, std::make_shared<ops::Sigmoid>()},
    {luojianet_ms::TANH, std::make_shared<ops::Tanh>()},
    {luojianet_ms::LEAKY_RELU, std::make_shared<ops::LeakyRelu>()}};
  ...
  auto activate_prim = luojianet_ms::api::MakeShared<luojianet_ms::ops::Activation>(src_prim);
