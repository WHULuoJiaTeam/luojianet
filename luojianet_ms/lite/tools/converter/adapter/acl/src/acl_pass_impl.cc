...
std::shared_ptr<luojianet_ms::Context> AclPassImpl::CreateModelContext() {
  auto model_context = std::make_shared<luojianet_ms::Context>();
...
CNodePtr AclPassImpl::CreateCustomNode(const FuncGraphPtr &func_graph) {
  MS_CHECK_TRUE_MSG(func_graph != nullptr, nullptr, "func_graph is nullptr.");
  auto prim = std::make_shared<luojianet_ms::ops::Custom>();
