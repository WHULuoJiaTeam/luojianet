...
namespace dpico {
bool ArgMaxChecker::Check(CNodePtr op, int32_t output_num, luojianet_ms::Format format) {
  ...
  if (primitive->GetAttr(ops::kTopK) != nullptr) {
    auto top_k = static_cast<uint32_t>(GetValue<int64_t>(primitive->GetAttr(ops::kTopK)));
    if (top_k != 1) {
  ...
  if (primitive->GetAttr(ops::kAxis) != nullptr) {
    auto axis = GetValue<int64_t>(primitive->GetAttr(ops::kAxis));
  ...
  if (primitive->GetAttr(dpico::kSelectLastIndex) != nullptr) {
    auto select_last_index = GetValue<bool>(primitive->GetAttr(dpico::kSelectLastIndex));
