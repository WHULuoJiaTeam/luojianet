...
namespace dpico {
bool BatchNormChecker::Check(CNodePtr op, int32_t output_num, luojianet_ms::Format format) {
  ...
  if (primitive->GetAttr(kUseGlobalStats) != nullptr) {
    auto use_global_stats = GetValue<bool>(primitive->GetAttr(kUseGlobalStats));
    if (!use_global_stats) {
