...
bool CheckResizeOp(const CNodePtr &op, const ShapeVector &input_shape, const ShapeVector &output_shape, size_t index_h,
  ...
  if (prim->GetAttr(ops::kNearestMode) != nullptr) {
    auto nearest_mode = static_cast<luojianet_ms::NearestMode>(GetValue<int64_t>(prim->GetAttr(ops::kNearestMode)));
...
bool ResizeChecker::Check(CNodePtr op, int32_t output_num, luojianet_ms::Format format) {
