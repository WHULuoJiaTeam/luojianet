...
bool CheckResizeOp(const api::CNodePtr &op, const ShapeVector &input_shape, const ShapeVector &output_shape,
  ...
  if (prim->GetAttr(ops::kNearestMode) != nullptr) {
    auto nearest_mode = static_cast<luojianet_ms::NearestMode>(api::GetValue<int64_t>(prim->GetAttr(ops::kNearestMode)));
    if (nearest_mode != luojianet_ms::NearestMode::FLOOR) {
...
bool ResizeChecker::Check(api::CNodePtr op, int32_t output_num, luojianet_ms::Format format) {
