...
namespace dpico {
STATUS RoiAlignMapper::Map(const CNodePtr &cnode, std::vector<BaseOperatorPtr> *base_operators,
  ...
  if (prim->GetAttr(ops::kMode) != nullptr) {
    auto pool_mode = GetValue<std::string>(prim->GetAttr(ops::kMode));
    if (pool_mode == "avg") {
  ...
  if (prim->GetAttr(dpico::kOutputHeight) != nullptr) {
    roi_align_operator->SetPooledHeight(GetValue<uint32_t>(prim->GetAttr(dpico::kOutputHeight)));
  }
  if (prim->GetAttr(dpico::kOutputWidth) != nullptr) {
    roi_align_operator->SetPooledWidth(GetValue<uint32_t>(prim->GetAttr(dpico::kOutputWidth)));
  }
  if (prim->GetAttr(dpico::kSamplingRatio) != nullptr) {
    roi_align_operator->SetSamplingRatio(GetValue<uint32_t>(prim->GetAttr(dpico::kSamplingRatio)));
  }
  if (prim->GetAttr(dpico::kSpatialScale) != nullptr) {
    roi_align_operator->SetSpatialScale(GetValue<float>(prim->GetAttr(dpico::kSpatialScale)));
  }
