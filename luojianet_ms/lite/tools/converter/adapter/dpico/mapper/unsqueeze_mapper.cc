...
namespace dpico {
STATUS UnsqueezeMapper::Map(const CNodePtr &cnode, std::vector<BaseOperatorPtr> *base_operators,
                            const PrimitivePtr &prim, const CNodePtrList &output_cnodes) {
  if (base_operators == nullptr) {
    MS_LOG(ERROR) << "base_operators is nullptr.";
    return RET_ERROR;
  }
  auto unsqueeze_prim = utils::cast<std::shared_ptr<ops::Unsqueeze>>(prim);
  ...
  if (unsqueeze_prim->GetAttr(ops::kAxis) != nullptr) {
    auto axes = GetValue<std::vector<int64_t>>(unsqueeze_prim->GetAttr(ops::kAxis));
