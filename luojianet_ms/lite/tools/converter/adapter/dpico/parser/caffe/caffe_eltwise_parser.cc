  ...
  if (eltwiseParam.has_operation()) {
    switch (eltwiseParam.operation()) {
      case caffe::EltwiseParameter::PROD:
        (void)prim->AddAttr(ops::kMode, MakeValue(static_cast<int64_t>(luojianet_ms::EltwiseMode::PROD)));
        break;
      case caffe::EltwiseParameter::SUM:
        (void)prim->AddAttr(ops::kMode, MakeValue(static_cast<int64_t>(luojianet_ms::EltwiseMode::SUM)));
        break;
      case caffe::EltwiseParameter::MAX:
        (void)prim->AddAttr(ops::kMode, MakeValue(static_cast<int64_t>(luojianet_ms::EltwiseMode::MAXIMUM)));
        break;
      default:
