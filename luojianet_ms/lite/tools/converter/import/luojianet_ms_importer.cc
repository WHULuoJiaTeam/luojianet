...
#include "tools/converter/import/luojianet_ms_importer.h"
#include <memory>
#include <map>
#include <set>
#include <vector>
...
STATUS LUOJIANET_MSImporter::Mindir2AnfAdjust(const FuncGraphPtr &func_graph, const converter::Flags &flag) {
...
STATUS LUOJIANET_MSImporter::GetFuncGraphOutputName(const CNodePtr &return_node) {
...
STATUS LUOJIANET_MSImporter::TraceOutput(const AnfNodePtr &node) {
...
void LUOJIANET_MSImporter::RemoveUnusedGraphInput(const FuncGraphPtr &func_graph) {
  MS_ASSERT(func_graph != nullptr);
...
FuncGraphPtr LUOJIANET_MSImporter::ImportMindIR(const converter::Flags &flag, const void *buff, const size_t &size) {
  MindIRLoader mindir_loader;
  auto func_graph = mindir_loader.LoadMindIR(buff, size);
  return CheckAndUpdateFuncGraph(flag, func_graph);
}

FuncGraphPtr LUOJIANET_MSImporter::ImportMindIR(const converter::Flags &flag) {
...
FuncGraphPtr LUOJIANET_MSImporter::CheckAndUpdateFuncGraph(const converter::Flags &flag, FuncGraphPtr func_graph) {
