...
#include "tools/converter/import/luojianet_importer.h"
#include <memory>
#include <map>
#include <set>
#include <vector>
...
STATUS LuoJiaNetImporter::Mindir2AnfAdjust(const FuncGraphPtr &func_graph, const converter::Flags &flag) {
...
size_t LuoJiaNetImporter::Hex2ByteArray(const std::string &hex_str, unsigned char *byte_array, size_t max_len) {
...
STATUS LuoJiaNetImporter::ProcessDependCnode(const CNodePtr &cnode) {
...
STATUS LuoJiaNetImporter::GetFuncGraphOutputName(const CNodePtr &return_node) {
  ...
  for (size_t i = 0; i < return_node->inputs().size(); i++) {
    ...
    if (output_node == nullptr) {
      ...
      return RET_ERROR;
    } else if (output_node->isa<luojianet_ms::CNode>()) {
...
void LuoJiaNetImporter::RemoveUnusedGraphInput(const FuncGraphPtr &func_graph) {
  MS_ASSERT(func_graph != nullptr);
...
FuncGraphPtr LuoJiaNetImporter::ImportMindIR(const converter::Flags &flag, const void *buff, const size_t &size) {
  MindIRLoader mindir_loader;
  auto func_graph = mindir_loader.LoadMindIR(buff, size);
  return CheckAndUpdateFuncGraph(flag, func_graph);
}

FuncGraphPtr LuoJiaNetImporter::ImportMindIR(const converter::Flags &flag) {
...
FuncGraphPtr LuoJiaNetImporter::CheckAndUpdateFuncGraph(const converter::Flags &flag, FuncGraphPtr func_graph) {
  ...
  if (ConverterInnerContext::GetInstance()->GetGraphInputTensorShapeMapSize() > 0) {
    for (const auto &input : func_graph->get_inputs()) {
      ...
      if (shape.empty()) {
        ...
        input->abstract()->set_shape(std::make_shared<luojianet_ms::abstract::Shape>(shape));
      }
    }
  }
