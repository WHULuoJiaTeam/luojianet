...
luojianet_ms::RoundMode CaffePoolingParser::ParseRoundMode(const caffe::PoolingParameter &poolingParam) {
  luojianet_ms::RoundMode roundMode = luojianet_ms::RoundMode::CEIL;
  if (poolingParam.has_round_mode()) {
    if (poolingParam.round_mode() == caffe::PoolingParameter_RoundMode_FLOOR) {
      roundMode = luojianet_ms::RoundMode::FLOOR;
    } else if (poolingParam.round_mode() == caffe::PoolingParameter_RoundMode_CEIL) {
      roundMode = luojianet_ms::RoundMode::CEIL;
    }
  } else if (poolingParam.has_ceil_mode()) {
    roundMode = poolingParam.ceil_mode() ? luojianet_ms::RoundMode::CEIL : luojianet_ms::RoundMode::FLOOR;
  }
  return roundMode;
}
...
PrimitiveCPtr CaffePoolingParser::Parse(const caffe::LayerParameter &proto, const caffe::LayerParameter &weight) {
  ...
  if (poolingParam.pool() == caffe::PoolingParameter::MAX) {
    ...
    prim_c->AddAttr(luojianet_ms::ops::kOriginalFormat, MakeValue<int64_t>(luojianet_ms::Format::NCHW));
    prim->set_pad_mode(luojianet_ms::PadMode::PAD);
