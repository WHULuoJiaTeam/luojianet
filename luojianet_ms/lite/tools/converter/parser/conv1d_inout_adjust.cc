...
bool Conv1DInOutAdjust::Run(const FuncGraphPtr &func_graph) {
  ...
  for (auto &node : nodes) {
    ...
    auto conv2d_prim = ops::GetOperator<luojianet_ms::ops::Conv2D>(cnode->input(0));
    ...
    switch (Format(GetValue<int64_t>(conv2d_prim->GetAttr(ops::kOriginalFormat)))) {
      case luojianet_ms::Format::NWC:
        (void)conv2d_prim->AddAttr(luojianet_ms::ops::kOriginalFormat, api::MakeValue<int64_t>(luojianet_ms::NHWC));
        axis = {1};
        break;
      case luojianet_ms::Format::NCW:
        (void)conv2d_prim->AddAttr(luojianet_ms::ops::kOriginalFormat, api::MakeValue<int64_t>(luojianet_ms::NCHW));
        axis = {2};
        break;
      default:
        continue;
