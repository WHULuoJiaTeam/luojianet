...
STATUS OnnxConvParser::ParseOnnxAttr(const onnx::NodeProto &onnx_node, int64_t *group, luojianet_ms::Format *format,
                                     luojianet_ms::PadMode *pad_mode) {
    ...
    if (onnx_node_attr.name() == "group") {
      ...
      *pad_mode = GetOnnxPadMode(onnx_node_attr);
      ...
      if (onnx_node_attr.s() == "NHWC") {
        *format = luojianet_ms::Format::NHWC;
      } else {
        MS_LOG(ERROR) << "Unsupported format: " << onnx_node_attr.s();
        return RET_ERROR;
      }
...
PrimitiveCPtr OnnxConvParser::Parse(const onnx::GraphProto &onnx_graph, const onnx::NodeProto &onnx_node) {
  ...
  luojianet_ms::Format format = luojianet_ms::Format::NCHW;
  luojianet_ms::PadMode pad_mode = luojianet_ms::PadMode::PAD;
  ...
  prim_c->AddAttr(luojianet_ms::ops::kOriginalFormat, MakeValue<int64_t>(format));
  ...
  if (conv1d) {
    prim_c->AddAttr(luojianet_ms::ops::kOriginalFormat, MakeValue<int64_t>(NCW));
  }
  ...
  prim->set_activation_type(luojianet_ms::ActivationType::NO_ACTIVATION);
