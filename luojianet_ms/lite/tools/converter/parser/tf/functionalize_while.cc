...
namespace {
luojianet_ms::ValueNodePtr GetWhileAnfPrim() {
  auto while_primc = std::make_shared<luojianet_ms::lite::While>();
  if (while_primc == nullptr) {
    MS_LOG(ERROR) << "new while_primitive failed";
    return nullptr;
  }
  while_primc->set_cond_subgraph_index(luojianet_ms::opt::FunctionalizeControlOpPass::GetSubgraphIndex());
  while_primc->set_body_subgraph_index(luojianet_ms::opt::FunctionalizeControlOpPass::GetSubgraphIndex());
  luojianet_ms::ValueNodePtr partial_anf_prim = NewValueNode(while_primc);
