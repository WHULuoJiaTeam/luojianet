...
ops::PrimitiveC *TFReduceParser::Parse(const tensorflow::NodeDef &tf_op,
  ...
  if (tf_op.op() == "Sum") {
    ...
    prim->set_mode(luojianet_ms::ReduceMode::Reduce_Max);
  } else if (tf_op.op() == "Min") {
    prim->set_mode(luojianet_ms::ReduceMode::Reduce_Min);
  } else if (tf_op.op() == "Mean") {
    prim->set_mode(luojianet_ms::ReduceMode::Reduce_Mean);
  } else if (tf_op.op() == "Prod") {
    prim->set_mode(luojianet_ms::ReduceMode::Reduce_Prod);
  } else if (tf_op.op() == "All") {
    prim->set_mode(luojianet_ms::ReduceMode::Reduce_All);
  } else {
