...
PrimitiveCPtr TFResizeParser::Parse(const tensorflow::NodeDef &tf_op,
  ...
  prim_c->AddAttr(luojianet_ms::ops::kOriginalFormat, MakeValue<int64_t>(luojianet_ms::Format::NHWC));
  ...
  if (tf_op.op() == "ResizeBilinear") {
    ...
    prim->set_method(luojianet_ms::ResizeMethod::CUBIC);
  } else {
    prim->set_method(luojianet_ms::ResizeMethod::UNKNOWN);
  }
