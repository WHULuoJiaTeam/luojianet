...
PrimitiveCPtr TflitePadParser::Parse(const std::unique_ptr<tflite::OperatorT> &tflite_op,
  ...
  if (tflite_op_type == tflite::BuiltinOperator_PAD) {
    ...
    prim->set_padding_mode(luojianet_ms::PaddingMode::CONSTANT);
    ...
    switch (tflite_attr->mode) {
      case tflite::MirrorPadMode_REFLECT:
        prim->set_padding_mode(luojianet_ms::PaddingMode::REFLECT);
        break;
      case tflite::MirrorPadMode_SYMMETRIC:
        prim->set_padding_mode(luojianet_ms::PaddingMode::SYMMETRIC);
        break;
      default:
