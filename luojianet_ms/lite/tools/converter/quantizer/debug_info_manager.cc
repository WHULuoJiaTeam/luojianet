...
int DebugInfoManager::SetQuantStaticInfo(OpParameter *op_parameter, int tensor_index, QuantDebugInfo *quant_debug_info,
  ...
  auto preferred_dim = luojianet_ms::lite::WeightDecoder::GetPreferredDim(op_parameter, tensor_index, tensor.shape());
  float *quant_data;
...
int DebugInfoManager::AddComparedInfo(const luojianet_ms::CallBackParam &call_back_param, OpParameter *op_parameter,
                                      bool is_input, int tensor_index, luojianet_ms::lite::Tensor *compared_tensor) {
  CHECK_NULL_RETURN(op_parameter);
