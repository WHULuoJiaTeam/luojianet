...
int DebugInfoManager::SetOriginStaticInfo(QuantDebugInfo *quant_debug_info, const luojianet_ms::lite::Tensor &tensor) {
...
int DebugInfoManager::SetQuantStaticInfo(const std::vector<luojianet_ms::tensor::MSTensor *> &inputs,
  ...
  auto preferred_dim =
    luojianet_ms::lite::WeightDecoder::GetPreferredDim(inputs, op_parameter, tensor_index, tensor.shape(), Version());
  float *quant_data;
  if (tensor.data_type() == kNumberTypeInt8) {
    quant_data = luojianet_ms::lite::WeightDecoder::DequantData<int8_t, float>(&tensor, preferred_dim);
  } else if (tensor.data_type() == kNumberTypeInt16) {
    quant_data = luojianet_ms::lite::WeightDecoder::DequantData<int16_t, float>(&tensor, preferred_dim);
  } else if (tensor.data_type() == kNumberTypeInt32) {  // Bias
    quant_data = luojianet_ms::lite::WeightDecoder::DequantData<int, float>(&tensor, preferred_dim);
...
int DebugInfoManager::AddOriginInfo(const luojianet_ms::CallBackParam &call_back_param, OpParameter *op_parameter,
                                    bool is_input, int tensor_index, luojianet_ms::lite::Tensor *origin_tensor) {
...
int DebugInfoManager::AddComparedInfo(const luojianet_ms::CallBackParam &call_back_param,
                                      const std::vector<luojianet_ms::tensor::MSTensor *> &inputs,
                                      OpParameter *op_parameter, bool is_input, int tensor_index,
                                      luojianet_ms::lite::Tensor *compared_tensor) {
  CHECK_NULL_RETURN(op_parameter);
...
std::map<std::string, luojianet_ms::schema::Tensor *> DebugInfoManager::ParseInputTensorFromModel(const Model &model) {
  std::map<std::string, luojianet_ms::schema::Tensor *> maps;
...
std::map<std::string, luojianet_ms::schema::Tensor *> DebugInfoManager::ParseOutputTensorFromModel(const Model &model) {
  std::map<std::string, luojianet_ms::schema::Tensor *> maps;
  for (auto node : model.all_nodes_) {
...
int DebugInfoManager::GetDataFromTensorMap(const luojianet_ms::schema::Tensor &schema_tensor,
                                           luojianet_ms::lite::Tensor *dst_tensor) {
...
int DebugInfoManager::GetConstTensor(const std::map<std::string, luojianet_ms::schema::Tensor *> &input_tensor_map,
  ...
  new_tensor->set_category(static_cast<luojianet_ms::lite::Tensor *>(tensor)->category());
...
KernelCallBack DebugInfoManager::GetOriginBeforeCallBack(
  const std::map<std::string, luojianet_ms::schema::Tensor *> &input_tensor_map,
  const std::map<std::string, OpParameter *> &op_parameters) {
  auto before_callback = [&](const std::vector<luojianet_ms::tensor::MSTensor *> &inputs,
                             const std::vector<luojianet_ms::tensor::MSTensor *> &outputs,
                             const CallBackParam &call_param) {
    for (size_t i = 0; i < inputs.size(); ++i) {
      auto tensor = inputs.at(i);
      MS_LOG(INFO) << " Get " << tensor->tensor_name() << " statistics info.";
      auto is_const = static_cast<luojianet_ms::lite::Tensor *>(tensor)->category() == CONST_TENSOR ||
                      static_cast<luojianet_ms::lite::Tensor *>(tensor)->category() == CONST_SCALAR;
      if (is_const) {
        luojianet_ms::lite::Tensor new_tensor;
        ...
        auto ret = AddOriginInfo(call_param, op_parameters.at(call_param.node_name), true, i,
                                 static_cast<luojianet_ms::lite::Tensor *>(tensor));
        if (ret != RET_OK) {
          MS_LOG(ERROR) << tensor->tensor_name() << " add origin info failed.";
          return false;
        }
...
KernelCallBack DebugInfoManager::GetAfterCallBack(const std::map<std::string, OpParameter *> &op_parameters,
  ...
  if (is_origin) {
    after_callback = [&](const std::vector<luojianet_ms::tensor::MSTensor *> &inputs,
                         const std::vector<luojianet_ms::tensor::MSTensor *> &outputs, const CallBackParam &call_param) {
...
int DebugInfoManager::GetClipAndCos() {
  for (auto &info : compared_info_) {
    ...
    info.cos_similarity = luojianet_ms::lite::GetCosSimilarity(iter->second.tensor_data.data, info.tensor_data.data,
                                                            info.tensor_data.elements_num, info.tensor_data.data_type);
    info.clip = luojianet_ms::lite::GetClipRate(iter->second.tensor_data.data, info.tensor_data.data,
                                             info.tensor_data.elements_num, info.tensor_data.data_type);
  }
  return RET_OK;
}
