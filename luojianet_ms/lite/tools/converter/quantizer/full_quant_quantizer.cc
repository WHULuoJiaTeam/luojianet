...
int FullQuantQuantizer::QuantNodeSimpleOp(const CNodePtr &cnode) {
  ...
  for (size_t i = 1; i < cnode->inputs().size(); i++) {
    ...
    if (is_graph_input) {
      ...
      auto input_cnode = input_node->cast<luojianet_ms::CNodePtr>();
    ...
    } else if (input_node->isa<luojianet_ms::Parameter>()) {
    ...
    } else if (input_node->isa<luojianet_ms::ValueNode>()) {
...
int FullQuantQuantizer::QuantNode(const FuncGraphPtr &func_graph) {
  ...
  for (auto &cnode : cnodes) {
    ...
    if (op_type == luojianet_ms::ops::kNameTupleGetItem) {
      ...
      auto index_value_node = index_node->cast<luojianet_ms::ValueNodePtr>();
...
int FullQuantQuantizer::CheckFp32TensorVec(const std::string &node_name,
                                           const std::vector<luojianet_ms::tensor::MSTensor *> &tensor_vec) {
  if (tensor_vec.empty()) {
...
int FullQuantQuantizer::DoInference(CollectType collect_type) {
  // get input tensor
  vector<luojianet_ms::tensor::MSTensor *> inputs = fp32_session_->GetInputs();
  ...
  for (size_t calib_index = 0; calib_index < calibrator_->GetBatchNum(); calib_index++) {
    ...
    KernelCallBack beforeCallBack = [&](const std::vector<luojianet_ms::tensor::MSTensor *> &beforeInputs,
                                        const std::vector<luojianet_ms::tensor::MSTensor *> &beforeOutputs,
    ...
    // func
    KernelCallBack afterCallBack = [&](const std::vector<luojianet_ms::tensor::MSTensor *> &afterInputs,
                                       const std::vector<luojianet_ms::tensor::MSTensor *> &afterOutputs,
                                       const CallBackParam &callParam) -> bool {
...
int FullQuantQuantizer::Int8Inference() {
  // int8 inference
  vector<luojianet_ms::tensor::MSTensor *> inputs = int8_session_->GetInputs();
...
KernelCallBack FullQuantQuantizer::GetBeforeCallBack(bool int8_op) {
  ...
  if (!int8_op) {
    before_call_back = [this](const std::vector<luojianet_ms::tensor::MSTensor *> &before_inputs,
...
KernelCallBack FullQuantQuantizer::GetInt8AfterCallBack() {
  KernelCallBack after_call_back = [this](const std::vector<luojianet_ms::tensor::MSTensor *> &afterInputs,
                                          const std::vector<luojianet_ms::tensor::MSTensor *> &afterOutputs,
                                          const CallBackParam &callParam) -> bool {
    if (callParam.node_type == kTypeConv2D) {
