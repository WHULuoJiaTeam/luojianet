...
#include "luojianet_ms/lite/tools/converter/quantizer/insert_quant_node_manager.h"
#include <memory>
#include <set>
#include <vector>
#include <string>
...
int InsertQuantNodeManager::InsertCastNode(const FuncGraphPtr &graph, const CNodePtr &cnode, size_t input_index,
                                           bool is_graph_input) {
  auto primitive = GetValueNode<std::shared_ptr<luojianet_ms::Primitive>>(cnode->input(0));
  ...
  std::shared_ptr<luojianet_ms::Primitive> input_cnode_primitive_c = nullptr;
  if (!is_graph_input) {
    auto input_cnode = std::dynamic_pointer_cast<luojianet_ms::CNode>(input_node);
    input_cnode_primitive_c = GetValueNode<std::shared_ptr<luojianet_ms::Primitive>>(input_cnode->input(0));
...
int InsertQuantNodeManager::CheckDataType(const AnfNodePtr &input_node, TypeId check_type_id) {
  ...
  if (!input_node->isa<luojianet_ms::CNode>() && !is_graph_input) {
    return RET_NO_CHANGE;
  }
  bool is_special_node =
    input_node->isa<luojianet_ms::CNode>() && opt::IsSpecialType(input_node->cast<luojianet_ms::CNodePtr>());
...
int InsertQuantNodeManager::NewDynamicQuantNode(const FuncGraphPtr &graph, const CNodePtr &cnode) {
  ...
  if (input->isa<luojianet_ms::CNode>() || IsGraphInput(input)) {
  ...
  if (weight->isa<luojianet_ms::CNode>() || IsGraphInput(weight)) {
