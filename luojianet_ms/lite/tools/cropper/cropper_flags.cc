...
CropperFlags::CropperFlags() {
  AddFlag(&CropperFlags::package_file_, "packageFile", "The libluojianet_ms-lite.a file that needs to be cropped", "");
...
STATUS CropperFlags::CheckOutputFile() {
  if (this->output_file_.empty()) {
    ...
    if (ValidFileSuffix(this->output_file_, "a") != RET_OK) {
      MS_LOG(ERROR) << "INPUT ILLEGAL: packageFile need to pass package name, such as libluojianet_ms-lite.a";
      return RET_INPUT_PARAM_INVALID;
    }
