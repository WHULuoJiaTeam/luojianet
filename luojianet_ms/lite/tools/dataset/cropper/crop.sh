...
{
  echo "Usage:"
  echo "bash crop.sh -p <path-to-luojianet_ms-directory> <source-file> [<more-source-files>] \\"
  echo "bash crop.sh -h \\"
  echo ""
  echo "Options:"
  echo "    -p path to luojianet_ms directory"
  echo "    -h print usage"
...
checkopts()
{
  ...
  do
    case "${opt}" in
      p)
        LUOJIANET_MS_PATH="$(cd "${OPTARG}" &> /dev/null && pwd )"
        ;;
      h)
        usage
        exit 1
...
# exit if luojianet_ms path is not given by user
if [ -z "${LUOJIANET_MS_PATH}" ]; then
  echo -e "\e[31mPlease set LUOJIANET_MS_PATH using -p flag.\e[0m"
  exit 1
fi
...
echo "MS PATH: $LUOJIANET_MS_PATH"
...
MD_LIB_PATH=`find $LUOJIANET_MS_PATH -name "${MD_LIB_FILENAME}" | head -n 1`
...
LIBJPEG_PATH=`find $LUOJIANET_MS_PATH -name "libjpeg.so*" | head -n 1`
LIBTURBOJPEG_PATH=`find $LUOJIANET_MS_PATH -name "libturbojpeg.so*" | head -n 1`
LIBSECUREC_PATH=`find $LUOJIANET_MS_PATH -name libsecurec.a | head -n 1`
