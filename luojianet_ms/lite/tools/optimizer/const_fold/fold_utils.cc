...
kernel::LiteKernel *GetLiteKernel(std::vector<Tensor *> inputs, std::vector<Tensor *> *outputs, const CNodePtr &cnode,
                                  lite::InnerContext *context, luojianet_ms::Context *ms_context) {
...
bool ConstFoldProcessor::Init() {
  ...
  if (ms_context_ == nullptr) {
    ms_context_ = std::shared_ptr<luojianet_ms::Context>(lite::MSContextFromContext(context_.get()));
...
int ConstFoldProcessor::DoConstantFold(const FuncGraphPtr &func_graph, const CNodePtr &cnode) {
  ...
  auto status = static_cast<luojianet_ms::kernel::InnerKernel *>(lite_kernel->kernel())->Run();
  delete (lite_kernel);
  lite_kernel = nullptr;
  if (status != lite::RET_OK) {
