...
int ConcatSplitEliminate(const FuncGraphPtr &func_graph, const CNodePtr &cnode) {
  ...
  auto pre_prim = ops::GetOperator<luojianet_ms::ops::Concat>(pre_cnode->input(kAnfPrimitiveIndex));
  MS_CHECK_TRUE_MSG(pre_prim != nullptr, lite::RET_ERROR, "pre_cnode is not a ops::Concat");
  auto prim = ops::GetOperator<luojianet_ms::ops::SplitWithOverlap>(cnode->input(kAnfPrimitiveIndex));
