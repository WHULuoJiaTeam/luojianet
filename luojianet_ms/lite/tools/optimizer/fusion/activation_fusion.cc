...
STATUS DoFusion(CNodePtr cur_cnode, const CNodePtr &pre_cnode) {
  auto cur_act_prim = ops::GetOperator<luojianet_ms::ops::Activation>(cur_cnode->input(0));
  MS_ASSERT(cur_act_prim != nullptr);
  auto pre_act_prim = ops::GetOperator<luojianet_ms::ops::Activation>(pre_cnode->input(0));
