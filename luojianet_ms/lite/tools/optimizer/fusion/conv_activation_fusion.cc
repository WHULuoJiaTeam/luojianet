...
const AnfNodePtr ConvActivationFusion::Process(const FuncGraphPtr &func_graph, const AnfNodePtr &node,
  ...
  auto act_prim = ops::GetOperator<luojianet_ms::ops::Activation>(act_node->input(0));
  MS_CHECK_TRUE_MSG(act_prim != nullptr, nullptr, "activation prim is nullptr.");
  auto act_prim_c = act_prim->GetPrim();
  if (act_prim_c->GetAttr(ops::kActivationType) != nullptr && act_prim->get_activation_type() != luojianet_ms::RELU &&
