...
const AnfNodePtr ConvActivationFusion::Process(const FuncGraphPtr &func_graph, const AnfNodePtr &node,
  ...
  auto act_prim = GetValueNode<std::shared_ptr<luojianet_ms::ops::Activation>>(act_node->input(0));
  if (act_prim == nullptr ||
      (act_prim->GetAttr(ops::kActivationType) != nullptr && act_prim->get_activation_type() != luojianet_ms::RELU &&
       act_prim->get_activation_type() != luojianet_ms::RELU6)) {
    return nullptr;
  }
  ...
  if (pre_node != nullptr && pre_node->isa<CNode>()) {
    ...
    if (CheckPrimitiveType(conv_node, prim::kPrimConv2DFusion) ||
      ...
      if (prim->GetAttr(ops::kActivationType) == nullptr ||
          static_cast<luojianet_ms::ActivationType>(GetValue<int64_t>(prim->GetAttr(ops::kActivationType))) ==
            luojianet_ms::NO_ACTIVATION) {
        auto type = act_prim->get_activation_type() == luojianet_ms::RELU ? luojianet_ms::RELU : luojianet_ms::RELU6;
