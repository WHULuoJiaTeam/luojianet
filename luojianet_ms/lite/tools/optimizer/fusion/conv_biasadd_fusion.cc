...
bool ConvBiasaddFusion::CheckCanFusion(const FuncGraphPtr &func_graph, const AnfNodePtr &node) const {
  ...
  auto add_act = add_act_ptr == nullptr ? luojianet_ms::NO_ACTIVATION
                                        : static_cast<luojianet_ms::ActivationType>(GetValue<int64_t>(add_act_ptr));
  ...
  auto conv_act = add_act_ptr == nullptr ? luojianet_ms::NO_ACTIVATION
                                         : static_cast<luojianet_ms::ActivationType>(GetValue<int64_t>(conv_act_ptr));
  if (add_act != luojianet_ms::NO_ACTIVATION) {
    if (conv_act != luojianet_ms::NO_ACTIVATION || (add_act != luojianet_ms::RELU && add_act != luojianet_ms::RELU6)) {
      return false;
    }
  }
