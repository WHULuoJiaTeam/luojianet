...
const AnfNodePtr ScaleActivationFusion::Process(const FuncGraphPtr &func_graph, const AnfNodePtr &node,
  ...
  auto act_prim = GetValueNode<std::shared_ptr<luojianet_ms::ops::Activation>>(act_node->input(FIRST_INPUT));
  MS_CHECK_TRUE_RET(act_prim != nullptr && act_prim->GetAttr(ops::kActivationType) != nullptr, nullptr);
  if (act_prim->get_activation_type() != luojianet_ms::RELU && act_prim->get_activation_type() != luojianet_ms::RELU6) {
    return nullptr;
  }
