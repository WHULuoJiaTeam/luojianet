...
const AnfNodePtr SigmoidMulFusion::Process(const FuncGraphPtr &func_graph, const AnfNodePtr &node,
  ...
  auto activation_prim = GetValueNode<std::shared_ptr<luojianet_ms::ops::Activation>>(activation_cnode->input(0));
  ...
  if (activation_prim == nullptr || (activation_prim->GetAttr(ops::kActivationType) != nullptr &&
                                     activation_prim->get_activation_type() != luojianet_ms::SIGMOID)) {
    return nullptr;
  }
  activation_prim->set_activation_type(luojianet_ms::SWISH);
