...
bool CheckInputTransposeNode(const CNodePtr &cnode, bool *indices, size_t indices_len) {
...
bool TransposeMatMulFusion::Run(const FuncGraphPtr &func_graph) {
  ...
  for (auto &node : node_list) {
    ...
    if (!CheckInputTransposeNode(cnode, indices_need_fuse, sizeof(bool) * DIMENSION_2D)) {
      continue;
    }
    auto matmul_prim = GetValueNode<std::shared_ptr<luojianet_ms::ops::MatMul>>(cnode->input(0));
    ...
    if (indices_need_fuse[FIRST_INPUT]) {
      ...
      auto left_pre_cnode = left_trans_cnode->input(SECOND_INPUT);
      (void)manager->Replace(left_trans_cnode, left_pre_cnode);
    }
    if (indices_need_fuse[SECOND_INPUT]) {
      ...
      auto right_pre_cnode = right_trans_cnode->input(SECOND_INPUT);
      (void)manager->Replace(right_trans_cnode, right_pre_cnode);
    }
  }
  return false;
}
