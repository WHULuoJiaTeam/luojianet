...
std::shared_ptr<luojianet_ms::ops::SliceFusion> GetSlice(const CNodePtr &cnode) {
  if (cnode == nullptr) {
    return nullptr;
  }
  return GetValueNode<std::shared_ptr<luojianet_ms::ops::SliceFusion>>(cnode->input(0));
}

std::shared_ptr<luojianet_ms::ops::Softmax> GetSoftmax(const CNodePtr &cnode) {
  if (cnode == nullptr) {
    return nullptr;
  }
  return GetValueNode<std::shared_ptr<luojianet_ms::ops::Softmax>>(cnode->input(0));
}

std::shared_ptr<luojianet_ms::ops::Reshape> GetReshape(const CNodePtr &cnode) {
  if (cnode == nullptr) {
    return nullptr;
  }
  return GetValueNode<std::shared_ptr<luojianet_ms::ops::Reshape>>(cnode->input(0));
}

std::shared_ptr<luojianet_ms::ops::FullConnection> GetFc(const CNodePtr &cnode) {
  if (cnode == nullptr) {
    return nullptr;
  }
  return GetValueNode<std::shared_ptr<luojianet_ms::ops::FullConnection>>(cnode->input(0));
}
...
ValueNodePtr SlicePreposePass::CreateSliceValueNode(const std::vector<int64_t> &axes) {
  MS_ASSERT(graph != nullptr);
  MS_ASSERT(slice_cnode != nullptr);
  auto new_slice = std::make_shared<luojianet_ms::ops::SliceFusion>();
...
ValueNodePtr SlicePreposePass::CopySliceValueNode(const CNodePtr &slice_cnode) {
  MS_ASSERT(graph != nullptr);
  MS_ASSERT(slice_cnode != nullptr);
  auto slice_c = GetValueNode<std::shared_ptr<luojianet_ms::ops::SliceFusion>>(slice_cnode->input(0));
  if (slice_c == nullptr) {
    MS_LOG(ERROR) << "slice node is nullptr";
    return nullptr;
  }
  auto new_slice_c = std::make_shared<luojianet_ms::ops::SliceFusion>();
...
CNodePtr SlicePreposePass::CreateReshapeCNode(const FuncGraphPtr &graph, const std::vector<int64_t> &shape_vector,
  ...
  MS_ASSERT(abstract != nullptr);
  MS_ASSERT(preceed_cnode != nullptr);
  auto new_reshape = std::make_shared<luojianet_ms::ops::Reshape>();
