...
api::SharedPtr<luojianet_ms::ops::SliceFusion> GetSlice(const CNodePtr &cnode) {
  if (cnode == nullptr) {
    return nullptr;
  }
  return ops::GetOperator<luojianet_ms::ops::SliceFusion>(cnode->input(0));
}

api::SharedPtr<luojianet_ms::ops::Softmax> GetSoftmax(const CNodePtr &cnode) {
  if (cnode == nullptr) {
    return nullptr;
  }
  return ops::GetOperator<luojianet_ms::ops::Softmax>(cnode->input(0));
}

api::SharedPtr<luojianet_ms::ops::Reshape> GetReshape(const CNodePtr &cnode) {
  if (cnode == nullptr) {
    return nullptr;
  }
  return ops::GetOperator<luojianet_ms::ops::Reshape>(cnode->input(0));
}

api::SharedPtr<luojianet_ms::ops::FullConnection> GetFc(const CNodePtr &cnode) {
  if (cnode == nullptr) {
    return nullptr;
  }
  return ops::GetOperator<luojianet_ms::ops::FullConnection>(cnode->input(0));
}
...
ValueNodePtr SlicePreposePass::CopySliceValueNode(const CNodePtr &slice_cnode) {
  MS_ASSERT(graph != nullptr);
  MS_ASSERT(slice_cnode != nullptr);
  auto slice_c = ops::GetOperator<luojianet_ms::ops::SliceFusion>(slice_cnode->input(0));
  if (slice_c == nullptr) {
