...
STATUS UpdateConv2DParamPass::UpdateConv2DAttr(const CNodePtr &cnode) {
  ...
  auto format = static_cast<luojianet_ms::Format>(GetValue<int64_t>(prim->GetAttr(ops::kFormat)));
  if (format != luojianet_ms::NHWC && format != luojianet_ms::NCHW) {
    MS_LOG(ERROR) << "conv2d's format only support nhwc or nchw, now is " << format;
    return lite::RET_ERROR;
  }
  auto kernel_size = format == luojianet_ms::NHWC ? ShapeVector{shape[1], shape[kInputIndexTwo]}
                                               : ShapeVector{shape[kInputIndexTwo], shape[kInputIndexThree]};
  int64_t in_channel = format == luojianet_ms::NHWC ? shape[kInputIndexThree] : shape[1];
