...
from luojianet_ms.common.api import _LuoJiaNetFunctionExecutor
...
def get_parse_method_of_class(obj, parse_method=None):
    ...
    Args:
        ...
        parse_method(str): Save the method name. Module object has default method named 'call'.
...
def resolve_symbol(namespace, symbol):
        ...
        if support_fallback_ == '0':
            ...
            if namespace.name == "numpy" and \
                ...
                raise NotImplementedError("LuoJiaNet does not support to use the numpy methods " \
                                          "within the call() or @ms_function decorated function in graph mode.")
    ...
    if isinstance(resolve_, _LuoJiaNetFunctionExecutor):
        logger.debug("Resolve class _LuoJiaNetFunctionExecutor, resolve fn instead.")
...
def convert_to_ms_tensor(data):
    """Convert C++ tensor to luojianet_ms tensor."""
    return Tensor(data)
...
class Parser:
    ...
    def __init__(self, fn: (types.FunctionType, types.MethodType), parse_method=None) -> None:
        ...
        # Used to resolve luojianet_ms builtin ops namespace.
        self.ms_common_ns = CellNamespace('luojianet_ms.common')
        self.ms_nn_ns = CellNamespace('luojianet_ms.nn')
        self.ms_ops_ns = CellNamespace('luojianet_ms.ops')
        self.ms_ops_c_ns = CellNamespace('luojianet_ms.ops.composite')
        self.ms_ops_c_multitype_ns = CellNamespace('luojianet_ms.ops.composite.multitype_ops')
        self.ms_ops_p_ns = CellNamespace('luojianet_ms.ops.operations')
        ...
        if isinstance(self.fn, (types.FunctionType, types.MethodType)):
            try:
                ...
                if e.__str__() == "could not get source code":
                    raise OSError(f"LuoJiaNet can not compile temporary source code in terminal. "
    ...
    def is_supported_namespace_module(self, value):
        ...
        # Check `luojianet_ms` namespace.
        if not hasattr(value, '__name__'):
            logger.debug(f"'{str(value)}' has no '__name__' attribute, we suppose it's supported.")
            return True
        name = value.__name__
        if name == 'luojianet_ms':
            logger.debug(f"Found 'luojianet_ms' root namespace.")
            return True
        if name == 'luojianet_ms.ops':
            logger.debug(f"Found 'luojianet_ms.ops' namespace.")
            return True
        if name == 'luojianet_ms.nn':
            logger.debug(f"Found 'luojianet_ms.nn' namespace.")
            return True
        if name == 'luojianet_ms.numpy':
            logger.debug(f"Found 'luojianet_ms.numpy' namespace.")
            return True
        if name == 'luojianet_ms.context':
            logger.debug(f"Found 'luojianet_ms.context' namespace.")
            return True
        ...
        logger.info(f"Not found '{name}' in luojianet_ms supported namespace.")
        return False

    def get_builtin_namespace_symbol(self, var: str):
        """Get luojianet_ms builtin namespace and symbol."""
        if var in self.closure_namespace:
            logger.debug(f"Found '{var}' in closure_namespace {self.closure_namespace.__str__()}.")
            return self.closure_namespace, var
        if var in self.global_namespace:
