...
from luojianet_ms._extends.remote import kernel_build_server
...
def __get_compile_cache_dep_files(file_path, compile_cache_dep_files, pkg):
    ...
    for node in ast.iter_child_nodes(root):
        ...
        if isinstance(node, ast.ImportFrom):
            ...
            continue
        # Do not care the files in luojianet_ms package
        if module_name.startswith("luojianet_ms"):
            continue

        for n in node.names:
            if n.name.startswith("luojianet_ms"):
                continue
            if module_name == "":
                whole_module = n.name
            else:
...
class _LuoJiaNETFunctionExecutor:
    """
    Represents a function compiled by graph compiler.

    _LuoJiaNETFunctionExecutor will compile the original function for every combination
...
def ms_function(fn=None, obj=None, input_signature=None):
    """
    Create a callable LuoJiaNET graph from a Python function.

    This allows the LuoJiaNET runtime to apply optimizations based on graph.
    ...
    Examples:
        ...
        ...
        >>> # create a callable LuoJiaNET graph by calling ms_function
        >>> def tensor_add(x, y):
        ...     z = x + y
        ...     return z
        ...
        ...
        ...
        >>> # create a callable LuoJiaNET graph through decorator @ms_function
        >>> @ms_function
        ...
        ...
        >>> # create a callable LuoJiaNET graph through decorator @ms_function with input_signature parameter
    ...
    def wrap_luojianet_ms(func):
        ...
        def staging_specialize(*args):
            ...
            if process_obj is None and is_pynative_parallel():
                process_obj = obj
            out = _LuoJiaNETFunctionExecutor(func, ms_create_time, input_signature, process_obj)(*args)
            return out

        return staging_specialize

    if fn is not None:
        return wrap_luojianet_ms(fn)
    return wrap_luojianet_ms
...
def ms_class(cls):
    ...
    This allows LuoJiaNET to identify user-defined classes and thus obtain their attributes and methods.
    ...
    Examples:
        ...
        >>> from luojianet_ms import ms_class
        ...
        >>> @ms_class
...
def _build_broadcast_graph(broadcast_params_dict, broadcast_phase):
    ...
    from luojianet_ms.nn.wrap.cell_wrapper import _BroadCastCell
...
def ms_memory_recycle():
    """
    Recycle memory used by LuoJiaNET.
    When train multi Neural network models in one process, memory used by LuoJiaNET is very large,
    this is because LuoJiaNET cached runtime memory for every model.
