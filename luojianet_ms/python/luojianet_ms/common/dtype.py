...
def pytype_to_dtype(obj):
    """
    Convert python type to LuoJiaNet type.

    Args:
        obj (type): A python type object.

    Returns:
        Type of LuoJiaNet type.

    Raises:
        NotImplementedError: If the python type cannot be converted to LuoJiaNet type.
    """
    ...
    raise NotImplementedError(f"The python type {obj} cannot be converted to LuoJiaNet type.")


def get_py_obj_dtype(obj):
    """
    Get the LuoJiaNet data type, which corresponds to python type or variable.
    ...
    # Primitive or Module
    if hasattr(obj, '__primitive_flag__') or hasattr(obj, 'call'):
        return function
    # luojianet_ms type
    if isinstance(obj, typing.Type):
        return type_type
...
def dtype_to_nptype(type_):
    """
    Convert LuoJiaNet dtype to numpy data type.

    Args:
        type_ (:class:`luojianet_ms.dtype`): LuoJiaNet's dtype.
...
def dtype_to_pytype(type_):
    """
    Convert LuoJiaNet dtype to python data type.
...
def issubclass_(type_, dtype):
    ...
    Args:
        type_ (:class:`luojianet_ms.dtype`): Target LuoJiaNet dtype.
        dtype (:class:`luojianet_ms.dtype`): Compare LuoJiaNet dtype.
