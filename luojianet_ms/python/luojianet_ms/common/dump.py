...
def set_dump(target, enabled=True):
    ...
    `target` should be an instance of :class:`luojianet_ms.nn.Module` or :class:`luojianet_ms.ops.Primitive` .
    Please note that this API takes effect only when Asynchronous Dump is enabled and the `dump_mode`
    field in dump config file is "2". See the `dump document <https://www.luojianet_ms.cn/docs/tutorials/experts/
    en/r1.7/debug/dump.html>`_ for details.
    The default enabled status for a :class:`luojianet_ms.nn.Module` or :class:`luojianet_ms.ops.Primitive` is False.
    ...
    Note:
        ...
        3. After using `set_dump(Module, True)` , operators in forward and backward
           computation  (computation generated by the grad operations) of the
           cell will be dumped.
        4. For :class:`luojianet_ms.nn.SoftmaxCrossEntropyWithLogits` layer, the forward
           ...
           Please note that :class:`luojianet_ms.nn.SoftmaxCrossEntropyWithLogits` layer will also use
           the above operators internally when initialized with `sparse=True` and
           `reduction="mean"` .

    Args:
        target (Union[Module, Primitive]): The Module instance or Primitive instance
    ...
    import luojianet_ms.nn as nn  # avoid circular import
    from luojianet_ms.ops import Primitive
    if not isinstance(target, nn.Module) and not isinstance(target, Primitive):
        raise ValueError(f"The \"target\" parameter must be an instance of "
                         f"Module or Primitive, "
