...
class Parameter(Tensor_):
    ...
    Args:
        ...
        parallel_optimizer (bool): It is used to filter the weight shard operation in semi auto or auto parallel
            mode. It works only when enable parallel optimizer in `luojianet_ms.context.set_auto_parallel_context()`.
            Default: True.
    ...
    Examples:
        ...
        >>> print(net(x))
        [[2.]]
        >>> net.weight.set_data(Tensor(np.zeros((1, 2)), luojianet_ms.float32))
        >>> print(net(x))
        [[0.]]
    """
    __base_type__ = {}
    ...
    def parallel_optimizer(self):
        ...
        when enable parallel optimizer in `luojianet_ms.context.set_auto_parallel_context()`.
        """
