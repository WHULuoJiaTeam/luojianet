...
def set_seed(seed):
    ...
    Note:
        The global seed is used by numpy.random, luojianet_ms.common.Initializer, luojianet_ms.ops.composite.random_ops and
        luojianet_ms.nn.probability.distribution.

        If global seed is not set, these packages will use their own default seed independently, numpy.random and
        luojianet_ms.common.Initializer will choose a random seed, luojianet_ms.ops.composite.random_ops and
        luojianet_ms.nn.probability.distribution will use zero.
    ...
    Examples:
        ...
        >>> from luojianet_ms import Tensor, set_seed, Parameter
        ...
        >>> w1 = Parameter(initializer("uniform", [2, 2], ms.float32), name="w1") # W2
        >>>
        >>> # 3. If neither global seed nor op seed is set, luojianet_ms.ops.composite.random_ops and
        >>> # luojianet_ms.nn.probability.distribution will choose a random seed:
        ...
        >>>
        >>> # 4. If global seed is set, but op seed is not set, luojianet_ms.ops.composite.random_ops and
        >>> # luojianet_ms.nn.probability.distribution will calculate a seed according to global seed and
        ...
        >>> c2 = ops.uniform((1, 4), minval, maxval) # C2
        >>>
        >>> # 5. If both global seed and op seed are set, luojianet_ms.ops.composite.random_ops and
        >>> # luojianet_ms.nn.probability.distribution will calculate a seed according to global seed and
        ...
        >>> # luojianet_ms.ops.composite.random_ops and luojianet_ms.nn.probability.distribution act as in
        >>> # condition 5.
        >>> c1 = ops.uniform((1, 4), minval, maxval, seed=2) # C1
        >>> c2 = ops.uniform((1, 4), minval, maxval, seed=2) # C2
        >>> # Rerun the program will get the same results:
        ...
        >>> # luojianet_ms.ops.composite.random_ops and luojianet_ms.nn.probability.distribution.
        >>> set_seed(1234)
        >>> np_1 = np.random.normal(0, 1, [1]).astype(np.float32) # A1
        >>> c1 = ops.uniform((1, 4), minval, maxval, seed=2) # C1
        >>> set_seed(1234)
