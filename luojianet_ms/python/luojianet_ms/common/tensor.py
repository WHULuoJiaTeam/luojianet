...
class Tensor(Tensor_):
    ...
    def astype(self, dtype, copy=True):
        ...
        Args:
            dtype (Union[:class:`luojianet_ms.dtype`, str]): Designated tensor dtype, can be in format
                of :class:`luojianet_ms.dtype.float32` or `float32`.
...
class RowTensor:
    ...
    RowTensor can only be used in the `Module`'s call method.
...
class SparseTensor:
    ...
    SparseTensor can only be used in the `Module`'s call method.
