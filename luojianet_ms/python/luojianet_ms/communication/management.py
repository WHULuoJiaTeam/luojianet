...
def _check_parallel_envs():
    ...
    try:
        ...
        pass
    rank_table_file_str = os.getenv("LUOJIANET_MS_HCCL_CONFIG_PATH")
    ...
    if not rank_table_file_str and not rank_table_file_str_old:
        raise RuntimeError("Get hccl rank_table_file failed, "
                           "please export LUOJIANET_MS_HCCL_CONFIG_PATH or RANK_TABLE_FILE.")
...
def init(backend_name=None):
    ...
    Note:
        ...
        before running the following example, please see the docstring of the luojianet_ms.management.
    ...
    Raises:
        ...
        RuntimeError: If device target is invalid, or backend is invalid, or distributed initialization fails,
                      or the environment variables RANK_ID/LUOJIANET_MS_HCCL_CONFIG_PATH
...
def release():
    ...
    Note:
        ...
        before running the following example, please see the docstring of the luojianet_ms.managerment.

    Raises:
        RuntimeError: If failed to release distributed resource.

    Examples:
        >>> from luojianet_ms.communication import init, release
        >>> init()
        >>> release()
    """
    finalize_hccl()
...
def get_rank(group=GlobalComm.WORLD_COMM_GROUP):
    ...
    Examples:
        >>> from luojianet_ms.communication import init, get_rank
        >>> init()
        >>> rank_id = get_rank()
        >>> print(rank_id)
...
def get_local_rank(group=GlobalComm.WORLD_COMM_GROUP):
    ...
    Note:
        GPU version of LuoJiaNET doesn't support this method.
        This method should be used after init(). The user needs to preset communication environment variables
        before running the following example, please see the docstring of the luojianet_ms.managerment.
    ...
    Raises:
        TypeError: If group is not a string.
        ValueError: If backend is invalid.
        RuntimeError: If HCCL is not available or LuoJiaNET is GPU version.
    Examples:
        >>> from luojianet_ms.context import set_context, set_auto_parallel_context
        >>> from luojianet_ms.communication.management import init, get_rank, get_local_rank
...
def get_group_size(group=GlobalComm.WORLD_COMM_GROUP):
    ...
    Note:
        ...
        running the following example, please see the docstring of the luojianet_ms.managerment.
...
def get_local_rank_size(group=GlobalComm.WORLD_COMM_GROUP):
    ...
    Examples:
        >>> from luojianet_ms.context import set_context, set_auto_parallel_context
        >>> from luojianet_ms.communication.management import init, get_local_rank_size
        >>> set_context(device_target="Ascend")
        >>> set_auto_parallel_context(device_num=16) # 2 server, each server with 8 NPU.
        >>> init()
...
def get_world_rank_from_group_rank(group, group_rank_id):
    ...
    Examples:
        >>> from luojianet_ms.context import set_context
        >>> from luojianet_ms.communication.management import init, create_group, get_world_rank_from_group_rank
        >>> set_context(device_target="Ascend")
        >>> init()
        >>> group = "0-4"
        >>> rank_ids = [0,4]
...
def get_group_rank_from_world_rank(world_rank_id, group):
    ...
    Note:
        ...
        the docstring of the luojianet_ms.managerment.
    ...
    Examples:
        >>> from luojianet_ms.context import set_context
        >>> from luojianet_ms.communication.management import init, create_group, get_group_rank_from_world_rank
        >>> set_context(device_target="Ascend")
        >>> init()
        >>> group = "0-4"
        >>> rank_ids = [0,4]
...
def create_group(group, rank_ids):
    ...
    Examples:
        >>> from luojianet_ms.context import set_context
        >>> from luojianet_ms.ops import operations as ops
        >>> from luojianet_ms.communication.management import init, create_group
        >>> set_context(device_target="Ascend")
        >>> init()
        >>> group = "0-8"
        >>> rank_ids = [0,8]
