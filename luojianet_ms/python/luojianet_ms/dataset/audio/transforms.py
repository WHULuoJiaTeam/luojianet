...
class AmplitudeToDB(AudioTensorOperation):
    ...
    Raises:
        TypeError: If `stype` is not of type :class:`luojianet_ms.dataset.audio.utils.ScaleType`.
    ...
    Examples:
        >>> import numpy as np
        >>> from luojianet_ms.dataset.audio import ScaleType
        >>>
...
class ComputeDeltas(AudioTensorOperation):
    ...
    Examples:
        >>> import numpy as np
        >>> from luojianet_ms.dataset.audio import BorderType
        >>>
        >>> waveform = np.random.random([1, 400//2+1, 30])
        >>> numpy_slices_dataset = ds.NumpySlicesDataset(data=waveform, column_names=["audio"])
...
class Fade(AudioTensorOperation):
    ...
    Examples:
        >>> import numpy as np
        >>> from luojianet_ms.dataset.audio import FadeShape
        >>>
        >>> waveform = np.array([[2.716064453125e-03, 6.34765625e-03, 9.246826171875e-03, 1.0894775390625e-02]])
...
class Vol(AudioTensorOperation):
    ...
    Examples:
        >>> import numpy as np
        >>> from luojianet_ms.dataset.audio import GainType
        >>>
