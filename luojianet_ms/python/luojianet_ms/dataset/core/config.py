...
def set_enable_autotune(enable, filepath_prefix=None):
    ...
    Note:
        ...
        - The JSON file can be loaded by API `luojianet_ms.dataset.deserialize` to build a tuned pipeline.
        - In distributed training scenario, set_enable_autotune() must be called after cluster communication has been
          initialized (luojianet_ms.communication.management.init()), otherwise the AutoTune file will always suffix with
          rank id 0.
