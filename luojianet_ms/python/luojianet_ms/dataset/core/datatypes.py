...
def mstype_to_detype(type_):
    """
    Get de data type corresponding to luojianet_ms dtype.

    Args:
        type_ (luojianet_ms.dtype): LuoJiaNET's dtype.
...
def mstypelist_to_detypelist(type_list):
    """
    Get list[de type] corresponding to list[luojianet_ms.dtype].

    Args:
        type_list (list[luojianet_ms.dtype]): a list of LuoJiaNET's dtype.
