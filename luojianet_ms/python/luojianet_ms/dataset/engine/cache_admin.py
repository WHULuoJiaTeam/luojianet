...
def main():
    ...
    cache_admin_dir = os.path.join(os.path.dirname(luojianet_ms.__file__), "bin")
    os.chdir(cache_admin_dir)
    cache_admin = os.path.join(cache_admin_dir, "cache_admin")

    if not os.path.exists(cache_admin):
        raise RuntimeError("Dataset cache is not supported on your luojianet_ms version.")
    ...
    python_lib_dir = os.path.join(os.path.dirname(luojianet_ms.__file__), "../../..")
    os.environ['LD_LIBRARY_PATH'] = python_lib_dir + ":" + os.environ.get('LD_LIBRARY_PATH')

    # LD_PRELOAD libnnacl.so
    nnacl_lib = os.path.join(os.path.dirname(luojianet_ms.__file__), "lib/libnnacl.so")
