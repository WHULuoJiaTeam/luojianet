...
from luojianet_ms.parallel._ps_context import _is_role_pserver, _is_role_sched
from luojianet_ms.parallel._utils import _get_device_num
from luojianet_ms.dataset.engine.offload import GetOffloadModel

import luojianet_ms.dataset.transforms.py_transforms as py_transforms
from luojianet_ms.dataset.text.utils import SentencePieceModel, DE_C_INTER_SENTENCEPIECE_MODE
...
try:
    context = import_module("luojianet_ms.context")
...
class Dataset:
    ...
    def map(self, operations, input_columns=None, output_columns=None, column_order=None,
        ...
        Note:
            - Input `operations` mainly accept c_transforms, py_transforms operator in luojianet_ms.dataset part, plus user
              defined Python function(PyFuncs).
            - Do not add network computing operators from luojianet_ms.nn and luojianet_ms.ops or others into this
              `operations`.
        ...
        if hasattr(self, 'operator_mixed') and getattr(self, 'operator_mixed') is True:
            ...
            logger.warning(
                "Input 'operations' of 'map' includes network computing operators like in luojianet_ms.nn, luojianet_ms.ops, "
                "luojianet_ms.numpy module and etc, which do not support multi-thread compiling, recommend to replace it "
    ...
    def create_tuple_iterator(self, columns=None, num_epochs=-1, output_numpy=False, do_copy=True):
        ...
        Args:
            ...
            do_copy (bool, optional): when output data type is luojianet_ms.Tensor,
...
class FashionMnistDataset(MappableDataset):
    ...
    You can unzip the dataset files into this directory structure and read by LuoJiaNet's API.
...
class IWSLT2016Dataset(SourceDataset, TextBaseDataset):
    ...
    You can unzip the original IWSLT2016 dataset files into this directory structure and read by LuoJiaNet's API. After
...
class IWSLT2017Dataset(SourceDataset, TextBaseDataset):
    ...
    You can unzip the original IWSLT2017 dataset files into this directory structure and read by LuoJiaNet's API. You
...
class PhotoTourDataset(MappableDataset):
    ...
    You can unzip the original PhotoTour dataset files into this directory structure and read by LuoJiaNet's API.
...
class Places365Dataset(MappableDataset):
    ...
    You can unzip the original Places365 dataset files into this directory structure and read by LuoJiaNet's API.
...
def _convert_row(row):
    ...
    for x in row:
        ...
        elif isinstance(x, Tensor):      # got luojianet_ms.Tensor
...
class GeneratorDataset(MappableDataset, TextBaseDataset):
    ...
    Args:
        ...
        column_types (list[luojianet_ms.dtype], optional): List of column data types of the dataset (default=None).
    ...
    Note:
        - Input `source` accept user defined Python function(PyFuncs), Do not add network computing operators from
          luojianet_ms.nn and luojianet_ms.ops or others into this `source`.
        - This dataset can take in a `sampler`. `sampler` and `shuffle` are mutually exclusive.
          The table below shows what input arguments are allowed and their expected behavior.
    ...
    def __init__(self, source, column_names=None, column_types=None, schema=None, num_samples=None,
        ...
        if hasattr(self, 'operator_mixed') and getattr(self, 'operator_mixed') is True:
            ...
            logger.warning(
                "Input 'source' of 'GeneratorDataset' includes network computing operators like in luojianet_ms.nn, "
                "luojianet_ms.ops, luojianet_ms.numpy module and etc, which do not support multi-thread compiling, recommend"
    ...
    def __deepcopy__(self, memodict):
        ...
        if new_op.sampler is not None and hasattr(self.source, "__getitem__"):
            ...
            if self.source_len == -1:
                raise RuntimeError("Attempt to call a random access dataset, '__len__' method is required!")
            try:
...
class AGNewsDataset(SourceDataset, TextBaseDataset):
    ...
    You can unzip the dataset files into the following structure and read by LuoJiaNet's API:
...
class Cifar10Dataset(MappableDataset):
    ...
    You can unzip the dataset files into the following directory structure and read by LuoJiaNet's API.
...
class Schema:
    ...
    Examples:
        >>> from luojianet_ms import dtype as mstype
        >>>
        >>> # Create schema; specify column name, luojianet_ms.dtype and shape of the column
        >>> schema = ds.Schema()
    ...
    def parse_columns(self, columns):
        ...
        Examples:
            >>> from luojianet_ms.dataset import Schema
            >>> schema = Schema()
...
class USPSDataset(SourceDataset):
    ...
    You can download and unzip the dataset files into this directory structure and read by LuoJiaNet's API.
...
class VOCDataset(MappableDataset):
    ...
    You can unzip the original VOC-2012 dataset files into this directory structure and read by LuoJiaNet's API.
...
class CocoDataset(MappableDataset):
    ...
    You can unzip the original COCO-2017 dataset files into this directory structure and read by LuoJiaNet's API.
