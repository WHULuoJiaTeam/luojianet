...
class AGNewsDataset(SourceDataset, TextBaseDataset):
    ...
    Args:
        ...
        shuffle (Union[bool, Shuffle level], optional): Perform reshuffling of the data every epoch
            ...
            There are three levels of shuffling, desired shuffle enum defined by luojianet_ms.dataset.Shuffle.
...
class AmazonReviewDataset(SourceDataset, TextBaseDataset):
    ...
    Args:
        ...
        shard_id (int, optional): The shard ID within `num_shards` (default=None). This
            argument can only be specified when `num_shards` is also specified.
        num_parallel_workers (int, optional): Number of workers to read the data
            (default=None, number set in the  luojianet_ms.dataset.config).
        cache (DatasetCache, optional): Use tensor caching service to speed up dataset processing
            (default=None, which means no cache is used).
...
class SQuADDataset(SourceDataset, TextBaseDataset):
    ...
    You can get the dataset files into the following structure and read by LuoJiaNET's API,
