...
class JiebaTokenizer(TextTensorOperation):
    ...
    Examples:
        >>> from luojianet_ms.dataset.text import JiebaMode
...
class Lookup(TextTensorOperation):
    ...
    Args:
        ...
        data_type (luojianet_ms.dtype, optional): The data type that lookup operation maps
            string to(default=luojianet_ms.int32).

    Raises:
        TypeError: If `vocab` is not of type text.Vocab.
        TypeError: If `unknown_token` is not of type string.
        TypeError: If `data_type` is not of type luojianet_ms.dtype.
...
class Ngram(TextTensorOperation):
    ...
    Args:
        n (list[int]): n in n-gram, which is a list of positive integers. For example, if n=[4, 3], then the result
            ...
            for a n-gram, an empty string will be returned. For example, 3 grams on ["luojianet_ms", "best"] will result in
        ...
        separator (str, optional): Symbol used to join strings together. For example, if 2-gram is
            ["luojianet_ms", "amazing"] with separator="-", the result would be ["luojianet_ms-amazing"]
...
class SentencePieceTokenizer(TextTensorOperation):
    ...
    Examples:
        >>> from luojianet_ms.dataset.text import SentencePieceModel, SPieceTokenizerOutType
        >>> sentence_piece_vocab_file = "/path/to/sentence/piece/vocab/file"
...
class ToNumber(TextTensorOperation):
    ...
    Args:
        data_type (luojianet_ms.dtype): Type to be cast to. Must be a numeric type in luojianet_ms.dtype.
...
class WordpieceTokenizer(TextTensorOperation):
    ...
    Raises:
        TypeError: If `vocab` is not of type :class:`luojianet_ms.dataset.text.Vocab`.
...
if platform.system().lower() != 'windows':
    ...
    class BasicTokenizer(TextTensorOperation):
        ...
        Raises:
            ...
            TypeError: If `normalization_form` is not of type :class:`luojianet_ms.dataset.text.NormalizeForm`.
        ...
        Examples:
            >>> from luojianet_ms.dataset.text import NormalizeForm
            >>>
            >>> # If with_offsets=False, default output one column {["text", dtype=str]}
    ...
    class FilterWikipediaXML(TextTensorOperation):
        ...
        Examples:
            >>> import luojianet_ms.dataset.text.transforms as text
            >>>
