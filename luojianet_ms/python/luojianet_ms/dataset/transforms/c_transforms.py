...
class TypeCast(TensorOperation):
    """
    Tensor operation to cast to a given LuoJiaNET data type.

    Note:
        This operation supports running on Ascend or GPU platforms by Offload.

    Args:
        data_type (luojianet_ms.dtype): luojianet_ms.dtype to be cast to.
...
class Mask(TensorOperation):
    ...
    Args:
        ...
        dtype (luojianet_ms.dtype, optional): Type of the generated mask. Default: luojianet_ms.dtype.bool\_.

    Raises:
        TypeError: `operator` is not of type Relational.
        TypeError: `constant` is not of type string int, float or bool.
        TypeError: `dtype` is not of type luojianet_ms.dtype.

    Supported Platforms:
        ``CPU``

    Examples:
        >>> from luojianet_ms.dataset.transforms.c_transforms import Relational
        >>> # Data before
        >>> # |  col   |
        >>> # +---------+
        >>> # | [1,2,3] |
