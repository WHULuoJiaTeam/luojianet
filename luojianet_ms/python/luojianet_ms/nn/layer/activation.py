...
class CELU(Cell):
    ...
    Examples:
        >>> x = Tensor(np.array([-2.0, -1.0, 1.0, 2.0]), luojianet_ms.float32)
        >>> celu = nn.CELU()
        >>> output = celu(x)
        >>> print(output)
...
class Softmax(Cell):
    ...
    Examples:
        ...
        >>> x = Tensor(np.array([-1, -2, 0, 2, 1]), luojianet_ms.float16)
        >>> softmax = nn.Softmax()
        >>> output = softmax(x)
        >>> print(output)
...
class ELU(Cell):
    ...
    Examples:
        >>> x = Tensor(np.array([-1, -2, 0, 2, 1]), luojianet_ms.float32)
        >>> elu = nn.ELU()
        >>> result = elu(x)
        >>> print(result)
...
class ReLU(Cell):
    ...
    Inputs:
        - **x** (Tensor) - The input of ReLU is a Tensor of any dimension. The data type is `number <https://www.mind\
          spore.cn/docs/en/r1.7/api_python/luojianet_ms.html#luojianet_ms.dtype>`_ .
    ...
    Examples:
        >>> x = Tensor(np.array([-1, 2, -3, 2, -1]), luojianet_ms.float16)
        >>> relu = nn.ReLU()
        >>> output = relu(x)
        >>> print(output)
        [0. 2. 0. 2. 0.]
...
class PReLU(Cell):
    ...
    Examples:
        >>> x = Tensor(np.array([[[[0.1, 0.6], [0.9, 0.9]]]]), luojianet_ms.float32)
        >>> prelu = nn.PReLU()
        >>> output = prelu(x)
        >>> print(output)
        [[[[0.1 0.6]
...
class HShrink(Cell):
    ...
    Examples:
        ...
        >>> from luojianet_ms import Tensor, nn
        >>> import numpy as np
        >>> input_x = Tensor(np.array([[ 0.5,  1,  2.0],[0.0533,0.0776,-2.1233]]),luojianet_ms.float32)
        >>> hshrink = nn.HShrink()
