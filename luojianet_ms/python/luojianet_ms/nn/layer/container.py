...
class _CellListBase:
    ...
    The sequential cell may be iterated using the call method using for-in statement.
    But there are some scenarios that the call method built-in does not fit.
...
class SequentialCell(Module):
    ...
    Examples:
        ...
        >>> x = Tensor(np.ones([1, 3, 4, 4]), dtype=luojianet_ms.float32)
        >>> output = seq(x)
        >>> print(output)
        [[[[27. 27.]
           [27. 27.]]
...
class CellList(_CellListBase, Module):
    ...
    but cells it contains are properly registered, and will be visible by all Module methods.
    ...
    def insert(self, index, cell):
        ...
        Args:
            ...
            cell(Module): The subcell to be inserted.
        """
        cls_name = self.__class__.__name__
