...
from luojianet_ms.communication.management import get_group_size, get_rank
from luojianet_ms.context import ParallelMode
from luojianet_ms.parallel._utils import _get_parallel_mode, _get_full_batch
from luojianet_ms.parallel._ps_context import _is_role_worker, _get_ps_context
from luojianet_ms.parallel._ps_context import _insert_hash_table_size, _set_cache_enable, _set_rank_id
...
class Embedding(Cell):
    ...
    Note:
        When 'use_one_hot' is set to True, the type of the `x` must be luojianet_ms.int32.
    ...
    Examples:
        ...
        >>> x = Tensor(np.ones([8, 128]), luojianet_ms.int32)
...
class EmbeddingLookup(Cell):
    ...
    Args:
        ...
        slice_mode (str): The slicing way in semi_auto_parallel/auto_parallel. The value must get through
            :class:`luojianet_ms.nn.EmbeddingLookup`. Default: :class:`luojianet_ms.nn.EmbeddingLookup.BATCH_SLICE`.
    ...
    Examples:
        >>> input_indices = Tensor(np.array([[1, 0], [3, 2]]), luojianet_ms.int32)
...
class MultiFieldEmbeddingLookup(EmbeddingLookup):
    ...
    Examples:
        >>> input_indices = Tensor([[2, 4, 6, 0, 0], [1, 3, 5, 0, 0]], luojianet_ms.int32)
        >>> input_values = Tensor([[1, 1, 1, 0, 0], [1, 1, 1, 0, 0]], luojianet_ms.float32)
        >>> field_ids = Tensor([[0, 1, 1, 0, 0], [0, 0, 1, 0, 0]], luojianet_ms.int32)
