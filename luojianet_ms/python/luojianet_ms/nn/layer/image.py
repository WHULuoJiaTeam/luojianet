...
class ImageGradients(Module):
    ...
    Examples:
        ...
        >>> image = Tensor(np.array([[[[1, 2], [3, 4]]]]), dtype=luojianet_ms.int32)
        >>> output = net(image)
        >>> print(output)
...
class PSNR(Module):
    ...
    Outputs:
        Tensor, with dtype luojianet_ms.float32. It is a 1-D tensor with shape N, where N is the batch num of img1.
...
class CentralCrop(Module):
    ...
    Examples:
        ...
        >>> image = Tensor(np.random.random((4, 3, 4, 4)), luojianet_ms.float32)
        >>> output = net(image)
        >>> print(output.shape)
        (4, 3, 2, 2)
    """
