...
class GlobalBatchNorm(_BatchNorm):
    r"""
    The GlobalBatchNorm interface is deprecated, please use the :class:`luojianet_ms.nn.SyncBatchNorm` instead.
...
class LayerNorm(Module):
    ...
    Examples:
        >>> x = Tensor(np.ones([20, 5, 10, 10]), luojianet_ms.float32)
        >>> shape1 = x.shape[1:]
...
class InstanceNorm2d(Module):
    ...
    Examples:
        ...
        >>> x = Tensor(np.ones([2, 3, 2, 2]), luojianet_ms.float32)
        >>> output = net(x)
        >>> print(output.shape)
        (2, 3, 2, 2)
    """
