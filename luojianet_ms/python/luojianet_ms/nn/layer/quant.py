...
from luojianet_ms.ops.primitive import Primitive
from luojianet_ms.ops import operations as P
from luojianet_ms.common.parameter import Parameter
from luojianet_ms.common.initializer import initializer
from luojianet_ms.common.tensor import Tensor
from luojianet_ms._checkparam import Validator, twice
from luojianet_ms.compression.common import QuantDtype
...
class BatchNormFoldCell(Module):
    """
    ...
    def forward(self, x, mean, variance, global_step):
        if self.is_gpu:
            if self.training:
...
class FakeQuantWithMinMaxObserver(UniformQuantObserver):
    ...
    Examples:
        ...
        >>> x = Tensor(np.array([[1, 2, 1], [-2, 0, -1]]), luojianet_ms.float32)
        >>> result = fake_quant(x)
        >>> print(result)
...
class Conv2dBnFoldQuantOneConv(Module):
    ...
    subclass of `FakeQuantWithMinMaxObserver`, :class:`luojianet_ms.nn.FakeQuantWithMinMaxObserver`.
    ...
    Args:
        ...
        fake (bool): Whether Conv2dBnFoldQuant Module adds FakeQuantWithMinMaxObserver. Default: True.
        quant_config (QuantConfig): Configures the types of quant observer and quant settings of weight and
            ...
            and can be generated by :func:`luojianet_ms.compression.quant.create_quant_config` method.
    ...
    Examples:
        >>> import luojianet_ms
        >>> from luojianet_ms.compression import quant
        >>> from luojianet_ms import Tensor
        ...
        >>> x = Tensor(np.array([[[[1, 0, 3], [1, 4, 7], [2, 5, 2]]]]), luojianet_ms.float32)
...
class Conv2dBnFoldQuant(Module):
    r"""
    2D convolution with Batch Normalization operation folded forward.
...
class Conv2dBnWithoutFoldQuant(Module):
    r"""
    2D convolution and batchnorm without fold with fake quantized forward.

    This part is a more detailed overview of Conv2d operation. For more details about Quantization,
    please refer to the implementation of class of `FakeQuantWithMinMaxObserver`,
    :class:`luojianet_ms.nn.FakeQuantWithMinMaxObserver`.
    ...
    where :math:`quant` is the continuous execution of quant and dequant, you can refer to the implementation of
    class of `FakeQuantWithMinMaxObserver`, :class:`luojianet_ms.nn.FakeQuantWithMinMaxObserver`.
...
class DenseQuant(Module):
    ...
    Args:
        ...
        activation (Union[str, Module, Primitive]): The regularization function applied to the output of the layer,
    ...
    Raises:
        ...
        TypeError: If `has_bias` is not a bool.
        TypeError: If `activation` is not str, Module and Primitive.
    ...
    Examples:
        ...
        >>> x = Tensor(np.array([[1, 5], [3, 4]]), luojianet_ms.float32)
    ...
    def forward(self, x):
        """Use operators to forward the Dense layer.
...
class ActQuant(_QuantActivation):
    ...
    of subclass of `FakeQuantWithMinMaxObserver`, :class:`luojianet_ms.nn.FakeQuantWithMinMaxObserver`.

    Args:
        activation (Module): Activation cell.
        ema (bool): The exponential Moving Average algorithm updates min and max. Default: False.
        ema_decay (float): Exponential Moving Average algorithm parameter. Default: 0.999.
    ...
    Raises:
        TypeError: If `activation` is not an instance of Module.
    ...
    def __init__(self,
        ...
        self.act = Validator.check_isinstance("activation", activation, Module)
...
class TensorAddQuant(Module):
    ...
    Examples:
        ...
        >>> x1 = Tensor(np.array([[1, 2, 1], [-2, 0, -1]]), luojianet_ms.float32)
        >>> x2 = Tensor(np.ones((2, 3)), luojianet_ms.float32)
...
class MulQuant(Module):
    ...
    Examples:
        ...
        >>> x1 = Tensor(np.array([[1, 2, 1], [-2, 0, -1]]), luojianet_ms.float32)
        >>> x2 = Tensor(np.ones((2, 3)) * 2, luojianet_ms.float32)
