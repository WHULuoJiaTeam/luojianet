...
class BatchNormFoldCell(Module):
    ...
    def call(self, x, mean, variance, global_step):
        if self.is_gpu:
            if self.training:
...
class Conv2dBnFoldQuant(Module):
    r"""
    2D convolution with Batch Normalization operation folded call.
...
class Conv2dBnWithoutFoldQuant(Module):
    r"""
    2D convolution and batchnorm without fold with fake quantized call.
...
class DenseQuant(Module):
    ...
    def call(self, x):
        """Use operators to call the Dense layer.
