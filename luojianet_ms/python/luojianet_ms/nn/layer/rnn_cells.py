...
def _check_lstmcell_init(func):
    def wrapper(*args, **kwargs):
        ...
        if len(args) > 4 or 'batch_size' in kwargs or \
            ...
            raise ValueError(f"The arguments of `nn.LSTMCell` from old LuoJiaNET version(<1.6) are detected, "
                             f"if you still need use single LSTM layer, please use `nn.LSTM` instead.")
...
class RNNCell(RNNCellBase):
    ...
    Inputs:
        ...
        - **hx** (Tensor) - Tensor of data type luojianet_ms.float32 and shape :math:`(batch\_size, hidden\_size)` .
...
class GRUCell(RNNCellBase):
    ...
    Inputs:
        - **x** (Tensor) - Tensor of shape (batch_size, `input_size`).
        - **hx** (Tensor) - Tensor of data type luojianet_ms.float32 and shape (batch_size, `hidden_size`).
          Data type of `hx` must be the same as `x`.
