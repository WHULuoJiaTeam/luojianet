...
class RNN(_RNNBase):
    ...
    Inputs:
        - **x** (Tensor) - Tensor of data type luojianet_ms.float32 or luojianet_ms.float16 and
          shape :math:`(seq\_len, batch\_size, input\_size)` or :math:`(batch\_size, seq\_len, input\_size)` .
        - **hx** (Tensor) - Tensor of data type luojianet_ms.float32 or luojianet_ms.float16 and
...
class LSTM(_RNNBase):
    ...
    Inputs:
        ...
        - **hx** (tuple) - A tuple of two Tensors (h_0, c_0) both of data type luojianet_ms.float32
          or luojianet_ms.float16 and shape (num_directions * `num_layers`, batch_size, `hidden_size`).
          The data type of `hx` must be the same as `x`.
        - **seq_length** (Tensor) - The length of each sequence in an input batch.
          Tensor of shape :math:`(\text{batch_size})`. Default: None.
          This input indicates the real sequence length before padding to avoid padded elements
