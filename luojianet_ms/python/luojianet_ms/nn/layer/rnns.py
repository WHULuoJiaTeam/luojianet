...
class _DynamicRNNBase(Module):
    ...
    def call(self, x, h, seq_length, w_ih, w_hh, b_ih, b_hh):
...
class _DynamicGRUAscend(Module):
    ...
    def call(self, x, h_0, seq_length, w_ih, w_hh, b_ih, b_hh):
        if b_ih is None:
...
class _RNNBase(Module):
    ...
    def call(self, x, hx=None, seq_length=None):
...
class RNN(_RNNBase):
    ...
    Inputs:
        - **x** (Tensor) - Tensor of data type luojianet_ms.float32 and
          shape (seq_len, batch_size, `input_size`) or (batch_size, seq_len, `input_size`).
        - **hx** (Tensor) - Tensor of data type luojianet_ms.float32 and
...
class LSTM(_RNNBase):
    ...
    Inputs:
        - **x** (Tensor) - (Tensor) - Tensor of data type luojianet_ms.float32 and
          shape (seq_len, batch_size, `input_size`) or (batch_size, seq_len, `input_size`).
        - **hx** (tuple) - A tuple of two Tensors (h_0, c_0) both of data type luojianet_ms.float32
