...
from luojianet_ms.common.initializer import initializer, Initializer
...
class DenseThor(Module):
    ...
    Examples:
        >>> x = Tensor(np.array([[1, 2, 3], [3, 4, 5]]), luojianet_ms.float32)
...
class Conv2dThor(_ConvThor):
    ...
    Examples:
        ...
        >>> # for Ascend
        >>> x = Tensor(np.ones([1, 120, 1024, 640]), luojianet_ms.float16)
        >>> print(net(x).shape)
        (1, 240, 1024, 640)
    """
...
class EmbeddingThor(Module):
    ...
    Note:
        When 'use_one_hot' is set to True, the type of the input `x` must be luojianet_ms.int32.
    ...
    Args:
        ...
        dtype (:class:`luojianet_ms.dtype`): Data type of input `x`. Default: luojianet_ms.float32.
