...
class LossBase(Module):
    ...
    Other losses derived from this should implement their own `call` and use method `self.get_loss`
    ...
    def get_axis(self, x):
        ...
        Examples:
            ...
            >>> labels = Tensor(np.array([1, 2, 3]), luojianet_ms.float32)
            >>> output = net(logits, labels)
            >>> print(output)
            (0,)
            >>> # Case 2: logits.shape = labels.shape = (3, 3)
            >>> logits = Tensor(np.array([[1, 2, 3],[1, 2, 3],[1, 2, 3]]), luojianet_ms.float32)
            >>> labels = Tensor(np.array([[1, 2, 3],[1, 2, 3],[1, 2, 3]]), luojianet_ms.float32)
            >>> output = net(logits, labels)
            >>> print(output)
            (0, 1)
        """
    ...
    def get_loss(self, x, weights=1.0):
        ...
        Examples:
            ...
            >>> # Case 2: logits.shape = labels.shape = (3, 3)
            >>> logits = Tensor(np.array([[1, 2, 3],[1, 2, 3],[1, 2, 3]]), luojianet_ms.float32)
            >>> labels = Tensor(np.array([[1, 2, 2],[1, 2, 3],[1, 2, 3]]), luojianet_ms.float32)
            >>> output = net(logits, labels)
            >>> print(output)
            0.11111111
        """
...
class L1Loss(LossBase):
    ...
    Examples:
        ...
        >>> logits = Tensor(np.array([1, 2, 3]), luojianet_ms.float32)
        >>> labels = Tensor(np.array([[1, 1, 1], [1, 2, 2]]), luojianet_ms.float32)
        >>> output = loss(logits, labels)
        >>> print(output)
        [[0. 1. 2.]
         [0. 0. 1.]]
...
class MSELoss(LossBase):
    ...
    Examples:
        ...
        >>> loss = nn.MSELoss()
        >>> logits = Tensor(np.array([1, 2, 3]), luojianet_ms.float32)
        >>> labels = Tensor(np.array([1, 1, 1]), luojianet_ms.float32)
        >>> output = loss(logits, labels)
        >>> print(output)
        1.6666667
        >>> # Case 2: logits.shape = (3,), labels.shape = (2, 3)
...
class SoftmaxCrossEntropyWithLogits(LossBase):
    ...
    Examples:
        ...
        >>> logits = Tensor(np.array([[3, 5, 6, 9, 12, 33, 42, 12, 32, 72]]), luojianet_ms.float32)
...
class SampledSoftmaxLoss(LossBase):
    ...
    Examples:
        ...
        >>> weights = Tensor(np.random.randint(0, 9, [7, 10]), luojianet_ms.float32)
        >>> biases = Tensor(np.random.randint(0, 9, [7]), luojianet_ms.float32)
        >>> labels = Tensor([0, 1, 2])
        >>> logits = Tensor(np.random.randint(0, 9, [3, 10]), luojianet_ms.float32)
    ...
    def call(self, weights, biases, labels, logits):
...
class BCELoss(LossBase):
    ...
    Examples:
        >>> weight = Tensor(np.array([[1.0, 2.0, 3.0], [4.0, 3.3, 2.2]]), luojianet_ms.float32)
        >>> loss = nn.BCELoss(weight=weight, reduction='mean')
        >>> logits = Tensor(np.array([[0.1, 0.2, 0.3], [0.5, 0.7, 0.9]]), luojianet_ms.float32)
        >>> labels = Tensor(np.array([[0, 1, 0], [0, 0, 1]]), luojianet_ms.float32)
        >>> output = loss(logits, labels)
        >>> print(output)
        1.8952923
    """
...
class CosineEmbeddingLoss(LossBase):
    ...
    Examples:
        >>> logits_x1 = Tensor(np.array([[0.3, 0.8], [0.4, 0.3]]), luojianet_ms.float32)
        >>> logits_x2 = Tensor(np.array([[0.4, 1.2], [-0.4, -0.9]]), luojianet_ms.float32)
        >>> labels = Tensor(np.array([1, -1]), luojianet_ms.int32)
    ...
    def call(self, logits_x1, logits_x2, labels):
...
class FocalLoss(LossBase):
    ...
    def call(self, logits, labels):
        ...
        if labels.shape[1] == 1:
            log_probability = self.gather_d(log_probability, 1, self.cast(labelss, luojianet_ms.int32))
        ...
        if self.weight is not None:
            ...
            if labels.shape[1] == 1:
                convert_weight = self.gather_d(convert_weight, 1, self.cast(labelss, luojianet_ms.int32))
