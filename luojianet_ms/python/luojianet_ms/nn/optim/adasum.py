...
def _parallel_check():
    ...
    if context.get_auto_parallel_context("parallel_mode") in ["data_parallel", "hybrid_parallel"]:
        logger.warning("For data parallel mode or hybrid parallel mode, "
                       "it is recommended to using luojianet_ms.boost to enable adasum.")
...
class AdaSumByGradWrapCell(Cell):
    ...
    Examples:
        ...
        >>> from luojianet_ms.nn import AdaSumByGradWrapCell
        >>> net = Net()
...
class AdaSumByDeltaWeightWrapCell(Cell):
    ...
    Examples:
        >>> from luojianet_ms import nn
        >>> from luojianet_ms.nn import AdaSumByDeltaWeightWrapCell
        >>> net = Net()
