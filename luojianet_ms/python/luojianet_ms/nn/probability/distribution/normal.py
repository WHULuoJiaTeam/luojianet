...
class Normal(Distribution):
    ...
    Examples:
        ...
        >>> n1 = msd.Normal(3.0, 4.0, dtype=luojianet_ms.float32)
        >>> # A Normal distribution can be initialized without arguments.
        >>> # In this case, `mean` and `sd` must be passed in through arguments.
        >>> n2 = msd.Normal(dtype=luojianet_ms.float32)
        >>> # Here are some tensors used below for testing
        >>> value = Tensor([1.0, 2.0, 3.0], dtype=luojianet_ms.float32)
        >>> mean_a = Tensor([2.0], dtype=luojianet_ms.float32)
        >>> sd_a = Tensor([2.0, 2.0, 2.0], dtype=luojianet_ms.float32)
        >>> mean_b = Tensor([1.0], dtype=luojianet_ms.float32)
        >>> sd_b = Tensor([1.0, 1.5, 2.0], dtype=luojianet_ms.float32)
