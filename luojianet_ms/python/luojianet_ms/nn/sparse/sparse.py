...
class SparseToDense(Module):
    ...
    LuoJiaNet uses three independent dense tensors: indices, value and dense shape to represent the sparse tensor.
    ...
    Inputs:
        - **sparse_tensor** (:class:`luojianet_ms.SparseTensor`): the sparse tensor to convert.
...
class SparseTensorDenseMatmul(Module):
    ...
    def call(self, indices, values, sparse_shape, dense):
