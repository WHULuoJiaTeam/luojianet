...
class DynamicLossScaleUpdateCell(Module):
    ...
    `get_update_cell` method of :class:`luojianet_ms.nn.DynamicLossScaleManager` will return this class, it will be called
    by :class:`luojianet_ms.TrainOneStepWithLossScaleCell` during training to update loss scale.
    ...
    Examples:
        ...
        >>> input = Tensor(np.ones([out_features, in_features]), luojianet_ms.float32)
        >>> labels = Tensor(np.ones([out_features,]), luojianet_ms.float32)
    ...
    def call(self, loss_scale, overflow):
...
class FixedLossScaleUpdateCell(Module):
    ...
    `get_update_cell` method of :class:`luojianet_ms.nn.FixedLossScaleManager` will return this class, it will be called
    by :class:`luojianet_ms.TrainOneStepWithLossScaleCell` during trainning.
    ...
    def __init__(self, network, optimizer, scale_sense):
        ...
        if isinstance(scale_sense, Module):
            ...
            raise TypeError("For '_TrainPipelineWithLossScaleCell', "
                            "the 'scale_sense' must be Module or Tensor, but got {}".format(type(scale_sense)))
