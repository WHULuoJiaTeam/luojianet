...
def isscalar(element):
    ...
    Note:
        Only object types recognized by the luojianet_ms parser are supported,
        which includes objects, types, methods and functions defined within
        the scope of luojianet_ms. Other built-in types are not supported.
    ...
    Raises:
        TypeError: If the type of `element` is not supported by luojianet_ms parser.
...
def array_equal(a1, a2, equal_nan=False):
    ...
    Note:
        In luojianet_ms, a bool tensor is returned instead, since in Graph mode, the
