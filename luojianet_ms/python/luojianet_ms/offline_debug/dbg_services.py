...
from luojianet_ms.offline_debug.mi_validators import check_init, check_initialize, check_add_watchpoint,\
...
from luojianet_ms.offline_debug.mi_validator_helpers import replace_minus_one
from luojianet_ms import log as logger
if not security.enable_security():
    import luojianet_ms._luojianet_ms_offline_debug as cds
...
def get_version():
    ...
    Examples:
        >>> from luojianet_ms.ccsrc.debug.debugger.offline_debug import dbg_services
        >>> version = dbg_services.get_version()
    """
    if security.enable_security():
        raise ValueError("Offline debugger is not supported in security mode. "
                         "Please recompile luojianet_ms without `-s on`.")
...
class DbgServices:
    ...
    def transform_check_node_list(info_name, info_param, node_name, check_node_list):
        ...
        Examples:
        >>> from luojianet_ms.offline_debug import dbg_services
        >>> d = dbg_services.DbgServices(dump_file_path="dump_file_path")
