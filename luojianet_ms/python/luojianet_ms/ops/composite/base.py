...
def add_flags(fn=None, **flags):
    ...
    Examples:
        ...
        >>> print(hasattr(net, '_luojianet_ms_flags'))
        True
    """
    def deco(fn):
        # need set the attr and access on c++
        if not hasattr(fn, "_luojianet_ms_flags"):
            fn._luojianet_ms_flags = {}

        fn._luojianet_ms_flags.update({**flags})
        return fn
    ret = deco
    if fn is not None:
        ret = deco(fn)
...
class GradOperation(GradOperation_):
    ...
    Examples:
        ...
        >>>
        >>> class GradNetWrtXYWithSensParam(nn.Module):
        ...     def __init__(self, net):
        ...
        >>>
        >>> class GradNetWithWrtParams(nn.Module):
        ...     def __init__(self, net):
        ...
        >>>
        >>> class GradNetWrtInputsAndParams(nn.Module):
        ...     def __init__(self, net):
...
class MultitypeFuncGraph(MultitypeFuncGraph_):
    ...
    Examples:
        ...
        >>> from luojianet_ms.ops.composite import MultitypeFuncGraph
        >>>
        >>> tensor_add = ops.Add()
    ...
    def register(self, *type_names):
        ...
        Args:
            type_names (Union[str, :class:`luojianet_ms.dtype`]): Inputs type names or types list.
...
class HyperMap(HyperMap_):
    ...
    Examples:
        ...
        >>> from luojianet_ms.ops.composite.base import MultitypeFuncGraph, HyperMap
        >>> from luojianet_ms import dtype as mstype
...
class Map(Map_):
    ...
    Examples:
        >>> from luojianet_ms import dtype as mstype
        >>> from luojianet_ms import Tensor, ops
        >>> from luojianet_ms.ops import MultitypeFuncGraph, Map
