...
import numpy as np
from luojianet_ms.ops.composite.multitype_ops import _constexpr_utils as const_utils
...
from luojianet_ms.ops.operations._inner_ops import DynamicResizeNearestNeighbor
...
def count_nonzero(x, axis=(), keep_dims=False, dtype=mstype.int32):
    ...
    Args:
        ...
        dtype (Union[Number, luojianet_ms.bool\_]): The data type of the output tensor. Only constant value is allowed.
                                             Default: luojianet_ms.int32
    ...
    Examples:
        ...
        >>> nonzero_num = ops.count_nonzero(x=x, axis=[0, 1], keep_dims=True, dtype=luojianet_ms.int32)
        >>> print(nonzero_num)
        [[3]]
...
def tensor_dot(x1, x2, axes):
    ...
    Examples:
        ...
        >>> import luojianet_ms
        >>> import numpy as np
        >>> input_x1 = Tensor(np.ones(shape=[1, 2, 3]), luojianet_ms.float32)
        >>> input_x2 = Tensor(np.ones(shape=[3, 1, 2]), luojianet_ms.float32)
...
def dot(x1, x2):
    ...
    Examples:
        >>> from luojianet_ms import Tensor, ops
        >>> import luojianet_ms
        >>> input_x1 = Tensor(np.ones(shape=[2, 3]), luojianet_ms.float32)
        >>> input_x2 = Tensor(np.ones(shape=[1, 3, 2]), luojianet_ms.float32)
        ...
        (1, 2, 1, 2)
        >>> input_x1 = Tensor(np.ones(shape=[1, 2, 3]), luojianet_ms.float32)
        >>> input_x2 = Tensor(np.ones(shape=[2, 3, 2]), luojianet_ms.float32)
        >>> output = ops.dot(input_x1, input_x2)
        >>> print(output)
        [[[[3. 3.]
           [3. 3.]]
        ...
        >>> print(output.shape)
        (1, 2, 2, 2)
        >>> input_x1 = Tensor(np.ones(shape=[3, 2, 3]), luojianet_ms.float32)
        >>> input_x2 = Tensor(np.ones(shape=[2, 1, 3, 2]), luojianet_ms.float32)
        >>> output = ops.dot(input_x1, input_x2)
        >>> print(output)
        [[[[[3. 3.]]
           [[3. 3.]]]
...
def batch_dot(x1, x2, axes=None):
    ...
    Examples:
        >>> from luojianet_ms import Tensor, ops
        >>> import numpy as np
        >>> x1 = Tensor(np.ones(shape=[2, 2, 3]), luojianet_ms.float32)
        >>> x2 = Tensor(np.ones(shape=[2, 3, 2]), luojianet_ms.float32)
        >>> axes = (-1, -2)
        ...
        [[[3. 3.]
         ...
         [[3. 3.]
          [3. 3.]]]
        >>> x1 = Tensor(np.ones(shape=[2, 2]), luojianet_ms.float32)
        >>> x2 = Tensor(np.ones(shape=[2, 3, 2]), luojianet_ms.float32)
        >>> axes = (1, 2)
        >>> output = ops.batch_dot(x1, x2, axes)
        >>> print(output)
        ...
        >>> print(output.shape)
        (2, 3)
        >>> x1 = Tensor(np.ones(shape=[6, 2, 3, 4]), luojianet_ms.float32)
        >>> x2 = Tensor(np.ones(shape=[6, 5, 4, 8]), luojianet_ms.float32)
        >>> output = ops.batch_dot(x1, x2)
        >>> print(output.shape)
        (6, 2, 3, 5, 8)
        >>> x1 = Tensor(np.ones(shape=[2, 2, 4]), luojianet_ms.float32)
        >>> x2 = Tensor(np.ones(shape=[2, 5, 4, 5]), luojianet_ms.float32)
        >>> output = ops.batch_dot(x1, x2)
        >>> print(output.shape)
        (2, 2, 5, 5)
...
def matmul(x1, x2, dtype=None):
    ...
    Examples:
        ...
        >>> x1 = Tensor(np.arange(2*3*4).reshape(2, 3, 4), luojianet_ms.float32)
        >>> x2 = Tensor(np.arange(4*5).reshape(4, 5), luojianet_ms.float32)
        ...
        >>> x1 = Tensor(np.ones([1, 2]), luojianet_ms.float32)
        >>> x2 = Tensor(np.ones([2,]), luojianet_ms.float32)
        >>> output = ops.matmul(x1, x2)
        >>> print(output)
        [2.]
        >>> print(output.shape)
