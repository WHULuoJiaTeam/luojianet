...
def normal(shape, mean, stddev, seed=None):
    ...
    Examples:
        ...
        >>> import luojianet_ms
        >>> shape = (3, 1, 2)
        >>> mean = Tensor(np.array([[3, 4], [5, 6]]), luojianet_ms.float32)
        >>> stddev = Tensor(1.0, luojianet_ms.float32)
        >>> output = ops.normal(shape, mean, stddev, seed=5)
        >>> result = output.shape
        >>> print(result)
        (3, 2, 2)
        >>> shape = (3, 1, 3)
        >>> mean = Tensor(np.array([[3, 4, 3], [3, 5, 6]]), luojianet_ms.float32)
        >>> stddev = Tensor(1.0, luojianet_ms.float32)
        >>> output = ops.normal(shape, mean, stddev, seed=5)
        >>> result = output.shape
        >>> print(result)
        (3, 2, 3)
        >>> shape = (3, 1, 3)
        >>> mean = Tensor(np.array([[1, 2, 3], [3, 4, 3], [3, 5, 6]]), luojianet_ms.float32)
        >>> stddev = Tensor(1.0, luojianet_ms.float32)
        >>> output = ops.normal(shape, mean, stddev, seed=5)
        >>> result = output.shape
        >>> print(result)
...
def laplace(shape, mean, lambda_param, seed=None):
    ...
    Examples:
        ...
        >>> from luojianet_ms import ops as ops
        >>> shape = (2, 3)
        >>> mean = Tensor(1.0, luojianet_ms.float32)
        >>> lambda_param = Tensor(1.0, luojianet_ms.float32)
...
def uniform(shape, minval, maxval, seed=None, dtype=mstype.float32):
    ...
    Args:
        ...
        dtype (luojianet_ms.dtype): Type of the Uniform distribution. If it is int32, it generates numbers from discrete
          uniform distribution; if it is float32, it generates numbers from continuous uniform distribution. It only
          supports these two data types. Default: luojianet_ms.float32.
    ...
    Examples:
        ...
        >>> shape = (4, 2)
        >>> minval = Tensor(1, luojianet_ms.int32)
        >>> maxval = Tensor(2, luojianet_ms.int32)
        >>> output = ops.uniform(shape, minval, maxval, seed=5, dtype=luojianet_ms.int32)
        >>>
        >>> # For continuous uniform distribution, minval and maxval can be multi-dimentional:
        >>> shape = (3, 1, 2)
        >>> minval = Tensor(np.array([[3, 4], [5, 6]]), luojianet_ms.float32)
        >>> maxval = Tensor([8.0, 10.0], luojianet_ms.float32)
...
def gamma(shape, alpha, beta, seed=None):
    ...
    Examples:
        ...
        >>> shape = (3, 1, 2)
        >>> alpha = Tensor(np.array([[3, 4], [5, 6]]), luojianet_ms.float32)
        >>> beta = Tensor(np.array([1.0]), luojianet_ms.float32)
        ...
        >>> shape = (3, 1, 3)
        >>> alpha = Tensor(np.array([[1, 3, 4], [2, 5, 6]]), luojianet_ms.float32)
        >>> beta = Tensor(np.array([1.0]), luojianet_ms.float32)
        >>> output = ops.gamma(shape, alpha, beta, seed=5)
        >>> result = output.shape
        >>> print(result)
        ...
        >>> shape = (3, 1, 2)
        >>> alpha = Tensor(np.array([[3, 4], [5, 6]]), luojianet_ms.float32)
        >>> beta = Tensor(np.array([1.0, 2]), luojianet_ms.float32)
        >>> output = ops.gamma(shape, alpha, beta, seed=5)
        >>> result = output.shape
        >>> print(output)
        [[[ 2.2132034  5.8855834]]
        ...
        >>> shape = (3, 1, 2)
        >>> alpha = Tensor(np.array([[3, 4], [5, 6]]), luojianet_ms.float32)
        >>> beta = Tensor(np.array([[1.0], [2.0]]), luojianet_ms.float32)
        >>> output = ops.gamma(shape, alpha, beta, seed=5)
        >>> result = output.shape
        >>> print(output)
        [[[ 5.6085486  7.8280783]]
...
def poisson(shape, mean, seed=None):
    ...
    Examples:
        ...
        >>> shape = (4, 1)
        >>> mean = Tensor(np.array([5.0, 10.0]), luojianet_ms.float32)
        ...
        >>> shape = (2, 2)
        >>> mean = Tensor(np.array([[5.0, 10.0], [5.0, 1.0]]), luojianet_ms.float32)
        >>> output = ops.poisson(shape, mean, seed=5)
        >>> result = output.shape
        >>> print(result)
        (2, 2)
...
def multinomial(inputs, num_sample, replacement=True, seed=None):
    ...
    Examples:
        ...
        >>> x = Tensor([0, 9, 4, 0], luojianet_ms.float32)
