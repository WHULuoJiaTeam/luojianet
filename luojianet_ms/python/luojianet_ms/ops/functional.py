...
from luojianet_ms.common._register_for_tensor import tensor_operator_registry
...
from luojianet_ms.nn.grad.cell_grad import _JvpInner
from luojianet_ms.nn.grad.cell_grad import _VjpInner
...
def arange(start=0, stop=None, step=1, rtype=None):
    ...
    Args:
        ...
        rtype (Union[:class:`luojianet_ms.dtype`, str], optional): Designated tensor type.
...
def _raise_arange_type_error(arg, arg_value):
    ...
    """
    raise TypeError("For luojianet_ms.ops.arange, the argument '{}' must be int or float, but got {}."
...
def _raise_arange_value_error(start, stop):
    ...
    Raise TypeError in both graph/pynative mode
    """
    raise ValueError("For luojianet_ms.ops.arange, the argument 'start' must be < 'stop', but got 'start': {}, "
...
def select(cond, x, y):
    ...
    Examples:
        ...
        >>> x = Tensor([2,3], luojianet_ms.float32)
        >>> y = Tensor([1,2], luojianet_ms.float32)
