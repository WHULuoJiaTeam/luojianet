...
if context.get_context('device_target') == "Ascend":
    import luojianet_ms.ops._op_impl._custom_op
...
class MinMaxUpdatePerLayer(PrimitiveWithInfer):
    ...
    def __init__(self, ema=False, ema_decay=0.999):
        ...
        if context.get_context('device_target') == "Ascend":
            from luojianet_ms.ops._op_impl._custom_op import minmax_update_perlayer
...
class MinMaxUpdatePerChannel(PrimitiveWithInfer):
    ...
    def __init__(self, ema=False, ema_decay=0.999, channel_axis=1):
        ...
        if self.is_ascend:
            from luojianet_ms.ops._op_impl._custom_op import minmax_update_perchannel
        if ema and not ema_decay:
            raise ValueError(
                f"For '{self.name}' attr \'ema\' and \'ema_decay\' should set together.")
...
class FakeLearnedScaleQuantPerLayer(PrimitiveWithInfer):
    ...
    def __init__(self,
        ...
        if context.get_context('device_target') == "Ascend":
            from luojianet_ms.ops._op_impl._custom_op import fake_learned_scale_quant_perlayer
...
class FakeLearnedScaleQuantPerLayerGrad(PrimitiveWithInfer):
    ...
    Examples:
        ...
        >>> dout = Tensor(np.array([[-2.3, 1.2], [5.7, 0.2]]), luojianet_ms.float32)
        >>> input_x = Tensor(np.array([[18, -23], [0.2, 6]]), luojianet_ms.float32)
        >>> _alpha = Tensor(np.array([6]), luojianet_ms.float32)
        >>> _quant_max = Tensor(np.array([127]), luojianet_ms.float32)
...
class FakeLearnedScaleQuantPerLayerGradD(PrimitiveWithInfer):
    ...
    def __init__(self,
                 neg_trunc=False):
        from luojianet_ms.ops._op_impl._custom_op import fake_learned_scale_quant_perlayer_grad
        self.neg_trunc = validator.check_value_type(
            'neg_trunc', neg_trunc, (bool,), self.name)
        self.init_prim_io_names(
            inputs=['dout', 'x', 'alpha', 'quant_max'], outputs=['dx', 'dalpha'])
...
class FakeLearnedScaleQuantPerLayerGradDReduce(PrimitiveWithInfer):
    ...
    @prim_attr_register
    def __init__(self):
        from luojianet_ms.ops._op_impl._custom_op import fake_learned_scale_quant_perlayer_grad_reduce
        self.init_prim_io_names(
...
class FakeLearnedScaleQuantPerChannel(PrimitiveWithInfer):
    ...
    def __init__(self,
        ...
        if context.get_context('device_target') == "Ascend":
            from luojianet_ms.ops._op_impl._custom_op import fake_learned_scale_quant_perchannel
        self.is_ascend = context.get_context('device_target') == "Ascend"
        self.quant_delay = validator.check_non_negative_int(
            quant_delay, 'quant_delay', self.name)
        self.neg_trunc = validator.check_value_type(
...
class FakeLearnedScaleQuantPerChannelGrad(PrimitiveWithInfer):
    ...
    Examples:
        ...
        >>> dout = Tensor(np.array([[-2.3, 1.2], [5.7, 0.2]]), luojianet_ms.float32)
        >>> input_x = Tensor(np.array([[18, -23], [0.2, 6]]), luojianet_ms.float32)
        >>> _alpha = Tensor(np.array([6]*2), luojianet_ms.float32)
        >>> _quant_max = Tensor(np.array([127]), luojianet_ms.float32)
        >>> result = fake_learned_scale_grad(dout, input_x, _min, _max)
    """
...
class FakeLearnedScaleQuantPerChannelGradD(PrimitiveWithInfer):
    ...
    def __init__(self,
                 neg_trunc=False,
                 channel_axis=1):
        from luojianet_ms.ops._op_impl._custom_op import fake_learned_scale_quant_perchannel_grad
        self.neg_trunc = validator.check_value_type(
            'neg_trunc', neg_trunc, (bool,), self.name)
        self.channel_axis = validator.check_non_negative_int(channel_axis, 'channel axis', self.name)
        self.init_prim_io_names(
...
class FakeLearnedScaleQuantPerChannelGradDReduce(PrimitiveWithInfer):
    ...
    def __init__(self, channel_axis=1):
        from luojianet_ms.ops._op_impl._custom_op import fake_learned_scale_quant_perchannel_grad_reduce
        self.channel_axis = validator.check_non_negative_int(channel_axis, 'channel axis', self.name)
        self.init_prim_io_names(
            inputs=['dout_alpha'], outputs=['dalpha'])
...
class FakeQuantPerLayer(PrimitiveWithInfer):
    ...
    def __init__(self,
        ...
        if context.get_context('device_target') == "Ascend":
            from luojianet_ms.ops._op_impl._custom_op import fake_quant_perlayer
...
class FakeQuantPerLayerGrad(PrimitiveWithInfer):
    ...
    Examples:
        ...
        >>> dout = Tensor(np.array([[-2.3, 1.2], [5.7, 0.2]]), luojianet_ms.float32)
        >>> input_x = Tensor(np.array([[18, -23], [0.2, 6]]), luojianet_ms.float32)
        >>> _min = Tensor(np.array([-4]), luojianet_ms.float32)
        >>> _max = Tensor(np.array([2]), luojianet_ms.float32)
    ...
    def __init__(self,
                 ...
                 narrow_range=False):
        if context.get_context('device_target') == "Ascend":
            from luojianet_ms.ops._op_impl._custom_op import fake_quant_perlayer_grad
        if num_bits not in self.support_quant_bit:
            raise ValueError(
                f"For '{self.name}' attr \'num_bits\' is not support.")
...
class FakeQuantPerChannel(PrimitiveWithInfer):
    ...
    Examples:
        ...
        >>> input_x = Tensor(np.array([3, 4, 5, -2, -3, -1]).reshape(3, 2), luojianet_ms.float32)
        >>> _min = Tensor(np.linspace(-2, 2, 12).reshape(3, 2, 2), luojianet_ms.float32)
        >>> _max = Tensor(np.linspace(8, 12, 12).reshape(3, 2, 2), luojianet_ms.float32)
    ...
    def __init__(self,
        ...
        self.is_ascend = context.get_context('device_target') == "Ascend"
        if self.is_ascend:
            from luojianet_ms.ops._op_impl._custom_op import fake_quant_perchannel
        if num_bits not in self.support_quant_bit:
            raise ValueError(
                f"For '{self.name}' Attr \'num_bits\' is not support.")
        if ema and not ema_decay:
...
class FakeQuantPerChannelGrad(PrimitiveWithInfer):
    ...
    Examples:
        ...
        >>> input_x = Tensor(np.random.randint(-4, 4, (2, 3, 4)), luojianet_ms.float32)
        >>> dout = Tensor(np.random.randint(-2, 2, (2, 3, 4)), luojianet_ms.float32)
        >>> _min = Tensor(np.random.randint(-8, 2, (2, 3, 4)), luojianet_ms.float32)
        >>> _max = Tensor(np.random.randint(-2, 8, (2, 3, 4)), luojianet_ms.float32)
    ...
    def __init__(self,
        ...
        if context.get_context('device_target') == "Ascend":
            from luojianet_ms.ops._op_impl._custom_op import fake_quant_perchannel_grad
        if num_bits not in self.support_quant_bit:
            raise ValueError(
                f"For '{self.name}' attr \'num_bits\' is not support.")
...
class BatchNormFold(PrimitiveWithInfer):
    ...
    Examples:
        ...
        >>> input_x = Tensor(np.array([1, 2, -1, -2, -2, 1]).reshape(2, 3), luojianet_ms.float32)
        >>> mean = Tensor(np.array([0.5, -1, 1,]), luojianet_ms.float32)
        >>> variance = Tensor(np.array([0.36, 0.4, 0.49]), luojianet_ms.float32)
        >>> global_step = Tensor(np.arange(6), luojianet_ms.int32)
...
class BatchNormFoldGrad(PrimitiveWithInfer):
    ...
    Examples:
        ...
        >>> d_batch_mean = Tensor(np.random.randint(-2., 2., (1, 2, 2, 3)), luojianet_ms.float32)
        >>> d_batch_std = Tensor(np.random.randn(1, 2, 2, 3), luojianet_ms.float32)
        >>> input_x = Tensor(np.random.randint(0, 256, (4, 1, 4, 6)), luojianet_ms.float32)
        >>> batch_mean = Tensor(np.random.randint(-8., 8., (1, 2, 2, 3)), luojianet_ms.float32)
        >>> batch_std = Tensor(np.random.randint(0, 12, (1, 2, 2, 3)), luojianet_ms.float32)
        >>> global_step = Tensor([2], luojianet_ms.int32)
...
class CorrectionMul(PrimitiveWithInfer):
    ...
    Examples:
        ...
        >>> input_x = Tensor(np.random.randint(-8, 12, (3, 4)), luojianet_ms.float32)
        >>> batch_std = Tensor(np.array([1.5, 3, 2]), luojianet_ms.float32)
        >>> running_std = Tensor(np.array([2, 1.2, 0.5]), luojianet_ms.float32)
    ...
    def __init__(self, channel_axis=0):
        ...
        if context.get_context('device_target') == "Ascend":
            from luojianet_ms.ops._op_impl._custom_op import correction_mul
...
class CorrectionMulGrad(PrimitiveWithInfer):
    ...
    Examples:
        ...
        >>> dout = Tensor(np.array([1.5, -2.2, 0.7, -3, 1.6, 2.8]).reshape(2, 1, 1, 3), luojianet_ms.float32)
        >>> input_x = Tensor(np.random.randint(0, 256, (2, 1, 1, 3)), luojianet_ms.float32)
        >>> gamma = Tensor(np.array([0.2, -0.2, 2.5, -1.]).reshape(2, 1, 2), luojianet_ms.float32)
        >>> running_std = Tensor(np.array([1.2, 0.1, 0.7, 2.3]).reshape(2, 1, 2), luojianet_ms.float32)
    ...
    def __init__(self, channel_axis=0):
        """Initialize correction mul layer"""
        if context.get_context('device_target') == "Ascend":
            from luojianet_ms.ops._op_impl._custom_op import correction_mul_grad
        self.channel_axis = channel_axis
...
class BatchNormFold2(PrimitiveWithInfer):
    ...
    Examples:
        ...
        >>> input_x = Tensor(np.random.randint(-6, 6, (4, 3)), luojianet_ms.float32)
        >>> beta = Tensor(np.array([0.2, -0.1, 0.25]), luojianet_ms.float32)
        >>> gamma = Tensor(np.array([-0.1, -0.25, 0.1]), luojianet_ms.float32)
        >>> batch_std = Tensor(np.array([0.1, 0.2, 0.1]), luojianet_ms.float32)
        >>> batch_mean = Tensor(np.array([0, 0.05, 0.2]), luojianet_ms.float32)
        >>> running_std = Tensor(np.array([0.1, 0.1, 0.3]), luojianet_ms.float32)
        >>> running_mean = Tensor(np.array([-0.1, 0, -0.1]), luojianet_ms.float32)
        >>> global_step = Tensor(np.random.randint(1, 8, (8, )), luojianet_ms.int32)
...
class BatchNormFold2Grad(PrimitiveWithInfer):
    ...
    Examples:
        ...
        >>> input_x = Tensor(np.arange(3*3*12*12).reshape(6, 3, 6, 12), luojianet_ms.float32)
        >>> dout = Tensor(np.random.randint(-32, 32, (6, 3, 6, 12)), luojianet_ms.float32)
        >>> gamma = Tensor(np.random.randint(-4, 4, (3, 1, 1, 2)), luojianet_ms.float32)
        >>> batch_std = Tensor(np.random.randint(0, 8, (3, 1, 1, 2)), luojianet_ms.float32)
        >>> batch_mean = Tensor(np.random.randint(-6, 6, (3, 1, 1, 2)), luojianet_ms.float32)
        >>> running_std = Tensor(np.linspace(0, 2, 6).reshape(3, 1, 1, 2), luojianet_ms.float32)
        >>> running_mean = Tensor(np.random.randint(-3, 3, (3, 1, 1, 2)), luojianet_ms.float32)
        >>> global_step = Tensor(np.array([-2]), luojianet_ms.int32)
...
class BatchNormFoldD(PrimitiveWithInfer):
    ...
    def __init__(self, momentum=0.9, epsilon=1e-5, is_training=True, freeze_bn=0):
        ...
        from luojianet_ms.ops._op_impl._custom_op import batchnorm_fold
        self.momentum = validator.check_float_range(momentum, 0, 1, Rel.INC_BOTH, 'momentum', self.name)
        self.epsilon = validator.check_positive_float(epsilon, 'epsilon', self.name)
        self.is_training = validator.check_value_type('is_training', is_training, (bool,), self.name)
        self.freeze_bn = validator.check_value_type('freeze_bn', freeze_bn, (int,), self.name)
...
class BatchNormFoldGradD(PrimitiveWithInfer):
    ...
    def __init__(self, epsilon=1e-5, is_training=True, freeze_bn=0):
        ...
        from luojianet_ms.ops._op_impl._custom_op import batchnorm_fold_grad
        self.epsilon = validator.check_positive_float(epsilon, 'epsilon', self.name)
        self.is_training = validator.check_value_type('is_training', is_training, (bool,), self.name)
        self.freeze_bn = validator.check_value_type('freeze_bn', freeze_bn, (int,), self.name)
...
class BatchNormFold2D(PrimitiveWithInfer):
    ...
    def __init__(self, freeze_bn=0):
        """Initialize conv2d fold layer"""
        from luojianet_ms.ops._op_impl._custom_op import batchnorm_fold2
...
class BatchNormFold2GradD(PrimitiveWithInfer):
    ...
    def __init__(self, freeze_bn=False):
        """Initialize MulFold layer"""
        from luojianet_ms.ops._op_impl._custom_op import batchnorm_fold2_grad
        self.freeze_bn = freeze_bn
        self.init_prim_io_names(
...
class BatchNormFold2GradReduce(PrimitiveWithInfer):
    ...
    def __init__(self, freeze_bn=False):
        """Initialize MulFold layer"""
        from luojianet_ms.ops._op_impl._custom_op import batchnorm_fold2_grad_reduce
        self.freeze_bn = freeze_bn
...
class ActULQClampMinGrad(PrimitiveWithInfer):
    ...
    Examples:
        ...
        >>> clamp_min_grad = act_ulq_clamp_min_grad(Tensor(y_grad), Tensor(clamp_min_mask, luojianet_ms.bool_),
...
class ActULQClampMaxGrad(PrimitiveWithInfer):
    ...
    Examples:
        ...
        >>> clamp_max_grad = act_ulq_clamp_max_grad(Tensor(y_grad), Tensor(clamp_max_mask, luojianet_ms.bool_),
                                                    Tensor(x_clamped_loss))
    """
    @prim_attr_register
    def __init__(self):
...
class IFMR(PrimitiveWithInfer):
    ...
    Examples:
        >>> data = Tensor(np.random.rand(1, 3, 6, 4).astype(np.float32))
        >>> data_min = Tensor([0.1], luojianet_ms.float32)
        >>> data_max = Tensor([0.5], luojianet_ms.float32)
