...
import functools
from luojianet_ms.common.dtype import type_size_in_bytes
...
class EnvReset(PrimitiveWithInfer):
    ...
    Args:
        ...
        state_dtype (list[:class:`luojianet_ms.dtype`]): The type of the state.
        reward_shape (list[tuple[int]]): The dimensionality of the reward.
        reward_dtype (list[:class:`luojianet_ms.dtype`]): The type of the reward.echo
...
class EnvStep(PrimitiveWithInfer):
    ...
    Args:
        ...
        state_dtype (list[:class:`luojianet_ms.dtype`]): The type of the state.
        reward_shape (list[tuple[int]]): The dimensionality of the reward.
        reward_dtype (list[:class:`luojianet_ms.dtype`]): The type of the reward.
...
class DiscountedReturn(PrimitiveWithInfer):
    ...
    Examples:
        ...
        >>> reward = Tensor([[1, 1, 1, 1]], dtype=luojianet_ms.float32)
        >>> done = Tensor([[False, False, True, False]])
        >>> last_state_value = Tensor([2.], dtype=luojianet_ms.float32)
...
class CudnnGRU(PrimitiveWithInfer):
    ...
    For detailed information, please refer to :class:`luojianet_ms.nn.GRU`.
    ...
    Examples:
        ...
        >>> batch_size = 2
        >>>
        >>> import luojianet_ms.ops.operations._rl_inner_ops as rl_ops
...
class PriorityReplayBufferCreate(PrimitiveWithInfer):
    ...
    Args:
        ...
        dtypes (list[:class:`luojianet_ms.dtype`]): The type of the transition.
