...
class TensorArray(PrimitiveWithInfer):
    ...
    Args:
        dtype (luojianet_ms.dtype): the data type in the TensorArray.
    ...
    Outputs:
        - **output** (Tensor[luojianet_ms.int64]) - an unique handle binded to the TensorArray.
    ...
    Examples:
        ...
        >>> create_op = ops.TensorArray(luojianet_ms.int32, ())
        >>> handle = create_op()
        >>> print(handle)
        0
    """
...
class TensorArrayRead(PrimitiveWithInfer):
    ...
    Inputs:
        ...
        - **handle** (luojianet_ms.int64) - The handle pointed to the TensorArray.
    ...
    Examples:
        ...
        >>> write_op = ops.TensorArrayWrite()
        >>> write_op.write(handle, 0, 1)
        >>> read_op = ops.TensorArrayRead(luojianet_ms.int32, ())
...
class TensorArrayStack(Primitive):
    ...
    Examples:
        ...
        >>> stack_op = ops.TensorArrayStack(luojianet_ms.int32, ())
        >>> ans = stack_op(handle)
        >>> print(ans)
        [1 2]
    """
...
class TensorArraySize(PrimitiveWithInfer):
    ...
    Outputs:
        - **output** (Tensor[luojianet_ms.int64]) - the logical size of the TensorArray.
...
class TensorArrayGather(PrimitiveWithInfer):
    ...
    Inputs:
        - **handle** (luojianet_ms.int64) - The handle pointed to the TensorArray.
        - **indices** (luojianet_ms.int32) - The locations of the gathered elements.
    ...
    Examples:
        >>> import luojianet_ms
        >>> import luojianet_ms.ops as ops
        >>> from luojianet_ms import numpy as mnp
        >>> create_op = ops.TensorArray(luojianet_ms.float32, dynamic_size=False, element_shape=(8,))
        >>> handle = create_op()
        >>> indices = mnp.range(0, 25, 1, luojianet_ms.int32)
        >>> gather_op = ops.TensorArrayGather(dtype=luojianet_ms.float32, element_shape=(8,))
