...
class GeSwitch(PrimitiveWithInfer):
    ...
    Examples:
        ...
        ...         self.add = ops.Add()
        ...         self.value = Tensor(np.full((1), 3), luojianet_ms.float32)
        ...
        ...         return ret[0]
        ...
        >>> x = Tensor(10.0, dtype=luojianet_ms.float32)
        >>> y = Tensor(5.0, dtype=luojianet_ms.float32)
        >>> net = Net()
        >>> output = net(x, y)
        >>> print(output)
    """
...
class Merge(PrimitiveWithInfer):
    ...
    Examples:
        >>> merge = ops.Merge()
        >>> input_x = Tensor(np.linspace(0, 8, 8).reshape(2, 4), luojianet_ms.float32)
        >>> input_y = Tensor(np.random.randint(-4, 4, (2, 4)), luojianet_ms.float32)
