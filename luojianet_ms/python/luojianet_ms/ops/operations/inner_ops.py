...
class ScalarCast(PrimitiveWithInfer):
    ...
    Inputs:
        ...
        - **input_y** (luojianet_ms.dtype) - The type to be cast. Only constant value is allowed.
    ...
    Examples:
        ...
        >>> output = scalar_cast(255.0, luojianet_ms.int32)
        >>> print(output)
        255
    """
...
class Randperm(PrimitiveWithInfer):
    ...
    Args:
        ...
        dtype (luojianet_ms.dtype): The type of output. Default: luojianet_ms.int32.
    ...
    Examples:
        ...
        >>> n = Tensor([20], dtype=luojianet_ms.int32)
        >>> output = randperm(n)
        >>> print(output)
...
class NoRepeatNGram(PrimitiveWithInfer):
    ...
    Examples:
        ...
        ...                      [0.5, 0.8, 0.8, 0.7, 0.7, 0.8, 0.2, 0.7, 0.9, 0.7]]], dtype=luojianet_ms.float32)
...
class MakeRefKey(Primitive):
    ...
    Examples:
        ...
        ...         return ref * x
        ...
        >>> x = Tensor(np.array([[1, 2, 3], [4, 5, 6]]), luojianet_ms.int32)
        >>> net = Net()
        >>> output = net(x)
        >>> print(output)
        [[ 1  4  9]
...
class FusedWeightScaleApplyMomentum(PrimitiveWithInfer):
    ...
    Supported Platforms:
        ``GPU``
    Examples:
        Please refer to the usage in :class:`luojianet_ms.nn.Momentum`, and add weight_decay and loss_scale as inputs.
    """
