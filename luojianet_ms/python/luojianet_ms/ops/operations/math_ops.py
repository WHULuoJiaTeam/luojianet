...
class _BinaryOp(PrimitiveWithInfer):
    ...
    __luojianet_signature__ = (sig.sig_dtype.T, sig.sig_dtype.T)
...
class Div(_MathBinaryOp):
    ...
    Examples:
        ...
        >>> x = Tensor(np.array([-4.0, 5.0, 6.0]), luojianet_ms.int32)
        >>> y = Tensor(2, luojianet_ms.float32)
        >>> output = div(x, y)
        >>> print(output)
        [-2.  2.5  3.]
        >>> print(output.dtype)
...
class MulNoNan(_MathBinaryOp):
    ...
    Examples:
        ...
        >>> x = Tensor(np.array([[-1.0, 6.0, 0], [0, np.nan, 4.0]]), luojianet_ms.int32)
...
class NPUAllocFloatStatus(PrimitiveWithInfer):
    ...
    Note:
        Please refer to the Examples of class: `luojianet_ms.ops.NPUAllocFloatStatus`.
