...
def make_sig(name="var", rw=sig_rw.RW_READ,
    ...
    See `ApplyMomentum` in `luojianet_ms.ops.operation.nn_ops` as a example.

    Args:
         name (bool): Argument name. Default: "var".
         rw (:class:`luojianet_ms.ops.signature.sig_rw`): Tag the argument attribute for write and read. Choose in
         ...
         kind (:class:`luojianet_ms.ops.signature.kind`): Choose in `[signature_kind.KIND_POSITIONAL_KEYWORD,
         ...
         default (Any): The default value of argument or `sig_kind.KIND_EMPTY_DEFAULT_VALUE` for no default value.
            Default: sig_kind.KIND_EMPTY_DEFAULT_VALUE.
         dtype (:class:`luojianet_ms.ops.signature.sig_dtype`): Choose in `signature_dtype.T` or
    ...
    Returns:
        :class:`luojianet_ms.ops.signature.Signature`, signature for one argument.
    """
