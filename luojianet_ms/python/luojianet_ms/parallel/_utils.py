...
from luojianet_ms._c_expression import reset_op_id
...
from luojianet_ms.common.seed import get_seed
...
def _to_full_shapes(shapes, device_num):
    """Expanding batch dimension according to device_num, adapt to luojianet_ms minddata graph solution."""
    new_shapes = []
    dataset_strategy = ()
...
def _get_parameter_broadcast():
    ...
    if parallel_mode in ("data_parallel", "hybrid_parallel") and parameter_broadcast is False and get_seed() is None:
        logger.warning("You are suggested to use luojianet_ms.context.set_auto_parallel_context(parameter_broadcast=True)"
                       " or luojianet_ms.common.set_seed() to share parameters among multi-devices.")
...
def _get_python_op(op_name, op_path, instance_name, arglist):
    ...
    if op_path != "luojianet_ms.ops.functional":
        op = cls(*arglist)
    else:
        op = cls
