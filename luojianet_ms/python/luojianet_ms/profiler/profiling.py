...
from luojianet_ms import log as logger, context
from luojianet_ms.communication.management import GlobalComm, get_rank, get_group_size
import luojianet_ms._c_expression as c_expression
import luojianet_ms._c_dataengine as cde
from luojianet_ms.train.profiling_parallel_pb2 import ProfilingParallel
from luojianet_ms.profiler.common.exceptions.exceptions import ProfilerFileNotFoundException, \
    ProfilerIOException, ProfilerException, ProfilerRawFileException
from luojianet_ms.profiler.common.exceptions.exceptions import ProfilerPathErrorException
from luojianet_ms.profiler.common.exceptions.exceptions import ProfilerDirNotFoundException
from luojianet_ms.profiler.common.util import get_file_path, fwrite_format
from luojianet_ms.profiler.common.validator.validate_path import \
    validate_and_normalize_path
from luojianet_ms.profiler.parser.aicpu_data_parser import DataPreProcessParser
from luojianet_ms.profiler.parser.framework_parser import FrameworkParser
from luojianet_ms.profiler.parser.hwts_log_parser import HWTSLogParser
from luojianet_ms.profiler.parser.integrator import Integrator, DeviceTarget
from luojianet_ms.profiler.parser.integrator import GpuTimelineGenerator, CpuTimelineGenerator, AscendTimelineGenerator
from luojianet_ms.profiler.parser.memory_usage_parser import MemoryUsageParser
from luojianet_ms.profiler.parser.minddata_parser import MinddataParser
from luojianet_ms.profiler.parser.minddata_analyzer import MinddataProfilingAnalyzer
from luojianet_ms.profiler.parser.flops_parser import FlopsParser
from luojianet_ms.profiler.parser.minddata_pipeline_parser import \
    MinddataPipelineParser
from luojianet_ms.profiler.parser.optime_parser import OPComputeTimeParser
from luojianet_ms.profiler.parser.step_trace_parser import GpuStepTraceParser, AscendStepTraceParser
from luojianet_ms.profiler.parser.hccl_parser import HcclParser
from luojianet_ms.profiler.parser.op_intermediate_parser import OPIntermediateParser
...
def _environment_check():
    if c_expression.security.enable_security():
        raise RuntimeError("Profiler is not supported when LuoJiaNET is compiled with \'-s on\'.")


class Profiler:
    """
    LuoJiaNET users can use this class to collect the performance of neural networks.
    ...
    Raises:
        RuntimeError: When the version of CANN does not match the version of LuoJiaNET,
            LuoJiaNET cannot parse the generated ascend_job_id directory structure.
    ...
    def _decide_device_target(self, kwargs):
        ...
        if self._device_target and self._device_target == DeviceTarget.GPU.value:
            ...
            # use context interface to open profiling, for the new luojianet_ms version(after 2020.5.21)
    ...
    def start(self):
        ...
        if not self._has_started:
            if not self._has_started_twice:
                ...
                raise RuntimeError("LuoJiaNET Profiling has finished, repeated start and stop actions are not "
                                   "supported.")
