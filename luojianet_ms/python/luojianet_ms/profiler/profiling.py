...
from luojianet_ms.profiler.parser.integrator import Integrator
from luojianet_ms.profiler.parser.integrator import GpuTimelineGenerator, AscendTimelineGenerator
...
def deprecated(name, version):
    msg = f"The {name} is deprecated from LuoJiaNet {version} and will be removed in a future version."
    logger.warning(msg)
...
class Profiler:
    ...
    This API enables LuoJiaNet users to profile the performance of neural network.
    ...
    Raises:
        RuntimeError: If ascend_job_id is transferred with data that does not match the current LuoJiaNet version.
    ...
    def profile(network, profile_option):
        ...
        if profile_option == ProfileOption.trainable_parameters:
            if not isinstance(network, Module):
                msg = "Profiling: The network should be an object of nn.Module"
                raise ValueError(msg)
