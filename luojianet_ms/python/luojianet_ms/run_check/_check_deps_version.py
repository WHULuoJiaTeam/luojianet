...
def parse_args():
    ...
    parser.add_argument("--luojianet_ms_version", type=str, help="LuoJiaNET version.")
    parser.add_argument("--supported_version", type=str, action='append', help="Supported environment version.")
    args = parser.parse_args()
    return args


def check_deps_version(luojianet_ms_version, supported_version):
    """
       check te/hccl/topi version

    Args:
        luojianet_ms_version (str): this luojianet_ms package version
    ...
    try:
        ...
        if v not in supported_version:
            print(f"LuoJiaNET version {luojianet_ms_version} and \"hccl\" wheel package version {v} does not "
                  "match, reference to the match info on: https://www.luojianet_ms.cn/install")
        import te
        v = '.'.join(te.__version__.split('.')[0:2])
        if v not in supported_version:
            print(f"LuoJiaNET version {luojianet_ms_version} and \"te\" wheel package version {v} does not "
                  "match, reference to the match info on: https://www.luojianet_ms.cn/install")
        import topi
        v = '.'.join(topi.__version__.split('.')[0:2])
        if v not in supported_version:
            print(f"LuoJiaNET version {luojianet_ms_version} and \"topi\" wheel package version {v} does not "
                  "match, reference to the match info on: https://www.luojianet_ms.cn/install")
...
def main():
    args = parse_args()
    check_deps_version(args.luojianet_ms_version, args.supported_version)
