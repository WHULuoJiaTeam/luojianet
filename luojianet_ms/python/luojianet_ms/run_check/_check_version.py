...
class GPUEnvChecker(EnvChecker):
    ...
    def check_version(self):
        ...
        if not version_match:
            if self.v == "0":
                logger.warning("Can not found cuda libs, please confirm that the correct "
                               ...
                               "installation guidelines: https://www.luojianet_ms.cn/install")
            else:
                logger.warning(f"LuoJiaNet version {__version__} and cuda version {self.v} does not match, "
                               "please refer to the installation guide for version matching "
                               "information: https://www.luojianet_ms.cn/install")
        nvcc_version = self._get_nvcc_version(False)
        if nvcc_version and (nvcc_version not in self.version):
            logger.warning(f"LuoJiaNet version {__version__} and nvcc(cuda bin) version {nvcc_version} "
        ...
        if cudnn_version and int(cudnn_version) < 760:
            logger.warning(f"LuoJiaNet version {__version__} and cudDNN version {cudnn_version} "
                           "does not match, please refer to the installation guide for version matching "
                           "information: https://www.luojianet_ms.cn/install. The recommended version is "
    ...
    def _get_lib_path(self, lib_name):
        ...
        luojianet_path = os.path.join(current_path, "../")
        try:
            ldd_result = subprocess.run(["ldd " + luojianet_path + "/_c_expression*.so* | grep " + lib_name],
                                        timeout=10, text=True, capture_output=True, check=False, shell=True)
            if ldd_result.returncode:
                logger.error(f"{self.lib_key_to_lib_name[lib_name]} (need by luojianet_ms-gpu) is not found, please "
                             f"confirm that _c_expression.so is in directory:{luojianet_path} and the correct cuda "
                             "version has been installed, you can refer to the installation "
                             "guidelines: https://www.luojianet_ms.cn/install")
                return path_list
            result = ldd_result.stdout
            for i in result.split('\n'):
                path = i.partition("=>")[2]
                if path.lower().find("not found") > 0:
                    logger.warning(f"Cuda {self.version} version(need by luojianet_ms-gpu) is not found, please confirm "
...
class AscendEnvChecker(EnvChecker):
    ...
    def check_version(self):
        if not Path(self.fwk_version).is_file():
            logger.warning("Using custom Ascend AI software package (Ascend Data Center Solution) path, package "
                           ...
                           "https://www.luojianet_ms.cn/install")
            return

        v = self._read_version(self.fwk_version)
        if v not in self.version:
            v_list = str([x for x in self.version])
            logger.warning(f"LuoJiaNet version {__version__} and Ascend AI software package (Ascend Data Center "
                           f"Solution)version {v} does not match, the version of software package expect one of "
                           f"{v_list}, please reference to the match info on: https://www.luojianet_ms.cn/install")
    ...
    def check_deps_version(self):
        ...
        """
        input_args = ["--luojianet_version=" + __version__]
    ...
    def _check_env(self):
        ...
        if self.path is None or self.path_check not in self.path:
            logger.warning("Can not find ccec_compiler(need by luojianet_ms-ascend), please check if you have set env "
                           "PATH, you can reference to the installation guidelines https://www.luojianet_ms.cn/install")

        if self.python_path is None or self.python_path_check not in self.python_path:
            logger.warning(
                "Can not find tbe op implement(need by luojianet_ms-ascend), please check if you have set env "
        ...
        if self.ld_lib_path is None or not (self.ld_lib_path_check_fwk in self.ld_lib_path and
            ...
            logger.warning("Can not find driver so(need by luojianet_ms-ascend), please check if you have set env "
        ...
        if self.ascend_opp_path is None or self.ascend_opp_path_check not in self.ascend_opp_path:
            logger.warning(
                "Can not find opp path (need by luojianet_ms-ascend), please check if you have set env ASCEND_OPP_PATH, "
                "you can reference to the installation guidelines https://www.luojianet_ms.cn/install")
...
def check_version_and_env_config():
    ...
    if __package_name__.lower() == "luojianet_ms-ascend":
        ...
        try:
            ...
            logger.warning(
                ...
                "if so find solution in FAQ in https://www.luojianet_ms.cn/docs/faq/en/master/index.html.")
    elif __package_name__.lower() == "luojianet_ms-gpu":
