...
class GPUEnvChecker(EnvChecker):
    ...
    def _get_lib_path(self, lib_name):
        ...
        luojianet_ms_path = os.path.join(current_path, "../")
        try:
            real_path = glob.glob(luojianet_ms_path + "/_c_expression*.so*")
            if real_path == []:
                logger.error(f"{self.lib_key_to_lib_name[lib_name]} (need by luojianet_ms-gpu) is not found, please "
                             f"confirm that _c_expression.so is in directory:{luojianet_ms_path} and the correct cuda "
...
class AscendEnvChecker(EnvChecker):
    ...
    def check_deps_version(self):
        ...
        """
        input_args = ["--luojianet_ms_version=" + __version__]
    ...
    if __package_name__.lower() == "luojianet_ms-ascend":
        ...
        try:
            ...
            logger.warning(
                ...
                "if so find solution in FAQ in https://www.luojianet_ms.cn/docs/faq/en/r1.7/index.html.")
