...
def _check_mul():
    ...
    try:
        ms = import_module("luojianet_ms")
    except ModuleNotFoundError:
        ms = None
    finally:
        pass

    print(f"LuoJiaNet version: ", ms.__version__)

    input_x = ms.Tensor(np.array([1.0, 2.0, 3.0]), ms.float32)
    input_y = ms.Tensor(np.array([4.0, 5.0, 6.0]), ms.float32)
    mul = ms.ops.Mul()
    mul(input_x, input_y)
    print(f"The result of multiplication calculation is correct, LuoJiaNet has been installed successfully!")
...
def run_check():
    ...
    # pylint: disable=broad-except
        ...
        print("LuoJiaNet running check failed.")
        print(str(e))
    finally:
        pass
