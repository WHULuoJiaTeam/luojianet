...
class BatchedGmres(nn.Module):
    ...
    def call(self, b, x0=None, tol=1e-5, atol=0.0, restart=20, maxiter=None):
...
class IterativeGmres(nn.Module):
    """
    ...
    def call(self, b, x0, tol, atol, restart, maxiter):
        A = _normalize_matvec(self.A)
        M = _normalize_matvec(self.M)
...
def gmres(A, b, x0=None, *, tol=1e-5, atol=0.0, restart=20, maxiter=None,
    ...
    Note:
        In the future, LuoJiaNet will report the number of iterations when convergence
    ...
    Args:
        ...
        tol (float, optional): Tolerances for convergence,
            ...
            "legacy" behavior, so LuoJiaNet's tolerance will differ from SciPy unless you
    ...
    Examples:
        ...
        >>> from luojianet_ms.scipy.sparse.linalg import gmres
...
class CG(nn.Module):
    ...
    def call(self, b, x0, tol, atol, maxiter):
        A = _normalize_matvec(self.A)
        M = _normalize_matvec(self.M)
...
def cg(A, b, x0=None, *, tol=1e-5, atol=0.0, maxiter=None, M=None):
    ...
    The numerics of LuoJiaNet's `cg` should exact match SciPy's `cg` (up to
    numerical precision).
    ...
    Args:
        ...
        tol (float, optional): Tolerances for convergence, :math:`norm(residual) <= max(tol*norm(b), atol)`.
            We do not implement SciPy's "legacy" behavior, so LuoJiaNet's tolerance will
    ...
    Examples:
        >>> import numpy as onp
        >>> from luojianet_ms.common import Tensor
        >>> from luojianet_ms.scipy.sparse.linalg import cg
