...
def is_within_graph(x):
    """
    Returns true if x is None. It's aim to check whether the call is within LuoJiaNET graph.
    Because in graph mode, x should be None in constexpr when x is a variable of LuoJiaNET.
...
def mstype_to_pytype(type_):
    """
    Convert LuoJiaNET type to Python type.

    Args:
        type_: A LuoJiaNET type object.
