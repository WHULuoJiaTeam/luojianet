...
from luojianet_ms.train.summary.summary_record import SummaryRecord, process_export_options
from luojianet_ms.train.summary.enums import PluginEnum, ModeEnum
from luojianet_ms.train.callback import Callback, ModelCheckpoint
from luojianet_ms.train import lineage_pb2
from luojianet_ms.train.serialization import save_checkpoint
from luojianet_ms.train.callback._dataset_graph import DatasetGraph
from luojianet_ms.nn.optim.optimizer import Optimizer
from luojianet_ms.nn.loss.loss import LossBase
from luojianet_ms.train._utils import check_value_type, _make_directory
...
class SummaryCollector(Callback):
    ...
    Args:
        ...
        collect_specified_data (Union[None, dict]): Perform custom operations on the collected data.
            ...
            - collect_train_lineage (bool): Whether to collect lineage data for the training phase,
              ...
              <https://www.luojianet_ms.cn/mindinsight/docs/en/r1.7/lineage_and_scalars_comparison.html>`_
    ...
    def _get_optimizer(self, cb_params):
        ...
        if optimizer is None or not isinstance(optimizer, Optimizer):
            logger.warning("Can not find optimizer in network, or the optimizer does not inherit LuoJiaNET's "
    ...
    def _get_learning_rate(optimizer):
        ...
        Args:
            optimizer (Optimizer): A optimizer which inherit the LuoJiaNET Optimizer class.
    ...
    def _get_dataset_path(self, output_dataset):
        ...
        Args:
            output_dataset (Union[Dataset, ImageFolderDataset, MnistDataset, Cifar10Dataset, Cifar100Dataset,
                ...
                Refer to luojianet_ms.dataset.Dataset.
        ...
        """
        dataset_package = import_module('luojianet_ms.dataset')
    ...
    def _get_ckpt_file_path(cb_params):
        """
        Get checkpoint file path from LuoJiaNET callback list.
