...
class _DataWrapper(nn.Module):
    ...
    def __init__(self, network, dataset_types, dataset_shapes, queue_name, min_shapes=None, max_shapes=None):
        ...
        # Also copy the flag in `network` call
        flags = getattr(network.__class__.call, "_luojianet_flags", {})
...
def _check_add_offload(dataset, dataset_helper, network):
    ...
    from luojianet_ms.dataset.engine import offload
...
def connect_network_with_dataset(network, dataset_helper):
    ...
    Examples:
        >>> from luojianet_ms import DatasetHelper
        >>>
        >>> # call create_dataset function to create a regular dataset, refer to luojianet_ms.dataset
...
class DatasetHelper:
    ...
    Args:
        dataset (Dataset): The dataset iterator. The dataset can be generated by dataset generator API in
                           :class:`luojianet_ms.dataset`, such as :class:`luojianet_ms.dataset.ImageFolderDataset`.
