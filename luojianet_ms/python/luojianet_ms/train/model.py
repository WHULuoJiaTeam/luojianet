...
class Model:
    ...
    Args:
        ...
        eval_indexes (list): It is used when eval_network is defined. If `eval_indexes` is None by default, all outputs
                             ...
                             Default: None.
        amp_level (str): Option for argument `level` in :func:`luojianet_ms.build_train_network`, level for mixed
            ...
            `loss_scale_manager` should be a subclass of :class:`luojianet_ms.LossScaleManager`.
            The more detailed explanation of `amp_level` setting can be found at `luojianet_ms.build_train_network`.

        boost_level (str): Option for argument `level` in `luojianet_ms.boost`, level for boost mode
    ...
    Examples:
        ...
        >>> # https://www.luojianet_ms.cn/tutorials/zh-CN/r1.7/beginner/quick_start.html
    ...
    def train(self, epoch, train_dataset, callbacks=None, dataset_sink_mode=True, sink_size=-1):
        ...
        Examples:
            >>> from luojianet_ms import Model, nn, FixedLossScaleManager
            >>>
    ...
    def predict(self, *predict_data):
        ...
        Examples:
            ...
            >>> from luojianet_ms import Model, Tensor
            >>>
    ...
    def infer_train_layout(self, train_dataset, dataset_sink_mode=True, sink_size=-1):
        ...
        Examples:
            ...
            >>> import numpy as np
            >>> import luojianet_ms as ms
            >>> from luojianet_ms import Model, context, Tensor, nn, FixedLossScaleManager
    ...
    def infer_predict_layout(self, *predict_data):
        ...
        Examples:
            ...
            >>> import numpy as np
            >>> import luojianet_ms as ms
            >>> from luojianet_ms import Model, context, Tensor
            >>> from luojianet_ms.context import ParallelMode
            >>> from luojianet_ms.communication import init
            >>>
            >>> context.set_context(mode=context.GRAPH_MODE)
