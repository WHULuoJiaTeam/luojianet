...
from luojianet_ms.train.checkpoint_pb2 import Checkpoint
from luojianet_ms.train.mind_ir_pb2 import ModelProto as mindir_model
from luojianet_ms.train.node_strategy_pb2 import ParallelStrategyMap, ParallelLayouts, ParallelGroupMap
from luojianet_ms.train.print_pb2 import Print

import luojianet_ms
import luojianet_ms.nn as nn
from luojianet_ms import context
from luojianet_ms import log as logger
from luojianet_ms._checkparam import check_input_data, check_input_dataset, Validator
from luojianet_ms.common import dtype as mstype
from luojianet_ms.common.api import _cell_graph_executor as _executor
from luojianet_ms.common.initializer import initializer
from luojianet_ms.common.parameter import Parameter
from luojianet_ms.common.tensor import Tensor
from luojianet_ms.communication.management import get_rank, get_group_size
from luojianet_ms.compression.export import quant_export
from luojianet_ms.parallel._cell_wrapper import get_allgather_cell
from luojianet_ms.parallel._tensor import _load_tensor, _get_tensor_strategy, _get_tensor_slice_index
from luojianet_ms.parallel._tensor import _reshape_param_data
from luojianet_ms.parallel._tensor import _reshape_param_data_with_weight
from luojianet_ms.parallel._utils import _infer_rank_list, _remove_repeated_slices
...
def save_checkpoint(save_obj, ckpt_file_name, integrated_save=True,
    ...
    Args:
        save_obj (Union[Module, list]): The cell object or data list(each element is a dictionary, like
    ...
    Raises:
        TypeError: If the parameter save_obj is not `nn.Module` or list type. And if the parameter
    ...
    if not isinstance(save_obj, nn.Module) and not isinstance(save_obj, list):
        raise TypeError("For 'save_checkpoint', the argument 'save_obj' should be nn.Module or list, "
    ...
    if isinstance(save_obj, nn.Module):
...
def load(file_name, **kwargs):
    ...
    The returned object can be executed by a `GraphCell`, see class :class:`luojianet_ms.nn.GraphCell` for more details.
...
def load_checkpoint(ckpt_file_name, net=None, strict_load=False, filter_prefix=None, dec_key=None, dec_mode="AES-GCM"):
    ...
    Args:
        ...
        net (Module): The network where the parameters will be loaded. Default: None
...
def load_param_into_net(net, parameter_dict, strict_load=False):
    ...
    Args:
        net (Module): The network where the parameters will be loaded.
    ...
    Raises:
        TypeError: Argument is not a Module, or parameter_dict is not a Parameter dictionary.
    ...
    if not isinstance(net, nn.Module):
        ...
        msg = ("For 'load_param_into_net', the argument 'net' should be a Module, but got {}.".format(type(net)))
        raise TypeError(msg)
...
def _save_graph(network, file_name):
    ...
    Args:
        network (Module): Obtain a pipeline through network for saving graph.
...
def export(net, *inputs, file_name, file_format='AIR', **kwargs):
    """
    Export the luojianet_ms network into an offline model in the specified format.
    ...
    Args:
        ...
        file_format (str): LuoJiaNet currently supports 'AIR', 'ONNX' and 'MINDIR' format for exported model.
            Default: 'AIR'.

            - AIR: Ascend Intermediate Representation. An intermediate representation format of Ascend model.
            - ONNX: Open Neural Network eXchange. An open format built to represent machine learning models.
            - MINDIR: LuoJiaNet Native Intermediate Representation for Anf. An intermediate representation format
              for LuoJiaNet models.
    ...
    Examples:
        >>> import numpy as np
        >>> from luojianet_ms import export, Tensor
        >>>
        >>> net = LeNet()
    ...
    if check_input_dataset(*inputs, dataset_type=luojianet_ms.dataset.Dataset):
        if len(inputs) != 1:
...
def _export(net, file_name, file_format, *inputs, **kwargs):
    """
    It is an internal conversion function. Export the LuoJiaNet prediction model to a file in the specified format.
    """
    logger.info("exporting model file:%s format:%s.", file_name, file_format)
    check_input_data(*inputs, data_class=Tensor)
...
def _save_mindir(net, file_name, *inputs, **kwargs):
    ...
    if 'dataset' in kwargs.keys() and kwargs['dataset'] is not None:
        check_input_data(kwargs['dataset'], data_class=luojianet_ms.dataset.Dataset)
...
def _quant_export(network, *inputs, file_format, **kwargs):
    """
    Exports LuoJiaNet quantization predict model to deploy with AIR and MINDIR.
    """
...
def parse_print(print_file_name):
    """
    Parse saved data generated by luojianet_ms.ops.Print.  Print is used to print data to screen in graph mode.
    ...
    please refer to :func:`luojianet_ms.context.set_context` and :class:`luojianet_ms.ops.Print`.
    ...
    Examples:
        ...
        >>> class PrintInputTensor(nn.Module):
        ...         def __init__(self):
        ...             super().__init__()
        ...             self.print = ops.Print()
        ...
        ...         def call(self, input_pra):
        ...
        >>> import luojianet_ms
        >>> data = luojianet_ms.parse_print('./log.data')
        >>> print(data)
...
def build_searched_strategy(strategy_filename):
    ...
    `<https://www.luojianet_ms.cn/docs/programming_guide/en/master/save_load_model_hybrid_parallel.html>`_.
...
def merge_sliced_parameter(sliced_parameters, strategy=None):
    ...
    Examples:
        >>> import numpy as np
        >>> from luojianet_ms import Tensor, merge_sliced_parameter, Parameter
        >>>
        >>> sliced_parameters = [
...
def load_distributed_checkpoint(network, checkpoint_filenames, predict_strategy=None,
    ...
    `<https://www.luojianet_ms.cn/docs/programming_guide/en/master/distributed_inference.html>`_.

    Args:
        network (Module): Network for distributed predication.
