...
def test_dataset(model_path,data_path,result_path):
    '''test_dataset'''
    model = CDNet34(3)
    ...
    img1_path=os.path.join(data_path,"left")
    img2_path=os.path.join(data_path,"right")
    ...
    for i, data in tqdm.tqdm(enumerate(data_loader),ncols=100):
        ...
        output = np.array(output[2].squeeze())
...
def test_single(model_path,left_path,right_path,result_path):
    '''test_single'''
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_folder', type=str, default=None, help='Results path')
    ...
    if(args_opt.left_input_file):
        test_single(args_opt.checkpoint_path,args_opt.left_input_file,args_opt.right_input_file,args_opt.output_folder)
    elif(args_opt.dataset_path):
        test_dataset(args_opt.checkpoint_path,args_opt.dataset_path,args_opt.output_folder)
    else:
