...
from src.dataset_generator import create_my_fasterrcnn_dataset, create_inference_fasterrcnn_dataset
import cv2
...
def fasterrcnn_inference(ckpt_path):
    ...
    ds, img_paths = create_inference_fasterrcnn_dataset(config)
    ...
    for idx, data in enumerate(ds.create_dict_iterator(num_epochs=1)):
        ...
        if len(flt_all_label) != 0:
            for bbox, label in zip(flt_all_bbox, flt_all_label):
                ...
                # cv2.rectangle(img, (int(bbox[0]), int(bbox[1])), (int(bbox[2]), int(bbox[3])), (0, 0, 255), thickness=1)
                # font = cv2.FONT_HERSHEY_SIMPLEX
                # cv2.putText(img, '{:.2}, {}'.format(bbox[4], cls_name), (int(bbox[0]), int(bbox[1])), font, 0.4, (0, 0, 255), thickness=1)
