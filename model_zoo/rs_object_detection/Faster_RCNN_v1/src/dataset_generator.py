...
def create_my_fasterrcnn_dataset(config, batch_size=2, device_num=1, rank_id=0, is_training=True,
                              num_parallel_workers=1, python_multiprocessing=True):
    """Create FasterRcnn dataset with MindDataset."""
    cv2.setNumThreads(0)
    # cv2.ocl.setUseOpenCL(False)

    ds_generator = DatasetGenerator(cfg=config)
    ds = de.GeneratorDataset(ds_generator, column_names=["image", "annotation"], num_shards=device_num, shard_id=rank_id, num_parallel_workers=1, shuffle=is_training)
...
def preprocess_fn_inference(image, img_path, config):
    ...
    def _infer_data(image_bgr, image_shape):
        ...
        if config.keep_ratio:
            input_data = rescale_column_inference(*input_data, config=config)
        else:
            input_data = resize_column_inference(*input_data, config=config)
        input_data = imnormalize_column(*input_data)
...
def create_inference_fasterrcnn_dataset(config, batch_size=1, device_num=1, rank_id=0, is_training=False,
                              num_parallel_workers=1):
    """Create FasterRcnn dataset with MindDataset."""
    ds_generator = DatasetGenerator_inference(cfg=config)
    img_paths = ds_generator.img_files
    dataset = de.GeneratorDataset(ds_generator, column_names=["image", "img_paths"],num_shards=device_num, shard_id=rank_id, num_parallel_workers=1, shuffle=False)
...
class DatasetGenerator_inference(object):
    def __init__(self, cfg=None, is_training=False):
        self.cfg = cfg
        self.is_training = is_training
        self.img_files = glob.glob(cfg.inference_img_dir + '*.png')
...
class DatasetGenerator(object):
    def __init__(self, cfg=None, is_training=True):
        ...
        self.is_training = is_training
        if cfg.dataset == "coco":
            self.img_files, self.img_anno_dict = create_coco_label(config=self.cfg, is_training=self.is_training)
        elif cfg.dataset == "other":
            self.img_files, self.img_anno_dict = create_train_data_from_txt(self.cfg.image_dir, self.cfg.anno_path)
...
if __name__ == "__main__":
    ...
    # a, b = create_coco_label(config=config, is_training=True)
    # a = create_my_fasterrcnn_dataset(config)
    a = create_my_fasterrcnn_dataset(config)

    for data in a.create_dict_iterator():
    ...
    #
    # # get config file
    # cfg_path = 'standard.yaml'
    # with open(cfg_path, 'r') as f:
    #     cfg_ml = AttrDict(yaml.load(f, Loader=yaml.FullLoader))
    #
    # train_dataset = DatasetGenerator(cfg_ml)
    #
    # data_loader = create_ml_dataset(train_dataset, is_training=True, batch_size=1, is_aug=True)
    #
    ...
    #
    #     plt.figure()
    #     plt.imshow(img.asnumpy().squeeze().transpose(1, 2, 0))
    #     plt.show()
    #
    #     break
