...
class BaseMetricLossFunction(
    ...
    def zero_losses(self):
        ...
        # # { add for graph mode
        # loss_name_return = [loss_name for loss_name in self.sub_loss_names()]
        # loss_value_return = [self.zero_loss() for loss_name in self.sub_loss_names()]
        # return zip(loss_name_return, loss_value_return)  # change for graph mode
        # # }
...
class MultipleLosses(nn.Module):
    def __init__(self, losses, miners=None, weights=None):
        ...
        # )
        if self.is_dict:
            self.losses = nn.CellList([x for x in losses.values()])
        else:
        ...
        if miners is not None:
            ...
            # )
            if self.is_dict:
                self.miners = nn.CellList([x for x in miners.values()])
            else:
                self.miners = nn.CellList(miners)
