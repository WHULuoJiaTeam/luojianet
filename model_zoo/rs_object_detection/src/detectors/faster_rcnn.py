...
from ..bbox_assigners import BboxAssignSampleForRcnn
from ..backbones import ResNetFea, ResidualBlockUsing
from ..necks import FeatPyramidNeck
from ..anchor_heads import Proposal, RPN
from ..heads import RcnnCls
from ..roi_extractors import SingleRoIExtractor
from ..anchor_heads.anchor_generator import AnchorGenerator
...
class Faster_Rcnn_Resnet(nn.Module):
    ...
    def test_mode_init(self, config):
        ...
        self.test_score_thresh = Tensor(np.ones((self.rpn_max_num, 1)).astype(self.np_cast_type) * config.test_score_thr)
    ...
    def call(self, img_data, img_metas, gt_bboxes, gt_labels, gt_valids):
        """
...
class FasterRcnn_Infer(nn.Module):
    ...
    def init_tensor(self, config):
        ...
        roi_align_index = [
            np.array(np.ones((config.num_expected_pos_stage2 + config.num_expected_neg_stage2, 1)) * i,
                     dtype=self.np_cast_type) for i in range(self.train_batch_size)]
    ...
    def call(self, img_data, img_metas):
        x = self.backbone(img_data)
        x = self.fpn_neck(x)
        _, cls_score, bbox_pred, _, _, _ = self.rpn_with_loss(x,img_metas,self.anchor_list,None,None,None)
        proposal, proposal_mask = self.proposal_generator_test(cls_score, bbox_pred, self.anchor_list)
        ...
        rcnn_masks = F.stop_gradient(rcnn_masks)
        rcnn_mask_squeeze = self.squeeze(self.cast(rcnn_masks, mstype.bool_))
        x_cls, x_reg = self.rcnn_cls(roi_feats,
                                 bbox_targets,
                                 rcnn_labels,
                                 rcnn_mask_squeeze)
        output = self.get_det_bboxes(x_cls, x_reg, rcnn_masks, bboxes_all, img_metas)
        return output
