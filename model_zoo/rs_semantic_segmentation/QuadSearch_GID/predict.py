...
def saveResult(data, output_folder):
    ...
    if not os.path.exists(output_folder+'output2.tif'):
        gtif_driver = gdal.GetDriverByName("GTiff")
        out_ds = gtif_driver.Create(output_folder+'output2.tif', data.shape[1], data.shape[0], save_bandsize, gdal.GDT_Byte)
    ...
    print(output_folder)
    print(output_folder+'output2.tif')
    write_ds = gdal.Open(output_folder+'output2.tif', gdalconst.GA_Update)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--input_file', default=r'/media/xx/My Passport/QuadSearch/GF2_PMS2__L1A0001799015-MSS2.tif')
    parser.add_argument('--output_folder', default=r'./')
    parser.add_argument('--checkpoint_path', default=r'/media/xx/My Passport/QuadSearch/ckpt/hrnet_21.ckpt')
    parser.add_argument('--device_target', default='GPU')
    parser.add_argument('-u', '--uuid', type=str, default=None,help='Pred uuid(Only for HWC Modelarts)')
    ...
    #img=cv.imread('./input.pngoutput.tif')
