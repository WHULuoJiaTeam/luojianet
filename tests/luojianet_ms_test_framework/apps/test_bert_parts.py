...
import numpy as np
from luojianet_ms.tests.models.Bert_NEZHA import EmbeddingLookup, GetMaskedLMOutput, \
    BertConfig, BertPreTraining, BertNetworkWithLoss
from luojianet_ms.tests.models.Bert_NEZHA.bert_model import BertModel
...
class BertPreTrainingNet(nn.Module):
    ...
    def forward(self, input_ids_, input_mask_, token_type_id_,
...
@luojianet_ms_test(pipeline_for_compile_forward_anf_graph_for_case_by_case_config)
def test_compile():
...
@luojianet_ms_test(pipeline_for_compile_grad_anf_graph_for_case_by_case_config)
def test_backward():
    context.set_context(mode=context.GRAPH_MODE)
    return test_sets
