...
class ApplyAddSignNet(nn.Module):
    ...
    def call(self, lr, alpha, sign_decay, beta, grad):
...
class ApplyCenteredRMSPropNet(nn.Module):
    def __init__(self, var):
        super(ApplyCenteredRMSPropNet, self).__init__()
        self.apply_centered_rms_prop = P.ApplyCenteredRMSProp()
        self.var = Parameter(var, name="var")

    def call(self, mean_grad, mean_square, moment, grad, learning_rate):
...
class ApplyFtrlNet(nn.Module):
    ...
    def call(self, grad, lr, l1, l2, lr_power):
...
class ApplyMomentumNet(nn.Module):
    def __init__(self, var, accum):
...
class ApplyPowerSignNet(nn.Module):
    ...
    def call(self, lr, logbase, sign_decay, beta, grad):
...
class ApplyProximalGradientDescentNet(nn.Module):
    ...
    def call(self, alpha, l1, l2, delta):
...
class ApplyRMSPropNet(nn.Module):
    def __init__(self, var):
        super(ApplyRMSPropNet, self).__init__()
        self.apply_rms_prop = P.ApplyRMSProp()
        self.var = Parameter(var, name="var")

    def call(self, mean_square, moment, learning_rate, grad):
...
class FusedSparseAdamNet(nn.Module):
    def __init__(self, var, m, v):
...
class FusedSparseFtrlNet(nn.Module):
    def __init__(self, var, accum, linear):
...
class FusedSparseLazyAdamNet(nn.Module):
    def __init__(self, var, m, v):
...
class FusedSparseProximalAdagradNet(nn.Module):
    def __init__(self, var, accum):
    ...
    def call(self, lr, l1, l2, grad, indices):
...
class SGDNet(nn.Module):
    ...
    def call(self, gradient, learning_rate, accum, momentum, stat):
...
class ApplyProximalAdagradConstantNet(nn.Module):
    def __init__(self, var, accum):
        super().__init__()
