...
if [[ "${PACKAGE_TYPE}" == "python" ]]; then
    LUOJIANET_PKG_PATH=`python -m pip show luojianet_ms-ascend | grep Location | awk '{print $2"/luojianet_ms"}' | xargs realpath`
  if [[ "X${LUOJIANET_PKG_PATH}" == "X" ]]; then
    LUOJIANET_PKG_PATH=${PROJECT_PATH}/build/package/luojianet_ms:
  fi
...
# using acl tar package, extract tar package here
  ...
  LUOJIANET_PKG_PATH=$(find "${TEST_PATH}" -maxdepth 1 -name "luojianet_ms_ascend*")
fi

export LD_LIBRARY_PATH=${LUOJIANET_PKG_PATH}:${LUOJIANET_PKG_PATH}/lib:${PROJECT_PATH}/tests/st/cpp:$LD_LIBRARY_PATH
export GLOG_v=2
