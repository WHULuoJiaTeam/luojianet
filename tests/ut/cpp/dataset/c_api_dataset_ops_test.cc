...
class NoTransform final : public TensorTransform {
 ...
 protected:
  std::shared_ptr<TensorOperation> Parse() override {
    return std::make_shared<transforms::PreBuiltOperation>(std::make_shared<luojianet_ms::dataset::test::NoOp>());
  }
...
class ThreeToOneTransform final : public TensorTransform {
 ...
 protected:
  std::shared_ptr<TensorOperation> Parse() override {
    return std::make_shared<transforms::PreBuiltOperation>(std::make_shared<luojianet_ms::dataset::test::ThreeToOneOp>());
  }
...
class OneToThreeTransform final : public TensorTransform {
 ...
 protected:
  std::shared_ptr<TensorOperation> Parse() override {
    return std::make_shared<transforms::PreBuiltOperation>(std::make_shared<luojianet_ms::dataset::test::OneToThreeOp>());
  }
...
TensorRow VecToRow(const MSTensorVec &v) {
  TensorRow row;
  for (const luojianet_ms::MSTensor &t : v) {
    std::shared_ptr<Tensor> rt;
    (void)Tensor::CreateFromMemory(TensorShape(t.Shape()), MSTypeToDEType(static_cast<luojianet_ms::TypeId>(t.DataType())),
...
MSTensorVec RowToVec(const TensorRow &v) {
  ...
  std::transform(v.begin(), v.end(), std::back_inserter(rv), [](std::shared_ptr<Tensor> t) -> luojianet_ms::MSTensor {
    return luojianet_ms::MSTensor(std::make_shared<DETensor>(t));
  });
  return rv;
}

MSTensorVec BucketBatchTestFunction(MSTensorVec input) {
  luojianet_ms::dataset::TensorRow output;
  std::shared_ptr<Tensor> out;
  (void)Tensor::CreateEmpty(luojianet_ms::dataset::TensorShape({1}),
                            luojianet_ms::dataset::DataType(luojianet_ms::dataset::DataType::Type::DE_INT32), &out);
...
MSTensorVec Predicate1(MSTensorVec in) {
  ...
  TensorRow output;
  std::shared_ptr<Tensor> out;
  (void)Tensor::CreateEmpty(luojianet_ms::dataset::TensorShape({}),
                            luojianet_ms::dataset::DataType(luojianet_ms::dataset::DataType::Type::DE_BOOL), &out);
...
TEST_F(MindDataTestPipeline, TestBucketBatchByLengthSuccess2) {
  ...
  // Create a BucketBatchByLength operation on ds
  std::map<std::string, std::pair<std::vector<int64_t>, luojianet_ms::MSTensor>> pad_info = {};
...
TEST_F(MindDataTestPipeline, TestMap) {
  ...
  // Create objects for the tensor ops
  std::shared_ptr<TensorTransform> no_op = std::make_shared<luojianet_ms::dataset::test::NoTransform>();
...
TEST_F(MindDataTestPipeline, Test3to1) {
  ...
  // Create objects for the tensor ops
  std::shared_ptr<TensorTransform> three_to_one_op = std::make_shared<luojianet_ms::dataset::test::ThreeToOneTransform>();
...
TEST_F(MindDataTestPipeline, Test1to3) {
  ...
  // Create objects for the tensor ops
  std::shared_ptr<TensorTransform> one_to_three_op = std::make_shared<luojianet_ms::dataset::test::OneToThreeTransform>();
