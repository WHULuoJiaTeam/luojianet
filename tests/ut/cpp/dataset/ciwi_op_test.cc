...
class MindDataTestCIWIOp : public UT::CVOP::CVOpCommon {
 protected:
  MindDataTestCIWIOp() : CVOpCommon() {}

  std::shared_ptr<Tensor> output_tensor_;
};

TEST_F(MindDataTestCIWIOp, TestOp1) {
  MS_LOG(INFO) << "Doing testCIWI.";
  std::unique_ptr<CIWIOp> op(new CIWIOp(100.0));
  ...
  cv::imwrite("/test_luojianet/luojianet1/luojianet/temp_ciwi.tif", output_img);
  MS_LOG(INFO) << "testCIWI end.";
