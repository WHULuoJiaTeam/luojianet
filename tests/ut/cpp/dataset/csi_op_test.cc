...
class MindDataTestCSIOp : public UT::CVOP::CVOpCommon {
 protected:
  MindDataTestCSIOp() : CVOpCommon() {}

  std::shared_ptr<Tensor> output_tensor_;
};

TEST_F(MindDataTestCSIOp, TestOp1) {
  MS_LOG(INFO) << "Doing testCSI.";
  std::unique_ptr<CSIOp> op(new CSIOp());
  ...
  cv::imwrite("/test_luojianet/luojianet1/luojianet/temp_CSI.tif", output_img);
  MS_LOG(INFO) << "testCSI end.";
