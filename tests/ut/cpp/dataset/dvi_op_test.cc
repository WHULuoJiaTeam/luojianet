...
class MindDataTestDVIOp : public UT::CVOP::CVOpCommon {
 protected:
  MindDataTestDVIOp() : CVOpCommon() {}

  std::shared_ptr<Tensor> output_tensor_;
};

TEST_F(MindDataTestDVIOp, TestOp1) {
  MS_LOG(INFO) << "Doing testDVI.";
  std::unique_ptr<DVIOp> op(new DVIOp());
  ...
  cv::imwrite("/test_luojianet/luojianet1/luojianet/temp_dvi.tif", output_img);
  MS_LOG(INFO) << "testDVI end.";
