...
class MindDataTestEVIOp : public UT::CVOP::CVOpCommon {
 protected:
  MindDataTestEVIOp() : CVOpCommon() {}

  std::shared_ptr<Tensor> output_tensor_;
};

TEST_F(MindDataTestEVIOp, TestOp1) {
  MS_LOG(INFO) << "Doing testEVI.";
  std::unique_ptr<EVIOp> op(new EVIOp());
  ...
  cv::imwrite("/test_luojianet/luojianet1/luojianet/temp_evi.tif", output_img);
  MS_LOG(INFO) << "testEVI end.";
