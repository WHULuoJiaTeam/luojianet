...
class MindDataTestEWI_WOp : public UT::CVOP::CVOpCommon {
 protected:
  MindDataTestEWI_WOp() : CVOpCommon() {}

  std::shared_ptr<Tensor> output_tensor_;
};

TEST_F(MindDataTestEWI_WOp, TestOp1) {
  MS_LOG(INFO) << "Doing testEWI_W.";
  std::unique_ptr<EWI_WOp> op(new EWI_WOp(0.1, 0.5));
  ...
  cv::imwrite("/test_luojianet/luojianet1/luojianet/temp_ewi_w.tif", output_img);
  MS_LOG(INFO) << "testEWI_W end.";
