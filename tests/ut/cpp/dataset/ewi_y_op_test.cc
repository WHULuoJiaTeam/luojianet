...
class MindDataTestEWI_YOp : public UT::CVOP::CVOpCommon {
 protected:
  MindDataTestEWI_YOp() : CVOpCommon() {}

  std::shared_ptr<Tensor> output_tensor_;
};

TEST_F(MindDataTestEWI_YOp, TestOp1) {
  MS_LOG(INFO) << "Doing testEWI_Y.";
  std::unique_ptr<EWI_YOp> op(new EWI_YOp());
  ...
  cv::imwrite("/test_luojianet/luojianet1/luojianet/temp_ewi_y.tif", output_img);
  MS_LOG(INFO) << "testEWI_Y end.";
