...
class MindDataTestFNDWIOp : public UT::CVOP::CVOpCommon {
 protected:
  MindDataTestFNDWIOp() : CVOpCommon() {}

  std::shared_ptr<Tensor> output_tensor_;
};

TEST_F(MindDataTestFNDWIOp, TestOp1) {
  MS_LOG(INFO) << "Doing testFNDWI.";
  std::unique_ptr<FNDWIOp> op(new FNDWIOp(1, 40));
  ...
  cv::imwrite("/test_luojianet/luojianet1/luojianet/temp_fndwi.tif", output_img);
  MS_LOG(INFO) << "testfndwi end.";
