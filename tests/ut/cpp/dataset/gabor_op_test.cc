...
class MindDataTestGaborOp : public UT::CVOP::CVOpCommon {
 protected:
  MindDataTestGaborOp() : CVOpCommon() {}

  std::shared_ptr<Tensor> output_tensor_;
};

TEST_F(MindDataTestGaborOp, TestOp1) {
  MS_LOG(INFO) << "Doing testGabor.";
  std::unique_ptr<GaborOp> op(new GaborOp(true));
  ...
  cv::imwrite("/test_luojianet/luojianet1/luojianet/building_Gabor.tif", output_img);
  MS_LOG(INFO) << "testGabor end.";
