...
class MindDataTestGLCMOp : public UT::CVOP::CVOpCommon {
 protected:
  MindDataTestGLCMOp() : CVOpCommon() {}

  std::shared_ptr<Tensor> output_tensor_;
};

TEST_F(MindDataTestGLCMOp, TestOp1) {
  MS_LOG(INFO) << "Doing testGLCM.";
  std::unique_ptr<GLCMOp> op(new GLCMOp(3));
  ...
  cv::imwrite("/test_luojianet/luojianet1/luojianet/building_GLCM.tif", output_img);
  MS_LOG(INFO) << "testGLCM end.";
