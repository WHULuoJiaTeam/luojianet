...
class MindDataTestGNDWIOp : public UT::CVOP::CVOpCommon {
 protected:
  MindDataTestGNDWIOp() : CVOpCommon() {}

  std::shared_ptr<Tensor> output_tensor_;
};

TEST_F(MindDataTestGNDWIOp, TestOp1) {
  MS_LOG(INFO) << "Doing testGNDWI.";
  std::unique_ptr<GNDWIOp> op(new GNDWIOp());
  ...
  cv::imwrite("/test_luojianet/luojianet1/luojianet/temp_gndwi.tif", output_img);
  MS_LOG(INFO) << "testgndwi end.";
