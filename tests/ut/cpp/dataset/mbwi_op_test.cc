...
#include "minddata/dataset/kernels/image/mbwi_op.h"
...
class MindDataTestMBWIOp : public UT::CVOP::CVOpCommon {
 protected:
  MindDataTestMBWIOp() : CVOpCommon() {}

  std::shared_ptr<Tensor> output_tensor_;
};

TEST_F(MindDataTestMBWIOp, TestOp1) {
  MS_LOG(INFO) << "Doing testMBWI.";
  std::unique_ptr<MBWIOp> op(new MBWIOp());
  ...
  cv::imwrite("/test_luojianet/luojianet1/luojianet/temp_mbwi.tif", output_img);
  MS_LOG(INFO) << "testMBWI end.";
