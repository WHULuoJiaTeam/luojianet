...
class MindDataTestMCIWIOp : public UT::CVOP::CVOpCommon {
 protected:
  MindDataTestMCIWIOp() : CVOpCommon() {}

  std::shared_ptr<Tensor> output_tensor_;
};

TEST_F(MindDataTestMCIWIOp, TestOp1) {
  MS_LOG(INFO) << "Doing testMCIWI.";
  std::unique_ptr<MCIWIOp> op(new MCIWIOp());
  ...
  cv::imwrite("/test_luojianet/luojianet1/luojianet/temp_mciwi.tif", output_img);
  MS_LOG(INFO) << "testMCIWI end.";
