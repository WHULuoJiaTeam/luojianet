...
class MindDataTestMNDWIOp : public UT::CVOP::CVOpCommon {
 protected:
  MindDataTestMNDWIOp() : CVOpCommon() {}

  std::shared_ptr<Tensor> output_tensor_;
};

TEST_F(MindDataTestMNDWIOp, TestOp1) {
  MS_LOG(INFO) << "Doing testMNDWI.";
  std::unique_ptr<MNDWIOp> op(new MNDWIOp());
  ...
  cv::imwrite("/test_luojianet/luojianet1/luojianet/temp_mndwi.tif", output_img);
  MS_LOG(INFO) << "testMNDWI end.";
