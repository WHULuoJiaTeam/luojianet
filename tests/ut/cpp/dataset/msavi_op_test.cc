...
class MindDataTestMSAVIOp : public UT::CVOP::CVOpCommon {
 protected:
  MindDataTestMSAVIOp() : CVOpCommon() {}

  std::shared_ptr<Tensor> output_tensor_;
};

TEST_F(MindDataTestMSAVIOp, TestOp1) {
  MS_LOG(INFO) << "Doing testMSAVI.";
  std::unique_ptr<MSAVIOp> op(new MSAVIOp());
  ...
  cv::imwrite("/test_luojianet/luojianet1/luojianet/temp_msavi.tif", output_img);
  MS_LOG(INFO) << "testMSAVI end.";
