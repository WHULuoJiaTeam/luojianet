...
class MindDataTestNDPIOp : public UT::CVOP::CVOpCommon {
 protected:
  MindDataTestNDPIOp() : CVOpCommon() {}

  std::shared_ptr<Tensor> output_tensor_;
};

TEST_F(MindDataTestNDPIOp, TestOp1) {
  MS_LOG(INFO) << "Doing testNDPI.";
  std::unique_ptr<NDPIOp> op(new NDPIOp());
  ...
  cv::imwrite("/test_luojianet/luojianet1/luojianet/temp_ndpi.tif", output_img);
  MS_LOG(INFO) << "testNDPI end.";
