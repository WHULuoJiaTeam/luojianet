...
class MindDataTestNDWIOp : public UT::CVOP::CVOpCommon {
 protected:
  MindDataTestNDWIOp() : CVOpCommon() {}

  std::shared_ptr<Tensor> output_tensor_;
};

TEST_F(MindDataTestNDWIOp, TestOp1) {
  MS_LOG(INFO) << "Doing testNDWI.";
  std::unique_ptr<NDWIOp> op(new NDWIOp());
  ...
  cv::imwrite("/test_luojianet/luojianet1/luojianet/temp_ndwi.tif", output_img);
  MS_LOG(INFO) << "testNDWI end.";
