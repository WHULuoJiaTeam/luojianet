...
class MindDataTestNWIOp : public UT::CVOP::CVOpCommon {
 protected:
  MindDataTestNWIOp() : CVOpCommon() {}

  std::shared_ptr<Tensor> output_tensor_;
};

TEST_F(MindDataTestNWIOp, TestOp1) {
  MS_LOG(INFO) << "Doing testNWI.";
  std::unique_ptr<NWIOp> op(new NWIOp());
  ...
  cv::imwrite("/test_luojianet/luojianet1/luojianet/temp_nwi.tif", output_img);
  MS_LOG(INFO) << "testNWI end.";
