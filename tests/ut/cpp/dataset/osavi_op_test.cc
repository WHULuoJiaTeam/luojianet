...
class MindDataTestOSAVIOp : public UT::CVOP::CVOpCommon {
 protected:
  MindDataTestOSAVIOp() : CVOpCommon() {}

  std::shared_ptr<Tensor> output_tensor_;
};

TEST_F(MindDataTestOSAVIOp, TestOp1) {
  MS_LOG(INFO) << "Doing testOSAVI.";
  std::unique_ptr<OSAVIOp> op(new OSAVIOp(0.16));
  ...
  cv::imwrite("/test_luojianet/luojianet1/luojianet/temp_osavi.tif", output_img);
  MS_LOG(INFO) << "testOSAVI end.";
