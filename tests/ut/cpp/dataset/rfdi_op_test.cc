...
#include "minddata/dataset/kernels/image/rfdi_op.h"
...
class MindDataTestRFDIOp : public UT::CVOP::CVOpCommon {
 protected:
  MindDataTestRFDIOp() : CVOpCommon() {}

  std::shared_ptr<Tensor> output_tensor_;
};

TEST_F(MindDataTestRFDIOp, TestOp1) {
  MS_LOG(INFO) << "Doing testRFDI.";
  std::unique_ptr<RFDIOp> op(new RFDIOp());
  ...
  cv::imwrite("/test_luojianet/luojianet1/luojianet/temp_RFDI.tif", output_img);
  MS_LOG(INFO) << "testRFDI end.";
