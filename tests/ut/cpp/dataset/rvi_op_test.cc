...
#include "minddata/dataset/kernels/image/rvi_op.h"
...
class MindDataTestRVIOp : public UT::CVOP::CVOpCommon {
 protected:
  MindDataTestRVIOp() : CVOpCommon() {}

  std::shared_ptr<Tensor> output_tensor_;
};

TEST_F(MindDataTestRVIOp, TestOp1) {
  MS_LOG(INFO) << "Doing testRVI.";
  std::unique_ptr<RVIOp> op(new RVIOp());
  EXPECT_TRUE(op->OneToOne());
  
  //std::string MindDataPath = "data/dataset";
  //std::string folder_path = MindDataPath + "/testImageNetData/train/class1/1_1.tif";
  std::string folder_path = "/test_luojianet/luojianet1/luojianet/temp.tif";
  //std::string folder_path = "../data/dataset/temp.tif";
  ...
  cv::imwrite("/test_luojianet/luojianet1/luojianet/temp_rvi.tif", output_img);
  MS_LOG(INFO) << "testRVI end.";
