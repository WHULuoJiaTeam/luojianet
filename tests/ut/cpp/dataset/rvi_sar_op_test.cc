...
#include "minddata/dataset/kernels/image/rvi_sar_op.h"
...
class MindDataTestRVI_SAROp : public UT::CVOP::CVOpCommon {
 protected:
  MindDataTestRVI_SAROp() : CVOpCommon() {}

  std::shared_ptr<Tensor> output_tensor_;
};

TEST_F(MindDataTestRVI_SAROp, TestOp1) {
  MS_LOG(INFO) << "Doing testRVI_SAR.";
  std::unique_ptr<RVI_SAROp> op(new RVI_SAROp());
  ...
  std::string bandhv ="/test_luojianet/luojianet1/luojianet/test_data/SAR/HHHV/s1a-ew-grd-hh-20220117t144920-20220117t145019-041502-04ef79-001.tiff";
  ...
  cv::Mat input_band3 = cv::imread(bandvv, cv::IMREAD_LOAD_GDAL | cv::IMREAD_COLOR | cv::IMREAD_ANYDEPTH);
  ...
  cv::imwrite("/test_luojianet/luojianet1/luojianet/temp_RVI_SAR.tif", output_img);
  MS_LOG(INFO) << "testRVI_SAR end.";
