...
class MindDataTestSAVIOp : public UT::CVOP::CVOpCommon {
 protected:
  MindDataTestSAVIOp() : CVOpCommon() {}

  std::shared_ptr<Tensor> output_tensor_;
};

TEST_F(MindDataTestSAVIOp, TestOp1) {
  MS_LOG(INFO) << "Doing testSAVI.";
  std::unique_ptr<SAVIOp> op(new SAVIOp(0.5));
  ...
  cv::imwrite("/test_luojianet/luojianet1/luojianet/temp_savi.tif", output_img);
