...
class MindDataTestSRWIOp : public UT::CVOP::CVOpCommon {
 protected:
  MindDataTestSRWIOp() : CVOpCommon() {}

  std::shared_ptr<Tensor> output_tensor_;
};

TEST_F(MindDataTestSRWIOp, TestOp1) {
  MS_LOG(INFO) << "Doing testSRWI.";
  std::unique_ptr<SRWIOp> op(new SRWIOp());
  ...
  cv::imwrite("/test_luojianet/luojianet1/luojianet/temp_srwi.tif", output_img);
  MS_LOG(INFO) << "testSRWI end.";
