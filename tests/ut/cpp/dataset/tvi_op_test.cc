...
class MindDataTestTVIOp : public UT::CVOP::CVOpCommon {
 protected:
  MindDataTestTVIOp() : CVOpCommon() {}

  std::shared_ptr<Tensor> output_tensor_;
};

TEST_F(MindDataTestTVIOp, TestOp1) {
  MS_LOG(INFO) << "Doing testTVI.";
  std::unique_ptr<TVIOp> op(new TVIOp());
  ...
  cv::imwrite("/test_luojianet/luojianet1/luojianet/temp_tvi.tif", output_img);
  MS_LOG(INFO) << "testTVI end.";
