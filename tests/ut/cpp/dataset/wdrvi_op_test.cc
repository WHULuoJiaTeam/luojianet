...
class MindDataTestWDRVIOp : public UT::CVOP::CVOpCommon {
 protected:
  MindDataTestWDRVIOp() : CVOpCommon() {}

  std::shared_ptr<Tensor> output_tensor_;
};

TEST_F(MindDataTestWDRVIOp, TestOp1) {
  MS_LOG(INFO) << "Doing testWDRVI.";
  std::unique_ptr<WDRVIOp> op(new WDRVIOp(0.1));
  ...
  cv::imwrite("/test_luojianet/luojianet1/luojianet/temp_wdrvi.tif", output_img);
  MS_LOG(INFO) << "testWDRVI end.";
