...
#include "minddata/dataset/kernels/image/wi_f_op.h"
...
class MindDataTestWI_FOp : public UT::CVOP::CVOpCommon {
 protected:
  MindDataTestWI_FOp() : CVOpCommon() {}

  std::shared_ptr<Tensor> output_tensor_;
};

TEST_F(MindDataTestWI_FOp, TestOp1) {
  MS_LOG(INFO) << "Doing testWI_F.";
  std::unique_ptr<WI_FOp> op(new WI_FOp());
  ...
  cv::imwrite("/test_luojianet/luojianet1/luojianet/temp_wi_f.tif", output_img);
  MS_LOG(INFO) << "testWI_F end.";
