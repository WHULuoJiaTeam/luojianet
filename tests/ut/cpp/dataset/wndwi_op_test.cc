...
#include "minddata/dataset/kernels/image/wndwi_op.h"
...
class MindDataTestWNDWIOp : public UT::CVOP::CVOpCommon {
 protected:
  MindDataTestWNDWIOp() : CVOpCommon() {}

  std::shared_ptr<Tensor> output_tensor_;
};

TEST_F(MindDataTestWNDWIOp, TestOp1) {
  MS_LOG(INFO) << "Doing testWNDWI.";
  std::unique_ptr<WNDWIOp> op(new WNDWIOp(0.45));
  ...
  cv::imwrite("/test_luojianet/luojianet1/luojianet/temp_WNDWI.tif", output_img);
  MS_LOG(INFO) << "testWNDWI end.";
