...
namespace ops {
class TestAddN : public UT::Common {
 public:
  TestAddN() {}
  void SetUp() {}
  void TearDown() {}
...
TEST_F(TestAddN, test_ops_addn1) {
  auto addn = std::make_shared<AddN>();
  ...
  auto input_tuple = std::make_shared<ValueTuple>(std::vector<ValuePtr>{tensor_x1, tensor_x2});
  auto abstract = addn->Infer({input_tuple->ToAbstract()});
...
TEST_F(TestAddN, test_ops_addn2) {
  auto addn = std::make_shared<AddN>();
  auto tensor_x1 = TensorConstructUtils::CreateOnesTensor(kNumberTypeBool, std::vector<int64_t>{3, 4, 5});
  auto tensor_x2 = TensorConstructUtils::CreateOnesTensor(kNumberTypeBool, std::vector<int64_t>{3, 4, 5});
  auto tensor_x3 = TensorConstructUtils::CreateOnesTensor(kNumberTypeBool, std::vector<int64_t>{3, 4, 5});
  MS_EXCEPTION_IF_NULL(tensor_x1);
  MS_EXCEPTION_IF_NULL(tensor_x2);
