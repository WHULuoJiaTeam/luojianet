...
TEST_F(TestAvgPoolGrad, test_ops_avg_pool_grad1) {
  auto avg_pool_grad = std::make_shared<AvgPoolGrad>();
  avg_pool_grad->Init();
  EXPECT_EQ(avg_pool_grad->get_pad_mode(), VALID);
  auto origin_input = MakeValue(std::vector<int64_t>{1, 2});
  auto dout = TensorConstructUtils::CreateOnesTensor(kNumberTypeFloat32, std::vector<int64_t>{1});
  MS_EXCEPTION_IF_NULL(origin_input);
  MS_EXCEPTION_IF_NULL(dout);
  auto abstract = avg_pool_grad->Infer({origin_input->ToAbstract(), dout->ToAbstract()});
