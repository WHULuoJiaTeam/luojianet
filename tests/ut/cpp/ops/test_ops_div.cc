...
namespace ops {
class TestDiv : public UT::Common {
 public:
  TestDiv() {}
  void SetUp() {}
  void TearDown() {}
...
TEST_F(TestDiv, test_ops_div) {
  ...
  auto div_abstract = div->Infer({tensor_x->ToAbstract(), tensor_y->ToAbstract()});
  MS_EXCEPTION_IF_NULL(div_abstract);
  EXPECT_EQ(div_abstract->isa<abstract::AbstractTensor>(), true);
  auto shape_ptr = div_abstract->BuildShape();
  MS_EXCEPTION_IF_NULL(shape_ptr);
  EXPECT_EQ(shape_ptr->isa<abstract::Shape>(), true);
  auto div_shape = shape_ptr->cast<abstract::ShapePtr>();
  MS_EXCEPTION_IF_NULL(div_shape);
  auto shape_vec = div_shape->shape();
  auto type = div_abstract->BuildType();
