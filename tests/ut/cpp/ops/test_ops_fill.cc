...
namespace ops {
class TestFill : public UT::Common {
 public:
  TestFill() {}
  void SetUp() {}
  void TearDown() {}
...
TEST_F(TestFill, test_ops_fill1) {
  auto fill = std::make_shared<Fill>();
  auto dtype = kFloat32;
  auto dims = MakeValue(std::vector<int64_t>{2, 3, 4, 5});
  auto x = MakeValue(4);
  MS_EXCEPTION_IF_NULL(dtype);
  MS_EXCEPTION_IF_NULL(dims);
  MS_EXCEPTION_IF_NULL(x);
  auto abstract = fill->Infer({dtype->ToAbstract(), dims->ToAbstract(), x->ToAbstract()});
