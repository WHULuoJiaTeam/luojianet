...
TEST_F(TestLeakyRelu, test_ops_leakyrelu1) {
  auto leakyrelu = std::make_shared<LeakyRelu>();
  auto tensor_x = TensorConstructUtils::CreateOnesTensor(kNumberTypeFloat32, std::vector<int64_t>{3, 4, 5});
  MS_EXCEPTION_IF_NULL(tensor_x);
  auto abstract = leakyrelu->Infer({tensor_x->ToAbstract()});
