...
namespace ops {
class TestMul : public UT::Common {
 public:
  TestMul() {}
  void SetUp() {}
  void TearDown() {}
...
TEST_F(TestMul, test_ops_mul) {
  ...
  auto mul_abstract = mul->Infer({tensor_x->ToAbstract(), tensor_y->ToAbstract()});
  MS_EXCEPTION_IF_NULL(mul_abstract);
  EXPECT_EQ(mul_abstract->isa<abstract::AbstractTensor>(), true);
  auto shape_ptr = mul_abstract->BuildShape();
  MS_EXCEPTION_IF_NULL(shape_ptr);
  EXPECT_EQ(shape_ptr->isa<abstract::Shape>(), true);
  auto mul_shape = shape_ptr->cast<abstract::ShapePtr>();
  MS_EXCEPTION_IF_NULL(mul_shape);
  auto shape_vec = mul_shape->shape();
  auto type = mul_abstract->BuildType();
