...
class TestPack : public UT::Common {
 public:
  TestPack() {}
  void SetUp() {}
  void TearDown() {}
...
TEST_F(TestPack, test_ops_pack1) {
  auto pack = std::make_shared<Pack>();
  pack->Init();
  EXPECT_EQ(pack->get_axis(), 0);
  ...
  auto input_ = std::make_shared<ValueTuple>(std::vector<ValuePtr>{input0, input1});
  auto abstract = pack->Infer({input_->ToAbstract()});
