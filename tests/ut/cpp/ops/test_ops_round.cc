...
class TestRound : public UT::Common {
 public:
  TestRound() {}
  void SetUp() {}
  void TearDown() {}
...
TEST_F(TestRound, test_ops_round1) {
  auto round = std::make_shared<Round>();
  auto input0 = TensorConstructUtils::CreateOnesTensor(kNumberTypeFloat32, std::vector<int64_t>{5});
  MS_EXCEPTION_IF_NULL(input0);
  auto abstract = round->Infer({input0->ToAbstract()});
