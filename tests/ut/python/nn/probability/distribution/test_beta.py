...
class GammaProb1(nn.Module):
    ...
    def forward(self, value, concentration1, concentration0):
...
def test_gamma_prob1():
    """
    Test probability functions: passing concentration1/concentration0, value through forward.
    """
    net = GammaProb1()
    value = Tensor([0.5, 1.0], dtype=dtype.float32)
...
class GammaKl(nn.Module):
    ...
    def forward(self, concentration1_b, concentration0_b, concentration1_a, concentration0_a):
...
class GammaConstruct(nn.Module):
    """
    Gamma distribution: going through forward.
    """
    def __init__(self):
