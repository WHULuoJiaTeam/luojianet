...
class GammaProb1(nn.Module):
    ...
    def forward(self, value, concentration, rate):
...
def test_gamma_prob1():
    """
    Test probability functions: passing concentration/rate, value through forward.
    """
    net = GammaProb1()
    value = Tensor([0.5, 1.0], dtype=dtype.float32)
...
class GammaKl(nn.Module):
    ...
    def forward(self, concentration_b, rate_b, concentration_a, rate_a):
