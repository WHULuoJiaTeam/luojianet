...
class LogNormalProb1(nn.Module):
    ...
    def forward(self, value, mean, sd):
...
def test_lognormal_prob1():
    """
    Test probability functions: passing mean/sd, value through forward.
    """
    net = LogNormalProb1()
    value = Tensor([0.5, 1.0], dtype=dtype.float32)
...
class LogNormalKl(nn.Module):
    ...
    def forward(self, mean_b, sd_b, mean_a, sd_a):
...
class LogNormalConstruct(nn.Module):
    """
    LogNormal distribution: going through forward.
    """
