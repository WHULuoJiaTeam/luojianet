...
class FirstInputTupleNet(nn.Module):
    def forward(self, tuple_a, tensor_x, list_b, tensor_y, scalar, dict_c, flag):
        if flag:
...
class GradNet1(nn.Module):
    ...
    def forward(self, tuple_a, tensor_x, list_b, tensor_y, tensor_z, dict_c):
...
def test_grad_first_input_net():
    class FirstInputTensorNet(nn.Module):
        def forward(self, tensor_x, tuple_a, list_b, tensor_y, tensor_z, dict_c):
...
def test_net_inputs_including_str():
    ...
    assert "The inputs types of the outermost network 'GradNet.forward' support bool, int, float, None, tensor, " \
...
def test_outermost_net_pass_parameter():
    ...
    assert "The inputs types of the outermost network 'FirstInputTupleNet.forward' " \
           "support bool, int, float, None, tensor, " \
           "mstype.Number(mstype.bool, mstype.int, mstype.float, mstype.uint), " \
           "and tuple or list containing only these types, and dict whose values are these types, " \
           "but the 1th arg type is <class 'luojianet_ms.common.parameter.Parameter'>, " \
